...
void DefaultTransformStrategy::onSelectionChanged(const Gui::SelectionChanges& msg)
{
    ...
    if (!filter.empty()) {
        ...
        std::set_difference(update_selection.begin(),
    ...
    , strategy(nullptr)
{
    ui = new Ui_Placement();
    ui->setupUi(this);
    connect(ui->applyButton, &QPushButton::clicked, this, &Transform::onApplyButtonClicked);
...
Base::Placement Transform::getPlacementData() const
{
    ...
    pos = Base::Vector3d(ui->xPos->value().getValue(),
                         ui->yPos->value().getValue(),
                         ui->zPos->value().getValue());
    cnt = Base::Vector3d(ui->xCnt->value().getValue(),
