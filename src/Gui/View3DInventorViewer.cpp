...
void View3DInventorViewer::updateFPSLabel()
{
    ...
    const unsigned long axisLetterColor = hGrpView->GetUnsigned("AxisLetterColor", 4294902015);

    if (axisLetterColor != previousAxisLetterColor) {
        previousAxisLetterColor = axisLetterColor;
        const Base::Color color(static_cast<uint32_t>(axisLetterColor));
    ...
    const int xOffset = hGrpOverlayL->GetASCII("Widgets", "").empty() ? 10 : fpsCounter->width() + 20;
