 ...
 * still running we can assume that everything is fine. \note This test is necessary since \a wget
 * writes all its output on stderr and we cannot determine surely if an error occurred or not.
 *
...
bool NetworkRetriever::startDownload(const QString& startUrl)
{
    ...
    if (!d->dir.isEmpty()) {
        ...
        if (!dir.exists(d->dir)) {
            if (!dir.mkdir(d->dir)) {
                Base::Console().Error("Directory '%s' could not be created.",
...
void StdCmdDownloadOnlineHelp::activated(int iMsg)
{
    ...
    if (!wget->isDownloading()) {
        ...
        while (loop > 0) {
            ...
            QFileInfo fi(path);
            if (!fi.exists()) {
                if (QMessageBox::critical(getMainWindow(),
        ...
        if (canStart) {
            ...
            if (!ok) {
                Base::Console().Error(
                    "The tool 'wget' couldn't be found. Please check your installation.");
            }
