...
std::string Command::getObjectCmd(const char* Name,
...
void Command::_copyVisual(const char* file,
                          int line,
                          const char* to,
                          const char* attr,
                          const char* from)
...
void Command::adjustCameraPosition()
{
    ...
    if (doc) {
        ...
        if (radius >= distance_to_midpoint) {
            ...
            distance_to_midpoint = (camera->position.getValue() - box.getCenter()).length();
...
void MacroCommand::activated(int iMsg)
{
    ...
    if (!fi.exists()) {
        QMessageBox::critical(Gui::getMainWindow(),
                              ...
                              qApp->translate("Gui::MacroCommand", "No such macro file: '%1'")
...
void MacroCommand::load()
{
    ParameterGrp::handle hGrp =
        App::GetApplication().GetParameterGroupByPath("User parameter:BaseApp/Macro");
...
void MacroCommand::save()
{
    ...
    std::vector<Command*> macros =
        Application::Instance->commandManager().getGroupCommands("Macros");
    if (!macros.empty()) {
        for (const auto& it : macros) {
...
const char* PythonCommand::getHelpUrl() const
{
    ...
    if (!PyUnicode_Check(pcTemp)) {
        throw Base::TypeError("PythonCommand::CmdHelpURL(): Method CmdHelpURL() of the Python "
                              "command object returns no string");
    }
    return PyUnicode_AsUTF8(pcTemp);
}
...
void PythonGroupCommand::activated(int iMsg)
{
    try {
        ...
        // If the command group doesn't implement the 'Activated' method then invoke the command
        // directly
        else {
            Gui::CommandManager& rcCmdMgr = Gui::Application::Instance->commandManager();
...
void PythonGroupCommand::languageChange()
{
    ...
    for (const auto& it : a) {
        ...
        if (cmd) {
            ...
            const char* context =
                dynamic_cast<PythonCommand*>(cmd) ? cmd->getName() : cmd->className();
            const char* tooltip = cmd->getToolTipText();
            const char* statustip = cmd->getStatusTip();
            if (!statustip || '\0' == *statustip) {
                statustip = tooltip;
