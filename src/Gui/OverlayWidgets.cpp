...
void OverlayTabWidget::startShow()
{
    ...
    if (duration) {
        ...
        _animator->setEasingCurve(
            (QEasingCurve::Type)OverlayParams::getDockOverlayAnimationCurve());
        _animator->start();
    }
...
bool OverlayTabWidget::checkAutoHide() const
{
    ...
    if (OverlayParams::getDockOverlayAutoView()) {
        ...
        if (!view->onHasMsg("CanPan") && view->parentWidget()
        ...
        if (parent) {
            ...
            if (treeView) {
                ...
                if (!TreeParams::getHideHeaderView() || option == OverlayOption::Disable) {
                    ...
                    static QString _style = QStringLiteral("QHeaderView:section {"
                                                           "height: 0px;"
...
QLayoutItem* OverlayTabWidget::prepareTitleWidget(QWidget* widget, const QList<QAction*>& actions)
{
    ...
    auto spacer = new QSpacerItem(buttonSize,
                                  buttonSize,
    ...
    if (tabWidget) {
        ...
        QObject::connect(grip,
                         &OverlaySizeGrip::dragMove,
                         tabWidget,
                         &OverlayTabWidget::onSizeGripMove);
...
void OverlayTitleBar::mouseMoveEvent(QMouseEvent* me)
{
    ...
    if (OverlayTabWidget::_Dragging != this && mouseMovePending
...
void OverlayTitleBar::mousePressEvent(QMouseEvent* me)
{
    ...
    if (OverlayTabWidget::_Dragging || !parent || !getMainWindow()
    ...
    dragSize.setHeight(
        std::max(OverlayParams::getDockOverlayMinimumSize(),
...
void OverlaySplitterHandle::paintEvent(QPaintEvent* e)
{
    ...
    int flags = Qt::AlignCenter;
    auto tabWidget =
        qobject_cast<OverlayTabWidget*>(splitter() ? splitter()->parentWidget() : nullptr);
