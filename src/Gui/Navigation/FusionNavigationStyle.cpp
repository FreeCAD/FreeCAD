...
SbBool FusionNavigationStyle::processSoEvent(const SoEvent* const ev)
{
    ...
    // True while a middle-button pan or orbit is in progress.
    const bool panningOrDragging = curmode == NavigationStyle::PANNING
    ...
    if (type.isDerivedFrom(SoMouseButtonEvent::getClassTypeId())) {
        ...
        switch (button) {
            case SoMouseButtonEvent::BUTTON1:
                ...
                else if (!press && panningOrDragging) {
                    processed = true;
                }
            ...
            case SoMouseButtonEvent::BUTTON2:
                ...
                if (!press && !viewer->isEditing() && !panningOrDragging) {
            ...
            case SoMouseButtonEvent::BUTTON3:
                if (press) {
                    // A double-click of the middle button zooms to fit, matching the Autodesk
                    // Fusion default. The two clicks must be close in both time and position so
                    // that a rapid pan-release-pan gesture is not mistaken for a double-click.
                    SbTime tmp = (ev->getTime() - this->centerTime);
                    float dci = (float)QApplication::doubleClickInterval() / 1000.0f;
                    int maxDist = QApplication::startDragDistance();
                    SbVec2s delta = pos - this->doubleClickPos;
                    int distSq = delta[0] * delta[0] + delta[1] * delta[1];
                    if (tmp.getValue() < dci && distSq <= maxDist * maxDist) {
                        viewer->viewAll();
                        processed = true;
                    }
                    this->centerTime = ev->getTime();
                    this->doubleClickPos = pos;
    ...
    switch (combo) {
        ...
        case CTRLDOWN | BUTTON1DOWN:
            ...
            break;
        case BUTTON3DOWN:
            // Middle button without Shift pans. Keep orbiting if a drag is already in
            // progress, so releasing Shift mid-orbit does not switch to panning.
            if (curmode != NavigationStyle::DRAGGING) {
                newmode = NavigationStyle::PANNING;
            }
            break;
        case SHIFTDOWN | BUTTON3DOWN:
            // Shift and middle button orbits. Keep panning if a drag is already in
            // progress, so pressing Shift mid-pan does not switch to orbiting.
        ...
        default:
            // Stop panning or orbiting as soon as the middle button is released, even if a
            // modifier (Shift) is still held down.
            if (panningOrDragging && !this->button3down) {
                newmode = NavigationStyle::IDLE;
            }
            break;
    }
