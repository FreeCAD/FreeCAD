...
Gui::PreferencePackManager::getPacksFromDirectory(const fs::path& path) const
{
...
static void copyTemplateParameters(Base::Reference<ParameterGrp> templateGroup,
...
static std::vector<fs::path> scanForTemplateFolders(const std::string& groupName,
                                                    const fs::path& entry)
{
        ...
        if (fs::is_directory(entry)) {
            if (entry.filename() == "PreferencePackTemplates"
                ...
                std::string subgroupName =
                    groupName + "/" + Base::FileInfo::pathToString(entry.filename());
...
static std::vector<PreferencePackManager::TemplateFile>
scanForTemplateFiles(const std::string& groupName, const fs::path& entry)
{
    ...
    for (const auto& templateDir : templateFolders) {
        ...
        for (const auto& entry : fs::directory_iterator(templateDir)) {
            if (entry.path().extension() == ".cfg") {
                ...
                if (std::find_if(templateFiles.begin(),
                                 templateFiles.end(),
...
std::vector<PreferencePackManager::TemplateFile> PreferencePackManager::templateFiles(bool rescan)
{
    ...
    auto resourcePath =
        fs::path(Base::FileInfo::stringToPath(App::Application::getResourceDir())) / "Gui";
