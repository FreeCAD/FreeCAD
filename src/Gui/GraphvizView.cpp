...
namespace
{
const std::string userPref = "User parameter:BaseApp/Preferences/";
}
    ...
    , doc(_doc)
{
    ...
    auto hGrp = App::GetApplication().GetParameterGroupByPath((userPref + "View").c_str());
...
void GraphvizView::convertDotStarted()
{
    if (pathType == PathType::Selection) {
        auto hGrp = App::GetApplication().GetParameterGroupByPath((userPref + "Paths").c_str());
        hGrp->SetASCII("Graphviz", path.toUtf8().constBegin());
    }
...
void GraphvizView::convertDotError()
{
    if (dotProc->error() == QProcess::FailedToStart) {
        path = QString();
        pathType = (PathType)std::min((int)pathType + 1, (int)PathType::Selection);
        convertDotStart();
        return;
    }
    disconnectSignals();
...
void GraphvizView::convertUnflattenError()
{
    ...
    if (unflattenProc->error() == QProcess::FailedToStart) {
        convertDotWrite();
    }
}
