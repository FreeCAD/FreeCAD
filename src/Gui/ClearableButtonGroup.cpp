...
#include "ClearableButtonGroup.h"

using namespace Gui;

ClearableButtonGroup::ClearableButtonGroup(QObject *parent)
    : QButtonGroup(parent)
{
    _bExclusive = true;
...
void ClearableButtonGroup::buttonClicked(QAbstractButton *button)
{
    if (_bExclusive) {
        // just uncheck all other buttons regardless of the state of the clicked button
        QList<QAbstractButton *> buttonlist = buttons();
        for (auto &iBtn : buttonlist)
            if (iBtn && iBtn != button && iBtn->isCheckable())
