...
bool parseStringList(PyObject* object, std::list<std::string>& items, const char* errorMessage)
{
    ...
    PyObject** sequenceItems = PySequence_Fast_ITEMS(sequence);
    for (Py_ssize_t index = 0; index < size; ++index) {
        PyObject* item = sequenceItems[index];
        if (!PyUnicode_Check(item)) {
            Py_DECREF(sequence);
            PyErr_Format(
                PyExc_TypeError,
                "Expected string at cmds[%zd], got %.200s",
                index,
...
PyObject* PythonWorkbenchPy::appendToolbar(PyObject* args, PyObject* kwd)
{
    PY_TRY
    {
        ...
        std::list<std::string> items;
        if (!parseStringList(pObject, items, "Expected a sequence of strings or a string for cmds")) {
            return nullptr;
        }
