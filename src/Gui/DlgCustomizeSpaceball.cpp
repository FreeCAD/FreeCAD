...
{
    this->selectionModel()->select(this->model()->index(number, 0),
                                   QItemSelectionModel::ClearAndSelect);
    this->scrollTo(this->model()->index(number, 0), QAbstractItemView::EnsureVisible);
}

void ButtonView::goSelectionChanged(const QItemSelection& selected,
...
void ButtonModel::load3DConnexionButtonMapping(boost::property_tree::ptree ButtonMapTree)
{
    ...
    BOOST_FOREACH (const boost::property_tree::ptree::value_type& Map,
        ...
        if ("Map" == Map.first) {
            ...
            BOOST_FOREACH (const boost::property_tree::ptree::value_type& kv,
...
// For now the Button mapping file (3DConnexion.xml) is held the same folder as the FreeCAD
// executable.
void ButtonModel::load3DConnexionButtons(const char* RequiredDeviceName)
{
    try {
        ...
        BOOST_FOREACH (const boost::property_tree::ptree::value_type& ButtonMap,
                       tree.get_child("")) {
...
QVariant ButtonModel::data(const QModelIndex& index, int role) const
{
    ...
    if (role == Qt::DecorationRole) {
        static QPixmap icon(BitmapFactory()
...
void ButtonModel::insertButtonRows(int number)
{
    ...
    for (int index = buttonCount; index < number + 1; ++index) {
        ...
        Base::Reference<ParameterGrp> newGroup =
            spaceballButtonGroup()->GetGroup(groupName.toLatin1());  // builds the group.
...
void CommandView::goChangeCommandSelection(const QString& commandName)
{
    ...
    QModelIndexList index(this->model()->match(this->model()->index(0, 0),
                                               Qt::UserRole,
...
QVariant CommandModel::data(const QModelIndex& index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        if (node->nodeType == CommandNode::GroupType) {
            ...
            return {qApp->translate(childNode->aCommand->className(),
                                    childNode->aCommand->getGroupName())};
        }
        return {};
    }
    if (role == Qt::DecorationRole) {
...
{
    QModelIndexList indexList(this->match(this->index(0, 0),
                                          Qt::UserRole,
                                          QVariant(QString::fromLatin1("Macros")),
                                          1,
                                          Qt::MatchWrap | Qt::MatchRecursive));
...
{
    QModelIndexList macroList(this->match(this->index(0, 0),
                                          Qt::UserRole,
...
void CommandModel::groupCommands(const QString& groupName)
{
    ...
    std::vector<Command*> commands =
        Application::Instance->commandManager().getGroupCommands(groupName.toLatin1());
...
QVariant PrintModel::data(const QModelIndex& index, int role) const
{
    ...
    if (index.column() == 1) {
        ...
        QModelIndexList indexList(commandModel->match(commandModel->index(0, 0),
                                                      Qt::UserRole,
                                                      QVariant(commandName),
                                                      1,
                                                      Qt::MatchWrap | Qt::MatchRecursive));
...
void DlgCustomizeSpaceball::setupButtonModelView()
{
    ...
    // had to do this here as the views default selection model is not created until after
    // construction.
    connect(buttonView->selectionModel(),
...
void DlgCustomizeSpaceball::setupLayout()
{
    ...
    QString model = QString::fromStdString(App::GetApplication()
                                               .GetUserParameter()
                                               .GetGroup("BaseApp")
                                               ->GetGroup("Spaceball")
                                               ->GetASCII("Model", ""));
