            ...
            if (docpy->getDocumentPtr()) {
                ...
                if (document) {
                    ...
                    for (auto & object : objects)
                        pyList.append(Py::String(object->getNameInDocument()));
                    extractTipsFromObject(inst, pyList, tips);
                }
            }
        }
...
void CallTipsList::extractTipsFromProperties(Py::Object& obj, QMap<QString, CallTip>& tips) const
{
    ...
    for (const auto & iter : Map) {
        CallTip tip;
        QString str = QString::fromLatin1(iter.first.c_str());
        tip.name = str;
        tip.type = CallTip::Property;
        QString longdoc = QString::fromUtf8(container->getPropertyDocumentation(iter.second));
        // a point, mesh or shape property
        if (iter.second->isDerivedFrom(Base::Type::fromName("App::PropertyComplexGeoData"))) {
            Py::Object data(iter.second->getPyObject(), true);
