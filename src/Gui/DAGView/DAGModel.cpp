    ...
    : QGraphicsScene(parentIn)
{
    ...
    // NOLINTBEGIN
    connectNewObject =
        documentIn.signalNewObject.connect(std::bind(&Model::slotNewObject, this, sp::_1));
    connectDelObject =
        documentIn.signalDeletedObject.connect(std::bind(&Model::slotDeleteObject, this, sp::_1));
    ...
    connectResObject =
        documentIn.signalResetEdit.connect(std::bind(&Model::slotResetEdit, this, sp::_1));
    // NOLINTEND
...
void Model::setupViewConstants()
{
    ...
    forgroundBrushes = {QBrush(Qt::red),
                        QBrush(Qt::darkRed),
                        QBrush(Qt::green),
                        QBrush(Qt::darkGreen),
                        QBrush(Qt::blue),
...
void Model::updateSlot()
{
    ...
    // we have to do this first and in isolation because everything is dependent on an up to date
    // graph.
    BGL_FORALL_VERTICES(currentVertex, *theGraph, Graph)
    {
    ...
    for (const auto& currentVertex : sorted) {
            ...
            for (; it != itEnd; ++it) {
                ...
                if (currentParentIndex < farthestParentIndex) {
                    Path::const_iterator start =
                        sorted.begin() + currentParentIndex + 1;  // 1 after
        ...
        visiblePixmap->setTransform(QTransform::fromTranslate(
            0.0,
            rowHeight * currentRow + cheat));  // calculate x location later.
        ...
        for (; it != itEnd; ++it) {
            ...
            if (!(*theGraph)[target].dagVisible) {
                continue;  // we don't make it here if source isn't visible. So don't have to worry
                           // about that.
            }
            float dependentX =
        ...
        if (rect) {
            ...
            if (pixmapItem && (pixmapItem == (*theGraph)[record.vertex].visibleIcon.get())) {
                ...
                if ((*theGraph)[record.vertex].lastVisibleState == VisibilityState::Off) {
                    const_cast<ViewProviderDocumentObject*>(record.VPDObject)->show();  // const
                                                                                        // hack
                }
                else {
                    const_cast<ViewProviderDocumentObject*>(record.VPDObject)->hide();  // const
                                                                                        // hack
                }
            ...
            if (!rect->isEditing()) {
                const_cast<Gui::ViewProviderDocumentObject*>(record.VPDObject)
                    ->setupContextMenu(&contextMenu,
                                       this,
                                       SLOT(editingStartSlot()));  // const hack.
            }
            else {
...
void Model::visiblyIsolate(Gui::DAG::Vertex sourceIn)
{
    ...
    boost::breadth_first_search(boost::make_reverse_graph(*theGraph),
                                sourceIn,
