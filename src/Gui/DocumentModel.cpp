...
{
    std::map<const ViewProviderDocumentObject*,
             boost::unordered_set<ViewProviderIndex*>>::const_iterator it;
    it = vp_nodes.find(&vp);
    if (it != vp_nodes.end()) {
...
void DocumentModel::slotNewDocument(const Gui::Document& Doc)
{
    ...
    Doc.signalChangedObject.connect(
        std::bind(&DocumentModel::slotChangeObject, this, sp::_1, sp::_2));
...
void DocumentModel::slotChangeObject(const Gui::ViewProviderDocumentObject& obj,
