...
Document::Document(App::Document* pcDocument, Application* app)
{
    ...
    d->connectNewObject =
        pcDocument->signalNewObject.connect(std::bind(&Gui::Document::slotNewObject, this, sp::_1));
    ...
    d->connectSaveDocument =
        pcDocument->signalSaveDocument.connect(std::bind(&Gui::Document::Save, this, sp::_1));
    d->connectRestDocument =
        pcDocument->signalRestoreDocument.connect(std::bind(&Gui::Document::Restore, this, sp::_1));
    ...
    d->connectUndoDocument =
        pcDocument->signalUndo.connect(std::bind(&Gui::Document::slotUndoDocument, this, sp::_1));
    d->connectRedoDocument =
        pcDocument->signalRedo.connect(std::bind(&Gui::Document::slotRedoDocument, this, sp::_1));
...
{
    std::map<std::string, ViewProvider*>::const_iterator it;
    for (it = d->_ViewProviderMapAnnotation.begin(); it != d->_ViewProviderMapAnnotation.end();
         ++it) {
        if (it->second == vp) {
            return true;
        }
...
void Document::slotNewObject(const App::DocumentObject& Obj)
{
    ...
    if (!pcProvider) {
        ...
        for (;;) {
            ...
            Base::Type type =
                Base::Type::getTypeIfDerivedFrom(cName.c_str(),
    ...
    if (viewProvider) {
        try {
            ...
            if (d->_editingViewer && d->_editingObject && d->_editViewProviderParent
                ...
                Base::Matrix4D mat;
                auto sobj =
                    d->_editViewProviderParent->getObject()->getSubObject(d->_editSubname.c_str(),
                                                                          nullptr,
                                                                          &mat);
...
void Document::RestoreDocFile(Base::Reader& reader)
{
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt; i++) {
            ...
            auto pObj =
                dynamic_cast<ViewProviderDocumentObject*>(getViewProviderByName(name.c_str()));
...
MDIView* Document::createView(const Base::Type& typeId)
{
    ...
    if (typeId == View3DInventor::getClassTypeId()) {
        ...
        for (It2 = d->_ViewProviderMapAnnotation.begin();
             It2 != d->_ViewProviderMapAnnotation.end();
             ++It2) {
...
bool Document::canClose(bool checkModify, bool checkLink)
{
    ...
    //             moment.\n"
    //                         "You either have to finish or cancel the editing in the task
    //                         panel."));
    //         Gui::TaskView::TaskDialog* dlg = Gui::Control().activeDialog();
