...
void Document::slotActivatedContainer(App::Document* doc, App::PropertyContainer* newContainer, App::PropertyContainer* oldContainer)
{
    if (oldContainer){
        if (oldContainer->isDerivedFrom(App::DocumentObject::getClassTypeId())){
            auto obj = static_cast<App::DocumentObject*>(oldContainer);
    ...
    if (newContainer){
        if (newContainer->isDerivedFrom(App::DocumentObject::getClassTypeId())){
            auto obj = static_cast<App::DocumentObject*>(newContainer);
            ViewProvider* vp = this->getViewProviderByName(obj->getNameInDocument());
            if (vp->isDerivedFrom(ViewProviderDocumentObject::getClassTypeId())){
                ViewProviderDocumentObject* vpdo = static_cast<ViewProviderDocumentObject*>(vp);
