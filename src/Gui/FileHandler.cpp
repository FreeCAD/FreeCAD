...
bool FileHandler::openInternal()
{
    ...
    if (hasExtension(QStringList()
                     << QLatin1String("wrl") << QLatin1String("wrz") << QLatin1String("vrml"))) {
        openVRML();
        return true;
    }

    if (hasExtension(QStringList() << QLatin1String("py") << QLatin1String("fcmacro")
    ...
    std::transform(imageFormats.cbegin(),
                   imageFormats.cend(),
