...
void NaviCubeImplementation::addCubeFace(const Vector3f& x,
                                         const Vector3f& z,
                                         ShapeId shapeType,
                                         PickId pickId,
                                         float rotZ)
{
    ...
    // Rotate by an additional rotZ if vector x and vector z are not already the standard
    // orientation
...
void NaviCubeImplementation::prepare()
{
    ...
    m_PickingFramebuffer = new QtGLFramebufferObject(2 * m_CubeWidgetSize,
                                                     2 * m_CubeWidgetSize,
...
void NaviCubeImplementation::handleResize()
{
    ...
    if (viewSize != m_ViewSize) {
        m_PosAreaBase[0] =
            std::min((int)(m_PosOffset[0] + m_CubeWidgetSize * 0.55), viewSize[0] / 2);
        m_PosAreaBase[1] =
            std::min((int)(m_PosOffset[1] + m_CubeWidgetSize * 0.55), viewSize[1] / 2);
...
NaviCubeImplementation::PickId NaviCubeImplementation::pickFace(short x, short y)
{
    ...
    if (m_PickingFramebuffer && std::abs(x) <= m_CubeWidgetSize / 2
        ...
        glFinish();
        glReadPixels(2 * x + m_CubeWidgetSize,
                     2 * y + m_CubeWidgetSize,
                     1,
                     1,
                     GL_RGBA,
                     GL_UNSIGNED_BYTE,
...
bool NaviCubeImplementation::mouseReleased(short x, short y)
{
    ...
    if (m_Dragging) {
        ...
        if (m_Faces[pickId].type == ShapeId::Main || m_Faces[pickId].type == ShapeId::Edge
            ...
            m_View3DInventorViewer->setCameraOrientation(
                rotation * m_View3DInventorViewer->getCameraOrientation());
        }
        else {
            return false;
        }
...
{
    bool hovering = std::abs(x) <= m_CubeWidgetSize / 2 && std::abs(y) <= m_CubeWidgetSize / 2;
