...
{
    static const auto hGrp = App::GetApplication().GetParameterGroupByPath("User parameter:BaseApp/Preferences/View");

    const float size = hGrp->GetFloat("DatumPlaneSize", 62.0) * Base::fromPercent(hGrp->GetFloat("DatumScale", 100.0));
    const float offset = 8.0F;

    SbVec3f verts[4];

    bool isSelected = Selection().isSelected(getObject());
    ...
    } else if (!getRole().empty()) {
        verts[0] = SbVec3f(size, size, 0);
        verts[1] = SbVec3f(size, offset, 0);
        verts[2] = SbVec3f(offset, offset, 0);
        verts[3] = SbVec3f(offset, size, 0);
    } else {
        verts[0] = SbVec3f(size - offset, size - offset, 0);
        verts[1] = SbVec3f(size - offset, -size + offset, 0);
        verts[2] = SbVec3f(-size + offset, -size + offset, 0);
        verts[3] = SbVec3f(-size + offset, size - offset, 0);
    }

    pTextTranslation->translation.setValue(verts[0] / 2  - SbVec3f(2, 4, 0)); // NOLINT
