...
void ViewProviderPlane::attach(App::DocumentObject * obj) {
    ...
    textTranslation->translation.setValue(SbVec3f(size * 0.3, size * 0.3, 0));
    sep->addChild(textTranslation);

    pLabel->string.setValue(SbString(getLabelText(role).c_str()));
...
unsigned long ViewProviderPlane::getColor(std::string& role)
{
    ...
    if (role == planesRoles[0]) {
        rgb = ViewParams::instance()->getAxisZColor(); // XY-plane
    }
    else if (role == planesRoles[1]) {
        rgb = ViewParams::instance()->getAxisYColor(); // XZ-plane
    }
    else if (role == planesRoles[2]) {
        rgb = ViewParams::instance()->getAxisXColor(); // YZ-plane
    }
    return rgb;
}
