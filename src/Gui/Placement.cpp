...
public:
    ...
    static App::PropertyPlacement* getProperty(App::DocumentObject* obj,
...
PlacementHandler::getSelectedObjects(Gui::Document* document) const
{
    return Gui::Selection().getObjectsOfType(App::DocumentObject::getClassTypeId(),
...
void PlacementHandler::applyPlacement(Gui::Document* document,
...
void PlacementHandler::applyPlacement(App::DocumentObject* obj,
                                      const QString& data,
                                      bool incremental)
{
    auto property = find_placement::getProperty(obj, this->propertyName);
...
QString PlacementHandler::getIncrementalPlacement(App::DocumentObject* obj,
...
{
    return QString::fromLatin1("App.getDocument(\"%1\").%2.%3=%4")
        .arg(QString::fromLatin1(obj->getDocument()->getName()),
             QString::fromLatin1(obj->getNameInDocument()),
             QString::fromLatin1(this->propertyName.c_str()),
             data);
...
{
    connect(ui->applyButton, &QPushButton::clicked, this, &Placement::onApplyButtonClicked);
...
void Placement::onSelectedVertexClicked()
{
    ...
    if (picked.size() == 1) {
        ...
        if (QApplication::keyboardModifiers()
            == Qt::ShiftModifier) {  // copy to clipboard on Shift+click
            QLocale loc;
        ...
        if (QApplication::keyboardModifiers()
            ...
            Base::Console().Message("(Angle copied to clipboard, but you might need to use a "
                                    "negative (-) angle sometimes.)\n");
        }
        else {
            Base::Console().Message(
    ...
    if (!success) {
        ...
        QMessageBox msgBox;
        msgBox.setText(tr(
            "Please select 1, 2, or 3 points before clicking this button.  A point may be on a vertex, \
...
void Placement::onApplyAxialClicked()
{
    ...
    if (km == Qt::ShiftModifier) {  // go opposite direction on Shift+click
        newPos = Base::Vector3d(curPos.x - (axis.x * axPos),
...
void Placement::showErrorMessage()
{
    ...
    msg.setText(
        tr("There are input fields with incorrect input, please ensure valid placement values!"));
    msg.exec();
}
...
void Placement::bindObject()
{
    if (!selectionObjects.empty()) {
        ...
        if (path.getProperty()) {
            ui->xPos->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Base.x")));
            ui->yPos->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Base.y")));
            ui->zPos->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Base.z")));

            ui->xAxis->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Axis.x")));
            ui->yAxis->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Axis.y")));
            ui->zAxis->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Axis.z")));
            ui->angle->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Angle")));

            ui->yawAngle->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Yaw")));
            ui->pitchAngle->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Pitch")));
            ui->rollAngle->bind(
                App::ObjectIdentifier::parse(obj, propertyName + std::string(".Rotation.Roll")));
...
Base::Rotation Placement::getRotationData() const
{
    ...
    if (index == 0) {
        ...
        rot.setYawPitchRoll(ui->yawAngle->value().getValue(),
...
QString Placement::getPlacementFromAxisWithAngle() const
{
    ...
    return QString::fromLatin1("App.Placement(App.Vector(%1,%2,%3), "
                               "App.Rotation(App.Vector(%4,%5,%6),%7), App.Vector(%8,%9,%10))")
        .arg(pos.x)
        .arg(pos.y)
        .arg(pos.z)
        .arg(dir.x)
    ...
    : Placement(parent, fl)
{
    ...
    QDockWidget* dw =
        pDockMgr->addDockWindow(QT_TR_NOOP("Placement"), this, Qt::BottomDockWidgetArea);
