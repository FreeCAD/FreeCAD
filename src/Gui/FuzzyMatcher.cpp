...
)
{
    ...
    if (matchIndex >= 0) {
        ...
        // Keep exact/prefix matches ahead of embedded substrings, regardless of length.
        // Bound the position penalty so even late substrings outrank subsequence matches.
        score = matchIndex == 0 ? 1000 + coverage : 500 + coverage + qMax(0, 100 - matchIndex);
        return true;
    }
