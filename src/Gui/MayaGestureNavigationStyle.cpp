...
const char* MayaGestureNavigationStyle::mouseButtons(ViewerMode mode)
{
    switch (mode) {
        ...
        case NavigationStyle::DRAGGING:
            return QT_TR_NOOP("Drag screen with one finger OR press ALT + left mouse button. In "
                              "Sketcher and other edit modes, hold Alt in addition.");
        case NavigationStyle::ZOOMING:
            return QT_TR_NOOP("Pinch (place two fingers on the screen and drag them apart from or "
                              "towards each other) OR scroll middle mouse button OR press ALT + "
                              "right mouse button OR PgUp/PgDown on keyboard.");
        default:
 ...
 * constder it a drag. \param currentPos current position of mouse cursor, in local pixel
 * coordinates. \return true if the mouse was moved far enough. False if it's within the boundary.
 * Ignores MayaGestureNavigationStyle::mouseMoveThresholdBroken flag.
 */
...
SbBool MayaGestureNavigationStyle::processSoEvent(const SoEvent* const ev)
{
    ...
    bool evIsLoc3 =
        type.isDerivedFrom(SoMotion3Event::getClassTypeId());  // spaceball/joystick movement
    ...
    int cntMBBefore =
        (comboBefore & BUTTON1DOWN
             ? 1
             : 0)  // cntMBBefore = how many buttons were down when this event arrived?
    ...
    int cntMBAfter = (comboAfter & BUTTON1DOWN
                          ? 1
                          : 0)  // cntMBAfter = how many buttons were down when this event arrived?
    ...
    switch (curmode) {
        ...
        case NavigationStyle::INTERACT: {
                    ...
                    case SoMouseButtonEvent::BUTTON2:
                        if (press) {
                            if (this->thisClickIsComplex && this->mouseMoveThresholdBroken) {
                                ...
                                setupPanningPlane(viewer->getSoRenderManager()
                                                      ->getCamera());  // set up panningplane
                            ...
                            if (!processed) {
                                ...
                                for (int i = 0; i < this->mousedownConsumedCount; i++) {
                                    inherited::processSoEvent(
                                        &(this->mousedownConsumedEvents
                                              [i]));  // simulate the previously-comsumed mousedown.
                                }
                                this->mousedownConsumedCount = 0;
                                processed = inherited::processSoEvent(
                                    ev);  // explicitly, just for clarity that we are sending a full
                                          // click sequence.
                                propagated = true;
                            }
                        }
            ...
            if (evIsLoc2) {
                if (this->mouseMoveThresholdBroken && (this->button1down || this->button2down)
                    ...
                    if ((this->button1down && !suppressLMBDrag && this->altdown)
                        ...
                        setViewingMode(this->button1down ? NavigationStyle::DRAGGING
                        ...
                        this->mousedownConsumedCount = 0;
                        processed =
                            inherited::processSoEvent(ev);  // explicitly, just for clarity that we
                                                            // are sending a full click sequence.
                        propagated = true;
                    }
                }
            ...
            // gesture start
            if (evIsGesture
                && /*!this->button1down &&*/ !this
                       ->button2down) {  // ignore gestures when mouse buttons are down. Button1down
                                         // check was disabled because of wrong state after
                                         // doubleclick on sketcher constraint to edit datum
                auto gesture = static_cast<const SoGestureEvent*>(ev);
                if (gesture->state == SoGestureEvent::SbGSStart
                    || gesture->state
                        == SoGestureEvent::SbGSUpdate) {  // even if we didn't get a start, assume
                                                          // the first update is a start (sort-of
                                                          // fail-safe).
                    if (type.isDerivedFrom(SoGesturePanEvent::getClassTypeId())) {
                        setupPanningPlane(
                            viewer->getSoRenderManager()->getCamera());  // set up panning plane
                        setViewingMode(NavigationStyle::PANNING);
                        processed = true;
                    }
        ...
        case NavigationStyle::PANNING: {
            ...
            if (evIsButton) {
                ...
                switch (button) {
                    case SoMouseButtonEvent::BUTTON1:
                    case SoMouseButtonEvent::BUTTON2:
                    case SoMouseButtonEvent::BUTTON3:  // allows to release button3 into SELECTION
                                                       // mode
                        if (comboAfter & BUTTON1DOWN || comboAfter & BUTTON2DOWN) {
                            ...
                            setViewingMode((comboAfter & BUTTON1DOWN) ? NavigationStyle::DRAGGING
            ...
            if (evIsLoc2 && !this->inGesture && this->mouseMoveThresholdBroken) {
                if (curmode == NavigationStyle::ZOOMING) {  // doesn't happen
                    ...
                    if (comboAfter & BUTTON1DOWN && comboAfter & BUTTON2DOWN) {
                        ...
                        // pseudomouse is enough) this->addToLog(event->getPosition(),
                        // event->getTime());
                ...
                if (gesture->state == SoGestureEvent::SbGSEnd) {
                    ...
                    if (type.isDerivedFrom(SoGesturePinchEvent::getClassTypeId())) {
                        ...
                        if (this->zoomAtCursor) {
                            ...
                            SbVec2f panDist =
                                this->normalizePixelPos(event->deltaCenter.getValue());
                            NavigationStyle::panCamera(viewer->getSoRenderManager()->getCamera(),
                                                       ratio,
                                                       this->panningplane,
                                                       panDist,
                                                       SbVec2f(0, 0));
                        }
                        NavigationStyle::doZoom(viewer->getSoRenderManager()->getCamera(),
