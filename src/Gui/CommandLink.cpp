    ...
    : Command("Std_LinkMake")
{
    ...
    sToolTipText = QT_TR_NOOP(
        "A Link is an object that references or links to another object in the same document, "
    ...
    try {
        if (objs.empty()) {
            ...
            for (auto obj : objs) {
                std::string name = doc->getUniqueObjectName("Link");
                Command::doCommand(Command::Doc,
                                   "App.getDocument('%s').addObject('App::Link','%s').setLink(App."
                                   "getDocument('%s').%s)",
                                   doc->getName(),
                                   name.c_str(),
                                   obj->getDocument()->getName(),
...
void StdCmdLinkMakeRelative::activated(int)
{
    ...
    try {
        ...
        for (auto& v : linkInfo) {
            ...
            ss << ']';
            FCMD_DOC_CMD(doc,
                         "addObject('App::Link','" << name << "').setLink("
...
void StdCmdLinkSelectLinked::activated(int)
{
    ...
    Selection().clearCompleteSelection();
    if (!subname.empty()) {
        Selection().addSelection(linked->getDocument()->getName(),
