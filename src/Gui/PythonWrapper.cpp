...
#ifdef HAVE_SHIBOKEN_TYPEINITSTRUCT
static bool loadPySideModule(const std::string& moduleName,
                             Shiboken::Module::TypeInitStruct*& types)
#else
...
template<typename qttype>
Py::Object
qt_wrapInstance(qttype object, const std::string& className, const std::string& moduleName)
{
    Py::Module mainmod(importShiboken(), true);
...
#if defined(HAVE_SHIBOKEN2)
        type = reinterpret_cast<SbkObjectType*>(
            Shiboken::Conversions::getPythonTypeObject("QPrinter"));
#else
        ...
        if (!name.isEmpty() && !name.startsWith("_") && !name.startsWith("qt_")) {
            ...
            if (!hasAttr) {
                ...
                Shiboken::AutoDecRef pyChild(
                    Shiboken::Conversions::pointerToPython(getPyTypeObjectForTypeName<QObject>(),
                                                           child));
