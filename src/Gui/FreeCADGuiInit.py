    ...
    def RunInitGuiPy(Dir) -> bool:
        ...
        if os.path.exists(InstallFile):
            try:
                ...
                Log(f'Init:      Initializing {Dir}... failed\n')
                Log('-' * 100 + '\n')
                Log(traceback.format_exc())
                Log('-' * 100 + '\n')
                Err(f'During initialization the error "{str(inst)}" occurred in {InstallFile}\n')
                Err('Please look into the log file for further information\n')
            else:
                Log(f'Init:      Initializing {Dir}... done\n')
                return True
        else:
            Log(f'Init:      Initializing {Dir}(InitGui.py not found)... ignore\n')
        return False
    ...
    try:
        ...
        for _, freecad_module_name, \
            ...
            if freecad_module_ispkg:
                Log(f'Init: Initializing {freecad_module_name}\n')
                try:
                    ...
                    ispkg in pkgutil.iter_modules(freecad_module.__path__)):
                        ...
                        Log(f'Init: Initializing {freecad_module_name}... done\n')
                    else:
                        Log('Init: No init_gui module found in ' + freecad_module_name \
                            + ', skipping\n')
                except Exception as inst:
                    Err(f'During initialization the error "{str(inst)}" occurred in {freecad_module_name}\n')
                    Err('-' * 80 + '\n')
                    Err(traceback.format_exc())
                    Err('-' * 80 + '\n')
                    Log(f'Init:      Initializing {freecad_module_name}... failed\n')
                    Log('-' * 80 + '\n')
                    Log(traceback.format_exc())
                    Log('-' * 80 + '\n')
    except ImportError as inst:
        Err(f'During initialization the error "{str(inst)}" occurred\n')
