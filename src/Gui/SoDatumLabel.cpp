...
private:
    ...
    std::vector<SbVec3f> computeRadiusDiameterBBox() const
    {
        ...
        // include arc helper if present
        if (geom.range != 0.0) {
            // sample points along the arc helper
            int numArcSamples = 6;
            for (int i = 0; i <= numArcSamples; i++) {
                float t = (float)i / (float)numArcSamples;
                float angle = geom.startangle + t * geom.range;
...
void SoDatumLabel::generateDiameterPrimitives(SoAction * action, const SbVec3f& p1, const SbVec3f& p2)
{
    ...
    // generate selectable primitive for arc helper if present
    if (geom.range != 0.0) {
        int countSegments = std::max(6, abs(int(50.0 * geom.range / (2 * std::numbers::pi))));
        double segment = geom.range / (countSegments - 1);

        // create selectable line segments for the arc
        for (int i = 0; i < countSegments - 1; i++) {
            double theta1 = geom.startangle + segment * i;
            double theta2 = geom.startangle + segment * (i + 1);
...
void SoDatumLabel::drawRadiusOrDiameter(const SbVec3f* points, float& angle, SbVec3f& textOffset)
{
    ...
    if (geom.range != 0.0) {
        ...
        for (int i = 0; i < countSegments; i++) {
            ...
            SbVec3f v1 = geom.center + SbVec3f(geom.radius * cos(theta), geom.radius * sin(theta), 0);
            glVertex2f(v1[0], v1[1]);
        }
        glEnd();
    }
...
SoDatumLabel::DiameterGeometry SoDatumLabel::calculateDiameterGeometry(const SbVec3f* points) const
{
    ...
    geom.range = this->param4.getValue();
