                              ...
                              const Command* command)
{
    ...
    QString tooltip =
        QString::fromLatin1("<p style='white-space:pre; margin-bottom:0.5em;'><b>%1</b>%2</p>")
    ...
    tooltip += QString::fromLatin1("<p style='white-space:pre; margin:0;'>");
    ...
    if (helpText.indexOf(QLatin1Char('\n')) >= 0) {
        ...
        if (width <= tipWidth) {
            ...
            tooltip += helpText.left(index).toHtmlEscaped() + QString::fromLatin1("</p>")
    ...
    : ActionGroup(pcCmd, parent)
{
    ...
    connect(getMainWindow(),
            &MainWindow::workbenchActivated,
            this,
            &WorkbenchGroup::onWorkbenchActivated);
}
...
void RecentMacrosAction::setFiles(const QStringList& files)
{
    ...
    for (int index = 0; index < numRecentFiles; index++) {
        ...
        recentFiles[index]->setText(
            QString::fromLatin1("%1 %2").arg(index + 1).arg(fi.completeBaseName()));
        recentFiles[index]->setToolTip(
            files[index]);  // set the full name that we need later for saving
        recentFiles[index]->setData(QVariant(index));
