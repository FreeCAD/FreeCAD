...
Py::Object View3DInventorPy::setCornerCrossVisible(const Py::Tuple& args)
{
    ...
    if (!PyArg_ParseTuple(args.ptr(), "i", &ok))
        throw Py::Exception();
    _view->getViewer()->setFeedbackVisibility(ok!=0);
    _view->getViewer()->redraw(); // added because isViewing() returns False when focus is in Python Console
    return Py::None();
}
...
Py::Object View3DInventorPy::isCornerCrossVisible(const Py::Tuple& args)
{
    if (!PyArg_ParseTuple(args.ptr(), ""))
        throw Py::Exception();
    bool ok = _view->getViewer()->isFeedbackVisible();
    return Py::Boolean(ok ? true : false);
}
...
Py::Object View3DInventorPy::setCornerCrossSize(const Py::Tuple& args)
{
    ...
    if (!PyArg_ParseTuple(args.ptr(), "i", &size))
        throw Py::Exception();
    _view->getViewer()->setFeedbackSize(size);
    _view->getViewer()->redraw(); // added because isViewing() returns False when focus is in Python Console
    return Py::None();
}
...
Py::Object View3DInventorPy::getCornerCrossSize(const Py::Tuple& args)
{
    if (!PyArg_ParseTuple(args.ptr(), ""))
        throw Py::Exception();
    int size = _view->getViewer()->getFeedbackSize();
    return Py::Int(size);
}
