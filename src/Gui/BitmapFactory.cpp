...
BitmapFactoryInst& BitmapFactoryInst::instance()
{
    if (!_pcSingleton) {
        ...
        _pcSingleton->addPath(QString::fromLatin1("%1/icons")
                                  .arg(QString::fromStdString(App::Application::getHomePath())));
        _pcSingleton->addPath(
            QString::fromLatin1("%1/icons")
                .arg(QString::fromStdString(App::Application::getUserAppDataDir())));
...
QPixmap BitmapFactoryInst::pixmapFromSvg(const QByteArray& originalContents,
                                         const QSizeF& size,
                                         const ColorMap& colorMapping) const
{
    ...
    for (const auto& colorToColor : colorMapping) {
        ...
        QString fromColorString =
            QString::fromLatin1(":#%1;").arg(fromColor, 6, 16, QChar::fromLatin1('0'));
        QString toColorString =
            QString::fromLatin1(":#%1;").arg(toColor, 6, 16, QChar::fromLatin1('0'));
...
QPixmap
BitmapFactoryInst::fillRect(int x, int y, int w, int h, const QPixmap& p, Qt::BGMode bgmode) const
{
    QBitmap b = p.mask();
    if (b.isNull()) {
...
{
    // does the similar as the method above except that this method does not resize the resulting
    // pixmap
    int x = 0, y = 0;
