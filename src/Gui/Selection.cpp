...
SelectionObserver::SelectionObserver(const ViewProviderDocumentObject* vp,
                                     bool attach,
                                     ResolveMode resolve)
    : resolve(resolve)
    , blockedSelection(false)
...
void SelectionObserver::attachSelection()
{
    if (!connectSelection.connected()) {
        ...
        // NOLINTBEGIN
        connectSelection =
            signal.connect(std::bind(&SelectionObserver::_onSelectionChanged, this, sp::_1));
        // NOLINTEND
...
std::vector<SelectionSingleton::SelObj>
SelectionSingleton::getCompleteSelection(ResolveMode resolve) const
{
    return getSelection("*", resolve);
}

std::vector<SelectionSingleton::SelObj>
SelectionSingleton::getSelection(const char* pDocName, ResolveMode resolve, bool single) const
{
...
std::vector<SelectionSingleton::SelObj>
SelectionSingleton::getPickedList(const char* pDocName) const
{
    std::vector<SelObj> temp;
    SelObj tempSelObj;
    ...
    for (std::list<_SelObj>::const_iterator It = _PickedList.begin(); It != _PickedList.end();
         ++It) {
...
std::vector<SelectionObject> SelectionSingleton::getSelectionEx(const char* pDocName,
                                                                Base::Type typeId,
                                                                ResolveMode resolve,
                                                                bool single) const
{
...
std::vector<SelectionObject> SelectionSingleton::getPickedListEx(const char* pDocName,
                                                                 Base::Type typeId) const
{
    ...
    while (!NotificationQueue.empty()) {
        ...
        switch (msg.Type) {
            case SelectionChanges::AddSelection:
                notify =
                    isSelected(msg.pDocName, msg.pObjectName, msg.pSubName, ResolveMode::NoResolve);
                break;
            case SelectionChanges::RmvSelection:
                notify = !isSelected(msg.pDocName,
                                     msg.pObjectName,
                                     msg.pSubName,
                                                         ...
                                                         const char** subelement)
{
    ...
    if (!obj->isDerivedFrom(typeId)
        && (resolve != ResolveMode::FollowLink
            || !obj->getLinkedObject(true)->isDerivedFrom(typeId))) {
        return nullptr;
    }
...
void SelectionSingleton::slotSelectionChanged(const SelectionChanges& msg)
{
    ...
    if (!msg.Object.getSubName().empty()) {
        ...
        SelectionChanges msg2(msg.Type,
                              pObject->getDocument()->getName(),
                              pObject->getNameInDocument(),
                              !newElementName.empty() ? newElementName.c_str()
...
namespace Gui
{
std::array<std::pair<double, std::string>, 3>
schemaTranslatePoint(double x, double y, double z, double precision)
{
    ...
    std::array<std::pair<double, std::string>, 3> ret = {
        std::make_pair(xuser, xunit.toUtf8().constBegin()),
        ...
        if (!ActiveGate->allow(pObject ? pObject->getDocument() : temp.pDoc, pObject, subelement)) {
            if (getMainWindow()) {
                ...
                if (ActiveGate->notAllowedReason.length() > 0) {
                    ...
                    msg = QCoreApplication::translate("SelectionFilter",
...
SelectionSingleton::selStackGet(const char* pDocName, ResolveMode resolve, int index) const
{
                                       ...
                                       const std::vector<std::string>& pSubNames)
{
    ...
    for (const auto& pSubName : pSubNames) {
        ...
        FC_LOG("Add Selection " << Chng.pDocName << '#' << Chng.pObjectName << '.'
                                << Chng.pSubName);
                                         ...
                                         const char* pSubName)
{
    ...
    if (DocName == pDocName && FeatName == pObjectName && SubName == pSubName) {
        if (show) {
            ...
            notify(
                SelectionChanges(SelectionChanges::SetPreselectSignal, DocName, FeatName, SubName));
        }
        else {
            rmvPreselect();
        }
...
bool SelectionSingleton::addSelection(const SelectionObject& obj, bool clearPreselect)
{
    ...
    if (!subNames.empty() && subNames.size() == points.size()) {
        ...
        for (std::size_t i = 0; i < subNames.size(); i++) {
            ...
            ok &= addSelection(obj.getDocName(),
                               obj.getFeatName(),
                               name.c_str(),
                                      ...
                                      const std::vector<SelObj>* pickedList)
{
    ...
    if (!changes.empty()) {
        for (auto& Chng : changes) {
            FC_LOG("Rmv Selection " << Chng.pDocName << '#' << Chng.pObjectName << '.'
                                    << Chng.pSubName);
            notify(std::move(Chng));
        }
        getMainWindow()->updateActions();
...
void SelectionSingleton::setVisible(VisibleState vis)
{
    ...
    for (auto& sel : sels) {
        ...
        if (!filter.insert(std::make_pair(obj, static_cast<App::DocumentObject*>(nullptr)))
                 .second) {
            continue;
        }
                                      ...
                                      const std::vector<App::DocumentObject*>& sel)
{
    ...
    for (auto obj : sel) {
        ...
        _SelObj temp;
        int ret = checkSelection(pDocName,
...
int SelectionSingleton::checkSelection(const char* pDocName,
                                       ...
                                       ResolveMode resolve,
                                       _SelObj& sel,
                                       const std::list<_SelObj>* selList) const
{
    ...
    if (!sel.pResolvedObject) {
        if (!selList) {
            FC_ERR("Sub-object " << sel.DocName << '#' << sel.FeatName << '.' << sel.SubName
                                 << " not found");
        }
        return -1;
    }
...
PyMethodDef SelectionSingleton::Methods[] = {
    ...
    {"setPreselection",
     reinterpret_cast<PyCFunction>(
         reinterpret_cast<void (*)()>(SelectionSingleton::sSetPreselection)),
...
PyObject* SelectionSingleton::sAddSelection(PyObject* /*self*/, PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args,
                         ...
                         &PyBool_Type,
                         &clearPreselect)) {
        Selection().addSelection(docname,
                                 objname,
                                 subname,
                                 x,
                                 y,
    ...
    PyErr_SetString(PyExc_ValueError,
                    "type must be 'DocumentObject[,subname[,x,y,z]]' or 'DocumentObject, list or "
                    "tuple of subnames'");
...
PyObject* SelectionSingleton::sSetSelectionStyle(PyObject* /*self*/, PyObject* args)
{
    ...
    PY_TRY
    {
        Selection().setSelectionStyle(selStyle == 0 ? SelectionStyle::NormalSelection
...
PyObject* SelectionSingleton::sAddSelectionGate(PyObject* /*self*/, PyObject* args)
{
    ...
    PyErr_SetString(PyExc_ValueError,
                    "Argument is neither string nor SelectionFiler nor SelectionGate");
