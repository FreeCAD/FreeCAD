...
private:
    QString detectOverlayStyleSheetFileName() const
    {
        ...
        QString overlayStyleSheet =
            QString::fromUtf8(handle->GetASCII("OverlayActiveStyleSheet").c_str());
...
const QString OverlayStyleSheet::_default =
    QStringLiteral("overlay:Light Theme + Dark Background.qss");
...
struct OverlayInfo
{
        ...
        , overlayMap(map)
    {
        ...
        tabWidget->getProxyWidget()->setObjectName(tabWidget->objectName()
...
public:
    ...
    void onTimer()
    {
        ...
        if (focus) {
            if (focus->isOverlaid(OverlayTabWidget::QueryOption::TransparencyChanged)
                || updateFocus) {
        ...
        for (auto o : _overlayInfos) {
            if (o->tabWidget != focus && o->tabWidget != active && o->tabWidget != reveal
                && o->tabWidget->count()
                && !o->tabWidget->isOverlaid(
                    OverlayTabWidget::QueryOption::TransparencyNotChanged)) {
                o->tabWidget->setOverlayMode(true);
            }
        }
        ...
        _top.tabWidget->setRect(
            QRect(rectLeft.width() - ofs.width(), ofs.height(), tw, rect.height()));
    }
    ...
    void setOverlayMode(OverlayMode mode)
    {
        switch (mode) {
            ...
            case OverlayManager::OverlayMode::TransparentNone:
                for (auto o : _overlayInfos) {
                    o->tabWidget->setTransparent(mode
                                                 == OverlayManager::OverlayMode::TransparentAll);
                }
                refresh();
                return;
    ...
    void
    dragDockWidget(QPoint pos, QWidget* srcWidget, QPoint dragOffset, QSize dragSize, bool drop)
    {
        if (!getMainWindow()) {
            return;
        }
        ...
        else if (!drop && OverlayTabWidget::_DragFrame
                 && !OverlayTabWidget::_DragFrame->isVisible()) {
...
{
    QObject::connect(dw->toggleViewAction(),
                     &QAction::triggered,
                     this,
    ...
    QObject::connect(dw,
                     ...
                     this,
                     &OverlayManager::onDockVisibleChange);
    QObject::connect(dw,
                     &QDockWidget::featuresChanged,
                     this,
                     &OverlayManager::onDockFeaturesChange);
...
void OverlayManager::onDockVisibleChange(bool visible)
{
    ...
    FC_TRACE("dock " << dock->objectName().toUtf8().constData() << " visible change " << visible
                     << ", " << dock->isVisible());
}
    ...
    switch (ev->type()) {
        ...
        case QEvent::KeyPress: {
            ...
            if (ke->modifiers() == Qt::NoModifier && ke->key() == Qt::Key_Escape) {
                if (d->mouseTransparent) {
                    ...
                    if (auto titleBar =
                            qobject_cast<OverlayTitleBar*>(OverlayTabWidget::_Dragging)) {
                        titleBar->endDrag();
                    }
                    else if (auto splitHandle = qobject_cast<OverlaySplitterHandle*>(
        ...
        case QEvent::MouseMove: {
            ...
            if (OverlayParams::getDockOverlayAutoMouseThrough() && ev->type() != QEvent::Wheel
                ...
                d->wheelDelay =
                    QTime::currentTime().addMSecs(OverlayParams::getDockOverlayWheelDelay());
                d->wheelPos = pos;
                return false;
            }
            else {
                for (auto widget = qApp->widgetAt(pos); widget; widget = widget->parentWidget()) {
                    ...
                    if (ev->type() == QEvent::Wheel) {
                        if (qobject_cast<OverlayTitleBar*>(widget)) {
                            activeTabWidget =
                                qobject_cast<OverlayTabWidget*>(widget->parentWidget());
                        }
                        else if (qobject_cast<OverlaySplitterHandle*>(widget)) {
                            auto parent = widget->parentWidget();
                            if (parent) {
                                activeTabWidget =
                                    qobject_cast<OverlayTabWidget*>(parent->parentWidget());
                            }
                        }
                        if (activeTabWidget) {
                            break;
