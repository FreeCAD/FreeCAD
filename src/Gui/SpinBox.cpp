...
void ExpressionSpinBox::showIcon()
{
    ...
    lineedit->setStyleSheet(QString::fromLatin1("QLineEdit { padding-right: %1px } ")
...
void ExpressionSpinBox::resizeWidget()
{
    ...
    iconLabel->move(lineedit->rect().right() - frameWidth - sz.width(),
                    lineedit->rect().center().y() - sz.height() / 2);
    updateExpression();
}
