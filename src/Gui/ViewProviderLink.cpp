...
#define CHECK_NAME(_name, _end)                                                                    \
    ...
    bool
    getDetail(bool checkname, int type, const char* subname, SoDetail*& det, SoFullPath* path) const
    {
        if (!isLinked()) {
            return false;
        }
    ...
    QIcon getIcon(QPixmap px)
    {
        ...
        if (iconLink.isNull()) {
            ...
            iconLink = QIcon();
            iconLink.addPixmap(
                BitmapFactory().merge(icon.pixmap(iconSize, iconSize, QIcon::Normal, QIcon::Off),
                                      px,
                                      BitmapFactoryInst::BottomLeft),
                QIcon::Normal,
                QIcon::Off);
            iconLink.addPixmap(
                BitmapFactory().merge(icon.pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On),
                                      px,
                                      BitmapFactoryInst::BottomLeft),
                QIcon::Normal,
                QIcon::On);
...
void LinkView::setTransform(SoTransform* pcTransform, const Base::Matrix4D& mat)
{
    ...
    pcTransform->setMatrix(SbMatrix(dMtrx[0],
                                    dMtrx[1],
                                    dMtrx[2],
                                    dMtrx[3],
                                    dMtrx[4],
    ...
    for (const auto& v : subInfo) {
        ...
        if (!sub.subElements.empty()) {
            ...
            for (const auto& subelement : sub.subElements) {
                ...
                if (!sub.linkInfo
                         ->getDetail(false, SnapshotTransform, subelement.c_str(), det, &path)) {
                    continue;
                }
        ...
        if (subname[0] >= '0' && subname[0] <= '9') {
            ...
            while (true) {
                ...
                if (subname[0] == '$') {
                    ...
                    for (const auto& info : nodeArray) {
                        if (info->isLinked()
                            && boost::equals(name, info->linkInfo->getLinkedName())) {
                            idx = i;
                            break;
                        }
                        ++i;
    ...
    , overlayCacheKey(0)
{
    ...
    ADD_PROPERTY_TYPE(OverrideMaterial,
                      (false),
                      " Link",
                      App::Prop_None,
...
void ViewProviderLink::onChanged(const App::Property* prop)
{
    if (prop == &ChildViewProvider) {
        childVp =
            freecad_dynamic_cast<ViewProviderDocumentObject>(ChildViewProvider.getObject().get());
        if (childVp && getObject()) {
            if (strcmp(childVp->getTypeId().getName(), getObject()->getViewProviderName()) != 0
                ...
                if (pcModeSwitch->getNumChildren() > 1) {
                    childVpLink = LinkInfo::get(childVp, nullptr);
                    pcModeSwitch->replaceChild(
                        1,
                        childVpLink->getSnapshot(LinkView::SnapshotTransform));
                }
            }
        }
    }
...
const App::LinkBaseExtension* ViewProviderLink::getLinkExtension() const
{
    ...
    return const_cast<App::DocumentObject*>(pcObject)->getExtensionByType<App::LinkBaseExtension>(
        true);
}
...
void ViewProviderLink::updateDataPrivate(App::LinkBaseExtension* ext, const App::Property* prop)
{
    ...
    if (prop == &ext->_ChildCache) {
        ...
        if (!ext->_getShowElementValue()) {
            ...
            auto linked =
                freecad_dynamic_cast<ViewProviderDocumentObject>(getLinkedView(true, ext));
...
void ViewProviderLink::updateElementList(App::LinkBaseExtension* ext)
{
    ...
    if (OverrideMaterialList.getSize() || MaterialList.getSize()) {
        ...
        for (auto obj : elements) {
            ++i;
            auto vp =
                freecad_dynamic_cast<ViewProviderLink>(Application::Instance->getViewProvider(obj));
            if (!vp) {
                continue;
            }
...
bool ViewProviderLink::getDetailPath(const char* subname,
                                     SoFullPath* pPath,
                                     bool append,
                                     SoDetail*& det) const
{
...
void ViewProviderLink::setupContextMenu(QMenu* menu, QObject* receiver, const char* member)
{
    ...
    if (ext->isLinkedToConfigurableObject()) {
        ...
        if (src && !ext->getOnChangeCopyObjects(nullptr, src).empty()) {
            ...
            act->setToolTip(
                QObject::tr("Select which object to copy or exclude when configuration changes. "
        ...
        if (ext->getLinkCopyOnChangeValue() == 0) {
            ...
            act->setToolTip(
                QObject::tr("Copy the linked object when its configuration is changed.\n"
    ...
    if (ext->isLinkMutated()) {
        ...
        act->setToolTip(QObject::tr("Synchronize the original configurable source object by\n"
...
void ViewProviderLink::_setupContextMenu(App::LinkBaseExtension* ext,
                                         QMenu* menu,
                                         QObject* receiver,
                                         const char* member)
{
    ...
    if (ext->getLinkedObjectProperty() && ext->_getShowElementProperty()
        ...
        action->setToolTip(
            QObject::tr("Change whether show each link array element as individual objects"));
    }
                ...
                if (ret.isTuple()) {
                    ...
                    if (!PyArg_ParseTuple(ret.ptr(),
                        ...
                        FC_ERR("initDraggingPlacement() expects return of type "
                               "tuple(matrix,placement,boundbox)");
                        return false;
                    }
...
void ViewProviderLink::setEditViewer(Gui::View3DInventorViewer* viewer, int ModNum)
{
    ...
    if (pcDragger && viewer) {
        ...
        if (useCenterballDragger) {
            ...
            auto length = std::max(std::max(dragCtx->bbox.LengthX(), dragCtx->bbox.LengthY()),
                                   dragCtx->bbox.LengthZ());
            cube->width = length;
            cube->height = length;
            cube->depth = length;
...
ViewProviderDocumentObject* ViewProviderLink::getLinkedViewProvider(std::string* subname,
