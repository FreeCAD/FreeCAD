...
void AutoSaver::saveDocument(const std::string& name, AutoSaveProperty& saver)
{
    ...
    if (doc && !doc->testStatus(App::Document::PartialDoc)
        ...
        getMainWindow()->showMessage(
            tr("Please wait until the AutoRecovery file has been saved..."),
            5000);
        // qApp->processEvents();
...
{
public:
    RecoveryRunnable(const std::set<std::string>& modes,
                     const char* dir,
                     const char* file,
    ...
    void run() override
    {
        ...
        QMetaObject::invokeMethod(AutoSaver::instance(),
                                  "renameFile",
                                  Qt::QueuedConnection,
    ...
    while (index < FileList.size()) {
        ...
        if (shouldWrite(entry.FileName, entry.Object)) {
            ...
            if (entry.Object->isDerivedFrom(App::Property::getClassTypeId())) {
                ...
                QThreadPool::globalInstance()->start(new RecoveryRunnable(getModes(),
                                                                          DirName.c_str(),
