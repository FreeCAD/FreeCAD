    ...
    , wbActionGroup(aGroup)
{
    ...
    connect(aGroup,
            ...
            this,
            &WorkbenchTabWidget::updateWorkbenchList);
    connect(aGroup->groupAction(),
...
void WorkbenchTabWidget::updateLayout()
{
    ...
    if (auto toolBar = qobject_cast<QToolBar*>(parentWidget())) {
        ...
        if (toolBar->isFloating()) {
            setToolBarArea(toolBar->orientation() == Qt::Horizontal
                               ? Gui::ToolBarArea::TopToolBarArea
    ...
    tabBar->setSelectionBehaviorOnRemove(direction() == Qt::LeftToRight ? QTabBar::SelectLeftTab
...
int WorkbenchTabWidget::addWorkbenchTab(QAction* action, int tabIndex)
{
    ...
    // if tabIndex is negative we assume that tab must be placed at the end of tabBar (default
    // behavior)
    if (tabIndex < 0) {
...
void WorkbenchTabWidget::setToolBarArea(Gui::ToolBarArea area)
{
    switch (area) {
        ...
        case Gui::ToolBarArea::RightToolBarArea: {
            ...
            layout->setDirection(direction() == Qt::LeftToRight ? QBoxLayout::TopToBottom
                                                                : QBoxLayout::BottomToTop);
            tabBar->setShape(area == Gui::ToolBarArea::LeftToolBarArea ? QTabBar::RoundedWest
                                                                       : QTabBar::RoundedEast);
            break;
        }
        ...
        case Gui::ToolBarArea::StatusBarToolBarArea: {
            ...
            layout->setDirection(direction() == Qt::LeftToRight ? QBoxLayout::LeftToRight
