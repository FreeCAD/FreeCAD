...
ActionGroup::ActionGroup(const QPixmap& icon,
                         const QString& title,
                         bool expandable,
                         QWidget* parent)
    : QWidget(parent)
    ...
    if (myDummy->isVisible()) {
        if (myScheme->groupFoldThaw) {
            if (m_foldDirection < 0) {
                ...
                p.setOpacity((double)(myScheme->groupFoldSteps - m_foldStep)
                             / myScheme->groupFoldSteps);
            }
        }
        ...
        switch (myScheme->groupFoldEffect) {
            ...
            case ActionPanelScheme::SlideFolding:
                p.drawPixmap(myDummy->pos(),
                             m_foldPixmap,
                             QRect(0,
                                   m_foldPixmap.height() - myDummy->height(),
                                   m_foldPixmap.width(),
                                   myDummy->width()));
                break;
