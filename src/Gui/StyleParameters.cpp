...
struct FunctionCall: public Expr
{
    ...
    Value evaluate(const EvaluationContext& context) const override
    {
        ...
        throw std::runtime_error(fmt::format("Unknown function '{}'", functionName));
    }
};
...
BuiltInParameterSource::BuiltInParameterSource(const Metadata& metadata) : ParameterSource(metadata)
{
...
{
    if (auto value = hGrp->GetASCII(name.c_str(), ""); value != "") {
        return Parameter {
            .name = name,
            .value = value,
        };
...
std::vector<ParameterSource*> ParameterManager::sources() const
{
    return _sources;
}
