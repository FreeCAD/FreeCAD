    ...
    if (!nonIntrusive) {
        ...
        if constexpr (content == Base::ContentType::Translated) {
            ...
            if constexpr (
                ...
                Base::Console().send<type, recipient, content>(notifier->getFullNameLabel(), msg.toUtf8());
            }
            else if constexpr (
                std::is_base_of_v<
                    Gui::ViewProviderDocumentObject,
            ...
            if constexpr (
                ...
                Base::Console().send<type, recipient, content>(notifier->getFullNameLabel(), msg.c_str());
            }
            else if constexpr (
                std::is_base_of_v<
                    Gui::ViewProviderDocumentObject,
