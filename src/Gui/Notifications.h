    ...
    if (!nonIntrusive) {
        ...
        if constexpr (recipient == Base::IntendedRecipient::All) {
            ...
            if constexpr (std::is_base_of_v<
                ...
                Base::Console()
                    .Send<type,
                          Base::IntendedRecipient::Developer,
                          Base::ContentType::Untranslated>(notifier->getObject()->getFullLabel(),
                                                           msg.c_str());
            }
            else if constexpr (std::is_base_of_v<
                                   Gui::Document,
                ...
                Base::Console()
                    .Send<type,
                          Base::IntendedRecipient::Developer,
                          Base::ContentType::Untranslated>(notifier->Label.getStrValue(),
                                                           msg.c_str());
            }
            else {
                Base::Console()
        ...
        if constexpr (content == Base::ContentType::Translated) {
            ...
            if constexpr (std::is_base_of_v<
                              App::DocumentObject,
                              std::remove_pointer_t<typename std::decay<TNotifier>::type>>) {
                Base::Console().Send<type, recipient, content>(notifier->getFullLabel(),
                                                               msg.toUtf8());
            }
            else if constexpr (std::is_base_of_v<
                                   Gui::ViewProviderDocumentObject,
                ...
                Base::Console().Send<type, recipient, content>(
                    ...
                    msg.toUtf8());
                                   ...
                                   App::Document,
                                   std::remove_pointer_t<typename std::decay<TNotifier>::type>>) {
                Base::Console().Send<type, recipient, content>(notifier->Label.getStrValue(),
                                                               msg.toUtf8());
            }
            else {
...
template<typename TNotifier, typename TCaption, typename TMessage>
inline void
Gui::TranslatedNotification(TNotifier&& notifier, TCaption&& caption, TMessage&& message)
{
