...
QImage colorPreview(const QColor& color)
{
...
{
    explicit GroupItem(const QString& title, bool canAddNewParameters = false)
        : title(title)
        , canAddNewParameters(canAddNewParameters)
    {}
...
{
public:
    Node(std::unique_ptr<Item> data, Node* parent = nullptr)
        : _parent(parent)
        , _data(std::move(data))
    {}
...
public:
                          ...
                          const QModelIndex& index) const override
    {
        ...
        if (model->item<StyleParametersModel::ParameterItem>(index) && index.column() == StyleParametersModel::ParameterExpression) {
            return new QLineEdit(parent);
        }

        if (model->isAddPlaceholder(index) && index.column() == StyleParametersModel::ParameterName) {
            return new QLineEdit(parent);
        }
    ...
    QSize sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const override
    {
        ...
        return QSize(base.width(), std::max(base.height(), 36));
    }
               ...
               const QModelIndex& index) const override
    {
        ...
        if (model->isAddPlaceholder(index)) {
            ...
            painter->fillRect(option.rect, QColor("#a3a8ad"));
...
StyleParametersModel::StyleParametersModel(StyleParameters::ParameterManager* manager,
                                           QObject* parent)
...
void StyleParametersModel::refresh()
{
    beginResetModel();
    root = std::make_unique<Node>(std::make_unique<GroupItem>(tr("Root")));

    auto userParameters =
        std::make_unique<Node>(std::make_unique<GroupItem>(tr("User Parameters"), true));
    auto themeParameters =
        std::make_unique<Node>(std::make_unique<GroupItem>(tr("Theme Parameters"), true));
    ...
    for (const auto& name : manager->parameters()) {
        ...
        switch (parameter.source) {
            case StyleParameters::ParameterSource::Predefined:
                item->data<ParameterItem>()->flags = Qt::ItemIsEnabled | Qt::ItemIsSelectable;
                builtinParameters->appendChild(std::move(item));
                break;
            case StyleParameters::ParameterSource::Theme:
                themeParameters->appendChild(std::move(item));
                break;
            case StyleParameters::ParameterSource::User:
                userParameters->appendChild(std::move(item));
                break;
        }
    }

    root->appendChild(std::move(userParameters));
    root->appendChild(std::move(themeParameters));
    root->appendChild(std::move(builtinParameters));
    endResetModel();
}
...
int StyleParametersModel::columnCount(const QModelIndex& index) const
{
    return ColumnCount;
}
...
bool StyleParametersModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    if (auto parameterItem = item<ParameterItem>(index)) {
        if (index.column() == ParameterName) {
    ...
    walk([this](const QModelIndex& index) {
...
void StyleParametersModel::walk(std::function<void(const QModelIndex&)> func,
                                const QModelIndex& index) const
{
    ...
    for (int i = 0; i < rowCount(index); ++i) {
        walk(func, this->index(i, 0, index));
    }
}
...
DlgThemeEditor::DlgThemeEditor(QWidget* parent)
    ...
    , model(std::make_unique<StyleParametersModel>(manager.get(), this))
{
    ui->setupUi(this);
    ...
    static const std::initializer_list<ColumnDefinition> columnSizingDefinitions = {
        ...
        {StyleParametersModel::ParameterType, QHeaderView::ResizeMode::Fixed, 80},
    };
    ...
    ui->tokensTreeView->setColumnWidth(StyleParametersModel::ParameterName, 200);
    ui->tokensTreeView->expandAll();

    connect(ui->closeButton, &QPushButton::clicked, this, &QDialog::accept);

    connect(ui->tokensTreeView, &QTreeView::clicked, this, [this](const QModelIndex& index) {
        loadDataFromIndex(index);
    });

    connect(model.get(), &StyleParametersModel::newParameterAdded, this, [this](const QModelIndex& index) {
        const auto newParameterExpressionIndex = index.siblingAtColumn(StyleParametersModel::ParameterExpression);
    ...
    connect(ui->expressionLineEdit, &QLineEdit::textChanged, [this](const QString& text) {
        ui->resultDecoration->setPixmap(QPixmap());
        ui->resultDecoration->hide();

        try {
            auto value = manager->evaluate(text.toStdString());
            ui->resultLabel->setText(tr("Result (%1)").arg(typeOfTokenValue(value)));
            ui->resultOutput->setText(QString::fromStdString(value.toString()));

            if (auto* color = std::get_if<QColor>(&value)) {
                ui->resultDecoration->show();
                ui->resultDecoration->setPixmap(QPixmap::fromImage(colorPreview(*color)));
            }
        }
        catch (const std::exception& e) {
            ui->resultLabel->setText(tr("Result (%1)").arg(tr("Error")));
            ui->resultOutput->setText(QStringLiteral("%1").arg(QString::fromStdString(e.what())));
        }
    });
}
...
void DlgThemeEditor::loadDataFromIndex(const QModelIndex& index)
{
    auto item = model->item<StyleParametersModel::ParameterItem>(index);
    if (!item) {
        return;
    }

    const auto& [name, token, _] = *item;

    ui->expressionLineEdit->setText(QString::fromStdString(token.value));
}
