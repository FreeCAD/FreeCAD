...
/**
 * @brief Appends each gradient stop's packed ARGB to a cache key, in stop order.
 *
 * Value::toString() renders a stop's color as "#rrggbb" and drops alpha, so two
 * gradients that differ only in stop transparency would otherwise produce the same
 * string and collide in the gradient cache. This fills that gap.
 */
QString gradientStopsFingerprint(const Gui::StyleParameters::Tuple& tuple)
{
    using namespace Gui::StyleParameters;

    const Value value {tuple};
    ...
    else if (tuple.kind == TupleKind::RadialGradient) {
    ...
    for (const auto& stop : stops) {
        fingerprint += QStringLiteral("%1").arg(stop.color.getPackedValue(), 8, 16, QLatin1Char('0'));
    }
    return fingerprint;
}
...
QPixmap cachedGradientPreview(const Gui::StyleParameters::Tuple& tuple)
{
    ...
    const QString key = QString::fromStdString(Gui::StyleParameters::Value(tuple).toString())
        + gradientStopsFingerprint(tuple);
    if (const auto found = cache.constFind(key); found != cache.constEnd()) {
        return *found;
    }
