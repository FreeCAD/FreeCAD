...
{
    explicit GroupItem(const QString& title, ParameterSource* source)
        : title(title)
        , source(source)
...
public:
                          ...
                          const QModelIndex& index) const override
    {
        ...
        if (model->item<StyleParametersModel::ParameterItem>(index)
            && (index.column() == StyleParametersModel::ParameterExpression || index.column() == StyleParametersModel::ParameterName)) {
            return new QLineEdit(parent);
        }

        if (model->isAddPlaceholder(index)
            && index.column() == StyleParametersModel::ParameterName) {
            return new QLineEdit(parent);
        }
...
StyleParametersModel::StyleParametersModel(const std::list<StyleParameters::ParameterSource*>& sources,
                                           QObject* parent)
...
void StyleParametersModel::remove(const QModelIndex& index)
{
    if (auto parameterItem = item<ParameterItem>(index)) {
...
DlgThemeEditor::DlgThemeEditor(QWidget* parent)
    ...
    , model(std::make_unique<StyleParametersModel>(Base::provideServiceImplementations<StyleParameters::ParameterSource>(), this))
{
    ui->setupUi(this);
    ...
    connect(ui->tokensTreeView, &TokenTreeView::requestRemove, model.get(), qOverload<const QModelIndex&>(&StyleParametersModel::remove));
