...
void DlgDocumentObject::init(
    App::DocumentObject* owner,
    bool singleSelect,
    bool filterOwner,
    bool filterTypeOwner,
    bool expandTypeOwner
)
{
    ...
    if (filterTypeOwner) {
        ...
        if (!objType.isBad()) {
            const auto name = objType.getName();
...
QTreeWidgetItem* DlgDocumentObject::findItem(App::DocumentObject* obj, const char* subname, bool* pfound)
{
    if (pfound) {
        *pfound = false;
    }
    ...
    for (auto o : sobjs) {
        ...
        for (int i = 0, count = item->childCount(); i < count; ++i) {
            ...
            if (strcmp(o->getNameInDocument(), child->data(0, ObjectNameRole).toByteArray().constData())
                == 0) {
                item = child;
                found = true;
                break;
...
static QTreeWidgetItem* _getSubObjectFromItem(
...
QList<App::SubObjectT> DlgDocumentObject::getSubObjectFromItem(QTreeWidgetItem* item, bool needSubName) const
{
    ...
    App::SubObjectT sobj(
        parentItem->data(0, DocNameRole).toByteArray().constData(),
...
{
    if (filterOwner
        && strcmp(item->data(0, ObjectNameRole).toByteArray().constData(), owner->getNameInDocument())
            == 0
        && strcmp(item->data(0, DocNameRole).toByteArray().constData(), owner->getDocument()->getName())
            == 0) {
        item->setHidden(true);
        return;
    }
