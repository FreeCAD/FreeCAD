    ...
    , fontMetrics(font())
{
    ...
    const int height = qRound(pixmap.height() / dpr);
    const int width = qRound(pixmap.width() / dpr);
    const int margin = height / 50;
    const int available = height - 2 * margin - fontMetrics.height();
    startX = width / 50;
    maxWidth = width - 2 * startX;
    ...
    if ((al != cfg.end()) && al->second.starts_with("Top")) {
        startY = margin + fontMetrics.ascent();
    }
    else {
        lineSpacing = -lineSpacing;
        startY = height - margin - fontMetrics.descent();
    }

    // keep the messages within a quarter of the splash screen and inside the pixmap
    lineCount = std::max(3, std::min(height / 4, available) / fontMetrics.lineSpacing() + 1);
...
{
    lines.push_front(fontMetrics.elidedText(msg, Qt::ElideRight, maxWidth));
