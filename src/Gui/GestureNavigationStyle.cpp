    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        switch (ns.getViewingMode()) {
            case NavigationStyle::SEEK_WAIT_MODE: {
                if (ev.isPress(1)) {
                    ns.seekToPoint(ev.inventor_event
                                       ->getPosition());  // implicitly calls interactiveCountInc()
...
public:
    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        bool long_click =
            (ev.inventor_event->getTime() - this->since).getValue() * 1000.0 >= this->hold_timeout;
...
public:
    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        if (ev.isLocation2Event()) {
            ...
            auto& ns = this->outermost_context().ns;
            ns.spin_simplified(ns.viewer->getSoRenderManager()->getCamera(),
...
public:
    ...
    {
        auto& ns = this->outermost_context().ns;
        // a workaround for Qt not always sending release evends during touchecreen gestures on
        // Windows
...
const char* GestureNavigationStyle::mouseButtons(ViewerMode mode)
{
    switch (mode) {
        ...
        case NavigationStyle::DRAGGING:
            return QT_TR_NOOP("Drag screen with one finger OR press left mouse button. In Sketcher "
                              "and other edit modes, hold Alt in addition.");
        case NavigationStyle::ZOOMING:
            return QT_TR_NOOP(
                "Pinch (place two fingers on the screen and drag them apart from or towards each "
                "other) OR scroll middle mouse button OR PgUp/PgDown on keyboard.");
        default:
...
void GestureNavigationStyle::onRollGesture(int direction)
{
    ...
    try {
        ...
        Base::Console().Error("GestureNavigationStyle::onRollGesture: unknown C++ exception when "
                              "invoking command %s\n",
                              cmd.c_str());
    }
}
