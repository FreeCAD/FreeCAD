...
void DlgSettingsUI::loadThemeDefaults()
{
    ...
    try {
        ...
        auto userThemesGroup = App::GetApplication().GetParameterGroupByPath(
            "User parameter:BaseApp/Preferences/Themes"
        );

        // list of theme accent color parameters to copy from theme
        std::vector<std::string> accentColors
        ...
        Base::Console().message(
            "Theme '%s' does not define accent colors, using defaults\n",
            themeName.c_str()
        );
#endif
    }
    try {
        ...
        auto userMainWindowGroup = App::GetApplication().GetParameterGroupByPath(
            "User parameter:BaseApp/Preferences/MainWindow"
        );
...
#ifdef FC_DEBUG
        Base::Console().message(
            "Theme '%s' does not define stylesheet settings, using defaults\n",
            themeName.c_str()
        );
#endif
    }
...
{
    // get parameter group
    ParameterGrp::handle hThemes = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Themes"
    );
