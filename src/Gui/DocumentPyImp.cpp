...
PyObject* DocumentPy::setEdit(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "s|is", &psFeatStr, &mod, &subname)) {
        ...
        if (!obj) {
            PyErr_Format(Base::PyExc_FC_GeneralError,
                         "No such object found in document: '%s'",
                         psFeatStr);
            return nullptr;
        }
    }
...
PyObject* DocumentPy::toggleTreeItem(PyObject* args)
{
    ...
    switch (mod) {
        case 0:
            getDocumentPtr()->signalExpandObject(*ActiveVp,
                                                 TreeItemMode::ToggleItem,
                                                 parent,
                                                 subname);
            break;
        case 1:
...
void DocumentPy::setInEditInfo(Py::Object arg)
{
    PyObject* pyobj;
    const char* subname;
    if (!PyArg_ParseTuple(arg.ptr(),
                          "O!s",
                          &Gui::ViewProviderDocumentObjectPy::Type,
                          &pyobj,
                          &subname)) {
        throw Py::Exception();
    }
