...
void adjustDialogPosition(QDialog* dialog) {
    ...
    const QPoint initialDialogPos = dialog->mapToGlobal(QPoint(0, 0));
    const QRect dialogRect = QRect(initialDialogPos, dialog->frameGeometry().size());
    ...
    const int margin = 5;
    if (dialogRect.intersects(mainWindowRect) && (!mainWindowRect.contains(dialogRect.topLeft()) || !mainWindowRect.contains(dialogRect.bottomRight()))) {
