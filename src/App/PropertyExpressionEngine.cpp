...
const boost::any PropertyExpressionEngine::getPathValue(const App::ObjectIdentifier & path) const
{
    ...
    auto i = std::as_const(expressions).find(usePath);

    if (i != expressions.cend())
        return i->second;
    else
        return boost::any();
}
...
void PropertyExpressionEngine::renameExpressions(const std::map<ObjectIdentifier, ObjectIdentifier> & paths)
{
    ...
    for (const auto & expression : expressions) {
        std::map<ObjectIdentifier, ObjectIdentifier>::const_iterator j = canonicalPaths.find(expression.first);
    ...
    for (const auto & expression : expressions)
        expressionChanged(expression.first);
