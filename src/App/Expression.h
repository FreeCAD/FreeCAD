...
// Map of depending objects to a map of depending property name to the full referencing object
// identifier
using ExpressionDeps =
    std::map<App::DocumentObject*, std::map<std::string, std::vector<ObjectIdentifier>>>;
...
protected:
    ...
    void collectReplacement(Expression& e,
                            std::map<ObjectIdentifier, ObjectIdentifier>&,
...
template<class P>
class ExpressionModifier: public ExpressionVisitor
{
public:
    ...
    App::PropertyLinkBase* getPropertyLink() override
    {
        return propLink;
    }
...
public:
    ...
    getDepObjects(std::vector<std::string>* labels = nullptr) const;
    void getDepObjects(std::map<App::DocumentObject*, bool>&,
                       std::vector<std::string>* labels = nullptr) const;
    ...
    static Component* createComponent(Expression* e1,
...
protected:
    App::DocumentObject*
        owner; /**< The document object used to access unqualified variables (i.e local scope) */
