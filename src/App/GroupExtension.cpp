...
{
    std::set<DocumentObject*> objectsPendingRemoval;

    while (Group.getSize() > 0 &&
           objectsPendingRemoval.size() != static_cast<size_t>(Group.getSize())) {
        ...
        for (auto docObject : grp) {
            objectsPendingRemoval.insert(docObject);
        }
    }
}
