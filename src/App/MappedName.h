...
public:
    ...
    explicit MappedName(std::string_view name)
    {
        ...
        data = Base::ByteBuffer::copy(Base::asBytes(name.data(), name.size()));
    }
    ...
    explicit MappedName(const IndexedName& element)
        : data(Base::ByteBuffer::borrow(
              Base::asBytes(element.getType(), std::strlen(element.getType()))
          ))
        , raw(true)
    {
        if (element.getIndex() > 0) {
            const std::string index = std::to_string(element.getIndex());
            this->data.append(Base::asBytes(index));
            this->raw = false;
        }
    }
    ...
    {
        if (postfix) {
            this->postfix = Base::ByteBuffer::copy(Base::asBytes(postfix));
        }
        this->raw = false;
    }
    ...
    static MappedName fromRawData(const char* name, int size = -1)
    {
        ...
        if (name) {
            ...
            res.data = Base::ByteBuffer::borrow(
                Base::asBytes(name, static_cast<std::size_t>(size))
            );
            res.raw = true;
        }
        return res;
    ...
    {
        return fromRawData(reinterpret_cast<const char*>(bytes.data()),
    ...
    static MappedName fromRawData(const MappedName& other, int startPosition, int size = -1)
    {
        ...
        if (size < dataSize - startPosition) {
            res.data = Base::ByteBuffer::borrow(
                Base::asBytes(other.data.data() + startPosition, static_cast<std::size_t>(size))
            );
        }
        else {
            res.data = Base::ByteBuffer::borrow(
                Base::asBytes(other.data.data() + startPosition,
            ...
            if (size == postfixSize) {
                ...
                res.postfix = Base::ByteBuffer::borrow(
                    Base::asBytes(other.postfix.data(), static_cast<std::size_t>(size))
                );
            }
        }
        return res;
    ...
    {
        stream.write(reinterpret_cast<const char*>(mappedName.data.data()),
                     static_cast<std::streamsize>(mappedName.data.size()));
        stream.write(reinterpret_cast<const char*>(mappedName.postfix.data()),
    ...
    bool operator==(const MappedName& other) const
    {
        ...
        Base::BytesView tmp = Base::asBytes(
        ...
        Base::BytesView remaining = Base::asBytes(
            smaller.postfix.data() + tmp.size(),
            smaller.postfix.size() - tmp.size()
        );

        return Base::equalBytes(remaining, larger.postfix.view());
    }
    ...
    {
        if (other && (other[0] != 0)) {
            this->postfix.append(Base::asBytes(other));
        }
        return *this;
    }
        ...
        if (dataToAppend && (size != 0)) {
            ...
            if (empty()) {
                this->data = Base::ByteBuffer::copy(
                    Base::asBytes(dataToAppend, static_cast<std::size_t>(size))
                );
                this->raw = false;
            }
            else {
                this->postfix.append(Base::asBytes(dataToAppend, static_cast<std::size_t>(size)));
            }
        }
    }
    ...
    void append(const MappedName& other, int startPosition = 0, int size = -1)
    {
        ...
        if (startPosition < otherDataSize)  // if starting inside data
        {
            ...
            if (startPosition == 0 && count == otherDataSize && this->empty()) {
                ...
                append(reinterpret_cast<const char*>(other.data.data() + startPosition), count);
            }
        ...
        if (size != 0) {
            if (startPosition == 0 && size == otherPostfixSize) {
                ...
                append(reinterpret_cast<const char*>(other.postfix.data() + startPosition), size);
            }
        }
    }
    ...
    const char* appendToBuffer(std::string& buffer, int startPosition = 0, int len = -1) const
    {
        ...
        if (startPosition < dataSize) {
            ...
            buffer.append(reinterpret_cast<const char*>(this->data.data() + startPosition), count);
            len -= count;
        }
        buffer.append(reinterpret_cast<const char*>(this->postfix.data()), len);
    ...
    const char* toConstString(int offset, int& size) const
    {
        ...
        if (offset > dataSize) {
            ...
            return reinterpret_cast<const char*>(this->postfix.data() + offset);
        }
        size = dataSize - offset;
        return reinterpret_cast<const char*>(this->data.data() + offset);
    }
    ...
    Base::ByteBuffer toRawBytes(int offset = 0, int size = -1) const
    {
        ...
        if (offset >= dataSize) {
            ...
            return Base::ByteBuffer::borrow(
                Base::asBytes(this->postfix.data() + offset, static_cast<std::size_t>(size))
            );
        }
        if (size <= dataSize - offset) {
            return Base::ByteBuffer::borrow(
                Base::asBytes(this->data.data() + offset, static_cast<std::size_t>(size))
            );
        }

        Base::ByteBuffer res = Base::ByteBuffer::copy(
            Base::asBytes(this->data.data() + offset,
                            static_cast<std::size_t>(dataSize - offset))
        );
        const int remaining = size - (dataSize - offset);
        res.append(Base::asBytes(this->postfix.data(), static_cast<std::size_t>(remaining)));
        return res;
    }
    ...
    {
        return reinterpret_cast<const char*>(this->postfix.data());
    }
    ...
    {
        if (this->postfix.empty()) {
            return IndexedName(Base::asBytes(this->data.data(), this->data.size()));
        }
        return IndexedName();
    }
    ...
    char operator[](int index) const
    {
        ...
        if (idx >= dataSize) {
            if (postfixSize == 0U) {
                return std::to_integer<char>(this->data.data()[dataSize - 1U]);
            }
            idx -= dataSize;
            if (idx >= postfixSize) {
                idx = postfixSize - 1U;
            }
            return std::to_integer<char>(this->postfix.data()[idx]);
        }
        return std::to_integer<char>(this->data.data()[idx]);
    }
    ...
    int find(Base::BytesView searchTarget, int startPosition = 0) const
    {
        ...
        if (static_cast<std::size_t>(startPosition) < dataSize) {
            const std::size_t found = Base::findBytes(
                this->data.view(), searchTarget, static_cast<std::size_t>(startPosition));
            if (found != Base::bytesNpos) {
        ...
        const std::size_t found = Base::findBytes(
            this->postfix.view(), searchTarget, static_cast<std::size_t>(startPosition));
        if (found == Base::bytesNpos) {
            return -1;
        }
    ...
    int find(const char* searchTarget, int startPosition = 0) const
    {
        ...
        return find(Base::asBytes(searchTarget), startPosition);
    }

    /// @copydoc find(Base::BytesView,int) const
    int find(const std::string& searchTarget, int startPosition = 0) const
    {
        return find(Base::asBytes(searchTarget.data(), searchTarget.size()), startPosition);
    }
    ...
    int rfind(Base::BytesView searchTarget, int startPosition = -1) const
    {
        ...
        if (static_cast<std::size_t>(startPosition) >= dataSize) {
            ...
            const std::size_t found = Base::rfindBytes(
                this->postfix.view(), searchTarget, std::min(from, this->postfix.size() - 1U));
            if (found != Base::bytesNpos) {
                return static_cast<int>(found + dataSize);
            }
            if (dataSize == 0U) {
        ...
        const std::size_t found = Base::rfindBytes(this->data.view(), searchTarget, from);
        if (found == Base::bytesNpos) {
            return -1;
        }
        return static_cast<int>(found);
    ...
    int rfind(const char* searchTarget, int startPosition = -1) const
    {
        ...
        return rfind(Base::asBytes(searchTarget), startPosition);
    }

    /// @copydoc rfind(Base::BytesView,int) const
    int rfind(const std::string& searchTarget, int startPosition = -1) const
    {
        return rfind(Base::asBytes(searchTarget.data(), searchTarget.size()), startPosition);
    }
    ...
    bool endsWith(const char* searchTarget) const
    {
        ...
        return endsWith(Base::asBytes(searchTarget));
    }

    /// @copydoc endsWith(Base::BytesView) const
    bool endsWith(const std::string& searchTarget) const
    {
        return endsWith(Base::asBytes(searchTarget.data(), searchTarget.size()));
    }
    ...
    bool startsWith(Base::BytesView searchTarget, int offset = 0) const
    {
        ...
        if ((offset != 0) || (!this->data.empty() && this->data.size() < searchTarget.size())) {
            return Base::equalBytes(
                toRawBytes(offset, static_cast<int>(searchTarget.size())).view(), searchTarget);
        }
        if (!this->data.empty()) {
    ...
    bool startsWith(const char* searchTarget, int offset = 0) const
    {
        ...
        return startsWith(Base::asBytes(searchTarget), offset);
    }

    /// @copydoc startsWith(Base::BytesView,int) const
    bool startsWith(const std::string& searchTarget, int offset = 0) const
    {
        return startsWith(Base::asBytes(searchTarget.data(), searchTarget.size()), offset);
    }
...
private:
    static bool bytesStartsWith(Base::BytesView value, Base::BytesView prefix)
    {
        ...
        return Base::startsWithBytes(value, prefix);
    }
    ...
    static bool bytesEndsWith(Base::BytesView value, Base::BytesView suffix)
    {
        ...
        return Base::endsWithBytes(value, suffix);
    }
