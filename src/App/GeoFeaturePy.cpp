...
// This File is normaly build as an include in GeoFeaturePyImp.cpp! Its not intended to be in a project!
...
PyMethodDef GeoFeaturePy::Methods[] = {
    {"getPaths",
        (PyCFunction) staticCallback_getPaths,
        METH_VARARGS,
        "returns all posible paths to the root of the document"
    },
...
//--------------------------------------------------------------------------
PyObject *GeoFeaturePy::_getattr(char *attr)				// __getattr__ function: note only need to handle new state
{
    try {
...
int GeoFeaturePy::_setattr(char *attr, PyObject *value) // __setattr__ function: note only need to handle new state
{
    try {
