...
void ExtensionContainer::Restore(Base::DocumentReader& reader,
...
void ExtensionContainer::restoreExtensions(Base::DocumentReader& reader,
                                           XERCES_CPP_NAMESPACE_QUALIFIER DOMElement* containerEl)
{
...
void ExtensionContainer::readExtension(Base::DocumentReader& reader,
    ...
    try {
        ...
        if (!ext) {
            ...
            if (!ext->isPythonExtension()) {
                delete ext;
                std::stringstream str;
                str << "Extension is not a python addable version: '" << type_cstr << "'"
                    << std::ends;
                throw Base::TypeError(str.str());
            }
...
void ExtensionContainer::handleChangedPropertyName(Base::DocumentReader& reader,
                                                   const char* TypeName,
                                                   const char* PropName)
{
    // inform all extensions about changed property name. This includes all properties from the
    ...
    for (const auto& entry : _extensions) {
        ...
        if (handled) {
            // one property change needs only be handled once
            return;
        }
    }
...
void ExtensionContainer::handleChangedPropertyType(Base::DocumentReader& reader,
                                                   const char* TypeName,
                                                   Property* prop)
{
    // inform all extensions about changed property type. This includes all properties from the
