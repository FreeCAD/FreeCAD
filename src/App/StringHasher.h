...
public:
    ...
    /// Return the referenced content as bytes, including any index and postfix.
...
public:
    ...
    {
        return fromString(reinterpret_cast<const char*>(bytes.data()),
                          eof,
    ...
    Base::ByteBuffer dataToBytes(int index = 0) const
    {
        ...
        if (index != 0) {
            ...
            res.append(Base::asBytes(indexText));
        }
        if (!_postfix.empty()) {
...
inline Base::ByteBuffer StringIDRef::toBytes() const
{
    return _sid ? _sid->dataToBytes(_index) : Base::ByteBuffer {};
}
