...
void PropertyPythonObject::restoreObject(Base::DocumentReader &reader,
...
void PropertyPythonObject::Restore(Base::DocumentReader& reader,
                                   XERCES_CPP_NAMESPACE_QUALIFIER DOMElement* ContainerDOM)
{
        ...
        if (file_cstr) {
            ...
            try {
                ...
                if (module_cstr && class_cstr) {
                    ...
                    if (!cls) {
                        std::stringstream s;
                        s << "Module " << module_cstr << " has no class " << class_cstr;
                        throw Py::AttributeError(s.str());
                    }
                    if (PyType_Check(cls)) {
                        this->object = PyType_GenericAlloc((PyTypeObject*)cls, 0);
