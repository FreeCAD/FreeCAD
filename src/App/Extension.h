...
#define EXTENSION_PROPERTY_SOURCE_TEMPLATE(_class_, _parentclass_)                                 \
    ...
    template<>                                                                                     \
    App::PropertyData _class_::propertyData = App::PropertyData();                                 \
    template<>                                                                                     \
    const App::PropertyData* _class_::extensionGetPropertyDataPtr(void)                            \
    {                                                                                              \
        return &propertyData;                                                                      \
 ...
 *
 * For general documentation on why extension system exists and how to use it see the
 * ExtensionContainer documentation. Following is a description howto create custom extensions.
 *
 ...
 * added to provide functionality around the properties. There are 3 small difference to normal
 * objects:
 ...
 * This means one should carefully design the API and make only necessary methods public or
 * protected. Every internal method should be private.
 *
 * The automatic availability of methods in the class does not hold for the python interface, only
 * for c++ classes. This is like every where else in FreeCAD, there is no automatic creation of
 * python API from c++ classes. Hence the extension creator must also create a custom python object
 * of its extension, which works exactly like the normal FreeCAD python object workflow. There is
 * nothing special at all for extension python objects, the normal xml + imp.cpp approach is used.
 * It must only be taken care that the objects father is the correct extension base class. Of course
 * also make sure your extension returns the correct python object in its "getPyObject" call. Every
 * method you create in the extensions python will be later added to an extended object. This
 ...
 * provided by the extension. On c++ side this is trivial, such methods are simply marked as
 * "virtual" and can than be overridden in any derived class. This is more involved for the python
 ...
 * the implementations for the overridden methods. This design is equal to the ObjectPythonT<>
 * design of normal document objects. As this wrapper inherits the c++ extension class it can also
 * override the virtual functions the user designed to be overridden. What it should do at a call of
 * the virtual method is to check if this method is implemented in the proxy object and if so call
 * it, and if not call the normal c++ version. It is the extensions creators responsibility to
 * implement this check and call behaviour for every overridable method. This is done by creating a
 * custom wrapper just like ExtensionPythonT<> and overriding all virtual methods.
 * @code
 ...
 * };
 * @endcode
 * @Note As seen in the code there are multiple helper macros to ease the repetitive work of
 * querying and calling methods of the proxy object. See the macro documentation for how to use
 * them.
 *
 * To ensure that your wrapper is used when a extension is created from python the extension type
 * must be exposed as follows:
 * @code
...
protected:
    static void initExtensionSubclass(Base::Type& toInit,
    ...
    virtual bool extensionHandleChangedPropertyType(Base::XMLReader& reader,
                                                    const char* TypeName,
                                                    Property* prop);
