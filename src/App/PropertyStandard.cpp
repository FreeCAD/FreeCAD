...
void PropertySet::setValue(const std::string& item)
{
    aboutToSetValue();
    _lValueList.insert(item);
    hasSetValue();
}

void PropertySet::setValues(const std::set<std::string>& Set)
{
    aboutToSetValue();
    _lValueList=Set;
    hasSetValue();
...
bool PropertySet::operator[] (const std::string& key) const
{
    std::set<std::string>::const_iterator it = _lValueList.find(key);
...
PyObject *PropertySet::getPyObject(void)
{
    PyObject* s = PySet_New(NULL);
    for (std::set<std::string>::const_iterator it = _lValueList.begin();it!= _lValueList.end(); ++it) {
        PyObject* item = PyUnicode_DecodeUTF8((*it).c_str(), (*it).size(), 0);
        if (!item) {
            Py_DECREF(s);
            throw Base::Exception("UTF8 conversion failure at PropertySet::getPyObject()");
        }
        PySet_Add(s, item);
    }
...
void PropertySet::setPyObject(PyObject *value)
{
    if (PySet_Check(value)) {
        ...
        while (PySet_Size(copy) > 0) {
            ...
            values.insert(itemStr);
        }
        setValues(values);
    }
    else {
        std::string error = std::string("type must be a set object");
        error += value->ob_type->tp_name;
        throw Base::TypeError(error);
    }
}
...
{
    writer.Stream() << writer.ind() << "<Set count=\"" <<  getSize() <<"\">" << endl;
    writer.incInd();
    for (std::set<std::string>::const_iterator it = _lValueList.begin();it!= _lValueList.end(); ++it)
        writer.Stream() << writer.ind() << "<Item key=\"" <<  *it <<"\"/>" << endl;

    writer.decInd();
    writer.Stream() << writer.ind() << "</Set>" << endl ;
}

void PropertySet::Restore(Base::XMLReader &reader)
{
    // read my Element
    reader.readElement("Set");
    // get the value of my Attribute
    int count = reader.getAttributeAsInteger("count");
    ...
    for(int i = 0; i < count; i++) {
        ...
        values.insert(reader.getAttribute("value"));
    }
...
{
    aboutToSetValue();
    _lValueList = dynamic_cast<const PropertySet&>(from)._lValueList;
    hasSetValue();
}
