...
#define __EXPR_SET_MSG(_e, _msg, _expr)                                                            \
    do {                                                                                           \
        std::ostringstream ss;                                                                     \
...
#define _EXPR_RETHROW(_e, _msg, _expr)                                                             \
    do {                                                                                           \
        __EXPR_SET_MSG(_e, _msg, _expr);                                                           \
        throw;                                                                                     \
    } while (0)

#define _EXPR_PY_THROW(_msg, _expr)                                                                \
    do {                                                                                           \
        Base::PyException _e;                                                                      \
        __EXPR_SET_MSG(_e, _msg, _expr);                                                           \
        _e.raiseException();                                                                       \
...
#define RUNTIME_THROW(_msg)                                                                        \
    __EXPR_THROW(Base::RuntimeError, _msg, static_cast<Expression*>(nullptr))
...
bool ExpressionVisitor::renameObjectIdentifier(
    Expression& e,
...
static inline bool essentiallyEqual(T a, T b)
{
    ...
    return std::fabs(a - b)
        <= ((std::fabs(a) > std::fabs(b) ? std::fabs(b) : std::fabs(a)) * _epsilon);
}
...
static inline Quantity
pyToQuantity(const Py::Object& pyobj, const Expression* e, const char* msg = nullptr)
{
    Quantity q;
...
{
    return (e1 && e1->isTouched()) || (e2 && e2->isTouched()) || (e3 && e3->isTouched());
}
...
Expression::Component*
Expression::createComponent(Expression* e1, Expression* e2, Expression* e3, bool isRange)
{
...
void Expression::getDeps(ExpressionDeps& deps, int option) const
{
    for (auto& v : getIdentifiers()) {
        ...
        if ((hidden && option == DepNormal) || (!hidden && option == DepHidden)) {
            continue;
        }
...
std::map<App::DocumentObject*, bool>
Expression::getDepObjects(std::vector<std::string>* labels) const
{
    ...
    for (auto& dep : getDeps(DepAll)) {
        for (auto& info : dep.second) {
            for (auto& path : info.second) {
                ...
                std::string imported = PropertyLinkBase::tryImportSubName(obj,
                                                                          key.second.c_str(),
                                                                          owner->getDocument(),
                                                                          nameMap);
                if (!imported.empty()) {
...
{
public:
    UpdateLabelExpressionVisitor(App::DocumentObject* obj,
                                 const std::string& ref,
                                 const char* newLabel)
        : obj(obj)
        , ref(ref)
...
UnitExpression::UnitExpression(const DocumentObject* _owner,
                               const Base::Quantity& _quantity,
...
OperatorExpression::OperatorExpression(const App::DocumentObject* _owner,
                                       Expression* _left,
                                       Operator _op,
                                       Expression* _right)
...
static Py::Object
calc(const Expression* expr, int op, const Expression* left, const Expression* right, bool inplace)
{
    Py::Object l = left->getPyValue();

    // For security reason, restrict supported types
    if (!PyObject_TypeCheck(l.ptr(), &PyObjectBase::Type) && !l.isNumeric() && !l.isString()
        && !l.isList() && !l.isDict()) {
    ...
    if ((op != OperatorExpression::MOD || !l.isString())
        && !PyObject_TypeCheck(r.ptr(), &PyObjectBase::Type) && !r.isNumeric() && !r.isString()
        && !r.isList() && !r.isDict()) {
        __EXPR_THROW(Base::TypeError, "Unsupported operator", expr);
    }
...
#define _BINARY_OP(_pyop)                                                                          \
    res =                                                                                          \
        inplace ? PyNumber_InPlace##_pyop(l.ptr(), r.ptr()) : PyNumber_##_pyop(l.ptr(), r.ptr());  \
    if (!res)                                                                                      \
        EXPR_PY_THROW(expr);                                                                       \
    return Py::asObject(res);
...
void OperatorExpression::_toString(std::ostream& s, bool persistent, int) const
{
    ...
    switch (op) {
        case NEG:
            s << "-" << (needsParens ? "(" : "") << left->toString(persistent)
              << (needsParens ? ")" : "");
            return;
        case POS:
            s << "+" << (needsParens ? "(" : "") << left->toString(persistent)
...
Py::Object FunctionExpression::evalAggregate(const Expression* owner,
                                             int f,
...
Base::Vector3d
FunctionExpression::evaluateSecondVectorArgument(const Expression* expression,
                                                 const std::vector<Expression*>& arguments)
{
    ...
    if (arguments.size() == 2) {
        if (!secondParameter.isSequence()) {
            _EXPR_THROW("Second parameter is not a sequence type: '" << secondParameter.as_string()
                                                                     << "'.",
                        expression);
        }
        if (PySequence_Size(secondParameter.ptr()) != 3) {
            _EXPR_THROW("Second parameter provided has " << PySequence_Size(secondParameter.ptr())
...
void FunctionExpression::initialiseObject(const Py::Object* object,
                                          const std::vector<Expression*>& arguments,
...
Py::Object FunctionExpression::transformFirstArgument(const Expression* expression,
                                                      const std::vector<Expression*>& arguments,
                                                      const Base::Matrix4D* transformationMatrix)
{
    ...
    if (PyObject_TypeCheck(target.ptr(), &Base::MatrixPy::Type)) {
        ...
        return Py::asObject(
            new Base::PlacementPy(Base::Placement(*transformationMatrix * placementMatrix)));
    }
        ...
        return Py::asObject(
            new Base::RotationPy(Base::Rotation(*transformationMatrix * rotatioMatrix)));
    }

    _EXPR_THROW("Function requires the first argument to be either Matrix, Placement or Rotation.",
                expression);
}
...
double FunctionExpression::extractLengthValueArgument(const Expression* expression,
                                                      const std::vector<Expression*>& arguments,
                                                      int argumentIndex)
{
...
Base::Vector3d FunctionExpression::extractVectorArgument(const Expression* expression,
                                                         const std::vector<Expression*>& arguments,
                                                         int argumentIndex)
{
...
Py::Object
FunctionExpression::evaluate(const Expression* expr, int f, const std::vector<Expression*>& args)
{
    ...
    switch (f) {
        ...
        case MROTATE: {
            ...
            static_cast<Base::RotationPy*>(rotationObject.ptr())
                ->getRotationPtr()
                ->getValue(rotationMatrix);
        ...
        case MROTATEZ: {
            ...
            Quantity rotationAngle =
                pyToQuantity(rotationAngleParameter, expr, "Invalid rotation angle.");

            if (!(rotationAngle.isDimensionlessOrUnit(Unit::Angle))) {
                _EXPR_THROW("Unit must be either empty or an angle.", expr);
            }

            Rotation rotation = Base::Rotation(Vector3d(static_cast<double>(f == MROTATEX),
                                                        static_cast<double>(f == MROTATEY),
                                                        static_cast<double>(f == MROTATEZ)),
        ...
        case MTRANSLATE: {
            ...
            if (PyObject_TypeCheck(target.ptr(), &Base::RotationPy::Type)) {
                ...
                static_cast<Base::RotationPy*>(target.ptr())
                    ->getRotationPtr()
                    ->getValue(targetRotatioMatrix);
                return Py::asObject(
                    new Base::PlacementPy(Base::Placement(translateMatrix * targetRotatioMatrix)));
            }
        ...
        case TRANSLATIONM: {
            ...
            if (PySequence_Size(parameter.ptr()) != 3) {
                _EXPR_THROW("Sequence provided has " << PySequence_Size(parameter.ptr())
                                                     << " elements instead of 3.",
                            expr);
            }
            double x, y, z;
        ...
        case VSCALEZ: {
            ...
            switch (f) {
                case VANGLE:
                    return Py::asObject(new QuantityPy(
                        new Quantity(vector1.GetAngle(vector2) * 180 / M_PI, Unit::Angle)));
                case VCROSS:
            ...
            switch (f) {
                case VLINEDIST:
                    return Py::asObject(new QuantityPy(
                        new Quantity(vector1.DistanceToLine(vector2, vector3), Unit::Length)));
                case VLINESEGDIST:
                    return Py::asObject(
                        new Base::VectorPy(vector1.DistanceToLineSegment(vector2, vector3)));
                case VLINEPROJ:
                    vector1.ProjectToLine(vector2, vector3);
                    return Py::asObject(new Base::VectorPy(vector1));
                case VPLANEDIST:
                    return Py::asObject(new QuantityPy(
                        new Quantity(vector1.DistanceToPlane(vector2, vector3), Unit::Length)));
                case VPLANEPROJ:
    ...
    switch (f) {
        ...
        case SQRT: {
            ...
            if (!((s.Length % 2) == 0) && ((s.Mass % 2) == 0) && ((s.Time % 2) == 0)
                && ((s.ElectricCurrent % 2) == 0) && ((s.ThermodynamicTemperature % 2) == 0)
                && ((s.AmountOfSubstance % 2) == 0) && ((s.LuminousIntensity % 2) == 0)
                && ((s.Angle % 2) == 0)) {
        ...
        case CBRT: {
            ...
            UnitSignature s = unit.getSignature();
            if (!((s.Length % 3) == 0) && ((s.Mass % 3) == 0) && ((s.Time % 3) == 0)
                && ((s.ElectricCurrent % 3) == 0) && ((s.ThermodynamicTemperature % 3) == 0)
                && ((s.AmountOfSubstance % 3) == 0) && ((s.LuminousIntensity % 3) == 0)
                && ((s.Angle % 3) == 0)) {
                _EXPR_THROW("All dimensions must be multiples of 3 to compute the cube root.",
                            expr);
            }
        ...
        case CATH:
            ...
            break;
        case TRANSLATIONM:
            if (v1.isDimensionlessOrUnit(Unit::Length) && v2.isDimensionlessOrUnit(Unit::Length)
                && v3.isDimensionlessOrUnit(Unit::Length)) {
                break;
            }
    ...
    switch (f) {
        ...
        case HYPOT: {
            output = sqrt(pow(v1.getValue(), 2) + pow(v2.getValue(), 2)
                          + (!e3.isNone() ? pow(v3.getValue(), 2) : 0));
            break;
        }
        case CATH: {
            output = sqrt(pow(v1.getValue(), 2) - pow(v2.getValue(), 2)
        ...
        case ROTATIONZ:
            return Py::asObject(
                new Base::RotationPy(Base::Rotation(Vector3d(static_cast<double>(f == ROTATIONX),
                                                             static_cast<double>(f == ROTATIONY),
                                                             static_cast<double>(f == ROTATIONZ)),
                                                    value)));
        case TRANSLATIONM:
 ...
 *
 * Unqualified names (i.e the name only without any dots) are resolved in the owning
 * DocumentObjects. Qualified names are looked up in the owning Document. It is first looked up by
 * its internal name. If not found, the DocumentObjects' labels searched.
 *
 * If something fails, an exception is thrown.
 *
 * @returns The Property object if it is derived from either PropertyInteger, PropertyFloat, or
 * PropertyString.
 */
...
bool VariableExpression::_adjustLinks(const std::set<App::DocumentObject*>& inList,
                                      ExpressionVisitor& v)
{
...
void VariableExpression::_updateLabelReference(App::DocumentObject* obj,
                                               const std::string& ref,
                                               const char* newLabel)
{
...
bool VariableExpression::_updateElementReference(App::DocumentObject* feature,
                                                 bool reverse,
                                                 ExpressionVisitor& v)
{
...
void VariableExpression::_collectReplacement(std::map<ObjectIdentifier, ObjectIdentifier>& paths,
                                             const App::DocumentObject* parent,
                                             App::DocumentObject* oldObj,
                                             App::DocumentObject* newObj) const
{
...
void VariableExpression::_offsetCells(int rowOffset, int colOffset, ExpressionVisitor& v)
{
    ...
    if (!addr.isValid()) {
        FC_WARN("Not changing relative cell reference '"
                << comp.getName() << "' due to invalid offset " << '(' << colOffset << ", "
                << rowOffset << ')');
    }
    else {
        v.aboutToChange();
...
ConditionalExpression::ConditionalExpression(const DocumentObject* _owner,
...
Expression* ConditionalExpression::simplify() const
{
    ...
    if (!v) {
        return new ConditionalExpression(owner,
                                         condition->simplify(),
                                         trueExpr->simplify(),
                                         falseExpr->simplify());
    }
...
{
    return strcmp(name, "None") && strcmp(name, "True") && strcmp(name, "False");
}

TYPESYSTEM_SOURCE(App::RangeExpression, App::Expression)

RangeExpression::RangeExpression(const DocumentObject* _owner,
...
static const App::DocumentObject* DocumentObject =
    nullptr;                           /**< The DocumentObject that will own the expression */
...
static std::map<std::string, FunctionExpression::Function>
...
{
    // parse from buffer
    ExpressionParser::YY_BUFFER_STATE my_string_buffer =
        ExpressionParser::ExpressionParser_scan_string(buffer);
...
UnitExpression* ExpressionParser::parseUnit(const App::DocumentObject* owner, const char* buffer)
{
    ...
    if (!unitExpression) {
        ...
        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            ...
            // If not initially a unit expression, but value is equal to 1, it means the expression
            // is something like 1/unit
...
{
    ExpressionParser::YY_BUFFER_STATE buf =
        ExpressionParser::ExpressionParser_scan_string(str.c_str());
    ExpressionParser::StringBufferCleaner cleaner(buf);
