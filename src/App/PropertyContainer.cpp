...
void PropertyContainer::getPropertyNamedList(
...
const PropertyData* PropertyContainer::getPropertyDataPtr()
{
    return &propertyData;
}
const PropertyData& PropertyContainer::getPropertyData() const
{
    return propertyData;
}
...
void PropertyContainer::Save(Base::Writer& writer) const
{
    ...
    for (auto it = Map.begin(); it != Map.end();) {
        ...
        if (!prop->testStatus(Property::PropDynamic)
            && (prop->testStatus(Property::Transient) || getPropertyType(prop) & Prop_Transient)) {
    ...
    for (auto prop : transients) {
        writer.Stream() << writer.ind() << "<_Property name=\"" << prop->getName() << "\" type=\""
                        << prop->getTypeId().getName() << "\" status=\"" << prop->getStatus()
                        << "\"/>" << std::endl;
    }
    writer.decInd();
    ...
    for (const auto& it : Map) {
        ...
        if (it.second->testStatus(Property::Transient) || it.second->getType() & Prop_Transient) {
            writer.decInd();
...
The property framework introduces the ability to access attributes (member variables) of a class by
name without knowing the class type. It's like the reflection mechanism of Java or C#. This ability
is introduced by the App::PropertyContainer class and can be used by all derived classes.

This makes it possible in the first place to make an automatic mapping to python (e.g. in
App::FeaturePy) and abstract editing properties in Gui::PropertyEditor.
