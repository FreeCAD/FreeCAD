    ...
    for (auto& indexedName : this->indexedNames) {
        ...
        for (auto& vv : indexedName.second.children) {
            ...
            stream << child.indexedName.getIndex() << ' ' << child.offset << ' ' << child.count
                   << ' ' << child.tag << ' ' << mapIndex << ' ';
            stream.write(reinterpret_cast<const char*>(child.postfix.data()),
                         static_cast<std::streamsize>(child.postfix.size()));
            stream << ' ' << '0';
        ...
        for (auto& dequeueOfMappedNameRef : indexedName.second.names) {
            for (auto ref = &dequeueOfMappedNameRef; ref; ref = ref->next.get()) {
                    ...
                    if (prefixID.id != 0) {
                        for (auto& sid : ref->sids) {
                            if (sid.isMarked() && sid.value() == prefixID.id) {
                                stream << '$';
                                stream.write(
                                    reinterpret_cast<const char*>(ref->name.dataBytes().data()),
                ...
                if (printName) {
                    stream << ';';
                    stream.write(reinterpret_cast<const char*>(ref->name.dataBytes().data()),
                                 static_cast<std::streamsize>(ref->name.dataBytes().size()));
                }
                ...
                if (postfix.empty()) {
                    ...
                    auto it = postfixMap.find(
                        std::string(reinterpret_cast<const char*>(postfix.data()), postfix.size())
                    );
                                  ...
                                  const std::vector<std::string>& postfixes)
{
    ...
    for (int i = 0; i < typeCount; ++i) {
        ...
        for (int j = 0; j < outerCount; ++j) {
            ...
            child.postfix = Base::ByteBuffer::copy(Base::asBytes(tmp.data(), tmp.size()));
            ...
            while (true) {
                ...
                if (tokens[offset] != "0") {
                    ...
                    if (postfixIndex <= 0 || postfixIndex > (int)postfixes.size()) {
                        ...
                        ref->name += Base::asBytes(postfix.data(), postfix.size());
                    }
                }
                            ...
                            std::vector<std::string>& postfixes)
{
    ...
    auto res = postfixMap.insert(std::make_pair(
        std::string(reinterpret_cast<const char*>(postfix.data()), postfix.size()), 0));
    if (res.second) {
                                   ...
                                   bool forceTag) const
{
    if (!postfix.empty()) {
        ...
        ss.write(reinterpret_cast<const char*>(postfix.data()),
                 static_cast<std::streamsize>(postfix.size()));
    }
    long inputTag = 0;
        ...
        for (auto& indexedChild : indexedNameIndexedElements.second.children) {
            ...
            if (pos > 10) {
                const Base::BytesView childPostfixPrefix =
                    Base::asBytes(child.postfix.data(), static_cast<std::size_t>(pos));
                                  ...
                                  std::vector<std::string>& postfixes) const
{
    ...
    for (auto& indexedName : this->indexedNames) {
        addPostfix(Base::asBytes(indexedName.first), postfixMap, postfixes);
                ...
                if (!entry->postfix.empty()) {
                    ...
                    const bool entryHasPrefix = entryPostfix.size() >= ELEMENT_MAP_PREFIX_SIZE
                        && std::memcmp(entryPostfix.data(),
                            ...
                            == 0;
                    if (!entryHasPrefix) {
                        merged.append(Base::asBytes(ELEMENT_MAP_PREFIX));
                    }
