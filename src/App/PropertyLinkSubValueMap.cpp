...
std::string indexedElementNameForGeometryCache(
    const App::PropertyLinkSubReferenceStore::Reference& reference)
{
    const std::string& subName =
        reference.shadow.oldName.empty() ? reference.subName : reference.shadow.oldName;
    const char* element = Data::findElementName(subName.c_str());
    if (!element || !element[0] || Data::hasMissingElement(element) || Data::isMappedElement(element)) {
        return {};
    }
...
std::string subNameFromCachedElement(
    const App::PropertyLinkSubReferenceStore::Reference& reference,
    const std::string& cachedElement,
                              ...
                              const App::PropertyLinkSubReferenceStore::Reference& reference)
{
    ...
    const std::string cachedElement = indexedElementNameForGeometryCache(reference);
    if (cachedElement.empty()) {
        return {};
    }

    auto names = geo->searchElementCache(cachedElement);
    if (names.empty()) {
        names = geo->searchElementCache(cachedElement, Data::SearchOptions());
    }
    if (names.empty()) {
        return {};
    }
