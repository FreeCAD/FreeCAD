...
StringIDRef StringHasher::getID(const Data::MappedName& name, const std::vector<StringIDRef>& sids)
{
    ...
    tempID._postfix = Base::ByteBuffer::borrow(
        Base::asBytes(name.postfixBytes().constData(), name.postfixBytes().size())
    );
    ...
    if (!tempID._postfix.empty()) {
        ...
        indexed = Data::IndexedName(
            Base::asBytes(name.dataBytes().constData(), name.dataBytes().size())
        );
    }
    if (indexed) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (!d.isPostfixed()) {
            ...
            if (d.isHashed() || d.isBinary()) {
                ...
                d._data = Base::ByteBuffer::copy(Base::asBytes(decoded));
            }
            else {
...
void StringHasher::restoreStream(std::istream& stream, std::size_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (sid.isHashed() || sid.isBinary()) {
            std::string decoded;
            Base::base64_decode(decoded, content);
            sid._sid->_data = Base::ByteBuffer::copy(Base::asBytes(decoded));
        }
        else {
    ...
    if ((count != 0U) && reader.FileVersion > 1) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            if (hashed || reader.hasAttribute("data")) {
                ...
                sid = new StringID(id,
                                   Base::ByteBuffer::copy(Base::asBytes(decoded)),
                                   StringID::Flag::Hashed);
            }
            else {
