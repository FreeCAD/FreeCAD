...
Base::ByteBuffer sha1DigestBuffer(Base::BytesView bytes)
{
    const auto digest = Base::sha1Digest(bytes);
    return Base::ByteBuffer::copy(
        Base::asBytes(reinterpret_cast<const char*>(digest.data()), digest.size()));
}

bool decodeBase64(std::string_view encoded, Base::ByteBuffer& decoded)
{
    ...
    const std::size_t consumedInputLength
        = Base::base64_decode(result, encoded.data(), encoded.size());
    if (consumedInputLength != dataLength) {
        return false;
    }

    decoded = Base::ByteBuffer::copy(Base::asBytes(result.data(), result.size()));
    return true;
}
...
struct StringIDHasher
{
    std::size_t operator()(const StringID* sid) const
    {
        ...
        return Base::hashCombine(Base::fnv1a64(sid->data().view()),
                                 Base::fnv1a64(sid->postfix().view()));
    }
...
std::string StringID::dataToText(int index) const
{
    ...
    std::string res(reinterpret_cast<const char*>(_data.data()), _data.size());
    if (index != 0) {
        res += std::to_string(index);
    }
    if (!_postfix.empty()) {
        res.append(reinterpret_cast<const char*>(_postfix.data()), _postfix.size());
    }
    return res;
}
...
StringIDRef StringHasher::getID(const char* text, int len, bool hashable)
{
    ...
    return getID(Base::asBytes(text, static_cast<std::size_t>(len)),
...
StringIDRef StringHasher::getID(Base::BytesView data, Options options)
{
    ...
    StringID dataID;
    if (hashed) {
        dataID._data = sha1DigestBuffer(data);
    }
    else {
...
StringIDRef StringHasher::getID(const Data::MappedName& name, const std::vector<StringIDRef>& sids)
{
    ...
    if (indexed) {
        ...
        tempID._data = Base::ByteBuffer::borrow(
            Base::asBytes(indexed.getType(), std::strlen(indexed.getType()))
        );
    }
    else {
    ...
    if (!tempID._postfix.empty()
        && Base::findBytes(tempID._postfix.view(), Base::asBytes("#")) == Base::bytesNpos) {
    ...
    if (!newStringID._postfix.empty() && !indexed) {
        ...
        if (res.id > 0) {  // If the data had an index
            if (res.index != 0) {
                ...
                const auto pos = Base::rfindBytes(v, Base::asBytes(":"));
                if (pos != Base::bytesNpos) {
...
void StringHasher::saveStream(std::ostream& stream) const
{
    ...
    for (auto& hasher : _hashes->right) {
        ...
        std::size_t offset = d.isPostfixEncoded() ? 1 : 0;
        ...
        if (d.isPostfixed()) {
            if (!d.isPrefixIDIndex() && !d.isIndexed() && !d.isPrefixID()) {
                stream << ' ';
                stream.write(reinterpret_cast<const char*>(d._data.data()),
                             static_cast<std::streamsize>(d._data.size()));
            }

            if (!d.isPostfixEncoded()) {
                stream << ' ';
                stream.write(reinterpret_cast<const char*>(d._postfix.data()),
                             static_cast<std::streamsize>(d._postfix.size()));
            }
            stream << '\n';
        }
        else {
            ...
            stream << ' ';
            textStreamWrapper
                << std::string(reinterpret_cast<const char*>(d._data.data()), d._data.size());
        }
    }
}
...
void StringHasher::restoreStreamNew(std::istream& stream, std::size_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        std::size_t j = 2;
        if (relative && last) {
            for (; j < tokens.size() && j - 2 < last->_sids.size(); ++j) {
        ...
        if (!d.isPostfixed()) {
            ...
            if (d.isHashed() || d.isBinary()) {
                Base::ByteBuffer decoded;
                if (!decodeBase64(content, decoded)) {
                    FC_THROWM(Base::RuntimeError, "Invalid Base64 string in string table");
                }
                d._data = std::move(decoded);
            }
            else {
                d._data = Base::ByteBuffer::copy(Base::asBytes(content));
            }
        }
        else {
            std::size_t offset = 0;
            ...
            if (d.isIndexed()) {
                ...
                d._data = Base::ByteBuffer::copy(Base::asBytes(d._sids[offset]._sid->toString(0)));
                if (d.isPrefixIDIndex()) {
                    d._data.append(Base::asBytes(":"));
                }
            }
            else {
                stream >> content;
            ...
            if (!d.isPostfixEncoded()) {
                stream >> content;
                d._postfix = Base::ByteBuffer::copy(Base::asBytes(content));
            }
        }
...
void StringHasher::restoreStream(std::istream& stream, std::size_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (sid.isHashed() || sid.isBinary()) {
            ...
            sid._sid->_data = std::move(decoded);
        }
        else {
            sid._sid->_data = Base::ByteBuffer::copy(Base::asBytes(content));
        }
        insert(sid);
    }
}
    ...
    if ((count != 0U) && reader.FileVersion > 1) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            if (hashed || reader.hasAttribute("data")) {
                ...
                sid = new StringID(id, std::move(decoded), StringID::Flag::Hashed);
            }
            else {
                const char* text = reader.getAttribute<const char*>("text");
                sid = new StringID(id,
                                   Base::ByteBuffer::copy(Base::asBytes(text)),
                                   StringID::Flag::None);
            }
            insert(sid);
        }
