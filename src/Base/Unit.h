...
using UnitExponentsExt = std::array<int, unitNumExponents>;
...
public:
    ...
    explicit constexpr Unit(const UnitExponentsExt exps, const std::string_view name = "")
        : _exps {castExponents(exps)}
        , _name {name}
    {}
...
private:
    ...
    constexpr UnitExponents castExponents(UnitExponentsExt exps)
    {
        auto cast = [](auto val) {
            if (val >= unitExponentLimit) {
                throw OverflowError("Unit exponent overflow");
            }
            if (val < -unitExponentLimit) {
                throw UnderflowError("Unit exponent underflow");
