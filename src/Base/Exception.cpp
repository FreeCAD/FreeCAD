...
bool Exception::getReported() const
{
    return hasBeenReported;
}

void Exception::setReported(const bool reported) const
{
    hasBeenReported = reported;
}

void Exception::setDebugInformation(const std::source_location& location)
{
...
#ifdef _DEBUG
    std::cout << "Set new signal handler" << '\n';
#endif
}
...
#ifdef _DEBUG
    std::cout << "Restore old signal handler" << '\n';
#endif
}

void SignalException::throw_signal(const int signum)
{
    std::cerr << "SIGSEGV signal raised: " << signum << '\n';
