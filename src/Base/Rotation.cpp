...
Rotation Rotation::makeRotationByAxes(Vector3d xdir, Vector3d ydir, Vector3d zdir, const char* priorityOrder)
{
    ...
    for (int i = 0; i < orderLen; ++i){
        ...
        if ((i > 0 && parsedDir == mainDirIndex) ||
            (i > 1 && parsedDir == hintDirIndex))
            THROWM(ValueError,"makeRotationByAxes: all axes listed in priorityOrder must be different");
        if (i == 0) mainDirIndex = parsedDir;
        else if (i == 1) hintDirIndex = parsedDir;
    }
    ...
    if (lastDir.Length() < tol) {
        Vector3d from;
        from[mainDirIndex] = 1.0;
        return Rotation(from, mainDir);
    }
    // make them all perpendicular.
    lastDir.Normalize();
    hintDir = lastDir.Cross(mainDir);
    hintDir.Normalize(); //redundant?
    // Order as XYZ and fix handedness.
    Vector3d finaldirs[3];
    finaldirs[mainDirIndex] = mainDir;
    finaldirs[hintDirIndex] = hintDir;
    finaldirs[lastDirIndex] = (2 + mainDirIndex - hintDirIndex)%3 ? lastDir : -lastDir;
    //build the rotation, by constructing a matrix first.
    Matrix4D m;
    m.setToUnity();
    for (int i = 0; i < 3; ++i){
        m.setCol(i, finaldirs[i]);
    }
    return Rotation(m);
}
