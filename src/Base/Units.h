...
inline std::array<std::string, numUnitSpecs> allStrings()
{
    auto getName = [](const std::pair<std::string_view, UnitVals>& def) {
        return def.first.data();
    };

    std::array<std::string, numUnitSpecs> arr {};
    std::transform(unitSpecs.begin(), unitSpecs.end(), arr.begin(), getName);

    return arr;
}

constexpr std::array<Unit, numUnitSpecs> all()
{
    std::array<Unit, numUnitSpecs> units;
    for (size_t i {0}; i < numUnitSpecs; ++i) {
        units.at(i) = makeFromSpec(unitSpecs.at(i));
    }
    return units;
}
