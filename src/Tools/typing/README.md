...
The docs command also writes `sidebar.ts` next to the generated content tree.
This fragment exports a `pythonApiSidebar` config with explicit `Submodules`
and `Types` group labels that can be imported into a Starlight site instead of
relying only on raw directory names in autogenerated sidebars.

A Starlight site scaffold lives under `src/Tools/typing/site/`. Its
`npm run generate:api` script writes generated docs content into
`src/content/docs/python-api/` and a generated sidebar fragment into
