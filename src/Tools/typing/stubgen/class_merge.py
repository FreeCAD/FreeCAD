...
from .parsing import decorator_kwargs, decorator_name, parse_python_source
    ...
    def deprecated_attributes(node: ast.ClassDef) -> dict[str, str]:
        ...
        for decorator in node.decorator_list:
            if decorator_name(decorator).split(".", 1)[-1] != "deprecated_attributes":
                continue
            for name, value in decorator_kwargs(decorator).items():
                if isinstance(value, str):
                    deprecated_attributes[name] = value
