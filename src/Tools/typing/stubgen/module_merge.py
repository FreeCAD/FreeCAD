...
GENERATED_STUB_HEADER = (
    ...
    "# Sources: binding metadata and source-adjacent typing contracts.",
)


def generated_stub_header() -> str:
...
def ensure_generated_stub_header(source: str) -> str:
    """Ensure a canonical provenance header leads generated stub source."""

    leading = leading_comment_block(source)
    if generated_stub_header() in leading:
        return source

    body = source[len(leading) :].lstrip("\n") if leading else source.lstrip("\n")
    parts = [part for part in (leading, generated_stub_header(), body) if part]
    return "\n\n".join(parts).rstrip() + "\n"
...
def merged_module_source(target_source: str, target_tree: ast.Module) -> str:
    ...
    if preamble:
        merged = f"{preamble}\n\n{merged}"
    return ensure_generated_stub_header(merged)
...
def copy_overlay_stubs(
    ...
    for source in sorted(overlay_dir.rglob("*.pyi")):
        ...
        if target.exists():
            ...
            target.write_text(ensure_generated_stub_header(overlay_source), encoding="utf-8")
        count += 1
