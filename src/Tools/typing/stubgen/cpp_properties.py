...
_QUALIFIED_NAME = re.compile(r"\b(?P<module>[A-Za-z_]\w*)\.[A-Za-z_]\w*\b")
...
def _annotation(expression: str) -> ast.expr:
    parsed = ast.parse(expression, mode="eval")
    return parsed.body


def _localize_expression(expression: str, module_name: str) -> str:
    """Remove a public module qualifier when it names the target module."""

    return re.sub(rf"\b{re.escape(module_name)}\.", "", expression)


def _docstring(value: str | None) -> ast.Expr | None:
    if not value:
        return None
    return ast.Expr(value=ast.Constant(value=value))


def _property_nodes(property_: TypedCppProperty) -> list[ast.FunctionDef]:
    getter_body = [node for node in [_docstring(property_.documentation)] if node]
    getter_body.append(ast.Expr(value=ast.Constant(value=Ellipsis)))
    getter = ast.FunctionDef(
        ...
        returns=_annotation(property_.getter),
        type_comment=None,
    )
    if property_.setter is None:
        return [getter]
    setter = ast.FunctionDef(
        ...
        args=ast.arguments(
            ...
            args=[
                ast.arg(arg="self"),
                ast.arg(arg="value", annotation=_annotation(property_.setter)),
            ],
            vararg=None,
            kwonlyargs=[],
            kw_defaults=[],
        ...
        decorator_list=[
            ast.Attribute(
                value=ast.Name(id=property_.name, ctx=ast.Load()),
                attr="setter",
                ctx=ast.Load(),
            )
        ],
...
def _is_any_annotation(annotation: ast.expr) -> bool:
...
def _any_placeholder_attributes(body: list[ast.stmt]) -> dict[str, tuple[int, str | None]]:
    placeholders: dict[str, tuple[int, str | None]] = {}
    for index, node in enumerate(body):
        if not isinstance(node, ast.AnnAssign) or not isinstance(node.target, ast.Name):
            continue
        if not _is_any_annotation(node.annotation):
            continue
        ...
        if index + 1 < len(body):
            ...
            if (
                ...
                documentation = next_node.value.value
        placeholders[node.target.id] = (index, documentation)
    return placeholders


def add_cpp_properties(
    target_source: str,
    properties: tuple[TypedCppProperty, ...],
) -> str:
    """Add generated C++ property-container members to public class stubs."""

    if not properties:
        return target_source
    tree = ast.parse(target_source)
    owners = {property_.owner for property_ in properties}
    module_names = {owner.module_name for owner in owners}
    if len(module_names) != 1:
        raise ValueError("C++ properties from multiple public modules cannot be merged together")
    module_name = next(iter(module_names))
    localized_properties = tuple(
        replace(
            property_,
            getter=_localize_expression(property_.getter, module_name),
            setter=(
                _localize_expression(property_.setter, module_name)
                if property_.setter is not None
                else None
            ),
        )
        for property_ in properties
    ...
    for owner in owners:
        class_node = next(
            (
                ...
                if isinstance(node, ast.ClassDef) and node.name == owner.python_name
            ),
            None,
        )
        if class_node is None:
            raise ValueError(f"Generated {owner.module_name} stub has no {owner.python_name} class")

        owner_properties = [
            property_ for property_ in localized_properties if property_.owner == owner
        ]
        existing_symbols = class_body_defined_symbols(class_node.body)
        placeholders = _any_placeholder_attributes(class_node.body)
        replaceable = {
            property_.name for property_ in owner_properties if property_.name in placeholders
        }
        collisions = [
            property_
            for property_ in owner_properties
            if property_.name in existing_symbols and property_.name not in replaceable
        ]
        if collisions:
            details = "; ".join(
                f"{property_.name} from {property_.source}:{property_.line}"
                for property_ in collisions
            )
            raise ValueError(
                f"Generated {owner.qualified_name} stub already declares C++ properties: {details}"
            )
        generated: list[ast.FunctionDef] = []
        replacements: dict[int, tuple[int, list[ast.FunctionDef]]] = {}
        for property_ in sorted(owner_properties, key=lambda item: item.name):
            placeholder = placeholders.get(property_.name)
            if placeholder is None:
                generated.extend(_property_nodes(property_))
                continue
            index, documentation = placeholder
            if property_.documentation is None and documentation is not None:
                property_ = replace(property_, documentation=documentation)
            end = index + (2 if documentation is not None else 1)
            replacements[index] = (end, _property_nodes(property_))

        if replacements:
            updated_body: list[ast.stmt] = []
            index = 0
            while index < len(class_node.body):
                replacement = replacements.get(index)
                if replacement is None:
                    updated_body.append(class_node.body[index])
                    index += 1
                    continue
                end, nodes = replacement
                updated_body.extend(nodes)
                index = end
            class_node.body = updated_body

        if generated:
            insert_at = len(class_node.body)
            for index, node in enumerate(class_node.body):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    insert_at = index
                    break
            class_node.body[insert_at:insert_at] = generated

    ast.fix_missing_locations(tree)
    merged = merged_module_source(target_source, tree)
    dependencies = {
        dependency
        for property_ in localized_properties
        for expression in (property_.getter, property_.setter or "")
        for dependency in _QUALIFIED_NAME.findall(expression)
        if dependency != module_name
    }
    imports: list[str] = []
    if any(
        re.search(r"\bSequence\b", expression)
        for property_ in localized_properties
        for expression in (property_.getter, property_.setter or "")
    ):
    ...
    imports.extend(f"import {dependency} as {dependency}" for dependency in sorted(dependencies))
    if not imports:
        return merged
