...
def snapUI(force=True):
    """ Get the snap toolbar user interface.
    @param force True if the dialog must be created if not exist.
    @return Snap toolbar user interface, None if not exist, and force=False.
    """
    ui = None
    # Loop over widgets
    ...
    if not ui and force:
        import snapWidget
        widget = snapWidget.load()
        widget.hide()
        ui = widget.ui
    return ui

__title__="FreeCAD alternative Snap tools"
__author__ = "Yorik van Havre, Jose Luis Cercos Pita"
__url__ = "http://free-cad.sourceforge.net"

class newSnapTracker(DraftTrackers.Tracker):
    ...
    def __init__(self):
        ...
        DraftTrackers.Tracker.__init__(self,children=[node])
    ...
    def setMarker(self,style):
        if (style == "point"):
            ...
            self.marker.markerIndex = coin.SoMarkerSet.PLUS_9_9
        elif (style == "well"):
            self.marker.markerIndex = coin.SoMarkerSet.WELL_9_9
        elif (style == "triangle"):
            self.marker.markerIndex = coin.SoMarkerSet.TRIANGLE_LINE_9_9
        elif (style == "slash"):
            self.marker.markerIndex = coin.SoMarkerSet.SLASH_9_9
...
class newSnapper:
    ...
    def snap(self,screenpos,lastpoint=None):
        """snap(screenpos,lastpoint=None,active=True,constrain=False): returns a snapped
        point from the given (x,y) screenpos (the position of the mouse cursor), lastpoint is an optional
        other point used to draw an imaginary segment and get additional snap locations.
        ...
        # Get state from user interface active=True,constrain=False
        self.ui   = snapUI()
        active    = self.ui.enabled()
        constrain = self.ui.constrain()
        # Alternative snap activation method (delete it?)
        oldActive = False
        ...
        # If snapping is unnactive, apparent point (orthogonality constrained if proceed) will used
        if not active:
            return cstr(self.getApparentPoint(screenpos[0],screenpos[1]))
        ...
        if not obj:
            # nothing has been snapped, check for grid snap
            if self.ui.grid():
                point = self.snapToGrid(point)
            return cstr(point)
        # Check if object is active to snap
        if hasattr(obj.ViewObject,"Selectable"):
            if not obj.ViewObject.Selectable:
                # nothing has been snapped, check for grid snap
                if active and self.ui.grid():
                    point = self.snapToGrid(point)
                return cstr(point)
            ...
            if (Draft.getType(obj) == "Wall") and not oldActive:
                if obj.Base:
                    for edge in obj.Base.Shape.Edges:
                        ...
                        if self.ui.intersection():
                            snaps.extend(self.snapToIntersection(edge))
                        if self.ui.perpendicular():
    ...
    def getApparentPoint(self,x,y):
        ...
        point = FreeCAD.DraftWorkingPlane.projectPoint(pt,dv)
                                ...
                                if isinstance(e.Curve,Part.Line):
                                    ...
                                    if not fcgeo.isPtOnEdge(np,e):
                                        if (np.sub(point)).Length < self.radius:
                                            ...
                                            if last and (fcgeo.vec(e).Length > 0.0001):
...
if not hasattr(FreeCADGui,"newSnapper"):
    FreeCADGui.newSnapper = newSnapper()
