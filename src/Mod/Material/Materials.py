...
class Materials:
    ...
    def __init__(self):
        ...
        self.widget = FreeCADGui.PySideUic.loadUi(filePath + "Resources" + os.sep + "ui" + os.sep + "materials.ui")
        ...
        self.widget.layoutAppearance.addWidget(self.widget.PreviewRender)
        ...
        model = QtGui.QStandardItemModel()
        treeView = widget.treeAppearance
        treeView.setModel(model)
        treeView.setUniformRowHeights(True)
        treeView.setItemDelegate(MaterialsDelegate())
        # model.itemChanged.connect(self.modelChange)
    ...
    def createMaterialTree(self):
        ...
        resources = get_material_libraries()

        # Favourites
        top = QtGui.QTreeWidgetItem(["Favorites"])
        materialTree.addTopLevelItem(top)

        # Recently used
        top = QtGui.QTreeWidgetItem(["Recent"])
        materialTree.addTopLevelItem(top)

        for name, value in resources.items():
            print("{0} Library: {1}".format(name, value))
            top = QtGui.QTreeWidgetItem(["{0} Library".format(name)])
            materialTree.addTopLevelItem(top)

            self.addCards(top, Path(value[0]), Path(value[0]), QtGui.QIcon(value[1]))

        # treeView.expandAll()
        self.edited = False

    def addCards(self, top, folder, library, icon):
        """
            Recursively add library folders and cards
        """

        for child in folder.iterdir():
            if child.is_dir():
                # item = QtGui.QTreeWidgetItem(top, [str(child.stem())])
                item = QtGui.QTreeWidgetItem([str(child.stem)])
                item.setFlags(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsDragEnabled | QtCore.Qt.ItemIsDropEnabled) # This should make it unselectable but seems to have no effect
                # print("setFlags({0})".format(int(QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsDragEnabled | QtCore.Qt.ItemIsDropEnabled)))
                top.addChild(item)
                self.addCards(item, folder / child.stem, library, icon)
            else:
    ...
    def onSelectMaterial(self, current, previous):
    ...
    def setOrClear(self, widget, data, name):
        if name in data:
            widget.setText(data[name])
        else:
            widget.clear()

    def updateGeneral(self, data):

        widget = self.widget
    
        self.setOrClear(widget.editName, data, "Name")
        self.setOrClear(widget.editAuthorLicense, data, "AuthorAndLicense")
        self.setOrClear(widget.editParent, data, "Parent")
        self.setOrClear(widget.editMaterialNumber, data, "MaterialNumber")
        self.setOrClear(widget.editStandardCode, data, "StandardCode")
        self.setOrClear(widget.editSourceURL, data, "SourceURL")
        self.setOrClear(widget.editSourceReference, data, "ReferenceSource")
    ...
    def implementSectionModel(self, template, param, treeView, groups):
        ...
        for group in template:
            ...
            if gg in groups:
                ...
                for propertyName in group[gg]:
                    item = QtGui.QStandardItem(propertyName)
                    item.setToolTip(group[gg][propertyName]["Description"])
                    # self.internalprops.append(propertyName)

                    value = QtGui.QStandardItem()
                    value.setToolTip(group[gg][propertyName]["Description"])

                    # print("Units '{0}'".format(group[gg][propertyName]["Units"]))
                    units = QtGui.QStandardItem(group[gg][propertyName]["Units"])
                    units.setToolTip(group[gg][propertyName]["Description"])

                    tt = group[gg][propertyName]["Type"]
    ...
    def storeSize(self):
        ...
        # root = self.widget.treeMaterials.model().invisibleRootItem()
        # for gg in range(root.rowCount()):
        #     group = root.child(gg)
        #     p.SetBool("TreeExpand"+group.text(), self.widget.treeMaterials.isExpanded(group.index()))

        # Save the state of the material tree
