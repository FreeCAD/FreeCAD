...
MaterialEntry::MaterialEntry(const ModelLibrary &library, const std::string &baseName, const std::string &modelName, const QDir &dir, 
...
MaterialLoader::MaterialLoader(std::map<std::string, Model*> *modelMap, std::list<ModelLibrary*> *libraryList) :
...
void MaterialLoader::addLibrary(ModelLibrary *model)
{
...
MaterialEntry *MaterialLoader::getModelFromPath(const ModelLibrary &library, const std::string &path) const
{
    YAML::Node yamlroot = YAML::LoadFile(path);
    std::string base = "Model";
    if (yamlroot["AppearanceModel"]) {
    ...
    MaterialEntry *model = new MaterialEntry(library, base, name, modelDir, uuid, yamlroot);
...
void MaterialLoader::loadLibrary(const ModelLibrary &library)
{
...
std::list<ModelLibrary *> *MaterialLoader::getModelLibraries()
{
    auto param =
