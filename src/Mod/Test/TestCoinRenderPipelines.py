...
class CoinRenderPipelineTestCase(unittest.TestCase):
    ...
    def test_indexed_face_set_per_vertex_color_matches_between_pipelines(self):
        FreeCAD, FreeCADGui, coin = _require_gui()
        ...
        try:
            if not {_RENDERER_LEGACY, _RENDERER_DRAW_LIST}.issubset(
                ...
                raise unittest.SkipTest(
                    "per-vertex color comparison requires both render pipelines"
                )

            fixture = get_snapshot_fixture("SoFCIndexedFaceSetPerVertexColor")
            runtime = SnapshotRuntime(coin, width, height, "SoFCIndexedFaceSetPerVertexColor")
            root = fixture.build(runtime)
            root.ref()
            try:
                frame_snapshot_camera(runtime, root, fixture.framing_policy)
                ...
                for renderer_name in (_RENDERER_LEGACY, _RENDERER_DRAW_LIST):
                    path = _renderer_output_path(
                        ...
                        renderer_name,
                        "actual",
                        "IndexedFaceSetPerVertexColor.png",
                    )
                    _render_png(harness, coin, root, path, renderer_name, frame_camera=False)
                    self.assertGreater(
                        _non_background_pixel_count(path),
                        10,
                        f"per-vertex color scene rendered empty in {renderer_name}: {path}",
                    )
                    rendered[renderer_name] = path
            finally:
                root.unref()
            ...
            self.assertTrue(
                ...
                "per-vertex SoFCIndexedFaceSet colors diverged between LegacyGL and DrawList",
            )
        finally:
            harness.close()

    def test_indexed_face_set_per_vertex_color_shaded_matches_between_pipelines(self):
        FreeCAD, FreeCADGui, coin = _require_gui()
        ...
        try:
            if not {_RENDERER_LEGACY, _RENDERER_DRAW_LIST}.issubset(
                ...
                raise unittest.SkipTest(
                    "shaded per-vertex color comparison requires both render pipelines"
                )
            ...
            root.ref()
            try:
                light_model = _find_descendant(coin, root, "SoLightModel")
                if light_model is None:
                    raise AssertionError("per-vertex color scene has no light model")
                light_model.model.setValue(coin.SoLightModel.PHONG)
                ...
                for renderer_name in (_RENDERER_LEGACY, _RENDERER_DRAW_LIST):
                    path = _renderer_output_path(
                        ...
                        renderer_name,
                        "actual",
                        "IndexedFaceSetPerVertexColorShaded.png",
                    )
                    _render_png(harness, coin, root, path, renderer_name, frame_camera=False)
                    self.assertGreater(
                        _non_background_pixel_count(path),
                        10,
                        f"shaded per-vertex color scene rendered empty in {renderer_name}: {path}",
                    )
                    rendered[renderer_name] = path
            finally:
                root.unref()
            ...
            self.assertTrue(
                ...
                "shaded per-vertex SoFCIndexedFaceSet colors diverged between LegacyGL and DrawList",
            )
        finally:
            harness.close()
    ...
    def test_string_label_sampler_matches_between_pipelines(self):
        ...
        try:
            if not {_RENDERER_LEGACY, _RENDERER_DRAW_LIST}.issubset(
                ...
                raise unittest.SkipTest("SoStringLabel sampler regression requires both pipelines")

            fixture = get_snapshot_fixture("SoStringLabel")
            runtime = SnapshotRuntime(coin, width, height, "SoStringLabel")
            root = fixture.build(runtime)
            root.ref()
            try:
                frame_snapshot_camera(runtime, root, fixture.framing_policy)
                ...
                for renderer_name in (_RENDERER_LEGACY, _RENDERER_DRAW_LIST):
                    path = _renderer_output_path(
                        ...
                        renderer_name,
                        "actual",
                        "SoStringLabelSampler.png",
                    )
                    _render_png(harness, coin, root, path, renderer_name, frame_camera=False)
                    self.assertGreater(
                        _non_background_pixel_count(path),
                        50,
                        f"SoStringLabel rendered empty in {renderer_name}: {path}",
                    )
                    rendered[renderer_name] = path
            finally:
                root.unref()

            text_region = (width // 4, height // 4, width * 3 // 4, height * 3 // 4)
            self.assertTrue(
                _region_pixels_similar(
                    rendered[_RENDERER_LEGACY],
                    rendered[_RENDERER_DRAW_LIST],
    ...
    def test_control_points_match_between_pipelines(self):
        ...
        try:
            if not {_RENDERER_LEGACY, _RENDERER_DRAW_LIST}.issubset(
                ...
                raise unittest.SkipTest("SoFCControlPoints regression requires both pipelines")

            fixture = get_snapshot_fixture("SoFCControlPoints")
            runtime = SnapshotRuntime(coin, width, height, "SoFCControlPoints")
            root = fixture.build(runtime)
            root.ref()
            try:
                frame_snapshot_camera(runtime, root, fixture.framing_policy)
                ...
                for renderer_name in (_RENDERER_LEGACY, _RENDERER_DRAW_LIST):
                    path = _renderer_output_path(
                        ...
                        renderer_name,
                        "actual",
                        "SoFCControlPointsPipeline.png",
                    )
                    _render_png(harness, coin, root, path, renderer_name, frame_camera=False)
                    self.assertGreater(
                        _non_background_pixel_count(path),
                        10,
                        f"SoFCControlPoints rendered empty in {renderer_name}: {path}",
                    )
                    rendered[renderer_name] = path
            finally:
                root.unref()
    ...
    def test_navi_cube_transparency_matches_between_pipelines(self):
        FreeCAD, FreeCADGui, coin = _require_gui()
        ...
        try:
            if not {_RENDERER_LEGACY, _RENDERER_DRAW_LIST}.issubset(
                ...
                raise unittest.SkipTest("SoNaviCube transparency requires both pipelines")

            fixture = get_snapshot_fixture("SoNaviCubeTranslucent")
            runtime = SnapshotRuntime(coin, width, height, "SoNaviCubeTranslucent")
            root = fixture.build(runtime)
            root.ref()
            try:
                frame_snapshot_camera(runtime, root, fixture.framing_policy)
                ...
                for renderer_name in (_RENDERER_LEGACY, _RENDERER_DRAW_LIST):
                    path = _renderer_output_path(
                        ...
                        renderer_name,
                        "actual",
                        "SoNaviCubeTranslucentPipeline.png",
                    )
                    _render_png(harness, coin, root, path, renderer_name, frame_camera=False)
                    self.assertGreater(
                        _non_background_pixel_count(path),
                        100,
                        f"Translucent SoNaviCube rendered empty in {renderer_name}: {path}",
                    )
                    rendered[renderer_name] = path
            finally:
                root.unref()
