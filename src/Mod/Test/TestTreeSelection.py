...
To run tests with debug output:
    FreeCAD --set-config Debug=1 -t TestTreeSelection.TestSelectAllInstances

To run tests without debug output:
...
class TestSelectAllInstances(unittest.TestCase):
    ...
    def _is_debug_enabled(self):
        """Check if debug output is enabled via --set-config Debug=1."""
        try:
            return FreeCAD.ConfigGet("Debug") == "1"
        except Exception:
            return False

    def _debug_print(self, *args, **kwargs):
        """Print only if Debug config is enabled."""
    ...
    def _debug_print_document_structure(self):
        """Print document structure for debugging."""
        self._debug_print("\n=== Document structure ===")
        for obj in self.doc.Objects:
            self._debug_print(f"{obj.Name} ({obj.TypeId})")
            if hasattr(obj, "Group"):
                self._debug_print(f"  Group contains: {[o.Name for o in obj.Group]}")
            if hasattr(obj, "Source"):
                self._debug_print(f"  Source: {obj.Source.Name if obj.Source else None}")
            if hasattr(obj, "LinkedObject"):
                linked = obj.LinkedObject.Name if obj.LinkedObject else None
                self._debug_print(f"  LinkedObject: {linked}")
            if hasattr(obj, "Base") and hasattr(obj, "Tool"):
                base = obj.Base.Name if obj.Base else None
                tool = obj.Tool.Name if obj.Tool else None
                self._debug_print(f"  Base: {base}, Tool: {tool}")

    def _debug_print_tree_selection(self, tree, count):
        """Print tree selection for debugging."""
        self._debug_print(f"\n=== Selected items: {count} ===")
        for item in tree.selectedItems():
            parent = item.parent()
            self._debug_print(
                f"  - {item.text(0)} | parent: {parent.text(0) if parent else 'root'}"
            )
