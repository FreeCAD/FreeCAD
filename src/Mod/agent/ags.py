...
class AgentAccountBridge(QtCore.QObject):
    _open_requested = QtCore.Signal(str)

    def __init__(self, url_provider, parent=None):
        super().__init__(parent)
        self._url_provider = url_provider
        self._open_requested.connect(
            self._open_url, QtCore.Qt.ConnectionType.QueuedConnection
        )

    @QtCore.Slot()
    def manageAccount(self):
        thread = threading.Thread(
            target=self._resolve_and_open, name="agent-account", daemon=True
        )
        thread.start()
...
def create_agent_account_bridge(url_provider):
    return AgentAccountBridge(url_provider)
