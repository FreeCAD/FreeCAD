...
def LinkFromStr(strlink, document):
    ...
    subname = ''
    if feature is None:
        raise ValueError(QtGui.QApplication.translate('AttachmentEditor',"No object named {name}", None).format(name= pieces[0]))
    if len(pieces) == 2:
        subname = pieces[1]
    elif len(pieces) > 2:
        raise ValueError(QtGui.QApplication.translate('AttachmentEditor',"Failed to parse link (more than one colon encountered)", None))
    ...
    def __init__(self, obj_to_attach,
        ...
        if hasattr(obj_to_attach,'Attacher'):
            ...
            if not movable:
                ...
                raise ValueError(QtGui.QApplication.translate('AttachmentEditor',"Object {name} is neither movable nor attachable, can't edit attachment",None)
            ...
            mb.setText(QtGui.QApplication.translate('AttachmentEditor',
            ...
            mb.setWindowTitle(QtGui.QApplication.translate('AttachmentEditor',"Attachment",None))
            btnAbort = mb.addButton(QtGui.QMessageBox.StandardButton.Abort)
            btnOK = mb.addButton(QtGui.QApplication.translate('AttachmentEditor',"Continue",None),QtGui.QMessageBox.ButtonRole.ActionRole)
        ...
        self.form.setWindowTitle(QtGui.QApplication.translate('AttachmentEditor',"Attachment",None))
        ...
        if self.create_transaction:
            self.obj.Document.openTransaction(QtGui.QApplication.translate('AttachmentEditor',"Edit attachment of {feat}",None).format(feat= self.obj.Name))
    ...
    def addSelection(self,docname,objname,subname,pnt):
        ...
        if i > -1:
            ...
            if objname == self.obj.Name:
                self.form.message.setText(QtGui.QApplication.translate('AttachmentEditor',"Ignored. Can't attach object to itself!",None))
                return
            if App.getDocument(docname).getObject(objname) in getAllDependent(self.obj):
                self.form.message.setText(QtGui.QApplication.translate('AttachmentEditor',"{obj1} depends on object being attached, can't use it for attachment",None).format(obj1= objname))
                return
        ...
        try:
            ...
            for m in sugr['reachableModes'].keys():
                ...
                if len(listlistrefs) == 1:
                    ...
                    txt = QtGui.QApplication.translate('AttachmentEditor',"{mode} (add {morerefs})",None).format(mode= txt,
                                                           morerefs= u"+".join(listrefs_userfriendly))
                else:
                    txt = QtGui.QApplication.translate('AttachmentEditor',"{mode} (add more references)",None).format(mode= txt)
                item.setText(txt)
                item.setData(self.KEYmode,m)
                item.setData(self.KEYon,True)
                if m == sugr['bestFitMode']:
            ...
            for item in list_widget.findItems('', QtCore.Qt.MatchContains):
                ...
                if (m != 'Deactivated'):
                    tip += u"\n\n"
                    tip += QtGui.QApplication.translate('AttachmentEditor', "Reference combinations:", None) + u" \n\n".join(cmb)
    ...
    def updateRefButtons(self):
        try:
            ...
            for i in range(len(self.refButtons)):
                ...
                typ = QtGui.QApplication.translate('AttachmentEditor',"Reference{i}",None).format(i= str(i+1))
                ...
                btn.setText(QtGui.QApplication.translate('AttachmentEditor',"Selecting...",None) if self.i_active_ref == i else typ)
        finally:
            self.block = old_selfblock
    ...
    def updatePreview(self):
        ...
        try:
            ...
            if self.last_sugr['message'] == 'LinkBroken':
                raise ValueError(QtGui.QApplication.translate('AttachmentEditor',"Failed to resolve links. {err}",None).format(err= self.last_sugr['error']))
            ...
            if new_plm is None:
                self.form.message.setText(QtGui.QApplication.translate('AttachmentEditor',"Not attached",None))
            else:
                self.form.message.setText(    QtGui.QApplication.translate('AttachmentEditor',"Attached with mode {mode}",None)
            ...
            self.form.message.setText(QtGui.QApplication.translate('AttachmentEditor',"Error: {err}",None).format(err= str(err)))

        if new_plm is not None:
            self.form.groupBox_AttachmentOffset.setTitle(QtGui.QApplication.translate('AttachmentEditor',"Attachment Offset (in local coordinates):",None))
            self.form.groupBox_AttachmentOffset.setEnabled(True)
        else:
            self.form.groupBox_AttachmentOffset.setTitle(QtGui.QApplication.translate('AttachmentEditor',"Attachment Offset (inactive - not attached):",None))
