...
// From:
// https://github.com/Celemation/FreeCAD/blob/joel_selection_summary_demo/src/Gui/SelectionSummary.cpp
...
TopoDS_Shape getLocatedShape(const App::SubObjectT& subject, Base::Matrix4D* mat = nullptr)
{
    ...
    Part::TopoShape shape =
        Part::Feature::getTopoShape(obj, subject.getElementName(), false, mat, nullptr, true);
    if (shape.isNull()) {
        Base::Console().Log(
...
bool getShapeFromStrings(TopoDS_Shape& shapeOut,
                         const App::SubObjectT& subject,
                         Base::Matrix4D* mat)
{
...
MeasurePositionInfoPtr MeasurePositionHandler(const App::SubObjectT& subject)
{
    ...
    return std::make_shared<MeasurePositionInfo>(true,
...
MeasureAngleInfoPtr MeasureAngleHandler(const App::SubObjectT& subject)
{
    ...
    auto info =
        std::make_shared<MeasureAngleInfo>(vAdapt.isValid(), (Base::Vector3d)vAdapt, position);
    return info;
}
