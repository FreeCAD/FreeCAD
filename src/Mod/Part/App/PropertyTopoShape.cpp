    ...
    if (has_ver) {
        ...
        if (owner && owner->getDocument()->testStatus(App::Document::PartialDoc)) {
            ...
            if (owner ? owner->checkElementMapVersion(this, _Ver.c_str())
                ...
                auto ver =
                    owner ? owner->getElementMapVersion(this) : _Shape.getElementMapVersion();
                if (!owner || !owner->getNameInDocument() || !_Shape.getElementMapSize()) {
                    ...
                    if (warnedDoc != owner->getDocument()->getName()) {
                        ...
                        FC_WARN("Recomputation required for document '"
                                << warnedDoc << "' on geo element version change in "
                                << getFullName() << ": " << _Ver << " -> " << ver);
                    }
                    owner->getDocument()->addRecomputeObject(owner);
                }
            }
...
void PropertyPartShape::loadFromFile(Base::Reader& reader)
{
    ...
    TopoDS_Shape shape;
    if (ulSize > 0) {
        if (!BRepTools::Read(shape,
                             static_cast<Standard_CString>(fi.filePath().c_str()),
                             builder)) {
...
ShapeHistory::ShapeHistory(BRepBuilderAPI_MakeShape& mkShape,
...
void ShapeHistory::reset(BRepBuilderAPI_MakeShape& mkShape,
                         TopAbs_ShapeEnum type,
                         const TopoDS_Shape& newS,
                         const TopoDS_Shape& oldS)
{
...
PropertyShapeCache* PropertyShapeCache::get(const App::DocumentObject* obj, bool create)
{
    ...
    prop =
        static_cast<PropertyShapeCache*>(const_cast<App::DocumentObject*>(obj)->addDynamicProperty(
 ...
 */
bool PropertyShapeCache::getShape(const App::DocumentObject* obj,
                                  TopoShape& shape,
                                  const char* subname)
{
 ...
 */
void PropertyShapeCache::setShape(const App::DocumentObject* obj,
