...
PyObject* BSplineSurfacePy::bounds(PyObject* args)
{
    ...
    Handle(Geom_BSplineSurface) surf =
        Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
    Py::Tuple bound(4);
...
PyObject* BSplineSurfacePy::getPole(PyObject* args)
{
    ...
    try {
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
        Standard_OutOfRange_Raise_if(uindex < 1 || uindex > surf->NbUPoles() || vindex < 1
                                         || vindex > surf->NbVPoles(),
...
PyObject* BSplineSurfacePy::approximate(PyObject* args, PyObject* kwds)
{
    ...
    static const std::array<const char*, 14> kwds_interp {"Points",
                                                          "DegMin",
                                                          "DegMax",
                                                          "Continuity",
                                                          "Tolerance",
...
{
    static const std::array<const char*, 11> kwlist {"poles",
                                                     "umults",
                                                     "vmults",
                                                     "uknots",
                                                     "vknots",
    ...
    try {
        ...
        Standard_Boolean genweights =
            (weights == Py_None) ? Standard_True : Standard_False;  // cache
        Standard_Integer index1 = 0;
        Standard_Integer index2 = 0;
        for (Py::Sequence::iterator it1 = list.begin(); it1 != list.end(); ++it1) {
            index1++;
        ...
        Handle(Geom_BSplineSurface) spline = new Geom_BSplineSurface(occpoles,
                                                                     occweights,
                                                                     occuknots,
                                                                     occvknots,
                                                                     occumults,
...
PyObject* BSplineSurfacePy::buildFromNSections(PyObject* args)
{
    ...
    try {
        ...
        if (Base::asBoolean(refSurf)) {
            Handle(Geom_BSplineSurface) ref =
                Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
