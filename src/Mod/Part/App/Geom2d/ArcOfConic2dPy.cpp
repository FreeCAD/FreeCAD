...
// This File is normaly build as an include in ArcOfConic2dPyImp.cpp! Its not intended to be in a project!
...
//--------------------------------------------------------------------------
PyObject *ArcOfConic2dPy::_getattr(char *attr)				// __getattr__ function: note only need to handle new state
{
    try {
...
int ArcOfConic2dPy::_setattr(char *attr, PyObject *value) // __setattr__ function: note only need to handle new state
{
    try {
