...
PyObject* BSplineCurve2dPy::isRational(PyObject* args)
{
    ...
    Handle(Geom2d_BSplineCurve) curve =
        Handle(Geom2d_BSplineCurve)::DownCast(getGeometry2dPtr()->handle());
...
PyObject* BSplineCurve2dPy::getPole(PyObject* args)
{
    ...
    try {
        Handle(Geom2d_BSplineCurve) curve =
            Handle(Geom2d_BSplineCurve)::DownCast(getGeometry2dPtr()->handle());
        Standard_OutOfRange_Raise_if(index < 1 || index > curve->NbPoles(),
...
PyObject* BSplineCurve2dPy::approximate(PyObject* args, PyObject* kwds)
{
    ...
    static const std::array<const char*, 11> kwds_interp {"Points",
                                                          "DegMax",
                                                          "Continuity",
                                                          "Tolerance",
                                                          "DegMin",
...
PyObject* BSplineCurve2dPy::interpolate(PyObject* args, PyObject* kwds)
{
    ...
    static const std::array<const char*, 9> kwds_interp {"Points",
                                                         "PeriodicFlag",
                                                         "Tolerance",
                                                         "InitialTangent",
                                                         "FinalTangent",
    ...
    try {
        Py::Sequence list(obj);
        Handle(TColgp_HArray1OfPnt2d) interpolationPoints =
            new TColgp_HArray1OfPnt2d(1, list.size());
        Standard_Integer index = 1;
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            Base::Vector2d pnt = Py::toVector2d(*it);
        ...
        if (parameters.IsNull()) {
            aBSplineInterpolation =
                std::make_unique<Geom2dAPI_Interpolate>(interpolationPoints,
...
PyObject* BSplineCurve2dPy::buildFromPolesMultsKnots(PyObject* args, PyObject* keywds)
{
    ...
    try {
        ...
        if (mults != Py_None) {  // mults are given
            ...
            for (Py::Sequence::iterator it = multssq.begin();
                ...
                if (index < occmults.Length() || !Base::asBoolean(periodic)) {
                    sum_of_mults += (int)
                        mult;  // sum up the mults to compare them against the number of poles later
                }
                occmults(index++) = mult;
            }
        }
        ...
        Handle(Geom2d_BSplineCurve) spline = new Geom2d_BSplineCurve(occpoles,
                                                                     occweights,
                                                                     occknots,
                                                                     occmults,
                                                                     degree,
...
PyObject* BSplineCurve2dPy::toBezier(PyObject* args)
{
    ...
    Handle(Geom2d_BSplineCurve) spline =
        Handle(Geom2d_BSplineCurve)::DownCast(this->getGeom2dBSplineCurvePtr()->handle());
...
PyObject* BSplineCurve2dPy::join(PyObject* args)
{
    ...
    Handle(Geom2d_BSplineCurve) spline =
        Handle(Geom2d_BSplineCurve)::DownCast(curve2->getGeom2dBSplineCurvePtr()->handle());
