...
// This File is normaly build as an include in BezierCurve2dPyImp.cpp! Its not intended to be in a project!
...
//--------------------------------------------------------------------------
PyObject *BezierCurve2dPy::_getattr(char *attr)				// __getattr__ function: note only need to handle new state
{
    try {
...
int BezierCurve2dPy::_setattr(char *attr, PyObject *value) // __setattr__ function: note only need to handle new state
{
    try {
