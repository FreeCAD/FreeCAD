...
protected:
    static bool intersect(const Handle(Geom_Curve) & c,
...
public:
    ...
    GeomBSplineCurve(const std::vector<Base::Vector3d>& poles,
...
PartExport bool find2DLinesIntersection(const Base::Vector3d& orig1,
...
PartExport bool find2DLinesIntersection(const GeomLineSegment* lineSeg1,
                                        ...
                                        Base::Vector3d& point);
PartExport bool findFilletCenter(const GeomLineSegment* lineSeg1,
                                 const GeomLineSegment* lineSeg2,
                                 double radius,
...
PartExport std::unique_ptr<GeomSurface> makeFromSurfaceAdaptor(const BRepAdaptor_Surface&,
                                                               bool silent = false);

PartExport std::unique_ptr<GeomCurve> makeFromCurve(const Handle(Geom_Curve) &,
                                                    bool silent = false);

PartExport std::unique_ptr<GeomCurve>
makeFromTrimmedCurve(const Handle(Geom_Curve) &, double f, double l, bool silent = false);
