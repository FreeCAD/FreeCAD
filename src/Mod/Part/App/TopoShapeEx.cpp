    ...
    for(int i=1;i<=otherMap.Extent();++i) {
        ...
        for(auto &v : other.getElementMappedNames(ss.str().c_str(),true)) {
            ...
            if(appendTag && other.Tag && other.Tag!=Tag) {
                ss.str("");
                ss << (op?op:TOPOP_TAG) << other.Tag << '_';
                prefix = ss.str();
            }else if(op) {
                prefix = op;
                prefix += '_';
        ...
        const Base::Matrix4D &rclTrf, const char *op, bool appendTag, bool checkScale) 
{
    ...
    std::string _op;
    if(appendTag && shape.Tag && shape.Tag!=Tag) {
        if(!op) op = TOPOP_TRANSFORM;
        if(_ElementMap) {
            _op = op;
            _op += std::to_string(shape.Tag);
            _op += '_';
            op = _op.c_str();
        }
    }else if(_ElementMap && op && op[0]) {
        _op = op;
        _op += '_';
        op = _op.c_str();
    }
    if(op && op[0] && !_ElementMap)
        mapSubElement(TopAbs_FACE,TopoShape(*this),op,true,appendTag);
    else
        copyElementMap(shape,op);
    return *this;
}
...
TopoShape &TopoShape::makECopy(const TopoShape &shape, const char *op, bool appendTag)
{
    ...
    _Shape = BRepBuilderAPI_Copy(shape.getShape()).Shape();
    std::string _op;
    if(appendTag && shape.Tag && shape.Tag!=Tag) {
        if(!op) op = TOPOP_COPY;
        if(_ElementMap) {
    ...
    if(shapesToProcess.size() > 0){
        ...
        if (!fill){
            ...
            if (!allowOpenResult || openWires.size() == 0){
                ...
                wiresForMakingFaces.push_back(TopoShape(Tag,Hasher).makEShape(mkWire,openWires,op,false));
            }
        }
...
TopoShape &TopoShape::makERefine(const TopoShape &shape, const char *op, bool no_fail) {
    ...
    try {
        ...
        return makEShape(mkRefine,shape,op,false);
    }catch (Standard_Failure &) {
        if(!no_fail) throw;
    }
    *this = shape;
        ...
        const std::vector<TopoShape> &shapes, const char *op, bool appendTag)
{
    ...
    for(auto &v : newNames) {
        ...
        GEN_NAME(first_name,element[0],first_key.tag);
        ...
        for(auto &v : newNames) {
            for(auto &name : v.second) {
                ...
                if(info.index>1)
                    ss << info.index;
                postfix = ss.str();
                GEN_NAME(name.first.name,v.first[0],0);
                setElementName(v.first.c_str(),newName.c_str(),
                        prefix.c_str(),postfix.c_str(),&info.sids);
            }
        }
    }
