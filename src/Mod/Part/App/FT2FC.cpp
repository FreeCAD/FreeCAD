...
PyObject* getGlyphContours(FT_Face FTFace,
                           UNICHAR currchar,
                           double PenPos,
                           double Scale,
                           int charNum,
                ...
                const double tracking)      // fc coords
{
    ...
    for (i = 0; i < length; i++) {
        ...
        PenPos += kern.x;
        try {
            Py::List WireList(getGlyphContours(FTFace, currchar, PenPos, scalefactor, i, tracking),
                              true);
                           ...
                           double tracking)
{
    ...
    for (std::vector<TopoDS_Wire>::iterator iWire = ctx.Wires.begin(); iWire != ctx.Wires.end();
        ...
        if ((ctx.wDir[wCount] == CLOCKWISE) && isTTF) {  // ttf outer wire. fill inside / right
        ...
        else if ((ctx.wDir[wCount] == CLOCKWISE) && !isTTF) {  // ps inner wire. fill outside /
                                                               // right
            (*iWire).Orientation(TopAbs_REVERSED);
        }
            ...
            (*iWire).Orientation(TopAbs_REVERSED);
                 ...
                 && !isTTF) {  // ps outer wire. fill inside / left
            (*iWire).Orientation(TopAbs_REVERSED);
        }
        else {
...
int calcClockDir(std::vector<Base::Vector3d> points)
{
    ...
    for (; iPoint < stop; iPoint++) {
        double area =
            points[iPoint].x * points[iPoint + 1].y - points[iPoint + 1].x * points[iPoint].y;
        bigArea += area;
    }
