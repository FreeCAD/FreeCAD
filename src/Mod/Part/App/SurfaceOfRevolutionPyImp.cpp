...
PyObject*
SurfaceOfRevolutionPy::PyMake(struct _typeobject*, PyObject*, PyObject*)  // Python wrapper
{
...
{
    Handle(Geom_SurfaceOfRevolution) curve =
        Handle(Geom_SurfaceOfRevolution)::DownCast(getGeometryPtr()->handle());
...
void SurfaceOfRevolutionPy::setBasisCurve(Py::Object arg)
{
    ...
    if (PyObject_TypeCheck(p, &(GeometryPy::Type))) {
        ...
        try {
            Handle(Geom_SurfaceOfRevolution) curve2 =
                Handle(Geom_SurfaceOfRevolution)::DownCast(getGeometryPtr()->handle());
