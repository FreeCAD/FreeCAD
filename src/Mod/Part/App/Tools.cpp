...
*/
Handle(Geom_Surface) Part::Tools::makeSurface(const TColStd_ListOfTransient& theBoundaries,
                                              ...
                                              const Standard_Integer theMaxDeg)
{
    ...
    GeomPlate_BuildPlateSurface
        aPlateBuilder(aDeg, aNbPnts, aNbIter, aTol2d, aTol3d, anAngTol, aCurvTol);
        ...
        for (; anIt.More(); anIt.Next(), i++) {
            ...
            if (aCur.IsNull()) {
                ...
                Handle(GeomPlate_CurveConstraint) aConst =
                    new GeomPlate_CurveConstraint(aHCOS,
                                                  1 /*GeomAbs_G1*/,
                                                  aNbPnts,
                                                  aTol3d,
                                                  anAngTol,
                ...
                Handle(GeomPlate_PointConstraint) aConst =
                    new GeomPlate_PointConstraint(aGP->Pnt(), 0);
                aPlateBuilder.Add(aConst);
            }
            else {
...
void Part::Tools::getPointNormals(const TopoDS_Face& theFace,
                                  ...
                                  TColgp_Array1OfDir& theNormals)
{
    ...
    if (aPolyTri->HasNormals()) {
        ...
        const TColgp_Array1OfPnt2d* aNodesUV =
            aPolyTri->HasUVNodes() && !aSurf.IsNull() ? &aPolyTri->UVNodes() : nullptr;
        ...
        for (Standard_Integer aNodeIter = aNodes.Lower(); aNodeIter <= aNodes.Upper();
            ...
            if (!aNodesUV
                || GeomLib::NormEstim(aSurf,
                                      aNodesUV->Value(aNodeIter),
                                      aTol,
        ...
        for (Standard_Integer aNodeIter = 1; aNodeIter <= numNodes; ++aNodeIter) {
            ...
            if (!hasNodesUV
                ...
                for (thePolyConnect.Initialize(aNodeIter); thePolyConnect.More();
                    ...
                    const gp_XYZ v1(aPolyTri->Node(aTri[1]).Coord()
                                    - aPolyTri->Node(aTri[0]).Coord());
                    const gp_XYZ v2(aPolyTri->Node(aTri[2]).Coord()
                                    - aPolyTri->Node(aTri[1]).Coord());
                    const gp_XYZ vv = v1 ^ v2;
                    const Standard_Real aMod = vv.Modulus();
