...
using refTypeStringList =
    std::vector<refTypeString>;  // a set of type strings, defines which selection sets are
                                 // supported by a certain mode
...
public:  // methods
    ...
    Base::Placement calculateAttachedPlacement(const Base::Placement& origPlacement,
                                               bool* subChanged = 0);

    virtual Base::Placement
    _calculateAttachedPlacement(const std::vector<App::DocumentObject*>& objs,
     ...
     *
     * @return the resulting placement. ReverseXY property of Attacher will be automatically
     * applied.
     */
