...
bool FaceTypedCylinder::isEqual(const TopoDS_Face& faceOne, const TopoDS_Face& faceTwo) const
{
    ...
    if (!cylinderOne.Axis().IsCoaxial(cylinderTwo.Axis(),
                                      Precision::Angular(),
                                      Precision::Confusion())
        && !cylinderOne.Axis().IsCoaxial(cylinderTwo.Axis().Reversed(),
                                         Precision::Angular(),
                                         Precision::Confusion())) {
        return false;
    }
...
bool wireEncirclesAxis(const TopoDS_Wire& wire, const Handle(Geom_CylindricalSurface) & cylinder)
{
    ...
    gp_Vec cv(cylAxis.Location().X(),
              cylAxis.Location().Y(),
              cylAxis.Location().Z());  // center of cylinder
    gp_Vec av(cylAxis.Direction().X(),
              cylAxis.Direction().Y(),
