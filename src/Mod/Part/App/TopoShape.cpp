...
{
#if OCC_VERSION_HEX >= 0x070600
    if (!BRepTools::Write(this->_Shape,
...
void TopoShape::exportStl(const char* filename, double deflection) const
{
    ...
    BRepMesh_IncrementalMesh aMesh(this->_Shape,
                                   deflection,
                              ...
                              std::ostream& str) const
{
    ...
    std::size_t index = 0;
    BRepMesh_IncrementalMesh MESH(this->_Shape,
                                  dev,
                                  /*isRelative*/ Standard_False,
                                  /*theAngDeflection*/
    ...
    if (!writer.isForceXML()) {
        ...
        if (writer.getMode("BinaryBrep")) {
            writer.Stream() << writer.ind() << "<TopoShape file=\""
                            << writer.addFile("TopoShape.bin", this) << "\"/>" << std::endl;
        }
        else {
            writer.Stream() << writer.ind() << "<TopoShape file=\""
                            << writer.addFile("TopoShape.brp", this) << "\"/>" << std::endl;
        }
    }
}
...
unsigned int TopoShape::getMemSize() const
{
    if (!_Shape.IsNull()) {
        ...
        unsigned int memsize =
            (sizeof(TopoDS_Shape) + sizeof(TopoDS_TShape)) * TopoShape_RefCountShapes(_Shape);
                ...
                case TopAbs_FACE: {
                    ...
                    switch (surface.GetType()) {
                        ...
                        case GeomAbs_BezierSurface:
                            ...
                            memsize +=
                                (surface.NbUPoles() * surface.NbVPoles()) * sizeof(Standard_Real);
                        ...
                        case GeomAbs_BSplineSurface:
                            ...
                            memsize +=
                                (surface.NbUKnots() + surface.NbVKnots()) * sizeof(Standard_Real);
                            memsize +=
                                (surface.NbUPoles() * surface.NbVPoles()) * sizeof(Standard_Real);
                            memsize += (surface.NbUPoles() * surface.NbVPoles())
                                * sizeof(Geom_CartesianPoint);
...
TopoDS_Shape
TopoShape::makeTube(double radius, double tol, int cont, int maxdegree, int maxsegm) const
{
    ...
    Handle(GeomFill_LocationLaw) aLoc =
                                  ...
                                  Standard_Boolean isSolid) const
{
    ...
    BRepPrimAPI_MakeRevol mkRevol(mkFace.Face(),
                                        ...
                                        bool fill) const
{
    ...
    mkOffset.PerformByJoin(inputShape,
                           offset,
                           tol,
    ...
    if (!shapesToProcess.empty()) {
        ...
        for (TopoDS_Shape& sh : shapesToProcess) {
            switch (sh.ShapeType()) {
                ...
                default:
                    throw Base::TypeError("makeOffset2D: input shape is not an edge, wire or face "
                                          "or compound of those.");
                    break;
            }
        }
        ...
        if (fabs(offset) > Precision::Confusion()) {
            ...
            offsetShape =
                sourceWires.size() > 1 ? TopoDS_Shape(compoundSourceWires) : sourceWires[0];
        }
        ...
        if (!fill) {
            ...
            if (!allowOpenResult || openWires.empty()) {
                ...
                if (openWires.size() != 2) {
                    throw Base::CADKernelError("makeOffset2D: collective offset with filling of "
                                               "multiple wires is not supported yet.");
                }
        ...
        if (!wiresForMakingFaces.empty()) {
            ...
            if (mkFace.Shape().IsNull()) {
                throw Base::CADKernelError(
                    "makeOffset2D: making face failed (null shape returned).");
            }
            ...
            Handle(TopTools_HSequenceOfShape) result_leaves =
                xp.SeqFromCompound(result, Standard_True);
                                       ...
                                       short join) const
{
    ...
    mkThick.MakeThickSolidByJoin(this->_Shape,
                                 remFace,
                                 offset,
                                 tol,
                                 BRepOffset_Mode(offsetMode),
...
TopoDS_Shape
TopoShape::replaceShape(const std::vector<std::pair<TopoDS_Shape, TopoDS_Shape>>& s) const
{
...
TopoShape&
TopoShape::makeFace(const std::vector<TopoShape>& shapes, const char* op, const char* maker)
{
    (void)op;
    _Shape.Nullify();
...
TopoShape&
TopoShape::makeTransform(const TopoShape& shape, const gp_Trsf& trsf, const char* op, bool copy)
{
    // resetElementMap();
