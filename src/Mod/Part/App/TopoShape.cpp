...
bool TopoShape::transformShape(const Base::Matrix4D& rclTrf, bool copy, bool checkScale)
{
    ...
    if(checkScale && rclTrf.hasScale()<0) {
        transformGeometry(rclTrf);
        return true;
    }
    
    auto trsf = convert(rclTrf);
    ...
    if(copy) {
        ...
        BRepBuilderAPI_Transform mkTrf(this->_Shape, trsf, Standard_True);
        this->_Shape = mkTrf.Shape().Moved(gp_Trsf());
    }else
        this->_Shape.Move(mat);
    return false;
}
