...
TopoShape::makeElementXor(const std::vector<TopoShape>& shapes, const char* op, double tol)
{
    ...
    std::vector<TopoShape> expanded_shapes;
    ...
    for (auto it = shapes.begin(); it != shapes.end(); ++it) {
        ...
        if (s.shapeType() == TopAbs_COMPOUND) {
            if (expanded_shapes.empty()) {
                expanded_shapes.insert(expanded_shapes.end(), shapes.begin(), it);
            }
            expandCompound(s, expanded_shapes);
        }
        else if (!expanded_shapes.empty()) {
            expanded_shapes.push_back(s);
        }
    }

    const auto& inputs = expanded_shapes.empty() ? shapes : expanded_shapes;
