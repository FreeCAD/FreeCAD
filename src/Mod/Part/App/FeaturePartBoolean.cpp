...
App::DocumentObjectExecReturn* Boolean::execute()
{
    try {
        ...
        shapes.push_back(Feature::getTopoShape(Base.getValue(), Feature::GetShapeOption::ResolveLink | Feature::GetShapeOption::Transform));
        auto BaseShape = shapes[0].getShape();
        if (BaseShape.IsNull()) {
            throw NullShapeException("Base shape is null");
        }
        shapes.push_back(Feature::getTopoShape(Tool.getValue(), Feature::GetShapeOption::ResolveLink | Feature::GetShapeOption::Transform));
