...
PartExport void
closestPointsOnLines(const gp_Lin& lin1, const gp_Lin& lin2, gp_Pnt& p1, gp_Pnt& p2);
PartExport bool intersect(const gp_Pln& pln1, const gp_Pln& pln2, gp_Lin& lin);
PartExport bool
tangentialArc(const gp_Pnt& p0, const gp_Vec& v0, const gp_Pnt& p1, gp_Pnt& c, gp_Dir& a);
...
public:
     ...
     */
    static bool getTriangulation(const TopoDS_Face& face,
     ...
     * \return true if a triangulation exists or false otherwise
     */
    static bool getPolygonOnTriangulation(const TopoDS_Edge& edge,
     ...
     * fails then the precomputed point normal of the triangulation is used. \param points \param
     * face \param vertexnormals
     */
    static void getPointNormals(const std::vector<gp_Pnt>& points,
                                const TopoDS_Face& face,
                                std::vector<gp_Vec>& vertexnormals);
     ...
     * vertexes. \param face \param aPoly \param vertexnormals
     */
    static void getPointNormals(const TopoDS_Face& face,
     ...
     * lengths the triangulation of a limited parameter range is computed. \param edge \param loc
     */
     ...
     * length the polygon of a limited parameter range is computed. \param edge \param loc
     */
     ...
     * \param done
     */
    static void getNormal(const Handle(Geom_Surface) & surf,
                          double u,
                          double v,
     ...
     * \param done
     */
    static void getNormal(const TopoDS_Face& face,
                          double u,
                          double v,
                          const Standard_Real tol,
                          gp_Dir& dir,
     ...
     */
    static bool
    isConcave(const TopoDS_Face& face, const gp_Pnt& pointOfVue, const gp_Dir& direction);
};
