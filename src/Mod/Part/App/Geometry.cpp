...
bool GeomCurve::isLinear(const Handle(Geom_Curve) & curve,
                         Base::Vector3d* dir,
                         Base::Vector3d* base)
{
...
bool GeomCurve::intersect(const Handle(Geom_Curve) & curve1,
    ...
    if (curve1->IsKind(STANDARD_TYPE(Geom_BoundedCurve))
        ...
        auto checkendpoints = [&points, tol](gp_Pnt p1, gp_Pnt p2) {
            if (p1.Distance(p2) < tol) {
                points.emplace_back(Base::Vector3d(p1.X(), p1.Y(), p1.Z()),
...
GeomBezierCurve::GeomBezierCurve(const std::vector<Base::Vector3d>& poles,
                                 const std::vector<double>& weights)
{
    ...
    for (std::size_t index = 1; index <= poles.size(); index++) {
        poleArray.SetValue(index,
                           gp_Pnt(poles[index - 1].x, poles[index - 1].y, poles[index - 1].z));
                                   ...
                                   bool checkrational)
{
    ...
    this->myCurve = new Geom_BSplineCurve(p,
                                          w,
                                          k,
                                          m,
                                          degree,
 ...
 * \param continuity
 */
void GeomBSplineCurve::approximate(double tol3d,
                                   int maxSegments,
                                   int maxDegree,
    ...
    try {
        ...
        if (approx.IsDone()) {
            ...
            throw Standard_Failure(
                "Approximation of B-Spline succeeded but is outside of tolerance");
        }
        else {
                                   ...
                                   double tol3d)
{
    try {
        ...
        GeomAPI_PointsToBSpline
            fit(coords, weight1, weight2, weight3, maxDegree, continuity, tol3d);
        const Handle(Geom_BSplineCurve)& spline = fit.Curve();
        if (!spline.IsNull()) {
            setHandle(spline);
        }
...
double GeomConic::getAngleXU() const
{
    ...
    gp_Ax2 xdirref(
        center,
        normal);  // this is a reference system, might be CCW or CW depending on the creation method
...
void GeomCircle::Save(Base::Writer& writer) const
{
    ...
    writer.Stream() << writer.ind() << "<Circle "
                    ...
                    << "\" CenterZ=\"" << center.Z() << "\" NormalX=\"" << normal.X()
                    << "\" NormalY=\"" << normal.Y() << "\" NormalZ=\"" << normal.Z()
                    << "\" AngleXU=\"" << AngleXU << "\" Radius=\"" << this->myCurve->Radius()
                    << "\"/>" << std::endl;
}
...
void GeomArcOfCircle::getRange(double& u, double& v, bool emulateCCWXY) const
{
    ...
    if (emulateCCWXY) {
        Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
        double angleXU = -conic->Position().XDirection().AngleWithRef(gp_Dir(1.0, 0.0, 0.0),
                                                                      gp_Dir(0.0, 0.0, 1.0));
...
void GeomArcOfCircle::Save(Base::Writer& writer) const
{
    ...
    writer.Stream() << writer.ind() << "<ArcOfCircle "
                    ...
                    << "\" CenterZ=\"" << center.Z() << "\" NormalX=\"" << normal.X()
                    << "\" NormalY=\"" << normal.Y() << "\" NormalZ=\"" << normal.Z()
                    << "\" AngleXU=\"" << AngleXU << "\" Radius=\"" << circle->Radius()
                    << "\" StartAngle=\"" << this->myCurve->FirstParameter() << "\" EndAngle=\""
                    << this->myCurve->LastParameter() << "\"/>" << std::endl;
}
...
void GeomEllipse::setMajorAxisDir(Base::Vector3d newdir)
{
    ...
    try {
        ...
        pos.SetXDirection(
            gp_Dir(newdir.x,
                   newdir.y,
                   newdir.z));  // OCC should keep the old main Direction (Z), and change YDirection
                                // to accommodate the new XDirection.
...
void GeomArcOfHyperbola::Restore(Base::XMLReader& reader)
{
    ...
    try {
        ...
        Handle(Geom_TrimmedCurve) tmpcurve = ma.Value();
        Handle(Geom_Hyperbola) tmphyperbola =
            Handle(Geom_Hyperbola)::DownCast(tmpcurve->BasisCurve());
        Handle(Geom_Hyperbola) hyperbola =
            Handle(Geom_Hyperbola)::DownCast(this->myCurve->BasisCurve());
...
void GeomArcOfParabola::Restore(Base::XMLReader& reader)
{
    ...
    double CenterX, CenterY, CenterZ, NormalX, NormalY, NormalZ, Focal, AngleXU, StartAngle,
        EndAngle;
    // read my Element
    ...
    try {
        ...
        Handle(Geom_Parabola) parabola =
            Handle(Geom_Parabola)::DownCast(this->myCurve->BasisCurve());
...
bool find2DLinesIntersection(const Base::Vector3d& orig1,
...
bool find2DLinesIntersection(const GeomLineSegment* lineSeg1,
...
bool findFilletCenter(const GeomLineSegment* lineSeg1,
                      const GeomLineSegment* lineSeg2,
                      double radius,
...
double suggestFilletRadius(const GeomLineSegment* lineSeg1,
                           const GeomLineSegment* lineSeg2,
                           const Base::Vector3d& refPnt1,
                           const Base::Vector3d& refPnt2)
{
...
GeomArcOfCircle* createFilletGeometry(const GeomLineSegment* lineSeg1,
                                      const GeomLineSegment* lineSeg2,
...
std::unique_ptr<GeomSurface> makeFromSurface(const Handle(Geom_Surface) & s, bool silent)
{
    ...
    if (s->IsKind(STANDARD_TYPE(Geom_ToroidalSurface))) {
        ...
        Handle(Geom_RectangularTrimmedSurface) hSurf =
        ...
        Handle(Geom_SurfaceOfLinearExtrusion) hSurf =
...
std::unique_ptr<GeomCurve>
makeFromTrimmedCurve(const Handle(Geom_Curve) & c, double f, double l, bool silent)
{
    if (c.IsNull()) {
        if (!silent) {
            throw Base::ValueError("Null curve");
