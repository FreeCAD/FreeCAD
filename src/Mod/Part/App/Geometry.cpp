 ...
 */
void transformAndConvertToGeometry(std::vector<std::unique_ptr<Part::Geometry>>& geos,
...
std::vector<TopoDS_Shape> makeTextWires(std::string& text, std::string& fontFile, double height, double tracking)
{
    if (text.empty()) {
        return {};
    }
    ...
    std::vector<char> fontBuffer((std::istreambuf_iterator<char>(fontStream)),
