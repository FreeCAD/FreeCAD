...
Mirroring::Mirroring()
{
    ...
    ADD_PROPERTY_TYPE(MirrorPlane,
                      ...
                      "Plane",
                      App::Prop_None,
                      "A reference for the mirroring plane, overrides Base and Normal if set, can "
                      "be face or circle");
}
...
void Mirroring::handleChangedPropertyType(Base::XMLReader& reader,
                                          const char* TypeName,
                                          App::Property* prop)
{
...
App::DocumentObjectExecReturn* Mirroring::execute()
{
    ...
    */
    if (refObject) {
        if (refObject->isDerivedFrom(Part::Plane::getClassTypeId())
            || refObject->isDerivedFrom<App::Plane>()
            ...
            // reference is an app::link or a part::feature or some subobject
                 ...
                 || refObject->isDerivedFrom<App::Link>()) {
            if (subStrings.size() > 1) {
                throw Base::ValueError(std::string(this->getFullLabel())
                                       + ": Only 1 subobject is supported for Mirror Plane "
                                         "reference, either a plane face or a circle edge.");
            }
            ...
            if (!subStrings.empty() && subStrings[0].length() > 0) {
                ...
                shape = Feature::getShape(
                    linked);  // no subobjects were selected, so this is entire shape of feature
            }
            ...
            if (!isFace && !isEdge) {
                throw Base::ValueError(std::string(this->getFullLabel())
                                       + ": Failed to extract mirror plane, unable to determine "
                                         "which face or edge to use.");
            }

            if (isFace) {
                BRepAdaptor_Surface adapt(face);
                if (adapt.GetType() != GeomAbs_Plane) {
                    throw Base::TypeError(std::string(this->getFullLabel())
                                          + ": Mirror plane face must be planar");
                }
                TopExp_Explorer exp;
            ...
            throw Base::ValueError(std::string(this->getFullLabel())
                                   + ": Mirror plane reference must be a face of a feature or a "
                                     "plane object or a circle");
        }
