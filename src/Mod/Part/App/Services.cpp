...
Base::Rotation rotationWithZAxis(
    ...
    const Base::Vector3d& xDirection = Base::Vector3d::UnitX
)
{
    return Base::Rotation::makeRotationByAxes(xDirection, Base::Vector3d {}, zDirection);
}
...
Part::TopoShape getSubTopoShape(const App::SubObjectT& object)
{
    ...
    if (auto shape = getShape(object.getObject(), subnameNoElement + elementName); !shape.isNull()) {
        return shape;
    }

    const auto newElementName = object.getNewElementName();
    if (!newElementName.empty() && newElementName != elementName) {
        if (auto shape = getShape(object.getObject(), subnameNoElement + newElementName);
            !shape.isNull()) {
            return shape;
        }
    }
...
std::optional<Base::Placement> snapPlacementFromFace(const TopoDS_Face& face, Part::TopoShape& shape)
{
    ...
    switch (surface.GetType()) {
        ...
        case GeomAbs_Cylinder: {
            ...
            return Base::Placement(
                origin.Perpendicular(toVector(axis.Location()), toVector(axis.Direction())),
