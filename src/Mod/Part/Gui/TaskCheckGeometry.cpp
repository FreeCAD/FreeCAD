...
QVector<QString> buildCheckStatusStringVector()
{
    ...
    names.push_back(
        QObject::tr("Invalid Same Parameter Flag"));  //    BRepCheck_InvalidSameParameterFlag
    names.push_back(
        QObject::tr("Invalid Degenerated Flag"));            //    BRepCheck_InvalidDegeneratedFlag
    ...
    names.push_back(
        QObject::tr("Invalid Imbrication Of Wires"));    //    BRepCheck_InvalidImbricationOfWires
    ...
    names.push_back(
        QObject::tr("Bad Orientation Of Sub Shape"));  //    BRepCheck_BadOrientationOfSubshape
...
QVector<QString> buildBOPCheckResultVector()
{
    ...
    results.push_back(
        QObject::tr("Boolean operation: Self-intersection found"));       // BOPAlgo_SelfIntersect
    results.push_back(QObject::tr("Boolean operation: Edge too small"));  // BOPAlgo_TooSmallEdge
    results.push_back(
        QObject::tr("Boolean operation: Non-recoverable face"));  // BOPAlgo_NonRecoverableFace
    results.push_back(QObject::tr(
        "Boolean operation: Incompatibility of vertex"));  // BOPAlgo_IncompatibilityOfVertex
    results.push_back(QObject::tr(
        "Boolean operation: Incompatibility of edge"));  // BOPAlgo_IncompatibilityOfEdge
    results.push_back(QObject::tr(
        "Boolean operation: Incompatibility of face"));            // BOPAlgo_IncompatibilityOfFace
    ...
    results.push_back(QObject::tr(
        "Boolean operation: Invalid curve on surface"));  // BOPAlgo_InvalidCurveOnSurface
...
void TaskCheckGeometryResults::goCheck()
{
    ...
    for (const auto& sel : selection) {
        ...
        if (shape.IsNull()) {
            ...
            reportViewStrings.append(entry->name + QLatin1String(" | ") + entry->type
                                     + QLatin1String(" | ") + entry->error);
...
#else
                Message_ProgressScope theInnerScope(theScope.Next(),
    ...
    aMessage +=
        QLatin1String(" <a href=\"#report\">") + tr("To Report view.") + QLatin1String("</a>");
...
void TaskCheckGeometryResults::recursiveCheck(const BRepCheck_Analyzer& shapeCheck,
...
void TaskCheckGeometryResults::buildShapeContent(App::DocumentObject* pObject,
    ...
    try {
        ...
        }
        {
            ...
            Py::Tuple args(1);
            {
                ...
                Py::Float result(Py::asObject(Part::TopoShape(shape).getPyObject())
...
#if OCC_VERSION_HEX < 0x070500
int TaskCheckGeometryResults::goBOPSingleCheck(const TopoDS_Shape& shapeIn,
                                               ResultEntry* theRoot,
                                               const QString& baseName,
...
#endif
{
    ...
    BOPCheck.SetParallelMode(
        !runSingleThreaded);  // this doesn't help for speed right now(occt 6.9.1).
    ...
    reportViewStrings.append(entry->name + QLatin1String(" | ") + entry->type + QLatin1String(" | ")
                             + entry->error);
    entry->viewProviderRoot = currentSeparator;
    entry->viewProviderRoot->ref();
    goSetupResultBoundingBox(entry);
    ...
    for (; BOPResultsIt.More(); BOPResultsIt.Next()) {
        ...
        for (; faultyShapes1It.More(); faultyShapes1It.Next()) {
            ...
            reportViewStrings.append(QLatin1String("  ") + faultyEntry->name + QLatin1String(" | ")
                                     + faultyEntry->error);
...
void TaskCheckGeometryResults::setupFunctionMap()
{
    ...
    functionMap.emplace_back(TopAbs_VERTEX,
                             BRepCheck_InvalidPointOnCurve,
                             goSetupResultInvalidPointCurve);
    functionMap.emplace_back(TopAbs_FACE,
                             BRepCheck_IntersectingWires,
                             goSetupResultIntersectingWires);
    functionMap.emplace_back(TopAbs_EDGE,
...
void TaskCheckGeometryResults::currentRowChanged(const QModelIndex& current,
...
void PartGui::goSetupResultBoundingBox(ResultEntry* entry)
{
    ...
    try {
        ...
        SbVec3f boundCenter((xmax - xmin) / 2 + xmin,
                            (ymax - ymin) / 2 + ymin,
                            (zmax - zmin) / 2 + zmin);
    ...
    , resultsBtn(nullptr)
{
    ...
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_CheckGeometry"),
                                         widget->windowTitle(),
                                         true,
                                         nullptr);
    ...
    connect(autoRunCheckBox,
            &QCheckBox::toggled,
            this,
            &TaskCheckGeometryDialog::onAutoRunCheckBoxToggled);
    ...
    connect(runBOPCheckBox,
            &QCheckBox::toggled,
            this,
            &TaskCheckGeometryDialog::onRunBOPCheckBoxToggled);
    ...
    connect(runSingleThreadedCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    connect(advancedShapeContentCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    connect(argumentTypeModeCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    connect(rebuildFaceModeCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    connect(continuityModeCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    connect(mergeVertexModeCheckBox,
            &QCheckBox::toggled,
            this,
    ...
    curveOnSurfaceModeCheckBox->setToolTip(
        tr("Check for invalid curves on surfaces.  Default: true"));
    curveOnSurfaceModeCheckBox->setChecked(group->GetBool("CurveOnSurfaceMode", true));
    connect(curveOnSurfaceModeCheckBox,
            &QCheckBox::toggled,
            this,
