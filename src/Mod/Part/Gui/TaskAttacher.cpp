...
void TaskAttacher::makeRefStrings(std::vector<QString>& refstrings, std::vector<std::string>& refnames) {
    ...
    for (size_t refs_index = 0; refs_index < 4; refs_index++) {
        if ((refs_index < refs.size()) && (refs[refs_index])) {
            refstrings.push_back(makeRefString(refs[refs_index], refnames[refs_index]));
            // for Origin or Datum features refnames is empty but we need a non-empty return value
            if (refnames[refs_index].empty())
                refnames[refs_index] = refs[refs_index]->getNameInDocument();
        } else {
