    ...
    , completed(false)
{
    ...
    if (!ViewProvider->getObject()->hasExtension(
    ...
    ui->attachmentOffsetX->bind(
        App::ObjectIdentifier::parse(ViewProvider->getObject(),
                                     std::string("AttachmentOffset.Base.x")));
    ui->attachmentOffsetY->bind(
        App::ObjectIdentifier::parse(ViewProvider->getObject(),
                                     std::string("AttachmentOffset.Base.y")));
    ui->attachmentOffsetZ->bind(
        App::ObjectIdentifier::parse(ViewProvider->getObject(),
                                     std::string("AttachmentOffset.Base.z")));
    ...
    if (errMessage.length() > 0) {
        ...
        if (!attached) {
            ...
            std::vector<QString> strs =
                AttacherGui::getUIStrings(pcAttach->attacher().getTypeId(),
...
void TaskAttacher::onSelectionChanged(const Gui::SelectionChanges& msg)
{
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection) {
        ...
        try {
            ...
            updateListOfModes();
            eMapMode mmode = getActiveMapMode();  // will be mmDeactivated, if selected or if no
                                                  // modes are available
...
void TaskAttacher::updateListOfModes()
{
    ...
    if (pcAttach->AttachmentSupport.getSize() > 0) {
        ...
        // add reachable modes to the list, too, but gray them out (using lastValidModeItemIndex,
        // later)
