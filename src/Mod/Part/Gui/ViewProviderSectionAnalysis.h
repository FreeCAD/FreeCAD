...
public:
    ...
    static App::Material partColor(const App::DocumentObject* part, std::size_t index, bool nudgeAlike);

    /// True when two harvested bodies carry colours very close to each other.
    bool sourcesLookAlike() const;
...
private:
    ...
    bool planeDragging = false;   //!< the dragger is moving the plane, write only
