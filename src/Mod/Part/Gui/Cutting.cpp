...
Cutting::Cutting(Gui::View3DInventor* view, QWidget* parent)
  : QDialog(parent)
    , ui(new Ui_Cutting)
{
    // create widgets
    ui->setupUi(this);
    ui->cutCustom->setRange(-INT_MAX,INT_MAX);
    ui->cutX->setRange(-INT_MAX,INT_MAX);
    ui->cutY->setRange(-INT_MAX,INT_MAX);
    ui->cutZ->setRange(-INT_MAX,INT_MAX);
    ui->dirX->setRange(-INT_MAX,INT_MAX);
    ui->dirY->setRange(-INT_MAX,INT_MAX);
    ui->dirZ->setRange(-INT_MAX,INT_MAX);
    ...
    if (!doc) {
        Base::Console().Error("CutView error: there is document\n");
        return;
    }

    std::vector<App::DocumentObject*> ObjectsList = doc->getObjects();
    if (ObjectsList.empty()) {
        Base::Console().Error("CutView error: there are no objects in the document\n");
        return;
    }
    ...
    connect(ui->cutCustom, SIGNAL(valueChanged(double)), this, SLOT(onCutCustomvalueChanged(double)));
...
void Cutting::noDocumentActions()
{
    blockSignals(true);
    doc = nullptr;
...
void Cutting::startCutting()
{
    ...
    if (!ui->groupBoxX->isChecked() && !ui->groupBoxY->isChecked()
        && !ui->groupBoxZ->isChecked() && !ui->groupBoxCustom->isChecked()) {
    ...
    // for now disable groupBoxCustom
    ui->groupBoxCustom->setEnabled(false);

    // create an empty compound
    auto CutCompound = doc->addObject("Part::Compound", CompoundName);
    if (!CutCompound) {
        Base::Console().Error( (std::string("CutView error: ")
...
Cutting* Cutting::makeDockWidget(Gui::View3DInventor* view)
{
    // embed this dialog into a QDockWidget
    Cutting* cutting = new Cutting(view);
    Gui::DockWindowManager* pDockMgr = Gui::DockWindowManager::instance();
    // the dialog is designed that you can see the tree, thus put it to the right side
    QDockWidget* dw = pDockMgr->addDockWindow("Cutting", cutting, Qt::RightDockWidgetArea);
...
void Cutting::onGroupBoxXtoggled(bool on)
{
    if (on)
        ui->groupBoxCustom->setChecked(false);
    // reset the cut
    startCutting();
}

void Cutting::onGroupBoxYtoggled(bool on)
{
    if (on)
        ui->groupBoxCustom->setChecked(false);
    startCutting();
...
void Cutting::onGroupBoxZtoggled(bool on)
{
    if (on)
        ui->groupBoxCustom->setChecked(false);
    startCutting();
...
void Cutting::onCutXvalueChanged(double val)
{
    // there might be no document
    if (!Gui::Application::Instance->activeDocument()) {
        noDocumentActions();
    ...
    ui->cutXHS->setToolTip(QString::number(val, 'g', Base::UnitsApi::getDecimals()));
        ...
        if (!CutObject) {
            Base::Console().Warning( (std::string("CutView warning: there is no ")
        ...
        if (hasBoxY && !hasBoxZ) { // only Y
            ...
            if (!CutFeatureZ) {
                Base::Console().Error( (std::string("CutView error: there is no ")
...
void Cutting::onCutXHSsliderMoved(int val)
{
    // we cannot cut to the edge because then the result is an empty shape
...
void Cutting::onCutYvalueChanged(double val)
{
    // there might be no document
    if (!Gui::Application::Instance->activeDocument()) {
        noDocumentActions();
...
void Cutting::onCutYHSsliderMoved(int val)
{
    // we cannot cut to the edge because then the result is an empty shape
...
void Cutting::onCutZvalueChanged(double val)
{
    // there might be no document
    if (!Gui::Application::Instance->activeDocument()) {
        noDocumentActions();
...
void Cutting::onCutZHSsliderMoved(int val)
{
    // we cannot cut to the edge because then the result is an empty shape
    ...
    if (hasBoxY && !hasBoxZ) { // only Y
        ...
        if (!CutFeatureY) {
            Base::Console().Warning(
                (std::string("CutView warning: the expected ")
        ...
        auto CutFeatureZ = doc->getObject(CutZName);
        if (!CutFeatureZ) {
            Base::Console().Warning((std::string("CutView warning: the expected ")
...
void Cutting::onRefreshCutPBclicked() 
{
    // get document
    doc = Gui::Application::Instance->activeDocument()->getDocument();
    if (!doc) {
        Base::Console().Error("CutView error: there is no document\n");
        return;
    }
    // get all objects in the document
    std::vector<App::DocumentObject*> ObjectsList = doc->getObjects();
...
void Cutting::onGroupBoxCustomtoggled(bool on)
{
    if (on) {
        ui->groupBoxX->setChecked(false);
        ui->groupBoxY->setChecked(false);
...
void Cutting::onCutCustomvalueChanged(double val)
{
    // ToDo
    ;
}

void Cutting::onDirXvalueChanged(double)
{
...
void Cutting::onDirYvalueChanged(double)
{
    double x = ui->dirX->value();
    double y = ui->dirY->value();
    double z = ui->dirZ->value();
...
void Cutting::onDirZvalueChanged(double)
{
    double x = ui->dirX->value();
    double y = ui->dirY->value();
    double z = ui->dirZ->value();
...
SbBox3f Cutting::getViewBoundingBox()
{
...
void Cutting::refreshCutRanges(SbBox3f BoundingBox,
    ...
    bool forXRange, bool forYRange, bool forZRange, bool forCustom)
{
    if (!BoundingBox.isEmpty()) {
        ...
        // set the values
        if (forCustom) {
            ui->cutCustom->setValue(cnt[2]);
            minlen = minlen / steps;
            int dim = static_cast<int>(log10(minlen));
            double singleStep = pow(10.0, dim);
            ui->cutCustom->setSingleStep(singleStep);
        ...
        // set decimals
        ui->cutCustom->setDecimals(minDecimals);
