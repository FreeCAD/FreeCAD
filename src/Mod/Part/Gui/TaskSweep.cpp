...
void SweepWidget::findShapes()
{
    ...
    for (auto obj : objs) {
        ...
        if (shape.ShapeType() == TopAbs_FACE || shape.ShapeType() == TopAbs_WIRE
...
bool SweepWidget::accept()
{
    ...
    if (count < 1) {
        QMessageBox::critical(this,
                              tr("Too few elements"),
                              tr("At least one edge or wire is required."));
        return false;
    }
    if (!ok) {
        QMessageBox::critical(this,
                              tr("Invalid selection"),
                              tr("Select one or more edges from a single object."));
        return false;
    }
    for (int i = 0; i < count; i++) {
        ...
        if (name == QLatin1String(spineObject.c_str())) {
            QMessageBox::critical(this,
                                  tr("Wrong selection"),
                                  tr("'%1' cannot be used as profile and path.")
    ...
    try {
        Gui::WaitCursor wc;
        QString cmd;
        cmd = QString::fromLatin1("App.getDocument('%5').addObject('Part::Sweep','Sweep')\n"
    ...
    if (on) {
        ...
        if (matchEdge) {
            ...
            if (!isPathValid(result.front())) {
                QMessageBox::critical(this,
                                      tr("Sweep path"),
                                      tr("The selected sweep path is invalid."));
                Gui::Selection().clearSelection();
            }
        }
        else if (matchPart) {
...
void TaskSweep::clicked(int id)
{
    if (id == QDialogButtonBox::Help) {
        QString help =
            QApplication::translate("PartGui::TaskSweep",
