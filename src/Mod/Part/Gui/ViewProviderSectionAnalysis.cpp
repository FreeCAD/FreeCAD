...
App::Material ViewProviderSectionAnalysis::partColor(
    const App::DocumentObject* part,
    std::size_t index,
    bool nudgeAlike
)
{
    ...
    if (nudgeAlike) {
        // If alike, just give the colour very gentle tint-nudge!
        constexpr float tint = 0.35F;
        const Base::Color apart = distinctColor(index).diffuseColor;
        mat.diffuseColor.set(
            mat.diffuseColor.r + (apart.r - mat.diffuseColor.r) * tint,
            mat.diffuseColor.g + (apart.g - mat.diffuseColor.g) * tint,
            mat.diffuseColor.b + (apart.b - mat.diffuseColor.b) * tint,
            0.0F
        );
    }
    return mat;
...
bool ViewProviderSectionAnalysis::sourcesLookAlike() const
{
    // Fuzzy match tolerance, per component
    ...
    for (const HarvestedBody& body : harvestCache) {
        ...
        auto* vp = dynamic_cast<Gui::ViewProviderGeometryObject*>(
            Gui::Application::Instance->getViewProvider(body.source)
        );
        if (!vp) {
            continue;
        }
        const auto& appearance = vp->ShapeAppearance.getValues();
        if (!appearance.empty()) {
            colours.push_back(appearance.front().diffuseColor);
        }
    }

    for (std::size_t i = 0; i < colours.size(); ++i) {
        for (std::size_t j = i + 1; j < colours.size(); ++j) {
            if (std::fabs(colours[i].r - colours[j].r) <= tolerance
                && std::fabs(colours[i].g - colours[j].g) <= tolerance
                && std::fabs(colours[i].b - colours[j].b) <= tolerance) {
                return true;
            }
        }
    }
...
void ViewProviderSectionAnalysis::updateCapFromScene()
{
    ...
    // Asked once per rebuild rather than per body: the answer is a property of
    // the set, and every cap has to agree on it.
    const bool alike = sourcesLookAlike();
    ...
    for (std::size_t index = 0; index < harvestCache.size(); ++index) {
        ...
        const App::Material colour = (perSolidColors() || appearance.empty())
            ? partColor(body.source, index, alike)
            : appearance.front();
...
void ViewProviderSectionAnalysis::updateData(const App::Property* prop)
{
    ...
    if (prop == &feat->PlaneNormal || prop == &feat->PlaneOffset || prop == &feat->FlipCut) {
        ...
        // Undo can move things too. Skip mid-drag, when dragger is the source.
        if (!planeDragging) {
            syncPlaneDragger();
        }
