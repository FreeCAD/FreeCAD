...
void CmdPartPointsFromMesh::activated(int iMsg)
{
    ...
    if (found != geoms.end()) {
        ...
        double defaultDistance =
            getDefaultDistance(Base::freecad_dynamic_cast<Part::Feature>(*found));
        ...
        bool ok;
        distance = QInputDialog::getDouble(Gui::getMainWindow(),
...
static void
_copyShape(const char* cmdName, bool resolve, bool needElement = false, bool refine = false)
{
    Gui::WaitCursor wc;
    ...
    if (parametric) {
        ...
        std::for_each(objs.begin(), objs.end(), [](App::DocumentObject* obj) {
            try {
                ...
                Gui::cmdAppDocumentArgs(obj->getDocument(),
                ...
                auto newObj = App::GetApplication().getActiveDocument()->getActiveObject();
                Gui::copyVisualT(newObj->getNameInDocument(),
                                 "ShapeAppearance",
                                 obj->getNameInDocument());
                Gui::copyVisualT(newObj->getNameInDocument(),
                                 "LineColor",
