...
void SectionCut::onCutXvalueChanged(double val)
{
    ...
    if (!pcBox) {
        Base::Console().error(
            "{}",
            (std::string("Section cut error: ") + std::string(BoxXName)
    ...
    if (hasBoxY && !hasBoxZ) {  // only Y
        ...
        if (!pcCut) {
            Base::Console().error(
                "{}",
                (std::string("Section cut error: ") + std::string(CutZName)
...
void SectionCut::onCutYvalueChanged(double val)
{
    ...
    if (!pcBox) {
        Base::Console().error(
            "{}",
            (std::string("Section cut error: ") + std::string(BoxYName)
             + std::string(" is no Part::Box object. Cannot proceed.\n"))
                .c_str()
        );
        return;
    ...
    if (hasBoxZ) {
        ...
        if (!CutFeatureZ) {
            Base::Console().error(
                "{}",
                (std::string("Section cut error: there is no ") + std::string(CutZName)
                 + std::string("\n"))
                    .c_str()
            );
            return;
...
void SectionCut::onCutZvalueChanged(double val)
{
    ...
    if (!pcBox) {
        Base::Console().error(
            "{}",
            (std::string("Section cut error: ") + std::string(BoxZName)
             + std::string(" is no Part::Box object. Cannot proceed.\n"))
                .c_str()
        );
        return;
...
void SectionCut::FlipClickedHelper(const char* BoxName)
{
    ...
    if (!pcBox) {
        Base::Console().error(
            "{}",
            (std::string("Section cut error: ") + std::string(BoxName)
             + std::string(" is no Part::Box object. Cannot proceed.\n"))
                .c_str()
        );
        return;
...
App::DocumentObject* SectionCut::findOrCreateObject(const char* objName)
{
    ...
    if (!object) {
        Base::Console().warning(
            "{}",
            (std::string("Section cut warning: there is no ") + std::string(objName)
...
App::DocumentObject* SectionCut::CreateBooleanFragments(App::Document* doc)
{
    ...
    if (!object) {
        Base::Console().error(
            "{}",
            (std::string("Section cut error: ") + std::string(CompoundName)
             + std::string(" could not be added\n"))
                .c_str()
        );
        return nullptr;
