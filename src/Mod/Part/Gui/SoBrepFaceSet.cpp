...
static void renderOverlayFaces(
    ...
    const SbColor& color,
    Gui::SelectionColors::VisualRole role,
    bool onTop
)
{
    ...
    Gui::SelectionColors::applyMaterial(state, faceSet, role, color, &packer);
...
void SoBrepFaceSet::renderHighlight(SoGLRenderAction* action, SelContextPtr ctx)
{
    ...
    renderOverlayFaces(
        ...
        overlayCoordIndex,
        ctx->highlightColor,
        Gui::SelectionColors::VisualRole::Preselection,
        onTop
    );
}
...
void SoBrepFaceSet::renderSelection(SoGLRenderAction* action, SelContextPtr ctx, bool /*push*/)
{
    ...
    if (ctx->isSelectAll()) {
        ...
        renderOverlayFaces(
            ...
            overlayCoordIndex,
            ctx->selectionColor,
            Gui::SelectionColors::VisualRole::Selection,
            false
        );
        return;
    }
...
bool SoBrepFaceSet::overrideMaterialBinding(SoGLRenderAction* action, SelContextPtr ctx, SelContextPtr ctx2)
{
    ...
    if (singleColor > 0 && !partialRender) {
        ...
        if (ctx && ctx->isHighlightAll()) {
            Gui::SelectionColors::applyPackedMaterial(
                state,
                this,
                Gui::SelectionColors::VisualRole::Preselection,
                ctx->highlightColor,
