...
void CmdPartCommon::activated(int iMsg)
{
    ...
    if (Sel.empty()) {
        QMessageBox::warning(
            Gui::getMainWindow(),
            QObject::tr("Wrong selection"),
            QObject::tr("Please select two shapes or more. Or, select one compound containing two "
                        "or more shapes to compute the intersection between."));
        return;
    }
...
void CmdPartFuse::activated(int iMsg)
{
    ...
    if (numShapes < 2) {
        QMessageBox::warning(Gui::getMainWindow(),
                             QObject::tr("Wrong selection"),
                             QObject::tr("Please select two shapes or more. Or, select one "
