...
class TaskFaceAppearancesGuiTest(unittest.TestCase):
    ...
    def test_face_selection_updates_widget_state(self):
        ...
        FreeCADGui.Selection.clearSelection()
        self.assertFalse(widget_material.isEnabled(), "Widget should be disabled when no face is selected.")
        self.assertFalse(btn_custom.isEnabled(), "Custom Appearance button should be disabled when no face is selected.")
        
        # Scenario B: Single face selected -> Widgets should be enabled
        FreeCADGui.Selection.addSelection(self.doc.Name, "Box", "Face1")
        self.assertTrue(widget_material.isEnabled(), "Widget should be enabled after selecting Face1.")
        ...
        self.assertTrue(widget_material.isEnabled(), "Widget should be enabled with multiple faces selected.")
        
        label_text = label_element.text().replace(" ", "")
        self.assertIn(label_text, ["[1,2]", "[2,1]"], f"Label did not update correctly for multiple faces. Got: {label_text}")

        # Scenario D: Clear selection -> Widgets should be disabled again
        FreeCADGui.Selection.clearSelection()
        self.assertFalse(widget_material.isEnabled(), "Widget should be disabled after clearing selection.")
