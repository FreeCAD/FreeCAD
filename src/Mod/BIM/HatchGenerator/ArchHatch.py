...
class CustomHatchFeature:
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "RotationDeg",
            "Hatch",
            "Legacy motif rotation around Z (degrees).",
        )
        obj.RotationDeg = 0.0
    ...
    def _execute_definition_preview(self, fp, start_time):
        ...
        try:
            ...
            hatch, _ = buildHatchShape(
                ...
                rotationDeg=(
                    float(getattr(fp, "GridRotationDeg", fp.RotationDeg))
                    if bool(getattr(fp, "LinkTileRotationToGrid", True))
                    else float(getattr(fp, "TileRotationDeg", fp.RotationDeg))
                ),
                gridRotationDeg=float(
                    getattr(fp, "GridRotationDeg", getattr(fp, "RotationDeg", 0.0))
                ),
                tileRotationDeg=(
                    float(getattr(fp, "GridRotationDeg", fp.RotationDeg))
                    if bool(getattr(fp, "LinkTileRotationToGrid", True))
