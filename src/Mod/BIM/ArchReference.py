...
class ArchReference:
    ...
    def getColors(self, obj):
        ...
        if filename.lower().endswith(".fcstd"):
            zdoc = zipfile.ZipFile(filename)
            if "GuiDocument.xml" not in zdoc.namelist():
                return None
            colorfile = None
            ...
            if not colorfile:
                return None
            if colorfile not in zdoc.namelist():
                return None
            colors = []
...
class ViewProviderArchReference:
    ...
    def getInventorString(self, obj):
        ...
        if not obj.Part:
            return None
        zdoc = zipfile.ZipFile(filename)
        if "Document.xml" not in zdoc.namelist():
            return None
        ivfile = None
        with zdoc.open("Document.xml") as docf:
            writemode1 = False
        ...
        if not ivfile:
            return None
        if ivfile not in zdoc.namelist():
            return None
        f = zdoc.open(ivfile)
        buf = f.read()
        buf = buf.decode("utf8")
