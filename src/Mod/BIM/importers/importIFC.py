...
def insert(srcfile, docname, skip=[], only=[], root=None, preferences=None):
    ...
    if preferences["MERGE_MODE_ARCH"] == 3:
        ...
        for host, children in additions.items():
            ...
            if (
                ...
                and (
                    ...
                    not in ["Site", "Building", "Floor", "BuildingPart", "Project"]
                )
            ):
                # avoid huge fusions
        ...
        for obj in objects.values():
            if obj.isDerivedFrom("Part::Feature"):
                if obj.Shape.isNull() and Draft.getType(obj) not in ["Site", "Project"]:
                    Arch.rebuildArchShape(obj)
    ...
    if filesize:
        _msg(
            "Finished importing {0} MB in {1} seconds, or {2} s/MB".format(
