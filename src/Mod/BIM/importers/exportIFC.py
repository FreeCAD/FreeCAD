    ...
    for k, v in products.items():
        if k not in treated:
            if (not buildings) or (k != buildings[0].Name):
                if Draft.getType(FreeCAD.ActiveDocument.getObject(k)) not in [
                    "Site",
                    "Building",
                    "Floor",
                    "BuildingPart",
                    ...
                    if FreeCAD.ActiveDocument.getObject(k).IfcType not in [
                        "Building",
                        "Building Storey",
                        "Site",
                        "Space",
