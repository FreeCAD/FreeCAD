...
class Transaction(object):
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        if exc_value is None:
            ...
            _err("Transaction failed but DEBUG_GEOMETRY is set. Committing transaction anyway.")
            self.document.commitTransaction()
        else:
...
class SH3DImporter:
    ...
    def import_sh3d_from_string(self, home: str):
        ...
        if self.progress_bar:
            self.progress_bar.start("Importing SweetHome 3D Home. Please wait ...", -1)
    ...
    def import_sh3d_from_filename(self, filename: str):
        ...
        if self.progress_bar:
            self.progress_bar.start(
                f"Importing SweetHome 3D file '{self.filename}'. Please wait ...", -1
            )
    ...
    def get_fc_object(self, id, sh_type):
        ...
        if self.preferences["MERGE"] and id in self.fc_objects:
            ...
            if sh_type:
                assert fc_object.shType == sh_type, (
    ...
    def _import_elements(self, parent, xpath):
        ...
        if self.progress_bar:
            ...
            self.progress_bar.start(
                f"Step {current_step}/{total_steps}: importing {total_elements} '{tag_name}' elements. Please wait ...",
                total_elements,
            )
    ...
    def _create_slabs(self):
        ...
        if self.progress_bar:
            self.progress_bar.stop()
            self.progress_bar.start(
                f"Step {current_step}/{total_steps}: Creating {total_elements} 'slab' elements. Please wait ...",
    ...
    def _decorate_surfaces(self):
        ...
        if self.progress_bar:
            self.progress_bar.stop()
            self.progress_bar.start(
                f"Decorating {total_elements} elements. Please wait ...", total_elements
            )
...
class WallHandler(BaseHandler):
    ...
    def _create_wall(self, floor, prev, next, elm):
        ...
        assert wall_details is not None, (
        ...
        if sweep.Shape.isNull() or not sweep.Shape.isValid():
            if is_wall_straight:
                _log("Sweep's shape is invalid, using ruled surface instead ...")
                ...
                _wrn("Sweep's shape is invalid, but mitigation is not available!")
...
class DoorOrWindowHandler(BaseFurnitureHandler):
    ...
    def _create_door(self, floor, elm):
        ...
        if not bb_face:
            _err(
                f"Weird: None of BoundingBox's faces for doorOrWindow#{elm.get('id')} has the expected angle ({angle}º). Can't create window."
            )
            if debug_geometry:
                self._debug_shape(
...
class FurnitureHandler(BaseFurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        floor = self.get_floor(level_id)
        assert floor != None, (
            f"Missing floor '{level_id}' for <pieceOfFurniture> '{furniture_id}' ..."
        )
