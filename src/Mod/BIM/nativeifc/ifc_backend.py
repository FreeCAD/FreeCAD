...
LEGACY_EXPORT_MODULES = (*GUID_MODULES, "ifcopenshell.geom")
...
_CAPABILITIES = (READ, GUID, IMPORT, MULTICORE_IMPORT, EXPLORER, EXPORT, LEGACY_EXPORT)
...
class BackendStatus:
    """Result of validating the installed IfcOpenShell ifc_backend."""
...
def _load(capability: str) -> tuple[dict[str, ModuleType], BackendStatus]:
    ...
    elif capability == LEGACY_EXPORT:
        required_modules = LEGACY_EXPORT_MODULES
    ...
    if capability == LEGACY_EXPORT:
        required_callables["ifcopenshell.file"] = getattr(module, "file", None)
        required_callables["ifcopenshell.guid.new"] = getattr(
            modules["ifcopenshell.guid"], "new", None
        )
