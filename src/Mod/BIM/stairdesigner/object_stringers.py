    ...
    def _update_stringers(
        ...
        for side_index, side in enumerate(("Left", "Right"), start=1):
            ...
            for (flight_index, sections), part in zip(
                ...
                flight = flights[flight_index]
                boards = [sections]
                starts_at_newel = ends_at_newel = False
                if (
                    ...
                    and (
                        ...
                        or (
                            ...
                            and str(flight.ExitDirection) != "Straight"
                        )
                    )
                ):
                    original = next(run for index, run in source_runs if index == flight_index)
                    boards = boundary_stringer_runs(original, side, balanced_footprint)
                    points = [getattr(section, side.lower()) for section in original]
                    starts_at_newel = (
                        flight_index == 0
                        and side.lower() in str(flight.EntryDirection).lower()
                        and math.dist(points[0], points[1]) < 1e-7
                    )
                    ends_at_newel = (
                        flight_index == len(flights) - 1
                        and side.lower() in str(flight.ExitDirection).lower()
                        and math.dist(points[-1], points[-2]) < 1e-7
                    )
                shapes = []
                if (
                    stringer_type == "Housed stringer"
                    and (starts_at_newel or ends_at_newel)
                    and not stair.StringerCustomWidth
                    and not part.OverrideWidth
                ):
                    back_extension = 0.0
                    if stair.RisersEnabled:
                        back_extension = _quantity_value(stair.StepRiserOverlap)
                        if not stair.PriorityToRiser:
                            back_extension += _quantity_value(stair.RiserThickness)
                    for index, board in enumerate(boards):
                        if (starts_at_newel and index == 0) or (
                            ends_at_newel and index == len(boards) - 1
                        ):
                            width = max(
                                width,
                                newel_stringer_width(
                ...
                for board_index, board in enumerate(boards):
                    if stringer_type == "Housed stringer":
                        shape = make_housed_stringer_shape(
                            ...
                            width,
                            overlap,
                            start_extension if board_index == 0 else 0.0,
                            end_extension if board_index == len(boards) - 1 else 0.0,
                            _quantity_value(part.NosingOffset),
                            str(part.NosingOffsetDirection),
                            _quantity_value(stair.Nosing),
                            straight_profile=(
                                (starts_at_newel and board_index == 0)
                                or (ends_at_newel and board_index == len(boards) - 1)
                            ),
                        )
                    else:
                        shape = make_notched_stringer_shape(
                            board,
                            riser_height,
                            _quantity_value(stair.StepThickness),
                            side,
                ...
                part.Shape = (
                    ...
                    if len(shapes) > 1
                    else shapes[0] if shapes else Part.Shape()
                )
                part.Label = (
