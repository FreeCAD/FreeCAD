...
def _local_step_expansion_faces(
    ...
    for section, distance in (
        ...
        if not footprint.isInside(probe, 1e-6, True):
            ...
            continue
        left_at_post = any(math.dist(section.left, point) < 1e-7 for point in corner_points)
        right_at_post = any(math.dist(section.right, point) < 1e-7 for point in corner_points)
        if distance < 0.0 and not local_expansion and not (left_at_post or right_at_post):
        ...
        # A fan tip ends in the newel. Extending its nosing back to the
        # boundary line magnifies the overhang by 1 / sin(winder angle),
        # potentially carrying it right through the post.
        shifted_left = (
            shifted_left
            if left_at_post
            else _continued_section_endpoint(
                section.left,
                shifted_left,
                side,
                section.tangent,
