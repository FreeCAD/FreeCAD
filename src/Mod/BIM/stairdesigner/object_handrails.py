...
class StairHandrailMixin:
    ...
    def _update_handrails(
        ...
        for flight_index, sections in flight_runs:
            ...
            endpoint = (
                balanced_footprint is not None
                and str(flight.FlightType) == "Straight"
                and (
                    (flight_index == 0 and str(flight.EntryDirection) != "Straight")
                    or (
                        flight_index == len(flights) - 1 and str(flight.ExitDirection) != "Straight"
                    )
                )
            )
            for side in ("Left", "Right"):
                runs = (
                    boundary_stringer_runs(
                        sections, side, balanced_footprint, include_post_sections=True
                    )
                    if endpoint
                    else [sections]
                )
                rail_runs.extend(
                    (flight_index, side, run, index, len(runs), endpoint)
                    for index, run in enumerate(runs)
                )
        prepared_runs = []
        for flight_index, side, sections, run_index, run_count, endpoint in rail_runs:
            flight = flights[flight_index]
            ...
            path_sections = (
                sections
                if endpoint
                else prepared_handrail_runs.get(side, {}).get(flight_index, sections)
            )
            post_tops = None
            if (
                endpoint
                and stringer is not None
                and str(stringer.StringerType) == "Housed stringer"
            ):
                points = [getattr(section, side.lower()) for section in path_sections]
                first = 0
                last = len(points) - 1
                while first < last and math.dist(points[first + 1], points[0]) < 1e-7:
                    first += 1
                while last > first and math.dist(points[last - 1], points[-1]) < 1e-7:
                    last -= 1
                if first or last < len(points) - 1:
                    height = _quantity_value(stair.HandrailHeightAboveNosing)
                    post_tops = (
                        max(section.top_elevation for section in path_sections[: first + 1])
                        + height,
                        max(section.top_elevation for section in path_sections[last:]) + height,
                    )
                    # The rail follows the board between the newels. Fan
                    # elevations determine post height, not the rail pitch.
                    path_sections = path_sections[first : last + 1]
            ...
            if path is None:
                continue
            if endpoint:
                fit_handrail_to_nosings(path, path_sections, side)
            if post_tops is not None:
                path["post_tops"] = post_tops
            prepared_runs.append(
                (flight_index, side, sections, run_index, endpoint, stringer, center_offset, path)
            )

        # Fit every board first, then join rails at a common corner elevation.
        # Independently raised rails otherwise leave a step beside a short post.
        for first, second in zip(prepared_runs, prepared_runs[1:]):
            if first[:2] != second[:2] or not (first[4] and second[4]):
                continue
            incoming, outgoing = first[-1], second[-1]
            joint = _line_intersection(
                (incoming["start"], incoming["tangent"]), (outgoing["start"], outgoing["tangent"])
            )
            if joint is not None:
                incoming["end"] = joint
                outgoing["start"] = joint
                for path in (incoming, outgoing):
                    path["length"] = math.dist(path["start"], path["end"])
            if "post_tops" in incoming:
                elevation = incoming["top_elevations"][-1]
            elif "post_tops" in outgoing:
                elevation = outgoing["top_elevations"][0]
            else:
                elevation = max(incoming["top_elevations"][-1], outgoing["top_elevations"][0])
            incoming["top_elevations"][-1] = elevation
            outgoing["top_elevations"][0] = elevation
        ...
        for (
            ...
            def stringer_span(sample):
                ...
                inward = (tangent[1], -tangent[0]) if side == "Left" else (-tangent[1], tangent[0])
                point = (
            ...
            for post_index, fraction in enumerate((0.0, 1.0)):
                ...
                first_floor_post = flight_index == 0 and run_index == 0 and post_index == 0
                if first_floor_post:
                    bottom = 0.0
                ...
                top = max(sample["top"], path.get("post_tops", (sample["top"],) * 2)[post_index])
                top += _quantity_value(stair.HandrailPostAboveTopRail)
                desired.append(
                    {
                        "role": "HandrailPost",
                        "flight": flight,
                        "side": side,
                        "element_index": run_index * 2 + post_index,
                        "shape": (
                            make_handrail_vertical_member_shape(
                                sample["point"],
                                sample["tangent"],
                ...
                desired.append(
                    {
                        ...
                        "flight": flight,
                        "side": side,
                        "element_index": run_index * 10000 + picket_index,
                        "shape": (
                            make_handrail_vertical_member_shape(
                                sample["point"],
                                sample["tangent"],
