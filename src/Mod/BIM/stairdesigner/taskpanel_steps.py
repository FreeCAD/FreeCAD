...
class StepPanelMixin:
    ...
    def _make_stair_panel(self):
        ...
        self.walking_line_offset = _length_spin(_value(self.stair.WalkingLineOffset), 0.0)
        self.walking_line_offset.setObjectName("walking_line_offset")
        self.walking_line_offset.setToolTip(
            translate(
                "BIM",
                "Distance from the outside of balanced turns; limited to half the flight width",
            )
        )
        widget.ui.dimensionForm.addRow(
            translate("BIM", "Walking line offset"), self.walking_line_offset
        )
        self.walking_line_offset.valueChanged.connect(self._apply)
