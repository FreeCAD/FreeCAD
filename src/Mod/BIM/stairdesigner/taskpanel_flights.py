...
class FlightPanelMixin:
    ...
    def _flight_type_changed(self, record):
        ...
        if flight_type.startswith("Circular") or flight_type.endswith("landing"):
            for name in ("entry_direction", "exit_direction"):
                editor = record.get(name)
                if editor is not None:
                    blocked = editor.blockSignals(True)
                    self._select_data(editor, "Straight")
