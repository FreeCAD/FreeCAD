...
def balanced_winder_sections(
    ...
    cumulative = _walking_path_stations(dense, walking_line_offset)
    ...
    for index in range(tread_count + 1):
        ...
        if index == 0 and entry_direction == "Straight":
            # Nosing is a physical distance, whereas stations on the offset
            # walking line include turning length. Anchor the first riser to
            # the footprint instead of extrapolating that station scale.
            center = vertices[0]
...
def _landing_winder_sections(
    ...
    chunk_lengths = [_walking_path_stations(chunk, walking_line_offset)[-1] for chunk in chunks]
    ...
    if landing_count == 0 or distributed_tread_count < required_free_treads:
        return balanced_winder_sections(
            ...
            walking_line_offset=walking_line_offset,
        )
    ...
    for index, (chunk, count) in enumerate(zip(chunks, chunk_tread_counts)):
        ...
        chunk_sections = _sample_dense_path(chunk, count, chunk_extras, walking_line_offset)
        extra_cursor += count
        if index:
...
def _walking_path_stations(points, outer_offset=None):
    """Measure going on the outer parallel of the balanced walking path.

    An offset curve gains ``offset * turning_angle`` in length at a turn.
    Measuring only the central curve therefore overestimates the number of
    straight treads and leaves too few winders. Clamp the requested distance
    to the outer half of narrow flights. None retains the centre-line API.
    """
    lengths = [math.hypot(b[0] - a[0], b[1] - a[1]) for a, b in zip(points, points[1:])]
    additions = [0.0] * len(lengths)
    if outer_offset is not None:
        for index in range(1, len(points) - 1):
            ...
            offset = max(b[2] / 2.0 - max(float(outer_offset), 0.0), 0.0)
            extra = offset * angle / 2.0
            additions[index - 1] += extra
            additions[index] += extra
    cumulative = [0.0]
    for length, extra in zip(lengths, additions):
        cumulative.append(cumulative[-1] + length + extra)
    return cumulative
...
def _sample_dense_path(points, interval_count, extra_widths=None, walking_line_offset=None):
    """Sample one uninterrupted straight/balanced path chunk."""

    cumulative = _walking_path_stations(points, walking_line_offset)
    total_length = cumulative[-1]
