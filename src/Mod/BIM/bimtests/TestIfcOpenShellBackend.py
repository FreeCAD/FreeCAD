...
class TestIfcOpenShellBackend(unittest.TestCase):
    ...
    def test_invalidate_refreshes_python_import_caches(self):
        with patch.object(ifc_backend.importlib, "invalidate_caches") as invalidate_caches:
            ifc_backend.invalidate()
    ...
    def test_missing_package_is_unavailable(self):
        ...
        with self.assertRaises(ifc_backend.IfcOpenShellUnavailable):
            ifc_backend.get_backend()
    ...
    def test_reports_validated_capabilities(self):
        ...
        with patch.object(ifc_backend.importlib, "import_module", side_effect=modules.__getitem__):
            status = ifc_backend.get_status()
            loaded = ifc_backend.get_backend()
    ...
    def test_import_does_not_require_export_apis(self):
        modules = _modules()
        import_modules = {name: modules[name] for name in ifc_backend.IMPORT_MODULES}
        with patch.object(
            ifc_backend.importlib, "import_module", side_effect=import_modules.__getitem__
        ):
            status = ifc_backend.get_status(capability=ifc_backend.IMPORT)
            loaded = ifc_backend.get_backend(capability=ifc_backend.IMPORT)
            geom = ifc_backend.get_module("ifcopenshell.geom", capability=ifc_backend.IMPORT)
    ...
    def test_explorer_does_not_require_serialized_or_export_apis(self):
        modules = _modules(serialized=False)
        explorer_modules = {name: modules[name] for name in ifc_backend.EXPLORER_MODULES}
        with patch.object(
            ifc_backend.importlib, "import_module", side_effect=explorer_modules.__getitem__
        ):
    ...
    def test_refresh_revalidates_after_environment_change(self):
        ...
        attempts = iter(
            (
                ...
                *(modules[name] for name in ifc_backend.REQUIRED_MODULES),
            )
        )
        ...
        with patch.object(ifc_backend.importlib, "import_module", side_effect=load):
            self.assertFalse(ifc_backend.get_status().available)
            self.assertTrue(ifc_backend.get_status(refresh=True).available)
