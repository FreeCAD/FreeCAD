...
"""Tests for structural IFC GUID generation through the runtime ifc_backend."""
...
class TestIfcStructuralExport(unittest.TestCase):
    ...
    def test_structural_entities_receive_unique_guids_in_supported_schemas(self):
        ...
        ifcopenshell = ifc_backend.get_backend(capability=ifc_backend.GUID)

        identifiers = set()
        for schema in ("IFC2X3", "IFC4"):
            model = ifcopenshell.file(schema=schema)
            identifier = ifc_backend.new_guid()
    ...
    def test_structural_relationship_uses_backend_guid(self):
        ...
        with patch.object(ifc_backend, "new_guid", return_value="structural-guid"):
