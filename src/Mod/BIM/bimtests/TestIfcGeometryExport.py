...
class TestIfcGeometryExport(unittest.TestCase):
    def _load_adapter(self, fake_exporter):
        ...
        # ``from importers import exportIFC`` resolves the package attribute
        # when the importers package is already loaded by the test runner.
        with patch.object(importers, "exportIFC", fake_exporter):
