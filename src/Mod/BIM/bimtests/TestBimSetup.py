...
def _status(available, *, version="", error=""):
    return ifc_backend.BackendStatus(available=available, version=version, error=error)
...
class TestBimSetup(unittest.TestCase):
    ...
    def test_status_refreshes_all_capability_profiles(self):
        ...
        with (
            patch.object(ifc_backend, "invalidate") as invalidate,
            patch.object(ifc_backend, "get_status", return_value=healthy) as get_status,
        ):
        ...
        self.assertEqual(
            ...
            [
                call(capability=ifc_backend.IMPORT),
                call(capability=ifc_backend.MULTICORE_IMPORT),
                call(capability=ifc_backend.EXPORT),
            ],
        )
