    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                ...
                "Create multiple BIM Structures from a selected base, using each selected edge as an extrusion path",
            ),
        }
    ...
    def setPreset(self, i):
        ...
        if elt:
            if elt in self.precast.PrecastTypes:
                ...
                self.vLength.setText(
                    FreeCAD.Units.Quantity(float(Presets[p][4]), FreeCAD.Units.Length).UserString
                )
                self.vWidth.setText(
                    FreeCAD.Units.Quantity(float(Presets[p][5]), FreeCAD.Units.Length).UserString
                )
...
class StructureTaskPanel(ArchComponent.ComponentTaskPanel):
    def __init__(self, obj):
        ...
        self.selectToolButton.setToolTip(
            QtGui.QApplication.translate(
                "Arch", "Select object or edges to be used as a Tool (extrusion path)", None
            )
        )
