...
# *   Copyright (c) 2020 onwards - The FreeCAD developers community         *
# *                                                                         *
...
"""This module provides the Wall object and view provider.  Walls are simple
...
class Wall(ArchComponent.Component):
    """The Wall object"""
    ...
    def execute(self, obj):
        ...
        if obj.Base:
            if hasattr(obj.Base, "Shape"):
                ...
                elif getattr(obj, "MakeBlocks", False):
                    blocks = self.make_blocks(obj, extdata)
                    if blocks:
                        base = blocks
            elif obj.Base.isDerivedFrom("Mesh::Feature"):
                shape = self.make_from_mesh(obj)
                if shape:
                    base = shape
        if not base:
    ...
    def make_blocks(self, obj, extdata):
        ...
        if not getattr(obj, "MakeBlocks", False):
            return None
        if not getattr(self, "basewires", None):
            return None
        if not getattr(self, "extv", None):
            return None
        if not getattr(self, "bplates", None):
            return None
        if not extdata:
            return None
        ...
        if len(self.basewires) == 1:
            ...
            for i in range(entire):
                ...
                blocks.append(b)
            if rest:
                rest = self.extv.Length - (entire * fsize)
    ...
    def make_extrusion(self, obj, extdata):
        ...
        base = None
        self.bplates = extdata[0]
        self.extv = extdata[2].Rotation.multVec(extdata[1])
        if isinstance(self.bplates, list):
            shps = []
            ...
            for b in self.bplates:
                b.Placement = extdata[2].multiply(b.Placement)
                b = b.extrude(self.extv)

                # See getExtrusionData() - not fusing baseplates there - fuse solids here
                # Remarks - If solids are fused, but exportIFC.py use underlying
                # baseplates w/o fuse, the result in ifc look slightly different from in FC.
                if sketchBaseToFuse:
                    if shps:
                    ...
                    shps.append(b)
                # TODO - To let user to select whether to fuse (slower) or to do a
                # compound (faster) only ?
            ...
            self.bplates.Placement = extdata[2].multiply(self.bplates.Placement)
        ...
        if obj.Shape:
            for f in obj.Shape.Faces:
                if f.normalAt(0, 0).getAngle(FreeCAD.Vector(0, 0, -1)) < 0.01:
                    if (
                        abs(abs(f.CenterOfMass.z) - abs(obj.Shape.BoundBox.ZMin))
                        < 0.001
                    ):
                        faces.append(f)
        return faces
...
class ViewProviderWall(ArchComponent.ViewProviderComponent):
                    ...
                    if hasattr(obj.Material, "Materials"):
                        ...
                        if len(activematerials) == len(obj.Shape.Solids):
                            ...
                            for i, mat in enumerate(activematerials):
                                ...
                                cols.extend(
                                    [c for j in range(len(obj.Shape.Solids[i].Faces))]
                                )
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        QtCore.QObject.connect(
            actionFlipDirection, QtCore.SIGNAL("triggered()"), self.flip_direction
        )
...
_Wall = Wall
_ViewProviderWall = ViewProviderWall
