...
class Arch_Wall:
    ...
    def Activated(self):
        ...
        self.WALL_BASE = PARAMS.GetInt(
            "WallBaseline", 0
        )  # 0 = no base, 1 = draft, 2 = sketch
        ...
        if not done:
            ...
            FreeCAD.activeDraftCommand = (
                self  # register as a Draft command for auto grid on/off
            )
    ...
    def create_wall(self):
        ...
        if self.WALL_BASE == 0:
            ...
            if self.MultiMat:
                FreeCADGui.doCommand(
                    "wall.Material = FreeCAD.ActiveDocument." + self.MultiMat.Name
                )
            ...
            FreeCADGui.doCommand(
                "trace=Part.LineSegment(FreeCAD."
                + str(l.StartPoint)
                + ",FreeCAD."
                + str(l.EndPoint)
                + ")"
    ...
    def taskbox(self):
        ...
        items = [
            translate("Arch", "Center"),
            translate("Arch", "Left"),
            translate("Arch", "Right"),
        ]
