...
class Arch_ToggleIfcBrepFlag:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_ToggleIfcBrepFlag", "Force an object to be exported as B-rep or not"
            ),
        }
        ...
        for obj in FreeCADGui.Selection.getSelection():
            if hasattr(obj, "Subtractions"):
                for sub in obj.Subtractions:
                    if Draft.getType(sub) not in ["Window", "Roof"]:
                        if mode is None:
        ...
        if len(walls) == 1:
            if Draft.getType(walls[0]) == "Wall":
                ...
                if ok:
                    ...
                    FreeCAD.Console.PrintWarning(
                        translate("Arch", "The selected wall contains no subwall to merge")
                    )
                    return
            else:
