        ...
        if os.path.exists(customModulePath):
            ...
            if customModules:
                ...
                for customModule in customModules:
                    ...
                    if "Preflight" not in mod.__file__:
    ...
    def testStoreys(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if (
                    hasattr(obj, "IfcRole")
                    and (obj.IfcRole not in ["Building", "Building Storey", "Site"])
                ) or (
                    hasattr(obj, "IfcType")
                    and (obj.IfcType not in ["Building", "Building Storey", "Site"])
                ):
                    ok = False
                    ancestors = obj.InListRecursive
                    # append extra objects not in InList
                    if hasattr(obj, "Host") and obj.Host not in ancestors:
                        ancestors.append(obj.Host)
                    if hasattr(obj, "Hosts"):
                        for h in obj.Hosts:
                            if h not in ancestors:
                                ancestors.append(h)
                ...
                if hasattr(obj, "IfcAttributes") and (Draft.getType(obj) != "BuildingPart"):
                    for prop in ["Length", "Width", "Height"]:
                        if prop in obj.PropertiesList:
                            if ("Export" + prop not in obj.IfcAttributes) or (
                        ...
                        if found:
                            for i in range(int(len(psets[found]) / 2)):
                                ...
                                if p in obj.IfcProperties:
                                    if (found not in obj.IfcProperties[p]) or (
                                        t not in obj.IfcProperties[p]
                                    ):
                                        ok = False
                                else:
                                    ok = False
