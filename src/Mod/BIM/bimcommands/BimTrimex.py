...
class BimTrimex(Trimex):
    ...
    def _setupObjectTrimex(self, sel):
        ...
        obj_type = utils.get_type(self.obj)
        if obj_type == "Wall":
            if self.obj.Base:
    ...
    def _setupBaseTrimex(self, face):
        ...
        def set_base(new_points):
            end = self.lockedActivePoint
            self._setBaseEndpoint(base, points[end], new_points[end])

        return self._setupAxisTrimex(face, points, axes, set_base)
    ...
    def _setupWallTrimex(self, face):
        ...
        def set_wall(new_points):
            ...
            if length > 0:
                obj.Length = length
                obj.Placement.Base = (new_points[0] + new_points[1]) * 0.5
    ...
    def _setBaseEndpoint(self, base, old, new):
        ...
        if utils.get_type(base) == "Part::Line":
...
Gui.addCommand("BIM_Trimex", BimTrimex())
