    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                ...
                "Create multiple BIM Structures from a selected base,"
                "using each selected edge as an extrusion path",
            ),
        }
    ...
    def Activated(self):
        ...
        if len(selex) >= 2:
            ...
            FreeCAD.Console.PrintError(
                translate(
                    "Arch",
                    "Please select the base object first"
                    "and then the edges to use as extrusion paths",
                )
                + "\n"
            )
...
class CommandStructure:
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_Structure",
                "Creates a structure from scratch or"
                "from a selected object (sketch, wire, face or solid)",
            ),
        }
    ...
    def createItemlist(self, baselist):
    ...
    def update(self, point, info):
        ...
        if FreeCADGui.Control.activeDialog():
            ...
            if self.Height >= self.Length:
                delta = FreeCAD.Vector(0, 0, self.Height / 2)
            else:
                delta = FreeCAD.Vector(self.Length / 2, 0, 0)
            delta = self.wp.get_global_coords(delta, as_vector=True)
            if self.modec.isChecked():
                ...
                if self.bpoint:
                    ...
                    self.tracker.on()
                    pl = (point.sub(self.bpoint)).Length
                    self.vLength.setText(
                        FreeCAD.Units.Quantity(pl, FreeCAD.Units.Length).UserString
                    )
                else:
                    self.tracker.off()
    ...
    def setCategory(self, i):
        ...
        if i > 1:
            ...
            fpresets = self.createItemlist(self.pSelect)
            self.vPresets.addItems(fpresets)
            self.setPreset(0)
        elif i == 1:
        ...
        if elt:
            if elt in self.precast.PrecastTypes:
                ...
                params.set_param_arch(
                    "StructurePreset", ";".join([str(i) for i in self.Profile])
                )
    ...
    def rotateLH(self):
        ...
        vl = self.vLength.text()
        vh = self.vHeight.text()
        self.vLength.setText(vh)
        self.vHeight.setText(vl)

    def rotateLW(self):

        vl = self.vLength.text()
        vw = self.vWidth.text()
        self.vLength.setText(vw)
        self.vWidth.setText(vl)
