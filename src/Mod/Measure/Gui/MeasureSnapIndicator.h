...
public:
    ...
    // Preview the resolved snap in the hovered document's 3D view: markers for point
    // types, a dashed line through the two Axis endpoints. Empty points, an
    // unpreviewable type, or no 3D view hides.
    void show(Gui::Document* guiDoc, const std::vector<gp_Pnt>& points, Measure::MeasureSnapMode type);
    void hide();

    // Remove the overlay and release the scene graph.
    void detach();

private:
    bool attach(Gui::Document* guiDoc);
