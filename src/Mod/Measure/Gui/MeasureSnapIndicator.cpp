...
// Scene-graph root of a document's 3D view, or nullptr when it has none. Each
// viewer owns its own root, so the overlay must follow the hovered document
// rather than whichever window happens to be active.
SoGroup* sceneGraphOf(Gui::Document* guiDoc)
{
    if (!guiDoc) {
        return nullptr;
    }
...
bool MeasureSnapIndicator::attach(Gui::Document* guiDoc)
{
    SoGroup* sg = sceneGraphOf(guiDoc);
    if (!sg) {
        detach();
        return false;
    }
    ...
    detach();
    sg->addChild(pRoot);
    // No per-view deletion signal exists, so ref the group to keep it valid.
    sg->ref();
    pSceneGraph = sg;
    attached = true;
    return true;
...
void MeasureSnapIndicator::show(
    Gui::Document* guiDoc,
    const std::vector<gp_Pnt>& points,
    Measure::MeasureSnapMode type
)
{
    if (type == Measure::MeasureSnapMode::Axis) {
        if (points.size() != 2 || !attach(guiDoc)) {
            hide();
            return;
        }
    ...
    if (points.empty() || !name || !attach(guiDoc)) {
        hide();
        return;
    }
