...
// Auto and None resolve to nothing by intent, and a requested snap that cannot
// apply warns and resolves to nothing so the pair uses nearest points.
...
)
{
    ...
    if (!snap) {
        Base::Console().warning(
            "{}: {} mode does not apply to {}; using nearest points for that element.\n",
            fullName,
...
AutoElement classifyAuto(const TopoDS_Shape& shape)
{
    ...
    if (shape.ShapeType() == TopAbs_EDGE) {
        ...
        if (!MeasureSnap::edgeHasCurve(edge)) {
            return AutoElement::Other;
        }
...
// Collinear or crossing lines would read 0 with the feet in empty space, so two
// straight edges only measure axis-to-axis when their lines are parallel and apart.
...
bool MeasureDistance::isValidSelection(const App::MeasureSelection& selection)
{
    ...
    for (auto element : selection) {
        ...
        if (type != App::MeasureElementType::POINT && type != App::MeasureElementType::LINE
            ...
            && type != App::MeasureElementType::CYLINDER && type != App::MeasureElementType::SPHERE
            && type != App::MeasureElementType::TORUS && type != App::MeasureElementType::SURFACE
            && type != App::MeasureElementType::DISC && type != App::MeasureElementType::CONE) {
            return false;
        }
    }
    return true;
...
)
{
    ...
    if (n1.IsParallel(n2, Precision::Angular())) {
        gp_Vec v(pl2.Location(), ref);
        Standard_Real dist = v.Dot(gp_Vec(n1));

        setValues(ref, ref.Translated(-dist * gp_Vec(n1)));
        return true;
    }
    ...
    if (inter.IsDone() && inter.NbSolutions() > 0) {
        ...
        Standard_Real t = ElCLib::Parameter(line, ref);
...
bool MeasureDistance::distanceLineLine(
...
)
{
    ...
    if (d1.IsParallel(d2, Precision::Angular())) {
        Standard_Real t2 = ElCLib::Parameter(l2, ref);
...
bool MeasureDistance::distancePlaneLine(
...
)
{
    ...
    if (!plane1.IsNull() && !plane2.IsNull()) {
        return distancePlanePlane(plane1, plane2, ref1);
    }
    if (!line1.IsNull() && !line2.IsNull()) {
        return distanceLineLine(line1, line2, ref1);
    }
    if (!plane2.IsNull() && !line1.IsNull()) {
        return distancePlaneLine(plane2, line1, ref1);
    }
    if (!plane1.IsNull() && !line2.IsNull()) {
        return distancePlaneLine(plane1, line2, ref2);
    }
...
)
{
    ...
    if (r1.kind == SnapKind::Axis && r2.kind == SnapKind::Axis) {
        ...
        // Extrema failed, and falling through to the mixed branch below would
        // silently measure a different pair.
        Base::Console().warning(
            "{}: could not resolve the axis-to-axis snap; using nearest points instead.\n",
            fullName.c_str()
        );
...
App::DocumentObjectExecReturn* MeasureDistance::execute()
{
    ...
    // Infinite datum pairs need their own handling, since extrema on an unbounded
    // shape has no meaningful solution.
