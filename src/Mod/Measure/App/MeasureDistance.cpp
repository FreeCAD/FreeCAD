...
)
{
    ...
    auto planeLine = [this](
        ...
        IntAna_IntConicQuad inter(ln, pl, Precision::Confusion());
...
App::DocumentObjectExecReturn* MeasureDistance::execute()
{
    ...
    if (subObject1 && subObject2) {
        if ((subObject1->isDerivedFrom<Part::DatumPlane>()
             || subObject1->isDerivedFrom<Part::DatumLine>())
            && (subObject2->isDerivedFrom<Part::DatumPlane>()
                || subObject2->isDerivedFrom<Part::DatumLine>())) {

            auto v = App::GeoFeature::getGlobalPlacement(
                         subObject1,
                         subject1.getObject(),
                         subject1.getSubName()
            )
            ...
            v = App::GeoFeature::getGlobalPlacement(
                    subObject2,
                    subject2.getObject(),
                    subject2.getSubName()
            )
            ...
            if (distanceInfiniteInfinite(shape1, shape2, ref1, ref2)) {
                return DocumentObject::StdReturn;
            }
        }
    }
