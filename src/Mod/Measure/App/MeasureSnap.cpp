...
// A single circular-edge wire is not recognized as a circle by
// BRepAdaptor_CompCurve, so wires are not handled.
...
// No cursor picks the first edge endpoint, which keeps recompute deterministic
...
// The point is the cursor, else the face bbox centre, projected onto the axis
// so it sits on the visible geometry.
...
bool MeasureSnap::edgeHasCurve(const TopoDS_Edge& edge)
{
...
TopoDS_Shape MeasureSnap::resolveShape(const App::SubObjectT& subject)
{
    try {
        ...
        // Same resolver the measurement engine uses, so the preview lands where the
        // snap will and follows link/assembly placement chains.
...
std::vector<gp_Pnt> MeasureSnap::previewPoints(const TopoDS_Shape& shape, MeasureSnapMode mode)
{
