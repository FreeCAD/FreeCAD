...
PythonDebugModule::PythonDebugModule()
  : Py::ExtensionModule<PythonDebugModule>("Debugger"), debugServer(0)
{
    add_varargs_method("start_embedded_debugger", &PythonDebugModule::start,
        "Start the debugger.");
    add_varargs_method("close", &PythonDebugModule::close,
        "Stop the debugger.");
...
Py::Object PythonDebugModule::start(const Py::Tuple &a)
{
    char* file;
    if (!PyArg_ParseTuple(a.ptr(), "s", &file))
        throw Py::Exception();

    quint16 port = 4567;
    QHostAddress host(QHostAddress::LocalHost);
    if (debugServer.state() != QProcess::Running) {
        QString app;
        app = QDir::toNativeSeparators(QString::fromUtf8(App::GetApplication().GetHomePath()) + 
#if defined (FC_DEBUG) && defined(_MSC_VER)
            QLatin1String("bin/DebugServerD"));
#else
            QLatin1String("bin/DebugServer"));
#endif

        QStringList args;
        args << QString::fromAscii("-host=%1").arg(host.toString())
             << QString::fromAscii("-port=%1").arg(port);
        debugServer.start(app, args);

        if (!debugServer.waitForStarted()) {
            throw Py::Exception("Start to debug server failed");
        }
    }

    Debugger::DebugClient* client = new Debugger::DebugClient();
    client->setScriptFile(QString::fromUtf8(file));
    client->connectToHost(host, port);

    return Py::None();
}

Py::Object PythonDebugModule::close(const Py::Tuple &a)
{
    debugServer.terminate();
    return Py::None();
}
...
PythonDebugger::PythonDebugger(DebugClient* client)
...
void PythonDebugger::sendContext(const QString& file, int line)
{
    d->client->sendContext(file, line);
    d->client->waitForReadyRead();
}
