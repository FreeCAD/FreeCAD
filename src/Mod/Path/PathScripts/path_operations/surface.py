...
__title__ = "Path Surface Operation Strategy"
__author__ = "russ4262 (Russell Johnson)"
__url__ = "http://www.freecadweb.org"
__doc__ = "Class and strategy implementation of 3D Surface operation."
__contributors__ = ""
...
class SurfaceOperation:
    ...
    produces a Surface path.
    '''

    def __init__(self, obj, parent, initialize=True):
        '''__init__(obj, parent) ... Constructor for Surface operation class.'''
        PathLog.debug('SurfaceOperation.__init__()')
    ...
    def getPropertyDefaultValues(self):
        ...
        if warn:
            ...
            msg += translate('PathSurface',
                             'Initializing LinearDeflection to 0.01 mm.')
    ...
    def getPropertyEditorModes(self):
        ...
        if not self.hasProperties:
            msg = translate('surface', 'No operation-specific properties.')
        ...
        return {
            ...
            'CutPatternAngle': P0,
            'PatternCenterAt': P2,
            'PatternCenterCustom': P2,
            'OptimizeStepOverTransitions': R2,
            'AvoidLastX_Faces': AF,
            'ProfileEdges': R2
        }
    ...
    def onChanged(self, prop):
        ...
        if prop in ['ScanType', 'CutPattern', 'Base', 'BoundBox']:
    ...
    # Primary public method to generate Surface path data
    def execute(self, parent, obj):
        '''execute() ...
        Execute the Surface operation.
    ...
    def _makePath(self, obj):
        '''_makePath(obj) ...
        Control the process for the Surface operation.
        '''
        PathLog.track()

        CMDS = list()

        self.module = 'PathSurface'
        ...
        if pPM:
            ...
            for m in range(0, len(self.job.Model.Group)):
                ...
                PathSurfaceSupport._prepareModelSTLs(self, self.job, obj, m, ocl)
                ...
                if FACES[m]:
                    ...
                    PathSurfaceSupport._makeSafeSTL(self, self.job, obj, m, FACES[m], VOIDS[m], ocl)
                    # Process model/faces - OCL objects must be ready
                    CMDS.extend(self._processCutAreas(self.job, obj, m, FACES[m], VOIDS[m]))
                else:
                    PathLog.debug('No data for model base: {}'.format(self.job.Model.Group[m].Label))
    ...
    def _opApplyPropertyLimits(self, obj):
        '''_opApplyPropertyLimits(obj) ...
        Apply necessary limits to user input property values before performing
        main operation.
        '''
        # Limit start index
    ...
    def _processPlanarOp(self, JOB, obj, mdlIdx, cmpdShp, fsi):
        ...
        if obj.ProfileEdges != 'None':
            ...
            if prflShp is False:
                ...
                PathLog.error(msg)
                return list()
            self.parent._addDebugObject(prflShp, 'NewProfileShape')
        ...
        if obj.ProfileEdges != 'Only':
            self.parent._addDebugObject(cmpdShp, 'CutArea')
