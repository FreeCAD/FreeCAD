...
class ObjectDressup:
    ...
    def getArcTrochoidal(self,obj,curCommand):
        ...
        #PathLog.info("current X{} Y{}".format(currendPos.x,currendPos.y))
        arcdir = 'CCW'
        ...
        #PathLog.info("arclength {} ".format(arclength))
        arccenter = FreeCAD.Vector(currendPos.x + curCommand.I, currendPos.y + curCommand.J, currendPos.z)
        #PathLog.info("arccenter X{} Y{}".format(arccenter.x,arccenter.y))
        radius = math.hypot(curCommand.I, curCommand.J)
        #PathLog.info("radius {} ".format(radius))
        #Data from circle command / Angle includet and direction Start End Angle  
        ...
        #PathLog.info("anglelineStartCenter deg {} ".format(math.degrees(anglelineStartCenter)))
        anglelineCenterEnd = D.angle(curCommand.Placement.Base.sub(arccenter), FreeCAD.Base.Vector(1, 0, 0), FreeCAD.Base.Vector(0, 0, -1))
        #PathLog.info("anglelineCenterEnd deg {} ".format(math.degrees(anglelineCenterEnd)))
        ...
        #arcangle = anglelineCenterStart+anglelineCenterEnd
        #PathLog.info("arcangle deg {} ".format(math.degrees(arcangle)))
        #PathLog.info("anglelineCenterStart deg {} ".format(math.degrees(anglelineCenterStart)))
        #neuarcangle= anglelineCenterStart+anglelineCenterEnd
        #PathLog.info("neuarcangle deg {} ".format(math.degrees(neuarcangle)))
        stepsAlongCircle = int(abs(arclength/obj.StepWidth.Value))#howmany steps to the circle
        ...
        results.append(extendcommand)
        #PathLog.info("NEXT ARC \n\n")
        return results          

    def getTrochoidal(self, obj,curCommand):
        '''returns Trochoidal Path G-code.'''
        global currLocation
        #PathLog.info("HIER Straight {}".format(obj.StepWidth))
        results = []
        op = PathDressup.baseOp(obj.Base)
        tc = PathDressup.toolController(obj.Base)
        horizFeed = tc.HorizFeed.Value
