...
class ObjectClearing(PathOp.ObjectOp):
    ...
    def opFeatures(self, obj):
        ...
        return PathOp.FeatureTool | PathOp.FeatureHeightsDepths \
            | PathOp.FeatureStartPoint | PathOp.FeatureCoolant | PathOp.FeatureBaseEdges \
            | PathOp.FeatureBaseFaces | PathOp.FeatureExtensions
    ...
    def opPropertyDefinitions(self):
        ...
        props = [
            ...
            ("App::PropertyFloat", "StepOver", "PathOptions", QtCore.QT_TRANSLATE_NOOP("App::Property", "Percent of cutter diameter to step over on each pass")),
            ...
            ('App::PropertyEnumeration', 'HandleMultipleFeatures', "SelectionOptions", QtCore.QT_TRANSLATE_NOOP("App::Property", 'Choose how to process multiple Base Geometry features.')),
    ...
    def opExecute(self, obj, getsim=False): # pylint: disable=arguments-differ
        ...
        for shape, isHole, details in shapes:
            ...
            if obj.CutPattern == "Adaptive":
                ...
                strategy.setAdaptiveAttributes(obj.StartDepth.Value,
            ...
            if obj.KeepToolDown:
                strategy.safeBaseShape = obj.Base[0][0].Shape  # Set safe base shape used to check
    ...
    def getTargetShape(self, obj, isPreview=False, facesOnly=False):
        ...
        if workingAreas:
            if facesOnly:
                removalShapes.extend([(wa, False, 'pathClearing') for wa in workingAreas])
            else:
                ...
                removalShapes.extend([(face.removeSplitter().extrude(extent), False, 'pathClearing') for face in workingAreas])
