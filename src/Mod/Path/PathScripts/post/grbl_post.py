...
def drill_translate(outstring, cmd, params):
  ...
  try:
    if MOTION_MODE == 'G91':
      trBuff += linenumber() + "G90" + "\n"  # force absolute coordonates during cycles
      ...
      if TRUE :
        while 1:
          if last_Stop_Z != RETRACT_Z :
            back_off = last_Stop_Z + a_bit
            trBuff += linenumber() + 'G0 Z' + format(float(back_off.getValueAs(UNIT_FORMAT)) , strFormat) + "\n" 
