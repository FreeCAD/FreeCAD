...
def processArguments(argstring):
	...
	global LINE_NUMBER_START        # Line number starting value
	global LINE_NUMBER_STEP         # Line number increment value
	global PREAMBLE                 # Preamble gcode
	global POSTAMBLE                # Postable gcode
	global PRE_OPERATION            # Pre operation text will be inserted before every operation
	global POST_OPERATION           # Post operation text will be inserted after every operation
	...
	global UNIT_FORMAT_US_IMP       #
	global UNIT_FORMAT_METRIC       #
	global UNIT_SPEED_FORMAT        # Text with specific units over time units
	global UNIT_SPEED_FORMAT_US_IMP #
	global UNIT_SPEED_FORMAT_METRIC #
...
def export(objectslist, filename, argstring):
	...
	if (0 < problems_count) or (0 < warnings_count):
		print("export: postprocessing: done, warnings: {}, problems: {}, see GCode output for details.".format(warnings_count, problems_count))
	else:
...
def linenumber():
	global LINE_NUMBER_START
	global LINE_NUMBER_STEP
	global LINENR
				...
				if param in c.Parameters:
					if param == 'F' and (currLocation[param] != c.Parameters[param] or REPEAT_ARGUMENTS):
						...
						if (not REPEAT_ARGUMENTS) and (param in currLocation) and (currLocation[param] == c.Parameters[param]):
							...
							commandlist.append(param + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string))
