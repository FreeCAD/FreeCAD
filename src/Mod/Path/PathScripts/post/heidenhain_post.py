...
parser.add_argument('--skip-params', action='store_true', help='suppress R F M parameters where already stored')
parser.add_argument('--use-fmax', action='store_true', help='suppress feedrate and use FMAX instead with rapid movements')
parser.add_argument('--fmax-value', default='8000', help='feedrate to use instead of FMAX, default=8000')
parser.add_argument('--axis-decimals', default='3', help='number of digits of axis precision, default=3')
parser.add_argument('--feed-decimals', default='0', help='number of digits of feedrate precision, default=0')
parser.add_argument('--spindle-decimals', default='0', help='number of digits of spindle precision, default=0')
parser.add_argument('--solve-comp', action='store_true', help='try to get RL or RR real path where compensation is active')
parser.add_argument('--solve-lbl', action='store_true', help='try to replace repetitive movements with LBL')
...
def HEIDEN_ToolCall(tool_Params):
    ...
    if H_Tool_ID == "0" and H_Tool_Speed == 0 and H_Tool_Comment == "":
        ...
        return(
            "TOOL CALL " + str(H_Tool_ID) + " " + H_Tool_Axis[MACHINE_WORK_AXIS] +
...
def HEIDEN_Arc(arc_Params, arc_direction, arc_comp, arc_feed, arc_rapid, arc_M_funct, Cmd_Number):
    ...
    H_Arc_CC = { # CC initial values to overwrite
        'X': 99999,
        'Y': 99999,
        'Z': 99999
        }
    ...
    def Axis_Select(a, b, c, incr):
        if a in arc_Params and b in arc_Params:
            _H_ArcCenter = (
                incr + HEIDEN_Format(a, H_Arc_CC[a]) + " " +
                incr + HEIDEN_Format(b, H_Arc_CC[b])
                )
            if c in arc_Params and arc_Params[c] != MACHINE_LAST_POSITION[c]:
                ...
                _H_ArcPoint = HEIDEN_PolarArc(
                    H_Arc_CC[a], H_Arc_CC[b], H_Arc_P_NEW[a], H_Arc_P_NEW[b], arc_Params[c], c, incr
                    )
            else:
                _H_ArcPoint = (
                    " " + incr + HEIDEN_Format(a, H_Arc_P_NEW[a]) +
                    " " + incr + HEIDEN_Format(b, H_Arc_P_NEW[b])
                    )
...
# COMPENSATION_DIFF_STATUS[1] = False # we can skip this control if already in compensation
    ...
    if(
        ...
        MACHINE_LAST_CENTER['Y'] == H_Arc_CC['Y'] and
...
def HEIDEN_Drill(drill_Obj, drill_Params, drill_Type, drill_feed): # create a drill cycle and movement
    ...
    if 'Q' in drill_Params: drill_Defs['INCR'] = drill_Params['Q']
    ...
    if GetPoint != None:
        if LBLIZE_PATH_LEVEL != GetLevel:
            ...
            if len(STORED_LBL) != 0 and len(STORED_LBL[-1][-1]) == 0:
                STORED_LBL[-1].pop()
            STORED_LBL.append(
                [[len(POSTGCODE) + GetAddit, len(POSTGCODE) + GetAddit]]
                )
        else:
