...
def convert(toolpath,Z=0.0,vf=1.0,hf=2.0,Z0=None,RampAngle=None,stop_after_length=None):
    '''convert(toolpath,Z=0.0,vf=1.0,hf=2.0,RampAngle=None) Converts lines and arcs to G1,G2,G3 moves. Returns a string.'''
    # create the path from the offset shape

    lastpt = None
    output = ""

    if RampAngle is None:
        tanA = 0.0
        Z0 = Z
    else:
        if Z0 is None:
...
def MakePath(wire,Side,radius,clockwise,ZClearance,StepDown,ZStart,ZFinalDepth,firstedge=None,PathClosed=True,SegLen=0.5,VertFeed=1.0,HorizFeed=2.0,RampAngle=None):
    ...
    if PathClosed:
        while ZCurrent > ZFinalDepth:
            paths += convert(toolpath,ZCurrent,VertFeed,HorizFeed,ZLast,RampAngle)
            ZLast = ZCurrent
            ZCurrent = ZCurrent - abs(StepDown)
        paths += convert(toolpath,ZFinalDepth,VertFeed,HorizFeed,ZLast,RampAngle)
    else:
        while ZCurrent > ZFinalDepth:
            paths += convert(toolpath,ZCurrent,VertFeed,HorizFeed,ZLast,RampAngle)
