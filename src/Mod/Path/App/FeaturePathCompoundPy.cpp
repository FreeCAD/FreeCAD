...
// This File is normaly build as an include in FeaturePathCompoundPyImp.cpp! Its not intended to be in a project!
...
//--------------------------------------------------------------------------
PyObject *FeaturePathCompoundPy::_getattr(char *attr)				// __getattr__ function: note only need to handle new state
{
    try {
...
int FeaturePathCompoundPy::_setattr(char *attr, PyObject *value) // __setattr__ function: note only need to handle new state
{
    try {
