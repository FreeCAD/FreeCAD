...
def process_emn(doc, filename):
    ...
    for emnline in emnlines:
        ...
        if not len(emnrecords): continue
        ...
        if current_section == ".BOARD_OUTLINE" and section_counter > 2:
            board_outline.append([int(emnrecords[0]), float(emnrecords[1]) * emn_unit, float(emnrecords[2]) * emn_unit,
                                  float(emnrecords[3])])
        if current_section == ".DRILLED_HOLES" and section_counter > 1 and float(
                emnrecords[0]) * emn_unit > ignore_hole_size:
            drills.append(
        ...
        if current_section == ".PLACEMENT" and section_counter > 1 and fmod(section_counter, 2) == 0:
            place_item = [emnrecords[2], emnrecords[1], emnrecords[0]]
    ...
    FreeCAD.Console.PrintMessage(
        "Proceed " + str(Process_board_outline(doc, board_outline, drills, board_thickness)) + " outlines\n")
...
def mid_point(prev_vertex, vertex, angle):
    ...
    midpoint = Base.Vector((vertex.x + prev_vertex.x) / 2 + shift * cos(basic_angle),
                           (vertex.y + prev_vertex.y) / 2 + shift * sin(basic_angle), 0)
    return midpoint
...
def process_emp(doc, filename, placement, board_thickness):
    ...
    for empline in emplines:
        ...
        if len(emprecords[0]) > 4 and emprecords[0][0:4] == ".END":
            ...
            if comp_PartNumber != "":
                if comp_height == 0:
                    comp_height = 0.1
                comps.append(
                    (comp_PartNumber, [Process_comp_outline(doc, comp_outline, comp_height), comp_GeometryName]))
                comp_PartNumber = ""
                comp_outline = []
        elif emprecords[0][0] == ".":
        ...
        if (current_section == ".ELECTRICAL" or current_section == ".MECHANICAL") and section_counter == 2 and \
                emprecords[2] == "THOU":
            emp_unit = 0.0254
        if (current_section == ".ELECTRICAL" or current_section == ".MECHANICAL") and section_counter == 2 and \
                emprecords[2] == "MM":
        ...
        if (current_section == ".ELECTRICAL" or current_section == ".MECHANICAL") and section_counter > 2:
            comp_outline.append([float(emprecords[1]) * emp_unit, float(emprecords[2]) * emp_unit,
                                 float(emprecords[3])])  #add point of outline
    FreeCAD.Console.PrintMessage("\n".join(passed_sections) + "\n")
    ...
    for place_item in placement:
        if place_item[1] in comps:
            ...
            placmnt = Base.Placement(Base.Vector(place_item[3], place_item[4], z_pos),
...
def Process_comp_outline(doc, comp_outline, comp_height):
    ...
    vertex_index = -1  #presume no vertex
    out_shape = []
    if not comp_outline:  #force 0.2mm circle shape for components without place outline definition
...
def place_steps(doc, placement, board_thickness):
    ...
    for validkey in validkeys:
        ...
        FreeCAD.Console.PrintMessage(
            "Reading step file " + str(model_dict[validkey]) + " for footprint " + str(validkey) + "\n")
