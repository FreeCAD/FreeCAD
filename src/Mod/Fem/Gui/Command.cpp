...
void CmdFemCreateElementsSet::activated(int)
{
    ...
    // start
        ...
        const auto elementsName = "ElementsSet";
        const auto uniqueElementsName = Command::getUniqueObjectName(elementsName).c_str();
        ...
        doCommand(Gui,
                  ...
                  uniqueElementsName,
                  MeshObj->getNameInDocument());
        doCommand(Gui, "Gui.activeDocument().setEdit('%s')", uniqueElementsName);
    }
    else {
        QMessageBox::warning(
            Gui::getMainWindow(),
