...
std::string resultMesh = "ResultMesh";
std::string lastName = "";
...
void copyResultsMesh(std::string oldName, std::string newName)
{
    int error = 0;
    ...
    {
        error = 1;
        Base::Console().Warning("the names are the same: %s and %s\n", oldName.c_str(), newName.c_str());
        QMessageBox::warning(Gui::getMainWindow(),
                             qApp->translate("CmdFemCreateElementsSet", "Wrong selection"),
                             qApp->translate("CmdFemCreateElementsSet", "Meshes must be Distinct"));
    }
    if ((oldName.find("Result") == std::string::npos || newName.find("Result") == std::string::npos) && error == 0)
    {
        error = 1;
        Base::Console().Warning("meshes must be both results: %s and %s\n", oldName.c_str(), newName.c_str());
        QMessageBox::warning(Gui::getMainWindow(),
                             qApp->translate("CmdFemCreateElementsSet", "Wrong selection"),
                             qApp->translate("CmdFemCreateElementsSet", "Mesh must be Results"));
    }
    if (error == 0)
    {
...
void generateMesh(std::string meshType)
{
    ...
    if (strcmp(meshType.c_str(), "result") == 0)
    {
        if (passResult == 0)
        {
            ...
            Gui::Command::doCommand(Gui::Command::Doc, "ne = Gui.ActiveDocument.getObject(\'%s\')", resultMesh.c_str());
    ...
    if (strcmp(meshType.c_str(), "result") == 0)
    {
        ...
        Gui::Command::doCommand(Gui::Command::Doc, "FreeCAD.ActiveDocument.%s.FemMesh = c.Object.FemMesh", resultMesh.c_str());
        Gui::Command::doCommand(Gui::Command::Doc, "Gui.ActiveDocument.getObject(\'%s\').BackfaceCulling = False", resultMesh.c_str());
        Gui::Command::doCommand(Gui::Command::Doc, "Gui.ActiveDocument.getObject(\'%s\').Visibility = True", resultMesh.c_str());
                 ...
                 double **nodeCoords, int rows, int requiredType)
{
    ...
    fprintf(fptr, "** written by FreeCAD inp file writer for CalculiX,Abaqus meshes\n");
...
void printData(const SMESHDS_Mesh *srcMesh)
{
    //     Base::Console().Warning("PRINT START\n");
    SMDS_ElemIteratorPtr srcElemIt;
    SMDS_NodeIteratorPtr srcNodeIt;
    srcElemIt = srcMesh->elementsIterator();
    srcNodeIt = srcMesh->nodesIterator();
    const SMDS_MeshNode *nSrc;
...
TaskCreateElementSet::TaskCreateElementSet(Fem::FemSetElementNodesObject *pcObject, QWidget *parent)
    : TaskBox(Gui::BitmapFactory().pixmap("FEM_CreateElementsSet"),
      ...
      selectionMode(none)
{
    //   Base::Console().Warning("TaskCreateElementSet::TaskCreateElementSet\n");
...
{
    //         Base::Console().Warning("POLY task\n");
...
void TaskCreateElementSet::Restore(void)
{
    ...
    for (std::vector<string>::reverse_iterator it = STR.rbegin(); it != STR.rend(); ++it)
    {
        ...
        {
            number++;
            copyResultsMesh(objectN, resultMesh);
        ...
        else if (objectN.find(resultMesh) != std::string::npos)
        {
        }
...
{
    //         Base::Console().Warning("SWITCH METHOD\n");
    if (Value == 1)
    {
...
{
    //    Base::Console().Warning("DEFINEELEMENSTCALLBACK\n");
    Gui::WaitCursor wc;
                                       ...
                                       bool inner)
{
    ...
    meshType = "NULL";
    std::size_t found = highLightMesh.find(resultMesh);
    ...
    Base::Console().Warning("mesh type %s %s\n", highLightMesh.c_str(), resultMesh.c_str());
    Base::Console().Warning("mesh type %s %s\n", resultMesh.c_str(), meshType.c_str());
    ...
    int requiredType = nVolumes > 0 ? 4 : 3;
...
*/
    ...
    int twenty = 20, keepElement = 0, maxNode = -1;
    while (srcNode->more())
    {
    ...
    while (srcElemIt->more())
    {
        ...
        /* add the bit tot determine which elements are outside the poly */
...
{
    //        Base::Console().Warning("ONSELECTIONCHANGED\n");
