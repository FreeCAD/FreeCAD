...
class _CommandNewMechanicalAnalysis:
    ...
    def Activated(self):
        ...
        FreeCADGui.addModule("CaeAnalysis") #<FemToCae> done, import at the file beginning
        #FreeCADGui.doCommand("FreeCADGui.ActiveDocument.ActiveView.setAxisCross(True)") 
        FreeCADGui.doCommand("FemGui.setActiveAnalysis(CaeAnalysis.makeCaeAnalysis('MechanicalAnalysis'))")
...
class _CommandFemFromShape:
    ...
    def Activated(self):
        ...
        FreeCADGui.addModule("FemGui")
        FreeCADGui.addModule("CaeAnalysis")  #Name changed
        sel = FreeCADGui.Selection.getSelection()
        if (len(sel) == 1):
            if(sel[0].isDerivedFrom("Part::Feature")):
                FreeCADGui.doCommand("App.activeDocument().addObject('Fem::FemMeshShapeNetgenObject', '" + sel[0].Name + "_Mesh')")
...
class _CommandMechanicalJobControl:  #consider name change from   _CommandMechanicalJobControl to _CommandAnalysisJobControl
    ...
    def Activated(self):
        taskd = JobControlTaskPanel(FemGui.getActiveAnalysis())  #name changed!!!
...
class _CommandPurgeFemResults:
    ...
    def Activated(self):
        makeCaeSolver('Calculix').reset_all  # <FemToCae>  = getActiveAnalysis().solver
...
def run_solver(analysis_type):
    solver = makeCaeSolver('Calculix') #<FemToCae>  = getActiveAnalysis().solver
