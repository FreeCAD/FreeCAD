...
PyObject* FemPostPipelinePy::renameArrays(PyObject* args)
{
    ...
    for (auto it = pyNames.begin(); it != pyNames.end(); ++it) {
        const auto& value = *it;
        if (!value.first.isString() || !static_cast<Py::Object>(value.second).isString()) {
            PyErr_SetString(PyExc_TypeError, "Names must be string objects");
            return nullptr;
        }
        names[value.first.as_string()] = static_cast<Py::Object>(value.second).as_string();
    }
