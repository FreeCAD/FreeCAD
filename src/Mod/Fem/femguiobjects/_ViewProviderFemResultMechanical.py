...
class _TaskPanelFemResultShow:
    ...
    def __init__(self, obj):
        ...
        self.form = [FreeCADGui.PySideUic.loadUi(FreeCAD.getHomePath() + "Mod/Fem/Resources/ui/ResultShow.ui"),
                     FreeCADGui.PySideUic.loadUi(FreeCAD.getHomePath() + "Mod/Fem/Resources/ui/ResultShowHints.ui")
        ]
        ...
        QtCore.QObject.connect(
            self.form[0].rb_none, QtCore.SIGNAL("toggled(bool)"),
            self.none_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_abs_displacement,
            QtCore.SIGNAL("toggled(bool)"),
            self.abs_displacement_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_x_displacement,
            QtCore.SIGNAL("toggled(bool)"),
            self.x_displacement_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_y_displacement,
            QtCore.SIGNAL("toggled(bool)"),
            self.y_displacement_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_z_displacement,
            QtCore.SIGNAL("toggled(bool)"),
            self.z_displacement_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_temperature,
            QtCore.SIGNAL("toggled(bool)"),
            self.temperature_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_vm_stress,
            QtCore.SIGNAL("toggled(bool)"),
            self.vm_stress_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_maxprin,
            QtCore.SIGNAL("toggled(bool)"),
            self.max_prin_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_minprin,
            QtCore.SIGNAL("toggled(bool)"),
            self.min_prin_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_max_shear_stress,
            QtCore.SIGNAL("toggled(bool)"),
            self.max_shear_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_massflowrate,
            QtCore.SIGNAL("toggled(bool)"),
            self.massflowrate_selected
        )
        QtCore.QObject.connect(
            self.form[0].rb_networkpressure,
            QtCore.SIGNAL("toggled(bool)"),
        ...
        # displacement
        QtCore.QObject.connect(
            self.form[0].cb_show_displacement,
            QtCore.SIGNAL("clicked(bool)"),
            self.show_displacement
        )
        QtCore.QObject.connect(
            self.form[0].hsb_displacement_factor,
            QtCore.SIGNAL("valueChanged(int)"),
            self.hsb_disp_factor_changed
        )

        self.form[0].sb_displacement_factor.valueChanged.connect(self.sb_disp_factor_changed)
        self.form[0].sb_displacement_factor_max.valueChanged.connect(self.sb_disp_factor_max_changed)

        # user defined equation
        QtCore.QObject.connect(
            self.form[0].user_def_eq,
            QtCore.SIGNAL("textchanged()"),
            self.user_defined_text
        )
        QtCore.QObject.connect(
            self.form[0].calculate,
        ...
        if self.restore_result_settings_in_dialog:
            ...
            self.form[0].sb_displacement_factor_max.setValue(10. * scale_factor)
            self.form[0].sb_displacement_factor.setValue(scale_factor)

    def restore_result_dialog(self):
        try:
            rt = FreeCAD.FEM_dialog["results_type"]
            if rt == "None":
                self.form[0].rb_none.setChecked(True)
                self.none_selected(True)
            elif rt == "Uabs":
                self.form[0].rb_abs_displacement.setChecked(True)
                self.abs_displacement_selected(True)
            elif rt == "U1":
                self.form[0].rb_x_displacement.setChecked(True)
                self.x_displacement_selected(True)
            elif rt == "U2":
                self.form[0].rb_y_displacement.setChecked(True)
                self.y_displacement_selected(True)
            elif rt == "U3":
                self.form[0].rb_z_displacement.setChecked(True)
                self.z_displacement_selected(True)
            elif rt == "Temp":
                self.form[0].rb_temperature.setChecked(True)
                self.temperature_selected(True)
            elif rt == "Sabs":
                self.form[0].rb_vm_stress.setChecked(True)
                self.vm_stress_selected(True)
            elif rt == "MaxPrin":
                self.form[0].rb_maxprin.setChecked(True)
                self.max_prin_selected(True)
            elif rt == "MinPrin":
                self.form[0].rb_minprin.setChecked(True)
                self.min_prin_selected(True)
            elif rt == "MaxShear":
                self.form[0].rb_max_shear_stress.setChecked(True)
                self.max_shear_selected(True)
            elif rt == "MFlow":
                self.form[0].rb_massflowrate.setChecked(True)
                self.massflowrate_selected(True)
            elif rt == "NPress":
                self.form[0].rb_networkpressure.setChecked(True)
                self.networkpressure_selected(True)
            elif rt == "Peeq":
                self.form[0].rb_peeq.setChecked(True)
                self.peeq_selected(True)

            sd = FreeCAD.FEM_dialog["show_disp"]
            self.form[0].cb_show_displacement.setChecked(sd)
            self.show_displacement(sd)

            df = FreeCAD.FEM_dialog["disp_factor"]
            dfm = FreeCAD.FEM_dialog["disp_factor_max"]
            # self.form[0].hsb_displacement_factor.setMaximum(dfm)
            # self.form[0].hsb_displacement_factor.setValue(df)
            self.form[0].sb_displacement_factor_max.setValue(dfm)
            self.form[0].sb_displacement_factor.setValue(df)
        except:
            self.restore_initial_result_dialog()
    ...
    def restore_initial_result_dialog(self):
        ...
        self.form[0].sb_displacement_factor_max.setValue(100.)    # init non standard values
    ...
    def calculate(self):
        ...
        self.update()
        self.restore_result_dialog()
        userdefined_eq = self.form[0].user_def_eq.toPlainText()  # Get equation to be used
    ...
    def set_result_stats(self, unit, minm, avg, maxm):
        self.form[0].le_min.setProperty("unit", unit)
        self.form[0].le_min.setProperty("rawText", "{:.6} {}".format(minm, unit))
        self.form[0].le_avg.setProperty("unit", unit)
        self.form[0].le_avg.setProperty("rawText", "{:.6} {}".format(avg, unit))
        self.form[0].le_max.setProperty("unit", unit)
        self.form[0].le_max.setProperty("rawText", "{:.6} {}".format(maxm, unit))

    def update_displacement(self, factor=None):
        if factor is None:
            if FreeCAD.FEM_dialog["show_disp"]:
                factor = self.form[0].sb_displacement_factor.value()
            else:
                factor = 0.0
    ...
    def hsb_disp_factor_changed(self, value):
        self.form[0].sb_displacement_factor.setValue(
            value / 100. * self.form[0].sb_displacement_factor_max.value()
        )
        self.update_displacement()

    def sb_disp_factor_max_changed(self, value):
        FreeCAD.FEM_dialog["disp_factor_max"] = value
        if value < self.form[0].sb_displacement_factor.value():
            self.form[0].sb_displacement_factor.setValue(value)
        if value == 0.:
            self.form[0].hsb_displacement_factor.setValue(0)
        else:
            self.form[0].hsb_displacement_factor.setValue(
                round(self.form[0].sb_displacement_factor.value() / value * 100.)
            )

    def sb_disp_factor_changed(self, value):
        FreeCAD.FEM_dialog["disp_factor"] = value
        if value > self.form[0].sb_displacement_factor_max.value():
            self.form[0].sb_displacement_factor.setValue(self.form[0].sb_displacement_factor_max.value())
        if self.form[0].sb_displacement_factor_max.value() == 0.:
            self.form[0].hsb_displacement_factor.setValue(0.)
        else:
            self.form[0].hsb_displacement_factor.setValue(
                round(value / self.form[0].sb_displacement_factor_max.value() * 100.)
            )
    ...
    def disable_empty_result_buttons(self):
        ...
        Peeq                --> rb_peeq"""
        if len(self.result_obj.DisplacementLengths) == 0:
            self.form[0].rb_abs_displacement.setEnabled(0)
        if len(self.result_obj.DisplacementVectors) == 0:
            self.form[0].rb_x_displacement.setEnabled(0)
            self.form[0].rb_y_displacement.setEnabled(0)
            self.form[0].rb_z_displacement.setEnabled(0)
        if len(self.result_obj.Temperature) == 0:
            self.form[0].rb_temperature.setEnabled(0)
        if len(self.result_obj.StressValues) == 0:
            self.form[0].rb_vm_stress.setEnabled(0)
        if len(self.result_obj.PrincipalMax) == 0:
            self.form[0].rb_maxprin.setEnabled(0)
        if len(self.result_obj.PrincipalMin) == 0:
            self.form[0].rb_minprin.setEnabled(0)
        if len(self.result_obj.MaxShear) == 0:
            self.form[0].rb_max_shear_stress.setEnabled(0)
        if len(self.result_obj.MassFlowRate) == 0:
            self.form[0].rb_massflowrate.setEnabled(0)
        if len(self.result_obj.NetworkPressure) == 0:
            self.form[0].rb_networkpressure.setEnabled(0)
        if len(self.result_obj.Peeq) == 0:
            self.form[0].rb_peeq.setEnabled(0)
