        ...
        try:
            for attr_name in dir(obj):
                ...
                try:
                    ...
                    if attr_name in ['Material', 'ShapeMaterial'] and attr_value and hasattr(attr_value, 'Name'):
                        ...
                        if material_name not in found_material_names:
                          ...
                          not isinstance(attr_value, (str, int, float, bool, list, tuple, dict))):
...
def modify_gltf_data(gltf_data):
    ...
    try:
        if ('materials' in gltf_data and 
            isinstance(gltf_data['materials'], list) and
            'found_material_names' in globals() and 
            found_material_names):
            
            for i, material in enumerate(gltf_data['materials']):
                if (isinstance(material, dict) and 
                    'name' in material and 
                    i < len(found_material_names) and
                    found_material_names[i]):
    ...
    try:
        if ('nodes' in gltf_data and 
            isinstance(gltf_data['nodes'], list) and
            'collected_varset_data' in globals() and 
            collected_varset_data):
            
            for node in gltf_data['nodes']:
                if (isinstance(node, dict) and 
                    'name' in node and 
                    isinstance(node['name'], str) and
                    node['name'] in collected_varset_data):
