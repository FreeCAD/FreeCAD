    ...
    ui (new Ui_TaskHelixParameters)
{
    ...
    PartDesign::Helix* pcFeat = static_cast<PartDesign::Helix*>(vp->getObject());
    // enable use of parametric expressions for the numerical fields
    ui->pitch->bind(pcFeat->Pitch);
    ui->height->bind(pcFeat->Height);
    ui->turns->bind(pcFeat->Turns);
    ui->coneAngle->bind(pcFeat->Angle);
...
void TaskHelixParameters::refresh()
{
    if (!vp || !vp->getObject())
        return;
    ...
    if (initing && !(rev->HasBeenEdited).getValue()) {
