...
bool ViewProviderBody::isHoleThreadVisible(const PartDesign::Hole* hole) const
{
    ...
    if (!body
        || !Visibility.getValue()
        || hole->Suppressed.getValue()
        || !hole->Threaded.getValue()
    ...
    if (holeIt == features.end()) {return false;}
...
void ViewProviderBody::updateThreadTextureForHole(const PartDesign::Hole* hole)
{
    ...
    if (it != m_threadOverlays.end()) {
        ...
        if (auto* holeVP = dynamic_cast<ViewProviderHole*>(Gui::Application::Instance->getViewProvider(hole))) {
...
{
    static const std::unordered_set<std::string> watchedProps{
        "Visibility", "CosmeticThread"
    };
