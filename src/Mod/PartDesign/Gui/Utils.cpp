...
bool setEdit(App::DocumentObject* obj, PartDesign::Body* body)
{
    ...
    Gui::cmdGuiDocument(parent,
                        std::ostringstream() << "setEdit(" << Gui::Command::getObjectCmd(parent)
                                             << ", 0, '" << subname << "')");
    return true;
}
                          ...
                          std::string* subname)
{
    ...
    if (activeView) {
        ...
        if (assertModern) {
            activeBody =
                activeView->getActiveObject<PartDesign::Body*>(PDBODYKEY, topParent, subname);
            ...
            if (!activeBody && messageIfNot) {
                DlgActiveBody dia(
                    ...
                    doc,
                    QObject::tr(
                        "In order to use PartDesign you need an active Body object in the "
                        "document. "
                                 ...
                                 std::string* subname)
{
    ...
    if (body) {
        ...
        Gui::cmdGuiDocument(_doc,
                            std::stringstream()
                                << "ActiveView.setActiveObject('" << PDBODYKEY << "',"
                                << Gui::Command::getObjectCmd(parent ? parent : body) << ",'" << sub
                                << "')");
        return Gui::Application::Instance->activeView()->getActiveObject<PartDesign::Body*>(
            PDBODYKEY,
            topParent,
            subname);
    }
...
PartDesign::Body* getBodyFor(const App::DocumentObject* obj,
                             bool messageIfNot,
                             bool autoActivate,
                             bool assertModern,
                             App::DocumentObject** topParent,
                             std::string* subname)
{
    ...
    if (messageIfNot) {
        QMessageBox::warning(Gui::getMainWindow(),
                             ...
                             QObject::tr("In order to use this feature it needs to belong to a "
                                         "body object in the document."));
    }
...
//     Gui::Command::doCommand(Gui::Command::Doc,
//     "PartDesignGui.setUpPart(App.activeDocument().%s)", PartName.c_str());
...
void fixSketchSupport(Sketcher::SketchObject* sketch)
{
    ...
    if (fabs(offset) < Precision::Confusion()) {
        ...
        FCMD_OBJ_CMD(sketch,
                     "AttachmentSupport = (" << Gui::Command::getObjectCmd(plane) << ",[''])");
        ...
        FCMD_OBJ_CMD(sketch,
                     "AttachmentSupport = (" << Gui::Command::getObjectCmd(obj) << ",[''])");
        FCMD_OBJ_CMD(sketch, "MapReversed = " << (reverseSketch ? "True" : "False"));
        FCMD_OBJ_CMD(sketch,
                     "MapMode = '" << Attacher::AttachEngine::getModeName(Attacher::mmFlatFace)
                                   << "'");
        ...
        if (!isPartDesignAwareObjecta(obj)) {
            ...
            for (auto prop : properties) {
                ...
                if (prop->isDerivedFrom(App::PropertyLink::getClassTypeId())) {
                    ...
                    App::PropertyLinkSubList* propLink =
                        static_cast<App::PropertyLinkSubList*>(prop);
                    std::vector<App::DocumentObject*> linkList = propLink->getValues();
                    bool valueChanged = false;
                    for (auto& link : linkList) {
                        if (link == feature) {
...
bool isFeatureMovable(App::DocumentObject* const feat)
{
    ...
    if (auto prim = dynamic_cast<PartDesign::ProfileBased*>(feat)) {
        ...
        if (auto prop = dynamic_cast<App::PropertyLinkList*>(prim->getPropertyByName("Sections"))) {
            if (std::any_of(prop->getValues().begin(),
                            prop->getValues().end(),
        ...
        if (auto prop =
                dynamic_cast<App::PropertyLinkSub*>(prim->getPropertyByName("AuxillerySpine"))) {
...
collectMovableDependencies(std::vector<App::DocumentObject*>& features)
{
    ...
    for (auto const& feat : features) {
        ...
        if (auto prim = dynamic_cast<PartDesign::ProfileBased*>(feat)) {
            ...
            if (auto prop =
                    dynamic_cast<App::PropertyLinkList*>(prim->getPropertyByName("Sections"))) {
            ...
            if (auto prop = dynamic_cast<App::PropertyLinkSub*>(
                    prim->getPropertyByName("AuxillerySpine"))) {
                App::DocumentObject* axis = prop->getValue();
                if (axis && !axis->isDerivedFrom<App::OriginFeature>()) {
                    unique_objs.insert(axis);
                }
...
void relinkToOrigin(App::DocumentObject* feat, PartDesign::Body* targetbody)
{
    if (feat->hasExtension(Part::AttachExtension::getExtensionClassTypeId())) {
        ...
        if (support && support->isDerivedFrom<App::OriginFeature>()) {
            ...
            App::OriginFeature* targetOriginFeature =
                targetbody->getOrigin()->getOriginFeature(originfeat->Role.getValue());
        ...
        if (auto prop =
                static_cast<App::PropertyLinkSub*>(prim->getPropertyByName("ReferenceAxis"))) {
            App::DocumentObject* axis = prop->getValue();
            if (axis && axis->isDerivedFrom<App::OriginFeature>()) {
                ...
                App::OriginFeature* targetOriginFeature =
                    targetbody->getOrigin()->getOriginFeature(originfeat->Role.getValue());
                if (targetOriginFeature) {
                    prop->setValue(static_cast<App::DocumentObject*>(targetOriginFeature),
