...
bool TaskBoxPrimitives::setPrimitive(App::DocumentObject *obj)
{
    try {
        ...
        // To allow e.g. a width of 2.2 um while the users has e.g. 2 user decimals
        // we cannot use Base::UnitsApi::getDecimals().
        // It is also more sensible to allow all values OCC allows, thus use OCC's limit.
        int minDigits = - 1 * (int)(log10(Precision::Confusion()));
        switch(ui->widgetStack->currentIndex()) {
            case 1:         // box
                cmd = QString::fromLatin1(
                    ...
                    "%1.Height=%4\n")
                    .arg(name)
                    .arg(ui->boxLength->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->boxWidth->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->boxHeight->value().getValue(), 0, 'f', minDigits);
                break;
            ...
            case 2:  // cylinder
                cmd = QString::fromLatin1(
                    ...
                    "%1.SecondAngle=%6\n")
                    .arg(name)
                    .arg(ui->cylinderRadius->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->cylinderHeight->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->cylinderAngle->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->cylinderXSkew->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->cylinderYSkew->value().getValue(), 0, 'f', minDigits);
                break;

            case 3:  // cone
                // the cone radii must not be equal
                if (ui->coneRadius1->value().getValue() == ui->coneRadius2->value().getValue()) {
                    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Cone radii are equal"),
                        QObject::tr("The radii for cones must not be equal!"));
                    return false;
                }
                cmd = QString::fromLatin1(
                    "%1.Radius1=%2\n"
                    ...
                    "%1.Angle=%5\n")
                    .arg(name)
                    .arg(ui->coneRadius1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->coneRadius2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->coneHeight->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->coneAngle->value().getValue(), 0, 'f', minDigits);
                 break;
            ...
            case 4:  // sphere
                cmd = QString::fromLatin1(
                    ...
                    "%1.Angle3=%5\n")
                    .arg(name)
                    .arg(ui->sphereRadius->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->sphereAngle1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->sphereAngle2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->sphereAngle3->value().getValue(), 0, 'f', minDigits);
                break;
            case 5:  // ellipsoid
                cmd = QString::fromLatin1(
                    "%1.Radius1=%2\n"
                    ...
                    "%1.Angle3=%7\n")
                    .arg(name)
                    .arg(ui->ellipsoidRadius1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->ellipsoidRadius2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->ellipsoidRadius3->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->ellipsoidAngle1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->ellipsoidAngle2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->ellipsoidAngle3->value().getValue(), 0, 'f', minDigits);
                break;
            ...
            case 6:  // torus
                cmd = QString::fromLatin1(
                    ...
                    "%1.Angle3=%6\n")
                    .arg(name)
                    .arg(ui->torusRadius1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->torusRadius2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->torusAngle1->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->torusAngle2->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->torusAngle3->value().getValue(), 0, 'f', minDigits);
                break;
            case 7:  // prism
                cmd = QString::fromLatin1(
                    "%1.Polygon=%2\n"
                    ...
                    .arg(ui->prismCircumradius->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->prismHeight->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->prismXSkew->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->prismYSkew->value().getValue(), 0, 'f', minDigits);
                break;
            case 8:  // wedge
                // Xmin/max, Ymin/max and Zmin/max must each not be equal
                if (ui->wedgeXmin->value().getValue() == ui->wedgeXmax->value().getValue()) {
                    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Invalid wedge parameters"),
                        QObject::tr("X min must not be equal to X max!"));
                    return false;
                }
                else if (ui->wedgeYmin->value().getValue() == ui->wedgeYmax->value().getValue()) {
                    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Invalid wedge parameters"),
                        QObject::tr("Y min must not be equal to Y max!"));
                    return false;
                }
                else if (ui->wedgeZmin->value().getValue() == ui->wedgeZmax->value().getValue()) {
                    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Invalid wedge parameters"),
                        QObject::tr("Z min must not be equal to Z max!"));
                    return false;
                }
                cmd = QString::fromLatin1(
                    "%1.Xmin=%2\n"
                    ...
                    "%1.Z2max=%11\n")
                    .arg(name)
                    .arg(ui->wedgeXmin->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeYmin->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeZmin->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeX2min->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeZ2min->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeXmax->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeYmax->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeZmax->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeX2max->value().getValue(), 0, 'f', minDigits)
                    .arg(ui->wedgeZ2max->value().getValue(), 0, 'f', minDigits);
                break;
