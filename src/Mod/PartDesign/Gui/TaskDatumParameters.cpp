...
TaskDatumParameters::TaskDatumParameters(ViewProviderDatum* ViewProvider, QWidget* parent)
    : PartGui::TaskAttacher(ViewProvider,
                            parent,
                            QString::fromLatin1("PartDesign_") + ViewProvider->datumType,
...
TaskDatumParameters::~TaskDatumParameters()
{
    if (this->ViewProvider
        && this->ViewProvider->isDerivedFrom(ViewProviderDatum::getClassTypeId())) {
...
bool TaskDlgDatumParameters::accept()
{
    ...
    if (extReference) {
        // TODO: rewrite this to be shared with CmdPartDesignNewSketch::activated() (2015-10-20,
        // Fat-Zer)
        ...
        if (result == QDialog::DialogCode::Rejected) {
            ...
            for (App::DocumentObject* obj : pcDatum->AttachmentSupport.getValues()) {
                if (pcActiveBody && !pcActiveBody->hasObject(obj)
                    && !pcActiveBody->getOrigin()->hasObject(obj)) {
                    auto* copy =
                        PartDesignGui::TaskFeaturePick::makeCopy(obj,
                                                                 subs[index],
