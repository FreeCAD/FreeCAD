...
void CmdPartDesignBody::activated(int iMsg)
{
    ...
    doCommand(Doc,
              "App.ActiveDocument.getObject('%s').Label = '%s'",
              bodyString,
              labelString.c_str());
    if (baseFeature) {
    ...
    if (baseFeature) {
        PartDesign::Body* body =
            dynamic_cast<PartDesign::Body*>(baseFeature->getDocument()->getObject(bodyString));
        if (body) {
...
void CmdPartDesignMigrate::activated(int iMsg)
{
    ...
    // Retrieve all PartDesign Features objects and filter out features already belonging to some
    // body
    ...
    for (auto featIt = migrateFeatures.begin(); !migrateFeatures.empty();) {
        ...
        if (!base || !base->isDerivedFrom(PartDesign::Feature::getClassTypeId())
            ...
            if (baseFeatSetIt != migrateFeatures.end()) {
                ...
                auto chainIt =
                    std::find_if(featureChains.begin(),
                                 featureChains.end(),
                                 [baseFeat, &baseFeatIt](
                ...
                if (chainIt != featureChains.end()) {
                    ...
                    if (std::next(baseFeatIt) == chainIt->end()) {
                        ...
                        newChainIt->splice(newChainIt->end(),
                                           *chainIt,
                                           std::next(baseFeatIt),
                                           chainIt->end());
                    }
            ...
            if (feature->isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
                ...
                if (sketch) {
                    ...
                    if (sketch->isDerivedFrom(Sketcher::SketchObject::getClassTypeId())) {
                        ...
                        // TODO: Message that sketchbased is based not on a sketch (2015-08-11,
                        // Fat-Zer)
                    }
                }
            }
...
void CmdPartDesignDuplicateSelection::activated(int iMsg)
{
    ...
    if (pcActiveBody) {
        ...
        std::set_difference(afterFeatures.begin(),
                            afterFeatures.end(),
        ...
        for (auto feature : newFeatures) {
            if (PartDesign::Body::isAllowed(feature)) {
                // if feature already is in a body, then we don't put it into the active body issue
                // #6278
...
void CmdPartDesignMoveFeatureInTree::activated(int iMsg)
{
    ...
    for (auto feat : features) {
        ...
        // TODO: if tip was moved the new position of tip is quite undetermined (2015-08-07,
        // Fat-Zer)
        ...
        FCMD_OBJ_CMD(body,
                     "insertObject(" << getObjectCmd(feat) << "," << getObjectCmd(lastObject)
                                     << ", True)");
    ...
    if (lastObject != target && body->Tip.getValue() == target
        ...
        msgBox.setInformativeText(
            qApp->translate("PartDesign_MoveFeatureInTree",
                            "Do you want the last feature to be the new tip?"));
