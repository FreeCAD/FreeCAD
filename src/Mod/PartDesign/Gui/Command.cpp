    ...
    try {
        ...
        if (bEditSelected) {
            ...
            cmd.doCommand(Gui::Command::Doc,
                          "App.activeDocument().recompute()");  // recompute the feature based on
                                                                // its references
            ...
            QMessageBox::warning(Gui::getMainWindow(),
                                 QObject::tr("Error"),
                                 QObject::tr("There is no active body. Please make a body active "
                                             "before inserting a datum entity."));
        }
    }
...
void CmdPartDesignClone::activated(int iMsg)
{
    ...
    if (objs.size() == 1) {
        ...
        Gui::cmdAppObject(bodyObj,
                          std::stringstream() << "Group = [" << getObjectCmd(cloneObj) << "]");
...
void finishFeature(const Gui::Command* cmd,
                          ...
                          std::vector<App::DocumentObject*>::iterator& firstFreeSketch)
{
    ...
    for (std::vector<App::DocumentObject*>::iterator s = sketches.begin(); s != sketches.end();
         s++) {
...
void prepareProfileBased(PartDesign::Body* pcActiveBody,
                         Gui::Command* cmd,
                         ...
                         std::function<void(Part::Feature*, App::DocumentObject*)> func)
{
    auto base_worker = [=](App::DocumentObject* feature, const std::vector<std::string>& subs) {
        ...
        if (which.compare("AdditiveLoft") == 0 || which.compare("SubtractiveLoft") == 0) {
            ...
            if (selection.size() == 2) {  // treat additional selected object as spine
                ...
                if (selection[1].getObject()->isDerivedFrom(Part::Part2DObject::getClassTypeId())
                    && subnames.empty()) {
    ...
    if ((which.find("Subtractive") != std::string::npos) || (which.compare("Groove") == 0)
        ...
        if (!pcActiveBody->isSolid()) {
            ...
            msgBox.setInformativeText(QObject::tr("Ensure that the body contains a feature before "
                                                  "attempting a subtractive command."));
    ...
    if (!selection.empty()) {
        ...
        if (!onlyAllowed) {
            ...
            msgBox.setInformativeText(QObject::tr("Consider using a ShapeBinder or a BaseFeature "
    ...
    bool extReference = std::find_if(status.begin(),
                                     status.end(),
    ...
    if (pcActiveBody && !bNoSketchWasSelected && extReference) {
        ...
        if (!dlg.radioXRef->isChecked()) {
            ...
            auto copy = PartDesignGui::TaskFeaturePick::makeCopy(sketches[0],
                                                                 "",
...
void finishProfileBased(const Gui::Command* cmd,
                        const Part::Feature* sketch,
                        App::DocumentObject* Feat)
{
...
void prepareProfileBased(Gui::Command* cmd, const std::string& which, double length)
{
    ...
    auto worker = [cmd, length](Part::Feature* profile, App::DocumentObject* Feat) {
        ...
        if (sketch) {
            ...
            Gui::cmdAppObject(Feat,
                              str << "ReferenceAxis = (" << Gui::Command::getObjectCmd(sketch)
                                  << ",['N_Axis'])");
        }
    ...
    : Command("PartDesign_SubtractivePipe")
{
    ...
    sToolTipText = QT_TR_NOOP(
        "Sweep a selected sketch along a path or to other profiles and remove it from the body");
    ...
    auto worker = [cmd, &pcActiveBody](Part::Feature* sketch, App::DocumentObject* Feat) {
        ...
        if (Feat->isError()) {
            App::DocumentObject* base =
                static_cast<PartDesign::Feature*>(Feat)->BaseFeature.getValue();
            if (base) {
...
bool dressupGetSelected(Gui::Command* cmd,
                        const std::string& which,
                        ...
                        bool& noSelection)
{
    ...
    if (selection.empty()) {
        ...
        QMessageBox::warning(Gui::getMainWindow(),
                             QObject::tr("Selection is not in Active Body"),
    ...
    std::function<void(App::DocumentObject*, std::vector<App::DocumentObject*>)> func)
{
    ...
    std::vector<App::DocumentObject*> features =
        cmd->getSelection().getObjectsOfType(PartDesign::Feature::getClassTypeId());
    ...
    if (features.empty()) {
        ...
        if (features.size() > 1) {
            ...
            Gui::Control().showDialog(
                new PartDesignGui::TaskDlgFeaturePick(features, status, accepter, worker, false));
            return;
        }
        else if (features.empty()) {
            QMessageBox::warning(Gui::getMainWindow(),
                                 QObject::tr("No valid features in this document"),
...
void CmdPartDesignMirrored::activated(int iMsg)
{
    ...
    auto worker = [cmd](App::DocumentObject* Feat, std::vector<App::DocumentObject*> features) {
        ...
        if (!direction) {
            auto body =
                static_cast<PartDesign::Body*>(Part::BodyBase::findBodyOf(features.front()));
            if (body) {
                FCMD_OBJ_CMD(Feat,
...
void CmdPartDesignLinearPattern::activated(int iMsg)
{
    ...
    auto worker = [cmd](App::DocumentObject* Feat, std::vector<App::DocumentObject*> features) {
        ...
        if (features.front()->isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
            ...
            if (sketch) {
                FCMD_OBJ_CMD(Feat,
                             "Direction = (" << Gui::Command::getObjectCmd(sketch)
                                             << ", ['H_Axis'])");
                direction = true;
            }
        }
        if (!direction) {
            ...
            if (body) {
                FCMD_OBJ_CMD(Feat,
                             "Direction = ("
                                 << Gui::Command::getObjectCmd(body->getOrigin()->getX())
                                 << ",[''])");
            }
        }
...
void CmdPartDesignPolarPattern::activated(int iMsg)
{
    ...
    auto worker = [cmd](App::DocumentObject* Feat, std::vector<App::DocumentObject*> features) {
        ...
        if (features.front()->isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
            ...
            if (sketch) {
                FCMD_OBJ_CMD(Feat,
                             "Axis = (" << Gui::Command::getObjectCmd(sketch) << ",['N_Axis'])");
                direction = true;
            }
        }
        if (!direction) {
            ...
            if (body) {
                FCMD_OBJ_CMD(Feat,
                             "Axis = (" << Gui::Command::getObjectCmd(body->getOrigin()->getZ())
                                        << ",[''])");
            }
        }
...
void CmdPartDesignMultiTransform::activated(int iMsg)
{
    ...
    if (!features.empty()) {
        ...
        for (std::vector<App::DocumentObject*>::iterator f = features.begin();
             f != features.end();) {
        ...
        Gui::Selection().clearSelection();
        if (prevFeature) {
            Gui::Selection().addSelection(prevFeature->getDocument()->getName(),
        ...
        if (pcActiveBody && oldTip != trFeat) {
            Gui::Selection().clearSelection();
            Gui::Selection().addSelection(oldTip->getDocument()->getName(),
                                          oldTip->getNameInDocument());
            rcCmdMgr.runCommandByName("PartDesign_MoveTip");
            Gui::Selection().clearSelection();
        ...
        auto worker = [cmd, pcActiveBody](App::DocumentObject* Feat,
            ...
            // Make sure the user isn't presented with an empty screen because no transformations
            // are defined yet...
