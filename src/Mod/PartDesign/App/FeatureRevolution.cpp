...
Revolution::MethodsArray Revolution::TypeEnums
    = {"Angle", "UpToLast", "UpToFirst", "UpToFace", "TwoAngles", nullptr};

Revolution::FuseOrdersArray Revolution::FuseOrderEnums = {"BaseFirst", "FeatureFirst", nullptr};
...
Revolution::Revolution()
{
    ...
    FuseOrder.setEnums(FuseOrderEnums.data());
}
...
{
    auto order = orderFromString(FuseOrder.getValueAsString());
    ...
    if (order == FuseArrangement::FeatureFirst) {
...
Revolution::FuseArrangement Revolution::orderFromString(const std::string& orderStr) const
{
    // clang-format off
    static constexpr auto orders = std::to_array<std::tuple<std::string_view, FuseArrangement>>({
        {"BaseFirst",       FuseArrangement::BaseFirst},
        {"FeatureFirst",    FuseArrangement::FeatureFirst},
    });
    // clang-format on

    auto it = std::find_if(orders.begin(), orders.end(), [orderStr](const auto& order) {
        return std::get<0>(order) == orderStr;
    });
    if (it != orders.end()) {
        return std::get<1>(*it);
    }

    throw Base::ValueError("Revolution: No such order");
}
