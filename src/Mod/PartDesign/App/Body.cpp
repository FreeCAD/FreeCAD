...
Body::Body()
{
    ADD_PROPERTY_TYPE(AllowCompound,
                      (false),
                      "Experimental",
                      App::Prop_None,
    ...
    static Base::Reference<ParameterGrp> hGrp =
        App::GetApplication().GetUserParameter().GetGroup("BaseApp/Preferences/Mod/PartDesign");
...
bool Body::isAllowed(const App::DocumentObject* obj)
{
    ...
    return (obj->isDerivedFrom<PartDesign::Feature>() || obj->isDerivedFrom<Part::Datum>() ||
            ...
            // Fat-Zer)
            // obj->isDerivedFrom<Part::FeaturePython>() // trouble with this line on Windows!?
...
std::vector<App::DocumentObject*> Body::removeObject(App::DocumentObject* feature)
{
    ...
    if (isSolidFeature(feature)) {
        ...
        // If the next feature is solid, reroute its BaseFeature property to the previous solid
        // feature
        if (nextSolidFeature) {
            assert(nextSolidFeature->isDerivedFrom(PartDesign::Feature::getClassTypeId()));
