...
const char* FeatureExtrude::SideTypesEnums[] = {"1 side", "2 sides", "Symmetric", nullptr};
...
App::DocumentObjectExecReturn* FeatureExtrude::buildExtrusion(ExtrudeOptions options)
{
    ...
    double L2 = (Sidemethod == "2 sides" && method2 == "Length") ? Length2.getValue() : 0;

    if ((Sidemethod == "1 side" && method == "Length")
        || (Sidemethod == "2 sides" && method == "Length" && method2 == "Length")) {
...
TopoShape FeatureExtrude::generateSingleExtrusionSide(const TopoShape& sketchshape,
                                                      ...
                                                      const Base::Vector3d& sketchNormalGlobal)
{
    TopoShape prism(0, getDocument()->getStringHasher());
