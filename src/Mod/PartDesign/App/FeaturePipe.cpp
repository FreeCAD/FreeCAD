...
Pipe::Pipe()
{
    ...
    ADD_PROPERTY_TYPE(Transformation,
                      (long(0)),
                      "Sweep",
                      App::Prop_None,
...
App::DocumentObjectExecReturn* Pipe::execute()
{
    ...
    try {
        ...
        if (!(spine && spine->isDerivedFrom<Part::Feature>())) {
            return new App::DocumentObjectExecReturn(
                QT_TRANSLATE_NOOP("Exception", "No spine linked"));
        }
        ...
        if (getAddSubType() == FeatureAddSub::Additive) {
            ...
            if (!mkFuse.IsDone()) {
                return new App::DocumentObjectExecReturn(
                    QT_TRANSLATE_NOOP("Exception", "Adding the pipe failed"));
            }
            ...
            if (!mkCut.IsDone()) {
                return new App::DocumentObjectExecReturn(
                    QT_TRANSLATE_NOOP("Exception", "Subtracting the pipe failed"));
            }
            // we have to get the solids (fuse sometimes creates compounds)
...
void Pipe::buildPipePath(const Part::TopoShape& shape,
    ...
    if (!shape.getShape().IsNull()) {
        try {
            if (!subedge.empty()) {
                ...
                if (len != 1) {
                    throw Base::ValueError(
                        QT_TRANSLATE_NOOP("Exception", "Spine is not connected."));
                }
                path = hWires->Value(1);
            }
            else {
...
void Pipe::handleChangedPropertyType(Base::XMLReader& reader,
                                     const char* TypeName,
                                     App::Property* prop)
{
