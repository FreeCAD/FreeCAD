                                                   ...
                                                   size_t expected_size)
{
    ...
    auto useEntireSketch =
        obj->isDerivedFrom(Part::Part2DObject::getClassTypeId()) && subName.find("Vertex") != 0;
    if (subs.empty() || std::find(subs.begin(), subs.end(), std::string()) != subs.end()
        || useEntireSketch) {
        shapes.push_back(Part::Feature::getTopoShape(obj));
        if (shapes.back().isNull()) {
            FC_THROWM(Part::NullShapeException,
                      "Failed to get shape of " << name << " "
                                                << App::SubObjectT(obj, "").getSubObjectFullName(
            ...
            if (shapes.back().isNull()) {
                FC_THROWM(Part::NullShapeException,
                          "Failed to get shape of "
                              << name << " "
                              << App::SubObjectT(obj, sub.c_str())
                                     .getSubObjectFullName(obj->getDocument()->getName()));
            }
        }
    }
    auto compound =
        TopoShape(0).makeElementCompound(shapes,
                                         "",
                                         TopoShape::SingleShapeCompoundCreationPolicy::returnShape);
    auto wires = compound.getSubTopoShapes(TopAbs_WIRE);
    auto edges = compound.getSubTopoShapes(TopAbs_EDGE,
                                           TopAbs_WIRE);  // get free edges and make wires from it
    if (!edges.empty()) {
...
App::DocumentObjectExecReturn* Loft::execute()
{
    ...
    try {
        ...
        if (!front.isNull() || !back.isNull()) {
            ...
            result = result.makeShapeWithElementMap(sewer.SewedShape(),
...
void Loft::handleChangedPropertyType(Base::XMLReader& reader,
                                     const char* TypeName,
                                     App::Property* prop)
{
