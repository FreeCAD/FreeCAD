...
const char* Helix::ModeEnums[] = {"pitch-height-angle",
                                  "pitch-turns-angle",
                                  "height-turns-angle",
                                  "height-turns-growth",
                                  nullptr};
...
Helix::Helix()
{
    ...
    ADD_PROPERTY_TYPE(HasBeenEdited,
                      (false),
                      group,
                      App::Prop_Hidden,
                      QT_TRANSLATE_NOOP("App::Property",
    ...
    ADD_PROPERTY_TYPE(Tolerance,
                      ...
                      App::Prop_None,
                      QT_TRANSLATE_NOOP("App::Property",
                                        "Fusion Tolerance for the Helix, increase if helical shape "
                                        "does not merge nicely with part."));
...
App::DocumentObjectExecReturn* Helix::execute()
{
    ...
    try {
        ...
        fix.LimitTolerance(path,
                           Precision::Confusion() * 1e-6
                               * size);  // needed to produce valid Pipe for very big parts
        ...
        BRepOffsetAPI_MakePipe mkPS(TopoDS::Wire(path),
                                    face,
        ...
        fix.LimitTolerance(
            result,
            Precision::Confusion() * size
                * Tolerance.getValue());  // significant precision reduction due to helical
...
TopoDS_Shape Helix::generateHelixPath(double breakAtTurn)
{
    ...
    // Find out in what quadrant relative to the axis the profile is located, and the exact
    // position.
    ...
    double startOffset =
        10000.0 * std::fabs(noAngle * (profileCenter * axisVector) - baseVector * axisVector);
...
void Helix::handleChangedPropertyType(Base::XMLReader& reader,
                                      const char* TypeName,
                                      App::Property* prop)
{
