...
TopoShape ProfileBased::getProfileShape(bool simplifyCompound) const
{
    TopoShape shape;
    ...
    if (subs.empty()) {
        ...
        for (auto& sub : subs) {
            shapes.push_back(
                Part::Feature::getTopoShape(profile, sub.c_str(), true, nullptr, nullptr, true, true, false, simplifyCompound));
        }
