...
static App::DocumentObjectExecReturn*
validateParameters(int chamferType, double size, double size2, double angle);
...
{
    // NOTE: Normally the Base property and the BaseFeature property should point to the same
    // object. The only difference is that the Base property also stores the edges that are to be
    // chamfered
...
void Chamfer::handleChangedPropertyType(Base::XMLReader& reader,
                                        const char* TypeName,
                                        App::Property* prop)
{
