...
void FeatureAddSub::updatePreviewShape()
{
    const auto notifyWarning = [this](auto message) {
        Base::Console().userWarning(
            QT_TRANSLATE_NOOP("Notifications", "Failure while computing preview: %s. That usually indicates an error with model."),
            message
        );
    };
    ...
    if (addSubType == Subtractive) {
        ...
        if (base.isNull()) {
            notifyWarning(QT_TRANSLATE_NOOP("FeatureAddSub", "Preview shape is empty"));
        }

        if (!base.isValid()) {
            notifyWarning(QT_TRANSLATE_NOOP("FeatureAddSub", "Preview shape is invalid"));
        }
    }
