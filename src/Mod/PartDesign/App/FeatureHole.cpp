...
const char* Hole::HoleCutType_None_Enums[] = {"None",
                                              "Counterbore",
                                              "Countersink",
                                              "Counterdrill",
                                              nullptr};
...
std::vector<std::string> Hole::HoleCutType_ISOmetric_Enums = {"None",
                                                              "Counterbore",
                                                              "Countersink",
                                                              "Counterdrill"};
...
std::vector<std::string> Hole::HoleCutType_ISOmetricfine_Enums = {"None",
                                                                  "Counterbore",
                                                                  "Countersink",
                                                                  "Counterdrill"};
...
// reproduced in: https://www.accu.co.uk/en/p/134-iso-metric-thread-tolerances [retrieved:
// 2021-01-11]
...
/* UTS coarse */
const char* Hole::HoleCutType_UNC_Enums[] = {"None",
                                             "Counterbore",
                                             "Countersink",
                                             "Counterdrill",
                                             nullptr};
...
/* UTS fine */
const char* Hole::HoleCutType_UNF_Enums[] = {"None",
                                             "Counterbore",
                                             "Countersink",
                                             "Counterdrill",
                                             nullptr};
...
/* UTS extrafine */
const char* Hole::HoleCutType_UNEF_Enums[] = {"None",
                                              "Counterbore",
                                              "Countersink",
                                              "Counterdrill",
                                              nullptr};
...
Hole::Hole()
{
    ...
    ADD_PROPERTY_TYPE(HoleCutCountersinkAngle,
                      (90.0),
                      "Hole",
                      App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(ThreadDepth,
                      (23.5),
                      "Hole",
                      App::Prop_None,
                      "Thread Length");  // default is assuming an M1

    ADD_PROPERTY_TYPE(UseCustomThreadClearance,
                      (false),
                      "Hole",
                      App::Prop_None,
                      "Use custom thread clearance");

    ADD_PROPERTY_TYPE(CustomThreadClearance,
                      (0.0),
                      "Hole",
                      App::Prop_None,
 ...
 */
static void
computeIntersection(gp_Pnt pa1, gp_Pnt pa2, gp_Pnt pb1, gp_Pnt pb2, double& x, double& y)
{
        ...
        if (isCountersink || isCounterbore || isCounterdrill) {
            ...
            computeIntersection(gp_Pnt(holeCutRadius, -holeCutDepth, 0),
                                gp_Pnt(holeCutRadius - sin(countersinkAngle),
                                       -cos(countersinkAngle) - holeCutDepth,
                                       0),
                                gp_Pnt(radius, 0, 0),
...
void Hole::rotateToNormal(const gp_Dir& helixAxis,
                          ...
                          const TopoDS_Shape& protoHole) const
{
    ...
    for (const auto& profileEdge : profileshape.getSubTopoShapes(TopAbs_EDGE)) {
        ...
        mapper.populate(Part::MappingStatus::Modified,
                        profileEdge,
...
TopoDS_Shape Hole::makeThread(const gp_Vec& xDir, const gp_Vec& zDir, double length)
{
    ...
    if (!mkPS.IsReady()) {
        throw Base::CADKernelError(
            QT_TRANSLATE_NOOP("Exception", "Error: Thread could not be built"));
    }
...
void Hole::addCutType(const CutDimensionSet& dimensions)
{
    ...
    if (std::all_of(list->begin(), list->end(), [name](const std::string& x) {
            return x != name;
        })) {
        list->push_back(name);
    }
...
{
    auto i =
        std::find_if(bore_data.begin(), bore_data.end(), [t](const Hole::CounterBoreDimension& x) {
            return x.thread == t;
        });
    if (i == bore_data.end()) {
...
{
    auto i =
        std::find_if(sink_data.begin(), sink_data.end(), [t](const Hole::CounterSinkDimension& x) {
            return x.thread == t;
        });
    if (i == sink_data.end()) {
