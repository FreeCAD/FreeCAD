...
const App::PropertyAngle::Constraints Draft::floatAngle = {
    0.0,
    90.0 - Base::toDegrees<double>(Precision::Angular()),
    0.1};
...
Draft::Draft()
{
    ...
    ADD_PROPERTY_TYPE(NeutralPlane,
                      (nullptr),
                      "Draft",
                      (App::PropertyType)(App::Prop_None),
                      "NeutralPlane");
    ADD_PROPERTY_TYPE(PullDirection,
                      (nullptr),
                      "Draft",
                      (App::PropertyType)(App::Prop_None),
                      "PullDirection");
...
App::DocumentObjectExecReturn* Draft::execute()
{
    ...
    try {
        ...
        //       wire, though.
        // BRepFeat_MakeDPrism requires a support for the operation but will probably support
        // multiple
        //       wires in the sketch
        ...
        do {
            ...
            for (std::vector<std::string>::iterator it = SubVals.begin(); it != SubVals.end();
                ...
                if (!mkDraft.AddDone()) {
                    // Note: the function ProblematicShape returns the face on which the error
                    // occurred Note: mkDraft.Remove() stumbles on a bug in
                    // Draft_Modification::Remove() and is
                    //       therefore unusable. See
                    //       https://forum.freecad.org/viewtopic.php?f=10&t=3209&start=10#p25341 The
                    //       only solution is to discard mkDraft and start over without the current
                    //       face
                    // mkDraft.Remove(face);
