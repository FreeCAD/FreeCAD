...
DrawViewPart::~DrawViewPart()
{
    ...
    geometryObject->removeReferenceVertexes();
}
...
void DrawViewPart::postHlrTasks()
{
    ...
    getAll<DrawHatch>();
    getAll<DrawGeomHatch>();
    getAll<DrawViewDimension>();
    getAll<DrawViewBalloon>();
...
template const VertexPtr DrawViewPart::getGeometry<Vertex>(std::string geomName) const;
template const BaseGeomPtr DrawViewPart::getGeometry<BaseGeom>(std::string geomName) const;
template const FacePtr DrawViewPart::getGeometry<Face>(std::string geomName) const;
...
// Explicit template instantiation:
template const VertexPtr DrawViewPart::getGeometry<Vertex>(int idx) const;
template const BaseGeomPtr DrawViewPart::getGeometry<BaseGeom>(int index) const;
template const FacePtr DrawViewPart::getGeometry<Face>(int index) const;
...
// Explicit template instantiation:
template const std::vector<VertexPtr> DrawViewPart::getAllGeometry<Vertex>() const;
template const std::vector<BaseGeomPtr> DrawViewPart::getAllGeometry<BaseGeom>() const;
template const std::vector<FacePtr> DrawViewPart::getAllGeometry<Face>() const;
...
std::pair<Base::Vector3d, Base::Vector3d> DrawViewPart::getDirsFromFront(const std::string& viewType) const
{
    ...
    if (viewType == "Right") {
        ...
        projDir = DrawUtil::toVector3d(newCS.Direction());
        rotVec = DrawUtil::toVector3d(newCS.XDirection());
    }
        ...
        projDir = DrawUtil::toVector3d(gYDir);
        rotVec = DrawUtil::toVector3d(gXDir);
    }
    else if (viewType == "Bottom") {
        projDir = DrawUtil::toVector3d(gYDir.Reversed());
        rotVec = DrawUtil::toVector3d(gXDir);
    }
    else if (viewType == "Rear") {
        projDir = DrawUtil::toVector3d(gDir.Reversed());
        rotVec = DrawUtil::toVector3d(gXDir.Reversed());
    }
    else if (viewType == "FrontTopLeft") {
        gp_Dir newDir = gp_Dir(gp_Vec(gDir) - gp_Vec(gXDir) + gp_Vec(gYDir));
        projDir = DrawUtil::toVector3d(newDir);
        gp_Dir newXDir = gp_Dir(gp_Vec(gXDir) + gp_Vec(gDir));
        rotVec = DrawUtil::toVector3d(newXDir);
    }
