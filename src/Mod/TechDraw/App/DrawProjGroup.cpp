...
Base::BoundBox3d DrawProjGroup::getBoundingBox() const
{
    ...
    if (anchorView == nullptr) {
        ...
        Base::Console().Message("PROBLEM - DPG::getBoundingBox - non DPGI entry in Views! %s\n",
                                getNameInDocument());
...
App::DocumentObject * DrawProjGroup::getProjObj(const char *viewProjType) const
{
    for( auto it : Views.getValues() ) {
        ...
        if (projPtr == nullptr) {
            //if an element in Views is not a DPGI, something really bad has happened somewhere
            Base::Console().Message("PROBLEM - DPG::getProjObj - non DPGI entry in Views! %s / %s\n",
...
DrawProjGroupItem* DrawProjGroup::getProjItem(const char *viewProjType) const
{
    ...
    if ( (result == nullptr) &&
        ...
        Base::Console().Message("PROBLEM - DPG::getProjItem finds non-DPGI in Group %s / %s\n",
                                getNameInDocument(),viewProjType);
        throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
    }
    return result;
...
bool DrawProjGroup::hasProjection(const char *viewProjType) const
{
    for( const auto it : Views.getValues() ) {
        ...
        if (view == nullptr) {
            //should never have a item in DPG that is not a DPGI. 
            Base::Console().Message("PROBLEM - DPG::hasProjection finds non-DPGI in Group %s / %s\n",
                                    getNameInDocument(),viewProjType);
            throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
        }
...
App::DocumentObject * DrawProjGroup::addProjection(const char *viewProjType)
{
    ...
    if ( checkViewProjType(viewProjType) && !hasProjection(viewProjType) ) {
        ...
        if ( (view == nullptr) &&
            ...
            Base::Console().Message("PROBLEM - DPG::addProjection - created a non DPGI! %s / %s\n",
                                    getNameInDocument(),viewProjType);
    ...
    if ( checkViewProjType(viewProjType) ) {
        ...
        for( auto it : Views.getValues() ) {
            ...
            if( projPtr != nullptr) {
                ...
                //if an element in Views is not a DPGI, something really bad has happened somewhere
                Base::Console().Message("PROBLEM - DPG::removeProjection - tries to remove non DPGI! %s / %s\n",
                                    getNameInDocument(),viewProjType);
                throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
            }
        }
...
int DrawProjGroup::purgeProjections()
{
    while (!Views.getValues().empty())   {
        ...
        if (dpgi != nullptr) {
            ...
            //if an element in Views is not a DPGI, something really bad has happened somewhere
            Base::Console().Message("PROBLEM - DPG::purgeProjection - tries to remove non DPGI! %s\n",
                                    getNameInDocument());
            throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
        }
    }
...
void DrawProjGroup::arrangeViewPointers(DrawProjGroupItem *viewPtrs[10]) const
{
    ...
    if ( strcmp(projType, "Third Angle") == 0 ||
        ...
        for (auto it : Views.getValues()) {
            ...
            if (oView == nullptr) {
                //if an element in Views is not a DPGI, something really bad has happened somewhere
                Base::Console().Message("PROBLEM - DPG::arrangeViewPointers - non DPGI in Views! %s\n",
                                    getNameInDocument());
                throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
            } else {
...
void DrawProjGroup::updateChildren(void)
{
    for( const auto it : Views.getValues() ) {
        ...
        if (view == nullptr) {
            //if an element in Views is not a DPGI, something really bad has happened somewhere
            Base::Console().Message("PROBLEM - DPG::updateChildren - non DPGI entry in Views! %s\n",
                                    getNameInDocument());
            throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
        } else {
...
void DrawProjGroup::updateChildrenSource(void)
{
    for( const auto it : Views.getValues() ) {
        ...
        if (view == nullptr) {
            //if an element in Views is not a DPGI, something really bad has happened somewhere
            Base::Console().Message("PROBLEM - DPG::updateChildrenSource - non DPGI entry in Views! %s\n",
                                    getNameInDocument());
            throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
        } else {
            view->Source.setValues(Source.getValues());
...
void DrawProjGroup::updateChildrenLock(void)
{
    for( const auto it : Views.getValues() ) {
        ...
        if (view == nullptr) {
            //if an element in Views is not a DPGI, something really bad has happened somewhere
            Base::Console().Message("PROBLEM - DPG::updateChildrenLock - non DPGI entry in Views! %s\n",
                                    getNameInDocument());
            throw Base::TypeError("Error: projection in DPG list is not a DPGI!");
        } else {
            view->requestPaint();
