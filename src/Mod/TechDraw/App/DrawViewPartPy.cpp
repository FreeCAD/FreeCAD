...
// This File is normaly build as an include in DrawViewPartPyImp.cpp! Its not intended to be in a project!
...
//--------------------------------------------------------------------------
PyObject *DrawViewPartPy::_getattr(char *attr)				// __getattr__ function: note only need to handle new state
{
    try {
...
int DrawViewPartPy::_setattr(char *attr, PyObject *value) // __setattr__ function: note only need to handle new state
{
    try {
