...
std::string PageRenderer::renderToSVGString() const
{
    ...
    try {
        ...
        const int pixelWidth = std::max(1, static_cast<int>(std::lround(width * mmToPixels)));
        const int pixelHeight = std::max(1, static_cast<int>(std::lround(height * mmToPixels)));
...
void PageRenderer::calculatePageBounds(double& width, double& height) const
{
    ...
    if (m_page && m_page->hasValidTemplate()) {
        try {
            width = m_page->getPageWidth();
            height = m_page->getPageHeight();
        }
        catch (const Base::Exception&) {
            // Keep default values if template access fails
            Base::Console().warning("PageRenderer: Failed to get page dimensions, using A4 defaults\n");
        }
    }
}
