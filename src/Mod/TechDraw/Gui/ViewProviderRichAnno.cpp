...
ViewProviderRichAnno::ViewProviderRichAnno()
{
    ...
    ADD_PROPERTY_TYPE(LineWidth, (getDefLineWeight()), group,App::Prop_None, "Frame line width");
    LineStyle.setEnums(LineStyleEnums);
    ADD_PROPERTY_TYPE(LineStyle, (1), group, App::Prop_None, "Frame line style");
...
void ViewProviderRichAnno::updateData(const App::Property* p)
{
    ...
    if (getViewObject() != nullptr) {
        if (getViewObject()->ShowFrame.getValue()) {
            LineWidth.setStatus(App::PropertyStatus::ReadOnly, false);
            LineStyle.setStatus(App::PropertyStatus::ReadOnly, false);
            LineColor.setStatus(App::PropertyStatus::ReadOnly, false);
        }
        else {
            LineWidth.setStatus(App::PropertyStatus::ReadOnly, true);
            LineStyle.setStatus(App::PropertyStatus::ReadOnly, true);
            LineColor.setStatus(App::PropertyStatus::ReadOnly, true);
        }
    }
