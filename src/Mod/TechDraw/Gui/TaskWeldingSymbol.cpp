...
//    Base::Console().message("TWS::accept()\n");
    if (m_createMode) {
        int tid = Gui::Command::openActiveDocumentCommand(QT_TRANSLATE_NOOP("Command", "Create WeldSymbol"));
    ...
    //    m_weldFeat->requestPaint();    //not a dv!
        ...
        int tid = Gui::Command::openActiveDocumentCommand(QT_TRANSLATE_NOOP("Command", "Edit WeldSymbol"));
        try {
            updateWeldingSymbol();
            updateTiles();
        }
