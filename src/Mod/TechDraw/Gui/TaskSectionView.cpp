                                            ...
                                            const Base::Vector3d& direction)
{
    ...
    for (const QPointF& corner : corners) {
        const double along = QPointF::dotProduct(corner - center, tangent);
        startAlong = std::min(startAlong, along);
        endAlong = std::max(endAlong, along);
    }

    const double span = std::max(0.0, endAlong - startAlong);
    const double padding = std::max(Rez::guiX(2.0), span * 0.05);
    startAlong -= padding;
    endAlong += padding;
...
private:
    ...
    std::list<Gui::InputHint> getToolHints() const override
    {
        ...
        if (m_mode == Mode::SeekCenter) {
            const QString action = m_task
                    && m_task->positionMode()
                        == TaskSectionView::PositionMode::TwoPoints
                ? QObject::tr("%1 place first section-line point")
                : QObject::tr("%1 place section-line center");
            return {
                {action, {MouseLeft}},
