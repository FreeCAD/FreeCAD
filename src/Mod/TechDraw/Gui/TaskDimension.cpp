...
{
    double selectionAngle = getAngleFromSelection();
    ui->dsbDimAngle->setValue(selectionAngle * 180.0 / M_PI);
}
...
{
    double selectionAngle = getAngleFromSelection();
    ui->dsbExtAngle->setValue(selectionAngle * 180.0 / M_PI);
}

double TaskDimension::getAngleFromSelection()
{
    ...
    if (!selection.empty()) {
        ...
        if (SubNames.size() == 2) {             //expecting Vertices
            ...
            if ((geomName0 == "Vertex") && (geomName1 == "Vertex"))  {
                ...
                double lineAngle = (v12 - v02).Angle();
                return lineAngle;
            }
