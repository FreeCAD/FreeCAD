...
void LineEdit::setDocumentObject(const App::DocumentObject *currentDocObj, bool checkInList)
{
    ...
    auto getPopupPos = [proxy_lineedit, zv](){
        const QPointF scene_pos = proxy_lineedit->mapToScene(proxy_lineedit->boundingRect().bottomLeft());
    ...
    auto getPopupWidth = [this, zv](){
        const int zoom_level = zv->zoomLevel(),
                  editors_width = this->width();
...
void LineEdit::keyPressEvent(QKeyEvent *event)
{
    ...
    if ((lastKeyPressed == Qt::Key::Key_Down || lastKeyPressed == Qt::Key::Key_Up) && completerActive()) {
