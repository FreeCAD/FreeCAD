...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const IntPoint& Pt)
{
    ...
    if (prevE && prevE->OutIdx >= 0 && (TopX(*prevE, Pt.Y) == TopX(*e, Pt.Y))
        && SlopesEqual(*e, *prevE, m_UseFullRange) && (e->WindDelta != 0) && (prevE->WindDelta != 0)) {
...
void Clipper::InsertLocalMinimaIntoAEL(const cInt botY)
{
    while (m_CurrentLM != m_MinimaList.end() && (m_CurrentLM->Y == botY)) {
        ...
        if (lb->OutIdx >= 0 && lb->PrevInAEL && lb->PrevInAEL->Curr.X == lb->Bot.X
            && lb->PrevInAEL->OutIdx >= 0 && SlopesEqual(*lb->PrevInAEL, *lb, m_UseFullRange)
        ...
        if (lb->NextInAEL != rb) {
            ...
            if (rb->OutIdx >= 0 && rb->PrevInAEL->OutIdx >= 0
                && SlopesEqual(*rb->PrevInAEL, *rb, m_UseFullRange) && (rb->WindDelta != 0)
        ...
        if (IsIntermediate(e, topY)) {
            ...
            if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y && op
                ...
                AddJoin(op, op2, e->Top);
                     ...
                     && SlopesEqual(*e, *eNext, m_UseFullRange) && (e->WindDelta != 0)
                     && (eNext->WindDelta != 0)) {
