...
struct ArcFittingMap
{
    ...
    // All intersections (multimap): maps each input z-value involved in an intersection to the
    // full intersection record. Each intersection inserts up to 4 entries (one per input z).
    // Format: input_z -> (e1bot.z, e1top.z, e2bot.z, e2top.z)
    std::multimap<int64_t, std::tuple<int64_t, int64_t, int64_t, int64_t>> all_intersections;

    // Maps (min_z, max_z) edge pairs to their original vertex and orientation.
    // Populated by _MakePoly, consumed by __SetFromResult.
    std::map<std::pair<int64_t, int64_t>, SegmentData> zData;

    // Tracks the orientation of a point, for use with point expansions. A point's z-value maps to
    // true if the arc generated by point expansion has positive orientation, and false if negative.
    std::map<int64_t, bool> orientations;
    ...
    // Track the next curve index for _MakePoly calls; persists across independent call paths
    int nextCurveIndex;

    // Track the previous z-label for arc connectivity
    int64_t z_prev;

    // Direction of the last processed segment as (dx, dy)
    std::optional<std::pair<double, double>> direction_prev;

    // Direction of the first segment, saved for loop-closure processing
    std::optional<std::pair<double, double>> direction_initial;
...
#define CAREA_PARAM_DECLARE(_type, _name) \
    ...
    void MakePolyPoly(Clipper2Lib::Paths64& pp);
...
private:
    struct EdgeInfo
    {
        ...
        int orientation;  // 1 = positive, -1 = negative, 0 = error. TODO/FIXME maybe this should be
                          // called an offsetting side tag or something like that, it's only
                          // relevant for offsetting and does not get reversed when edges are
                          // backwards. It isn't actually orientation
    };
    ...
    Clipper2Lib::Path64 _MakePoly(const CCurve& curve, std::list<int> edge_orientations = {});
