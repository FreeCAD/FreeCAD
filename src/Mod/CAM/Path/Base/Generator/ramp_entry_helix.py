...
from Path.Geom import pointsCoincide as coincide
...
helix_commands = helix.Helix(commands, tool_controller, ignoreAbove=10, angle=80)
helix_commands.generate()

Arguments:
    commands: list of Path.Command objects
    maxStepDown: maximum height for each helix turn
    tool_controller: tool controller to apply feed rate
    ignoreAbove: helix ramp will start from this height
...
class Helix:
    def __init__(self, commands, maxStepDown=9999, tc=None, ignoreAbove=None):
    ...
    def checks(self):
        if not isinstance(self.commands[0], Path.Command):
            raise TypeError("'commands' must be a list of Path.Command objects")

        if self.tc is not None:
            if (
                not hasattr(self.tc, "Proxy")
                or not self.tc.Proxy
                or not isinstance(self.tc.Proxy, Path.Tool.Controller.ToolController)
            ):
            ...
            if not self.tc.HorizRapid.Value:
                raise ValueError("'HorizRapid' is 0")
            if not self.tc.VertRapid.Value:
                raise ValueError("'VertRapid' is 0")
                ...
                if isRoughly(edge.xy_length, 0) and edge.end_point[2] < edge.start_point[2]:
                    ...
                    while j < len(edges) and not loopFound:
                        ...
                        if not isRoughly(candidate.start_point[2], candidate.end_point[2]):
                            ...
                            break
                        if coincide(edge.end_point, candidate.end_point):
                            loopFound = True
