...
class HelixRamp:
    ...
    def checks(self):
        ...
        Path.Log.debug("maxStepDown: {}".format(self.maxStepDown))
    ...
    def createHelix(self, rampedges, startZ):
        ...
        if self.maxStepDown and not isRoughly(rampheight, self.maxStepDown):
            num_loops = math.ceil(round(rampheight / self.maxStepDown, 6))
            rampedges *= num_loops
            ramplen *= num_loops
