...
def build_kinematic_chain(machine: Machine, restrict_axes: Optional[List[str]] = None) -> List[RotaryAxis]:
    ...
    if restrict_axes:
        rotary_axes = {name: axis for name, axis in machine.rotary_axes.items() 
                      if name in restrict_axes}
    else:
    ...
    root_axes = [name for name, axis in rotary_axes.items() 
...
def compute_rotation_matrix(chain: List[RotaryAxis], angles: Dict[str, float]) -> FreeCAD.Rotation:
    ...
    if all_table and len(chain) >= 2:
        ordered = sorted(chain,
                         key=lambda ax: 0 if abs(ax.rotation_vector.z) > 0.9 else 1)
    else:
        ordered = chain
...
def _decompose_2axis(first_axis: RotaryAxis, second_axis: RotaryAxis,
                     desired_axis: FreeCAD.Vector) -> List[Dict[str, float]]:
    ...
    first_valid = [a for a in first_candidates
                   if first_axis.min_limit <= a <= first_axis.max_limit]
...
def _decompose_2axis_head(first_axis: RotaryAxis, second_axis: RotaryAxis,
                          desired_axis: FreeCAD.Vector) -> List[Dict[str, float]]:
...
def _solve_analytical_2axis(chain: List[RotaryAxis], desired_axis: FreeCAD.Vector,
...
def _solve_generic_2axis(chain: List[RotaryAxis], desired_axis: FreeCAD.Vector,
                        current_state: Dict[str, float]) -> List[Dict[str, float]]:
...
def _solve_single_axis(chain: List[RotaryAxis], desired_axis: FreeCAD.Vector,
                      current_state: Dict[str, float]) -> List[Dict[str, float]]:
...
def _expand_solution_space(chain: List[RotaryAxis], base_solutions: List[Dict[str, float]]) -> List[Dict[str, float]]:
...
def _compute_solution_cost(chain: List[RotaryAxis], solution: Dict[str, float],
        ...
        if len(chain) >= 2:
            ...
            if all_table or all_head:
                ...
                for first_ax, second_ax in [(chain[0], chain[1]), (chain[1], chain[0])]:
                    ...
                    err = abs(check_second - candidate[second_ax.name])
                    if err < decomp_error:
                        decomp_error = err
                error = decomp_error
        elif len(chain) == 1:
            ...
            error = min(diff, abs(diff - 360), abs(diff + 360),
                       abs(diff - 180), abs(diff + 180))
