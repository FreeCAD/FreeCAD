...
def spiral(
    ...
    Path.Log.debug(
        f"tool_diameter: {tool_diameter}, stepover_percent: {stepover_percent}, angle: {theta}, milling_direction: {milling_direction}"
    )
    ...
    while True:
        ...
        if k == 0:
            Path.Log.debug(
                f"Layer {k}: start_corner_index={start_corner_index}, order={order}, start_idx={start_idx}"
            )
        ...
        valid_next = (
            s0n < s1n and t0n < t1n and (s1n - s0n) > tool_diameter and (t1n - t0n) > tool_diameter
        )
