...
def zigzag(
    ...
    Path.Log.debug(
        f"Zigzag: Before reverse - min_t={min_t}, max_t={max_t}, first_t={step_positions[0] if step_positions else None}, last_t={step_positions[-1] if step_positions else None}"
    )
    if reverse:
        ...
        Path.Log.debug(
            f"Zigzag: After reverse - center={center}, first_t={step_positions[0] if step_positions else None}, last_t={step_positions[-1] if step_positions else None}"
        )

    Path.Log.debug(
        f"Zigzag: {len(step_positions)} passes generated, reverse={reverse}, step_vec={step_vec}"
    )
