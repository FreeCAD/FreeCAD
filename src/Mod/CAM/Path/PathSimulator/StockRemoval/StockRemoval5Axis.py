...
class VolumetricStock:
    ...
    def __init__(
        self, bounds: Tuple[float, float, float, float, float, float], resolution: float = 0.5
    ):
        """
    ...
    def remove_cylinder(
        self, start: Tuple[float, float, float], end: Tuple[float, float, float], radius: float
    ):
        """
            ...
            for j in range(self.ny):
                for k in range(self.nz):
                    ...
                    cube_faces = [
                        ...
                        (base + 4, base + 6, base + 7),  # Top
                        (base, base + 1, base + 5),
                        (base, base + 5, base + 4),  # Front
                        (base + 2, base + 3, base + 7),
                        (base + 2, base + 7, base + 6),  # Back
                        (base, base + 3, base + 7),
                        (base, base + 7, base + 4),  # Left
                        (base + 1, base + 2, base + 6),
                        (base + 1, base + 6, base + 5),  # Right
                    ]
...
class StockRemoval5Axis:
    ...
    def remove_material(
        ...
        collision = False
        if check_collision:
            collision = self.collision_detector.check_tool_collision(tool, start_pos, (0, 0, 0))
            if collision:
                self.collisions += 1
        ...
        if tool.tool_type == ToolType.ENDMILL:
            ...
            for i in range(num_samples):
                t = i / (num_samples - 1)
                pos = tuple(start_pos[j] + t * (end_pos[j] - start_pos[j]) for j in range(3))
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "removal_percentage": (
                (self.stock.get_removed_volume() / self.stock.initial_volume * 100)
                if self.stock.initial_volume > 0
                else 0
            ),
            "operations": self.operations,
