...
def check_ghostgunner_spindle_clearance(toolstate, machine_name, tool_length=25.0):
    ...
    if tool_length > max_tool_length:
        warnings.append(f"Tool length {tool_length:.1f}mm exceeds maximum {max_tool_length:.1f}mm")
    ...
    if spindle_bottom_z < fixture["height"]:
        warnings.append(f"Spindle nose may collide with fixture at Z={z:.2f}")
...
def run_collision_checks(model, machine_cfg, holder):
    ...
    is_ghost_gunner = any(
        gg in machine_name for gg in ["GhostGunner", "ghostgunner", "GG2", "GG3", "GGR", "GGD"]
    )
    ...
    for s in sampled:
        ...
        if is_ghost_gunner:
            ...
            spindle_warnings = check_ghostgunner_spindle_clearance(s, machine_name, tool_length)
