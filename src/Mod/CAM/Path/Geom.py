...
def isVertical(obj):
    ...
    if type(obj) is FreeCAD.Vector:
        return isRoughly(obj.x, 0) and isRoughly(obj.y, 0)

    if obj.ShapeType == "Face":
        if type(obj.Surface) is Part.Plane:
            return isHorizontal(obj.Surface.Axis)
        if type(obj.Surface) in [Part.Cylinder, Part.Cone]:
            return isVertical(obj.Surface.Axis)
        if type(obj.Surface) is Part.Sphere:
            return True
        if type(obj.Surface) is Part.SurfaceOfExtrusion:
            return isVertical(obj.Surface.Direction)
        if type(obj.Surface) is Part.SurfaceOfRevolution:
            return isHorizontal(obj.Surface.Direction)
        if type(obj.Surface) is not Part.BSplineSurface:
            Path.Log.info(
    ...
    if obj.ShapeType == "Edge":
        ...
        if type(obj.Curve) is not Part.BSplineCurve:
            Path.Log.info(
...
def cmdsForEdge(edge, flip=False, approximation=False, segm=50, hSpeed=0, vSpeed=0):
    ...
    If approximation is True an Edge based on a BSplineCurve,
    the edge is considered will to represent by arcs and results in G2 or G3 command.
    Otherwise edge has no direct Path.Command mapping
    and will be approximated by straight segments.
    segm is a factor for the segmentation of arbitrary curves not mapped to G2|G3
    ...
    # Process Curve
        ...
        if type(edge.Curve) is Part.BSplineCurve and approximation:
            # Split B-Spline by arcs
            biArcs = edge.Curve.toBiArcs(0.05)
            edges = [Part.Edge(arc.toShape()) for arc in biArcs]
        else:
            edges = [edge]
        for edge in edges:
            pt = edge.valueAt(edge.LastParameter) if not flip else edge.valueAt(edge.FirstParameter)
            ...
            if hasattr(edge.Curve, "Axis") and (
                (
                    type(edge.Curve) is Part.Circle
                    and isRoughly(edge.Curve.Axis.x, 0)
                    and isRoughly(edge.Curve.Axis.y, 0)
                )
                or approximation
            ):
                # This is an arc and it should be represented by a simple G2|G3 command
