        ...
        if isinstance(edge.Curve, Part.BSplineCurve):
            ...
            for curve in curves:
                ...
                if isinstance(edge.Curve, Part.Circle) and not isVertical(edge.Curve.Axis):
                    edges.extend(splitArcEdge(edge))
                else:
                    edges.append(edge)
...
def splitArcEdge(edge):
    ...
    axisAngle = edge.Curve.Axis.getAngle(FreeCAD.Vector(0, 0, 1))
    if axisAngle < math.radians(5):
        maxArcAngle = math.pi / 2
    elif axisAngle < math.radians(10):
        maxArcAngle = math.pi / 4
    elif axisAngle < math.radians(20):
        maxArcAngle = math.pi / 6
    else:
        maxArcAngle = math.pi / 10
    edges = []
