...
def isVertical(obj):
    ...
    if obj.ShapeType == "Edge":
        ...
        if not isinstance(obj.Curve, Part.BSplineCurve):
            Path.Log.info(
...
def cmdsForEdge(edge, flip=False, approximation=False, segm=50, hSpeed=0, vSpeed=0):
    ...
    # Process Curve
        ...
        if isinstance(edge.Curve, Part.BSplineCurve) and approximation:
...
def flipEdge(edge):
    ...
    elif isinstance(edge.Curve, Part.Line) or isinstance(edge.Curve, Part.LineSegment):
