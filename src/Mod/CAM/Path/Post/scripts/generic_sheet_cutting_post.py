                ...
                if hasattr(item, "Path") and item.Path:
                    ...
                    for cmd in item.Path.Commands:
                        ...
                        if "Z" in cmd.Parameters:
                            z = CompValue(cmd.Parameters["Z"])
                            last_z = CompValue(self._last_z) if self._last_z is not None else None
                            if last_z is not None and z > last_z:
                                # Z+ = retract. Always retain the retraction.
                                new_commands.append(cmd)
                                in_cut = False
                            elif last_z is not None and z < last_z:
                                # Z- = entry/descent.
                                if not marked and z <= CompValue(cut_height):
                                    # First entry: retain it and mark it.
                                    new_commands.append(cmd)
                                    if marking_delay_sec:
                                        new_commands.append(
                                            Path.Command("G4", {"P": marking_delay_sec})
                                        )

                                    marked = True
                                    in_cut = True
                                elif marked:
                                    # Subsequent entry: discard it, but enter cut state
                                    # so all lateral cutting moves are discarded too.
                                    in_cut = True
                                else:
                                    # Positioning before first entry.
                                    new_commands.append(cmd)
                            else:
                                # Z unchanged.
