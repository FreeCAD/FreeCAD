...
# *   USA                                                                   *
# *   This post is based on work by lcorley to support Crossfire(tm) plasma tables*
# *   And Nicklas SB Karlsson to support Fangling Plasma Cutters            *
# *   The Post is untested on a real CNC cutter                             *
# ***************************************************************************
...
class Fangling(PostProcessor):
    ...
    def get_property_schema(cls):
         return [
            ...
            },
            {
                ...
                'help': translate(
                    'CAM',
                    'By default the machine does not support Z parameters'
                ),
            },
            {
                'name': 'STRIP_F',
                ...
                'help': translate(
                    'CAM',
                    'By default the machine does not support F parameters'
                ),
            },
    ...
    def convert_command_to_gcode(self, command: Path.Command):
        props = self._machine.postprocessor_properties
        out = ''
        if 'X' not in command.Parameters and 'Y' not in command.Parameters and command.Name in ["G0", "G00","G1", "G2", "G3","G01", "G02", "G03"]: 
                #No horizontal move detected so removed
                return ''
        if command.Name in ["G0", "G00"] and self.last_command in ["G1", "G2", "G3","G01", "G02", "G03"]:
        ...
        elif command.Name in ["G1", "G2", "G3","G01", "G02", "G03"] and self.last_command in ["G0", "G00"]:
        ...
        if command.Name in ["G0", "G00", 'G1','G01','G2','G02','G3','G03']:
            ...
            new_command.Parameters = {k: v for k, v in d.items() if (k != 'Z' or not self.values["STRIP_Z"]) and (k != 'F' or not self.values["STRIP_F"])}
