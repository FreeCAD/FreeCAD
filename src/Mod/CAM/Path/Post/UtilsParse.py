...
def drill_translate(
    ...
    param_x = Units.Quantity(params["X"], Units.Length)
    param_y = Units.Quantity(params["Y"], Units.Length)
    param_z = Units.Quantity(params["Z"], Units.Length)
    param_r = Units.Quantity(params["R"], Units.Length)
    if param_r < param_z:  # R less than Z is error
    ...
    if values["MOTION_MODE"] == "G91":
        # G91 relative movements (not generated by CAM WB drilling)
        param_x += Units.Quantity(motion_location["X"], Units.Length)
        param_y += Units.Quantity(motion_location["Y"], Units.Length)
        param_z += motion_z
        param_r += param_z
    ...
    #     (1) retract perpendicular to the selected plane to the position indicated
    #         by the R word, or
    #     (2) retract perpendicular to the selected plane to the position that axis was in
    #         just before the canned cycle started (unless that position is lower than
    #         the position indicated by the R word, in which case use the R word position).
    # To use option (1), program G99. To use option (2), program G98. Remember that the
    # R word has different meanings in absolute distance mode and incremental distance mode.
    if drill_retract_mode == "G99":
        clear_z = param_r
    if drill_retract_mode == "G98" and motion_z >= param_r:
        clear_z = motion_z
    else:
        clear_z = param_r

    cmd = format_command_line(values, ["G0", f"Z{format_for_axis(values, clear_z)}"])
    G0_clear_z = f"{cmd}"
    ...
    # At the very beginning of the execution of any of the canned cycles, with the
    # XY-plane selected, if the current Z position is below the R position, the Z-axis is
    # traversed to the R position. This happens only once, regardless of the value of L.
    ...
    #     2. a straight traverse of the Z-axis only to the R position, if it is not
    #        already at the R position.
    if motion_z < param_r:
        cmd = format_command_line(values, ["G0", f"Z{format_for_axis(values, param_r)}"])
        gcode.append(f"{linenumber(values)}{cmd}")
    cmd = format_command_line(
        values,
        [
            "G0",
            f"X{format_for_axis(values, param_x)}",
            f"Y{format_for_axis(values, param_y)}",
        ],
    )
    gcode.append(f"{linenumber(values)}{cmd}")
    if motion_z > param_r:
        ...
        cmd = format_command_line(values, ["G1", f"Z{format_for_axis(values, param_r)}"])
        gcode.append(f"{linenumber(values)}{cmd}{F_feedrate}")

    # drill moves
    if command in ("G81", "G82"):
        output_G81_G82_drill_moves(values, gcode, command, params, param_z, F_feedrate, G0_clear_z)
    elif command in ("G73", "G83"):
        output_G73_G83_drill_moves(
            values, gcode, command, params, param_z, clear_z, param_r, F_feedrate, G0_clear_z
        )
    ...
    if drill_step != 0:
        while True:
            ...
            if next_stop_z > param_z:
                # speed-controlled move to the next drill step down
                cmd = format_command_line(
                ...
                if command == "G73":
                    ...
                    gcode.append(f"{linenumber(values)}{G0_clear_z}")
                last_stop_z = next_stop_z
            else:
                # speed-controlled move to the (final) step down
                cmd = format_command_line(values, ["G1", f"Z{format_for_axis(values, param_z)}"])
                gcode.append(f"{linenumber(values)}{cmd}{F_feedrate}")
                gcode.append(f"{linenumber(values)}{G0_clear_z}")
                break
