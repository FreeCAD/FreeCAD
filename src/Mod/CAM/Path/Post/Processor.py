...
class PostProcessor:
    ...
    def _expand_prefix(self, postables) -> None:
        ...
        if (lines := self.values["SAFETYBLOCK"]) != None and lines != "":
        ...
        # PREAMBLE
        if (lines := self.values["PREAMBLE"]) != None and lines != "":
        ...
        # see _expand_pre_job()
        if (lines := self.values["PRE_JOB"]) != None and lines != "":
                ...
                if not self.values["TOOL_CHANGE"]:
                    ...
                    return (
                        ...
                        [
                            self._make_postable(
                                "Post: tool-change",
                                Path.command("(Tool change suppressed: M6 T{tool_num})"),
                            )
                        ],
                    )
                ...
                if item.path:
                    ...
                    for cmd in item.Path.Commands:
                        ...
                        # after
                            ...
                            raise ValueError(
                                f"Expected -1|0|1|None from edit_fn, saw {edit_flag.__class__.__name__} {edit_flag}"
                            )
                    if new_commands:
                        item.path = Path.Path(new_commands)
    ...
    def _expand_trailing_lines(self, postables) -> None:
        ...
        trailing = []
        if (lines := self.values["POST_JOB"]) != None and lines != "":
            trailing.append(self._make_postable("Post: post_job", lines))
        if (lines := self.values["POSTAMBLE"]) != None and lines != "":
