...
class ObjectDressup:
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "MirrorAxis",
            "Path",
            QT_TRANSLATE_NOOP("App::Property", "The mirror axis"),
        )
        obj.addProperty(
            "App::PropertyDistance",
            "MirrorOffsetX",
            "Path",
            QT_TRANSLATE_NOOP("App::Property", "The offset for mirror axis X"),
        )
        obj.addProperty(
            "App::PropertyDistance",
            "MirrorOffsetY",
            "Path",
            QT_TRANSLATE_NOOP("App::Property", "The offset for mirror axis Y"),
        )
        obj.addProperty(
            "App::PropertyBool",
            "CenterModel",
    ...
    def setEditorModes(self, obj):
        offsetXMode = 2 if obj.MirrorAxis in ("X", "None") else 0
        offsetYMode = 2 if obj.MirrorAxis in ("Y", "None") else 0
        centerModelMode = 2 if obj.MirrorAxis == "None" else 0
    ...
    def execute(self, obj):
        ...
        if obj.KeepBasePath:
            cmds = PathUtils.getPathWithPlacement(obj.Base).Commands
            cmds.extend(commandlist)
