...
class ObjectDressup:
    ...
    def createArcZ2MoveDown(self, obj, begin, end, radius):
        commands = []
        angle = math.acos((radius - begin.z + end.z) / radius)  # start angle
        stepAngle = self.getStepAngleArcZ(obj, radius, 0.5)
        iters = math.ceil(angle / stepAngle)
        distance = radius * math.sin(angle) + obj.OffsetIn.Value
        if distance >= 0:
            # get start point with positive distance
            overtravelIn = self.getOvertravelIn(obj, distance)
            iterBegin = overtravelIn[0].positionBegin()
        else:
            # get start point with negative distance
            overtravelOut = self.getOvertravelOut(obj, abs(distance))
            iterBegin = overtravelOut[-1].positionEnd()  # start point
        ...
        while iter <= iters:
            if iter < iters:
                ...
                if distance >= 0:
                    ...
                    overtravelOut = self.getOvertravelOut(obj, abs(distance))
                    iterEnd = overtravelOut[-1].positionEnd()
                iterEnd.z = end.z + radius * (1 - math.cos(angle))
            else:
                # exclude error of calculations for the last iteration
                iterEnd = copy.copy(end)
    ...
    def createArcZ2MoveUp(self, obj, begin, end, radius):
        commands = []
        angleMax = math.acos((radius - end.z + begin.z) / radius)  # finish angle
        stepAngle = self.getStepAngleArcZ(obj, radius, 0.5)
        iters = math.ceil(angleMax / stepAngle)
        distance = radius * math.sin(angleMax) + obj.OffsetOut.Value
        iterBegin = copy.copy(begin)  # start point of short segment
        iter = 1
        angle = 0  # start angle
        while iter <= iters:
            ...
            if distance >= 0:
                # get next point with positive distance
                overtravelOut = self.getOvertravelOut(obj, distance)
                iterEnd = overtravelOut[-1].positionEnd()
            else:
                # get next point with negative distance
                overtravelIn = self.getOvertravelIn(obj, abs(distance))
                iterEnd = overtravelIn[0].positionBegin()
            iterEnd.z = begin.z + radius * (1 - math.cos(angle))
            param = {"X": iterEnd.x, "Y": iterEnd.y, "Z": iterEnd.z, "F": self.horizFeed}
            commands.append(PathLanguage.MoveStraight(iterBegin, "G01", param))
        ...
        if obj.StyleOut not in ("No Retract", "Vertical"):
            ...
            # Should be apply only on straight Path segment
                ...
                if not self.closedProfile:
                    ...
                    pseudoArcEnd = end + tangent + normal
                    lead.extend(self.createArcZ2MoveUp(obj, end, pseudoArcEnd, arcRadius))
                import pprint
    ...
    # Increase travel length in the begin, take commands from profile end
    ...
    # Increase travel length in the end, take commands from profile start
    ...
    def cutTravelEnd(self, obj, commands, cutLength):
        ...
        for i, instr in enumerate(reversed(commands)):
            if instr.positionBegin() is None:
                # workaround if cut whole profile by neative offset
                cmds = commands[:-i]
    ...
    def generateLeadInOutCurve(self, obj):
        ...
        for i, instr in enumerate(source):
            ...
            if (last or not self.isCuttingMove(source[i + 1])) and obj.LeadOut:
                ...
                print("invertAlt", self.invertAlt)
...
class CommandPathDressupLeadInOut:
    ...
    def IsActive(self):
        ...
        if not selection[0].isDerivedFrom("Path::Feature"):
            return False
        if not hasattr(selection[0], "ClearanceHeight"):
            return False
        if not hasattr(selection[0], "SafeHeight"):
            return False
        if not hasattr(selection[0], "StartDepth"):
            return False
