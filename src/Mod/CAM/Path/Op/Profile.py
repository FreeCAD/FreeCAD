...
class ObjectProfile(PathAreaOp.ObjectOp):
                        ...
                        if flattened:
                            ...
                            for po in passOffsets:
                                ...
                                cutShp = self._getCutAreaCrossSection(
                                    obj, base, origWire, flatWire, diffDepth
                                )
                                if cutShp:
                                ...
                                if cutWireObjs:
                                    for cW in cutWireObjs:
                                        if diffDepth:
                                            ...
                                            cW.translate(FreeCAD.Vector(0, 0, -diffDepth))
                                        openEdges.append(cW)
                                else:
                                    Path.Log.error(self.inaccessibleMsg)
    ...
    # Open-edges methods
    def _getCutAreaCrossSection(self, obj, base, origWire, flatWire, diffDepth=0):
        ...
        sdv = wBB.ZMax
        fdv = obj.FinalDepth.Value + diffDepth
        extLenFwd = sdv - fdv
        if extLenFwd <= 0.0:
            extLenFwd = 0.1
