...
class ObjectOp(PathOp.ObjectOp):
    ...
    def buildpathocc(self, obj, wires, zValues, relZ=False, forward=True, start_idx=0):
        ...
        # checks edges coincidence
        for i in range(len(wires)):
            coincideTolerance = PathOpUtil.getCoincideTolerance(wires[i].Edges)
            if coincideTolerance is None or coincideTolerance > Path.Geom.Tolerance:
                # discretize wire with bad coincidence between edges
                wires[i] = PathOpUtil.discretizeWire(wires[i], tol)
