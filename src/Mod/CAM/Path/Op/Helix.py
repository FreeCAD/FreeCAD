...
class ObjectHelix(PathCircularHoleBase.ObjectOp):
    ...
    def initCircularHoleOperation(self, obj):
        ...
        obj.setEditorMode("StepMaxHeight", 2)  # hide
    ...
    def opOnDocumentRestored(self, obj):
        ...
        if not hasattr(obj, "StepMaxHeight"):
            obj.addProperty(
                "App::PropertyLength",
                "StepMaxHeight",
                "Helix Drill",
    ...
    def circularHoleExecute(self, obj, holes):
        ...
        if 0 < obj.OverrideProfileDiameter.Value < tooldiameter:
        ...
        for index, hole in enumerate(holes):
            ...
            stepHeight = obj.StepMaxHeight.Value if obj.StepMaxHeight.Value else work_distance
            ...
            while iter < iters:
                iter += 1
                if iters:
                    self.commandlist.append(Path.Command(f"(hole {index + 1}, step {iter})"))
                ...
                if obj.SpiralMill and obj.SingleHelix:
                    ...
                    if spiralOuterRadius <= spiralInnerRadius:
                        ...
                        spiralArgs = {
                            ...
                            "direction": obj.Direction,
                            "startAt": obj.StartSide,
                            "finish_circle": obj.FinishSpiralCircle,
                            "dir_angle_rad": dirAngleRad,
                        }

                        # create spiral
                        self.commandlist.extend(spiral.generate(**spiralArgs)[1:])
        ...
        for i, instr in enumerate(source):
            if instr.isMove() and instr.isArc():
                ...
                if profileSide == "Internal":
                    horizMoveLength = instr.arcAngle() * (instr.arcRadius() + tooldiameter / 2)
                else:
                    horizMoveLength = instr.arcAngle() * (instr.arcRadius() - tooldiameter / 2)
                ...
                factorH = factorH if factorH > 0.1 else 0.1
                ...
                # Do not increase feed by more than x10
                factorV = factorV if factorV > 0.1 else 0.1
