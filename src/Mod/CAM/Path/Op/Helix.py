...
class ObjectHelix(PathCircularHoleBase.ObjectOp):
    ...
    def circularHoleExecute(self, obj, holes):
        ...
        if obj.ProfileSide == "Internal":
            args["cone_angle"] = obj.HelixConeAngle
        else:
            args["cone_angle"] = -obj.HelixConeAngle
        ...
        for counter, hole in enumerate(holes):
            ...
            if (obj.ProfileSide == "Internal" and obj.OffsetInner.Value == -toolradius) or args[
                ...
                self.commandlist.append(Path.Command("G1", {"Z": obj.FinalDepth.Value}))
            else:
    ...
    def createSpiral(self, center, innerR, outerR, increment, startSide, direction, finish):
        ...
        if finish:
            ...
            commandlist.append(
                Path.Command(arcCmdName, {"X": p1.x, "Y": p1.y, "I": -spiralR, "J": 0})
            )
            p2 = FreeCAD.Vector(center.x + spiralR, center.y, 0)
            commandlist.append(
                Path.Command(arcCmdName, {"X": p2.x, "Y": p2.y, "I": spiralR, "J": 0})
            )
