...
def projectFacesToXY(faces, minEdgeLength=1e-10):
    ...
    for f in faces:
        ...
        if type(f.Surface) in [Part.Cone, Part.Cylinder, Part.Sphere] or (
                ...
                for e in TechDraw.findShapeOutline(f, 1, projdir).Edges:
                    ...
                    if type(e.Curve) is Part.BSplineCurve and vectorsOnStraightLine(
                        ...
                        for i in range(len(discretized) - 1):
                            el.append(Part.makeLine(discretized[i], discretized[i + 1]))

                        # I don't think the for loop should technically be required-
                        # everything should be a line -> 2 points, but it's more
                        # robust than the below
            ...
            if len(projwires) > 1:
                # FIXME: This is a bandaid over some nonsense faces I've seen
                # This is almost certainly HIDING a problem, not FIXING it
    ...
    if outfaces:
        ...
        # FIXME: Doing this sometimes causes things to barf, but is critical in
        # some cases to making things work correctly. BSplines again appear to
        # be problematic- as a band-aid, just return the fusion if concatenation
        # fails
        try:
