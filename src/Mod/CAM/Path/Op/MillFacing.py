...
class ObjectMillFacing(PathOp.ObjectOp):
    ...
    def opExecute(self, obj):
        ...
        Path.Log.debug(
            f"Depth parameters: clearance={obj.ClearanceHeight.Value}, safe={obj.SafeHeight.Value}, start={obj.StartDepth.Value}, step={obj.StepDown.Value}, final={obj.FinalDepth.Value + obj.MaterialAllowance.Value}"
        )
        ...
        Path.Log.debug(
            f"Milling direction: {milling_direction}, StepOver: {obj.StepOver}, stepover_percent: {stepover_percent}"
        )
            ...
            while True:
                ...
                if depth_count == 1:
                    ...
                    if first_xy is not None:
                        ...
                        if any(
                            abs(pre2[k] - self.commandlist[-1].Parameters.get(k, pre2[k] + 1))
                            > 1e-9
                            for k in ("X", "Y", "Z")
                        ):
