    ...
    def _executeDrilling(self, obj, holes):
        ...
        for edge in edgelist:
            ...
            if firstMove:  # Build manually
                ...
                target_z = safe_height
                if (
                    obj.CollisionAvoidanceStrategy
                    in ("Line of Sight", "Tool Diameter", "Tool Shape")
                    and obj.KeepToolDown
                ):
                    # check collision at peck retract depth,
                    # but latter need replace z in last linking move
                    target_z = peck_retract
                ...
                if linking_moves and not Path.Geom.isRoughly(linking_moves[0].z, current_pos.z):
                    ...
                    linking_moves[-1].z = safe_height  # linking ends only at safe height
                    if len(linking_moves) > 1 and Path.Geom.pointsCoincide(
                        Path.Geom.commandEndPoint(linking_moves[-1]),
                        Path.Geom.commandEndPoint(linking_moves[-2]),
                    ):
                        del linking_moves[-1]
