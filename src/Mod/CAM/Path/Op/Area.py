...
class ObjectOp(PathOp.ObjectOp):
    ...
    def _buildPathArea(self, obj, shape, isHole, linkingArgs):
        ...
        for areaIndex, areaParams in enumerate(areaParamsList):
            ...
            print("shapelist", len(shapelist))
            print("wires", len([w for sh in shapelist for w in sh.Wires]))
            for i, sh in enumerate(shapelist):
                print()
                print("shape", i)
                wires = sh.Wires
                if not wires:
                ...
                print("  sortFromPoint", sortFromPoint)
                while wires:
                    doHelix = helixRamp
                    ...
                    if middleEdge:
                        pathParams["start"] = self.getMiddlePointLongestEdge(wire)
                    print("  pathParams start", pathParams["start"])
                    ...
                    if pp.Size:
                        ...
                        if self.initmove:
                            ...
                            # and (not helixRamp or oneStepDown):
                            print("RetractThreshold")
                    ...
                    if self.initmove:
                        ...
                        for cmd in linkingMoves:
                            ...
                            if cmd.z < obj.SafeHeight.Value:
                                ...
                                cmd.Parameters = par
                        print()
                        print("  linkingMoves", linkingMoves)
