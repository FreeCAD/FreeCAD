...
class ObjectArray:
    ...
    def processGroup(self, obj):
        ...
        for i, op in enumerate(self.group):
            ...
            obj.ArrayGroup[i].Label = f"Array_{baseOp.Label}_"
    ...
    def processGroupCompound(self, obj):
        ...
        for op in self.group:
            ...
            if (
                not len(obj.ArrayGroup)
                or obj.ArrayGroup[-1].ToolController != toolController
                or obj.ArrayGroup[-1].CoolantMode != coolantMode
            ):
                self.addNewArrayElement(obj)
                cmName = f"_{coolantMode}" if coolantMode != "None" else ""
                obj.ArrayGroup[-1].Label = f"Array{cmName}_{toolController.Label}"
                obj.ArrayGroup[-1].ToolController = toolController
                obj.ArrayGroup[-1].CoolantMode = coolantMode

            obj.ArrayGroup[-1].Base += [op["opName"]]
            obj.ArrayGroup[-1].PathTemp.addCommands(op["path"].Commands)

            if obj.ArrayGroup[-1].Active:
                obj.ArrayGroup[-1].Path = obj.ArrayGroup[-1].PathTemp
            else:
                obj.ArrayGroup[-1].Path = Path.Path()
