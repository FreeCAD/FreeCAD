...
class PathArray:
    ...
    def getPointsArray(self):
        ...
        if self.pointsSorting == "Automatic":
            ...
            positions = [
                {"x": pos["point"].x, "y": pos["point"].y, "z": pos["point"].z, "a": pos["angle"]}
                for pos in points
            ]
            ...
            points = [
                {"point": FreeCAD.Vector(pos["x"], pos["y"], pos["z"]), "angle": pos["a"]}
    ...
    def getPath(self):
        ...
        if self.addBasePath:
            for base in self.base:
                ...
                self.pathGroup.append({"path": path, "opName": base.Name})
        ...
        if self.arrayType == "Polar":
            ...
            if self.swapDirection:
                self.getLinear2DXYArray()
            else:
                self.getLinear2DYXArray()

        elif self.arrayType == "Points":
            self.getPointsArray()

        else:
            # self.arrayType == "Linear1D"
            self.getLinear1DArray()
