...
class ToolBitBrowserWidget(QtGui.QWidget):
    ...
    _batch_size = 50  # Number of items to insert per batch
    ...
    def __init__(
        ...
        top_layout.addWidget(self._search_edit, 3)  # Give search more space
    ...
    def _fetch_all_assets(self):
        ...
        try:
            self._all_assets = self._asset_manager.fetch(
                asset_type="toolbit",
            ...
            self._is_fetching = False
            self._fetch_data()  # Ensure data is fetched even if trigger fails
    ...
    def _sort_assets(self):
        ...
        if self._sort_key == "label":
            ...
            self._all_assets.sort(key=lambda x: self._tool_no_factory(x) or 0)
    ...
    def _fetch_data(self):
        ...
        try:
            ...
            self._tool_list_widget.update()  # Force widget update
            self._tool_list_widget.viewport().update()  # Force viewport update

            # Insert additional batches if needed
            more_items = True
            while more_items and offset < len(self._all_assets):
