...
class FlatLocalStore(AssetStore):
    ...
    def _uri_to_path(self, uri: Uri | str) -> pathlib.Path:
        """
        Converts a URI or URI string to a file system path, ignoring asset type
        and version.
    ...
    async def list_assets(self,
        ...
        try:
            for entry in self.base_dir.iterdir():
                if entry.is_file() and entry.name.endswith(self.file_extension):
                    # Construct a dummy URI for the flat file
                    ...
                    uri = f"{self.protocol}:///flat_asset/{asset_name}/v1"
                    assets.append(uri)
        ...
        return [Uri(a) for a in assets]

    async def list_versions(self, uri: Uri) -> List[str]:
        """
        Returns a list of versions for a given asset.
        Since this store is unversioned, it returns ['v1'] if the asset exists,
    ...
    # TODO: Consider refactoring with other local stores