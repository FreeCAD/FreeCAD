...
"""
        ...
        gcode = self.get_gcode([], "--machine=A350 --toolhead=50W --spindle-percent --no-header --no-comments")
    ...
    def test_command(self):
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header")
    ...
    def test_precision(self):
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header --precision=2")
        result = gcode.splitlines()[18]
        self.assertEqual(result, expected)
    ...
    def test_lines(self):
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header --line-numbers --line-number=10 --line-increment=2")
        result = gcode.splitlines()[18]
        self.assertEqual(result, expected)

    def test_preamble(self):
        """ Test Pre-amble """
        gcode = self.get_gcode([], "--machine=A350 --toolhead=50W --spindle-percent --no-header --preamble='G18 G55' --no-comments")
        result = gcode.splitlines()[0]
        self.assertEqual(result, "G18 G55")

    def test_postamble(self):
        """ Test Post-amble """
        gcode = self.get_gcode([], "--machine=A350 --toolhead=50W --spindle-percent --no-header --postamble='G0 Z50\nM2' --no-comments")
    ...
    def test_inches(self):
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header --inches")
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header --inches --precision=2")
        result = gcode.splitlines()[18]
        self.assertEqual(result, expected)
    ...
    def test_axis_modal(self):
        ...
        gcode = self.get_gcode([c0, c1], "--machine=A350 --toolhead=50W --spindle-percent --no-header --axis-modal")
    ...
    def test_tool_change(self):
        ...
        gcode = self.get_gcode([c0, c1], "--machine=A350 --toolhead=50W --spindle-percent --no-header")
        self.assertEqual(gcode.splitlines()[19:22], ["M5", "M76", "M6 T2"])
        self.assertEqual(gcode.splitlines()[22], "M3 P25")  # no TLO on Snapmaker (G43 inserted after tool change)
    ...
    def test_spindle(self):
        ...
        # test 200W toolhead
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=200W --spindle-percent --no-header")
        self.assertEqual(gcode.splitlines()[18], "M3 P20")

        # test custom spindle speed extrema
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=200W --spindle-percent --no-header --spindle-speeds=3000,4000")
    ...
    def test_boundaries(self):
        ...
        gcode = self.get_gcode([command], "--machine=A350 --toolhead=50W --spindle-percent --no-header --boundaries-check")
        ...
        gcode = self.get_gcode([c0, c1], "--machine=A350 --toolhead=50W --spindle-percent --no-header --boundaries-check")
        self.assertFalse(self.post.check_boundaries(gcode.splitlines()))

        # check succeed with artisan (which base is bigger)
        gcode = self.get_gcode([c0, c1], "--machine=artisan --toolhead=50W --spindle-percent --no-header --boundaries-check")
        self.assertTrue(self.post.check_boundaries(gcode.splitlines()))

        # check fails with custom boundaries
        gcode = self.get_gcode([c0, c1], "--machine=A350 --toolhead=50W --spindle-percent --no-header --boundaries-check --boundaries='50,400,10'")
        self.assertFalse(self.post.check_boundaries(gcode.splitlines()))
