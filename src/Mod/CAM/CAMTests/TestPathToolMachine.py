...
class TestPathToolMachine(unittest.TestCase):
    ...
    def test_initialization_defaults(self):
        ...
        self.assertAlmostEqual(
            self.default_machine.max_power.getValueAs("W").Value, 2000
        )
        ...
        self.assertAlmostEqual(
            self.default_machine.min_feed.getValueAs("mm/min").Value, 1
        )
        self.assertAlmostEqual(
            self.default_machine.max_feed.getValueAs("mm/min").Value, 2000
        )
    ...
    def test_initialization_custom_values(self):
        ...
        self.assertAlmostEqual(
            custom_machine.max_power.getValueAs("W").Value, 5000
        )
        ...
        self.assertAlmostEqual(
            custom_machine.max_torque.getValueAs("Nm").Value, 50
        )
        self.assertAlmostEqual(
            custom_machine.get_peak_torque_rpm_value(), 15000
        )
        self.assertAlmostEqual(
            custom_machine.min_feed.getValueAs("mm/min").Value, 10
        )
        self.assertAlmostEqual(
            custom_machine.max_feed.getValueAs("mm/min").Value, 5000
        )
        self.assertEqual(custom_machine.id, "custom-id")

    def test_initialization_custom_torque_quantity(self):
        custom_torque_machine = Machine(
            max_torque=FreeCAD.Units.Quantity(100, "Nm")
        )
        self.assertAlmostEqual(
            custom_torque_machine.max_torque.getValueAs("Nm").Value, 100
        )
    ...
    def test_validate_peak_torque_rpm_greater_than_max_rpm(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Peak Torque RPM.*must be less than max RPM"
        ):
            self.default_machine.validate()
    ...
    def test_validate_max_rpm_less_than_min_rpm(self):
        ...
        self.default_machine.set_peak_torque_rpm(
            1000
        )  # peak_torque_rpm = 1000 RPM
        self.default_machine._max_rpm = (
            2000 / 60.0
        )  # max_rpm = 2000 RPM (33.33 1/s)
        self.assertLess(
            self.default_machine.get_max_rpm_value(),
            self.default_machine.get_min_rpm_value(),
        )
        with self.assertRaisesRegex(
            AttributeError, "Max RPM must be larger than min RPM"
        ):
            self.default_machine.validate()
    ...
    def test_validate_max_feed_less_than_min_feed(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Max feed must be larger than min feed"
        ):
            self.default_machine.validate()
    ...
    def test_get_torque_at_rpm(self):
        ...
        expected_peak_torque_rpm = 60000 / 3
        expected_max_torque_nm = 2000 * 9.5488 / expected_peak_torque_rpm
        expected_torque_below_peak = (
            expected_max_torque_nm / expected_peak_torque_rpm * 10000
        )
    ...
    def test_set_max_power(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Max power must be positive"
        ):
    ...
    def test_set_min_rpm(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Min RPM cannot be negative"
        ):
    ...
    def test_set_max_rpm(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Max RPM must be positive"
        ):
    ...
    def test_set_min_feed(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Min feed cannot be negative"
        ):
            self.default_machine.set_min_feed(-10)
        self.default_machine = Machine()
        self.default_machine.set_min_feed(3000)
        self.assertAlmostEqual(
            self.default_machine.min_feed.getValueAs("mm/min").Value, 3000
        )
        self.assertAlmostEqual(
            self.default_machine.max_feed.getValueAs("mm/min").Value, 3001
        )
    ...
    def test_set_max_feed(self):
        ...
        self.assertAlmostEqual(
            self.default_machine.max_feed.getValueAs("mm/min").Value, 3000
        )
        with self.assertRaisesRegex(
            AttributeError, "Max feed must be positive"
        ):
        ...
        self.assertAlmostEqual(
            self.default_machine.max_feed.getValueAs("mm/min").Value, 500
        )
        self.assertAlmostEqual(
            self.default_machine.min_feed.getValueAs("mm/min").Value, 499
        )
        self.default_machine = Machine()
        self.default_machine.set_max_feed(0.5)
        self.assertAlmostEqual(
            self.default_machine.max_feed.getValueAs("mm/min").Value, 0.5
        )
        self.assertAlmostEqual(
            self.default_machine.min_feed.getValueAs("mm/min").Value, 0
        )
    ...
    def test_set_peak_torque_rpm(self):
        ...
        self.assertAlmostEqual(
            self.default_machine.get_peak_torque_rpm_value(), 40000
        )
        with self.assertRaisesRegex(
            AttributeError, "Peak torque RPM cannot be negative"
        ):
    ...
    def test_set_max_torque(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "Max torque must be positive"
        ):
