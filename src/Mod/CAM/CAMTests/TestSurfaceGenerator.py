...
@unittest.skipIf(surface_cpp is None, "The 'surface_generator' C++ module is not available to test.")
class TestSurfaceGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.boundary_with_hole = [
            ...
            [(-2.5, -2.5), (2.5, -2.5), (2.5, 2.5), (-2.5, 2.5), (-2.5, -2.5)]   # Inner hole (wound the same way)
        ]
    ...
    def test10(self):
        ...
        self.assertGreater(len(faces_fine), len(faces_coarse), 
                           "Finer deflection should result in more faces.")
    ...
    def test20(self):
        ...
        endpoints = surface_cpp.generate_linear_pattern_cpp(
            0, 10, 0, 10,  # BBox
            100.0, 0.0,    # Use a large stepover to get only one line
            False, False,  # is_zigzag, reversed
        ...
        self.assertAlmostEqual(start_point[0], 0.0, places=2, msg="Start X should be on the boundary edge.")
        self.assertAlmostEqual(end_point[0], 10.0, places=2, msg="End X should be on the boundary edge.")
        self.assertAlmostEqual(start_point[1], 5.0, places=2, msg="Line should be at the correct Y height.")
    ...
    def test60(self):
        ...
        scan_lines = surface_cpp.generate_spiral_pattern_cpp(
            0, 10, 0, 10, 5.0, 5.0, # BBox, Center
            1.0, 0.1, False, [] # stepover, sample, reversed, empty boundary
        )
    ...
    def test70(self):
        ...
        scan_lines = surface_cpp.generate_spiral_pattern_cpp(
            0, 10, 0, 10, 5.0, 5.0,
            1.0, 0.1, False, self.square_boundary
        )
        ...
        for line in scan_lines:
            for point in line:
                self.assertTrue(0.0 <= point[0] <= 10.0 and 0.0 <= point[1] <= 10.0,
