...
Example, selecting an open edge for profiling when the operation expects a closed polygon.  

Example, selecting an apparently closed polygon for pocketing but the polygon is actually open or it is possible to guess the intent of the user.

The ideal behavior with suspect selection is to guess non-destructively and allow the user to validate the guess or cancel without losing the existing selection.


## Decision
The CAM user interface will support both flavors simultaneously. Operation GUI will evaluate the user's selection, if it exists, and also act intuitively if no selection exist. The user interface will guide the user as much as possible.
...
- If the selection contains invalid items, the operation should raise an error
  and stop.  Example: "The selection contains items (list of invalid items) that cannot be <drilled/pocketed/vcarved/etc> Please correct the suggestion and try again."
