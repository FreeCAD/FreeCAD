...
def horizontalFaceLoop(obj, faces, subNames):
    ...
    face = faces[0]
    clusters = [horizontalEdgeLoop(obj, e) for e in face.Edges]

    # use sorting by Area as simple optimization
    clustersSorted = sorted(
        [edges for edges in clusters if edges],
        key=lambda edges: Part.Face(Part.Wire(Part.sortEdges(edges)[0])).Area,
    )

    objFaces = obj.Shape.Faces
    for edges in clustersSorted:
        ...
        comp = Part.makeCompound([obj.Shape.getElement(f) for f in names])

        outlineWire = TechDraw.findShapeOutline(comp, 1, Vector(0, 0, 1))

        """
        Code below take a lot of time if shape contain thousands edges,
        but profit from this is not obvious
        https://github.com/FreeCAD/FreeCAD/pull/26644#issuecomment-3720755145
        This code can be uncomment if needed
        """
        ...
        """
        uniqueEdges = []
        for edge in outlineWire.Edges:
            if any(Path.Geom.edgesMatch(edge, e) for e in uniqueEdges):
                continue
            uniqueEdges.append(edge)
        outlineWire = Part.Wire(uniqueEdges)
        """

        if not outlineWire.isClosed():
            # skip if outline wire is not closed
            continue

        # if the faces really form the walls of a hole then the resulting has the same footprint
        bb1 = comp.BoundBox
        bb2 = outlineWire.BoundBox
        prec = 1  # used low precision because findShapeOutline() is dirty
        if (
            isRoughly(bb1.XMin, bb2.XMin, prec)
