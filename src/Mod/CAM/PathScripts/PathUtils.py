...
def horizontalEdgeLoop(obj, edge, verbose=False):
    ...
    if not isHorizontal(edge) and verbose:
        ...
        Path.Log.warning(
            translate(
                "CAM",
                "Selected edge should be horizontal",
            )
        )
        return None
...
def horizontalFaceLoop(obj, face, faceList=None):
    ...
    for edges in clusterSorted:
        ...
        if (
            w.isClosed()
            and Path.Geom.isRoughly(bb1.XMin, bb2.XMin, prec)
            and Path.Geom.isRoughly(bb1.XMax, bb2.XMax, prec)
            and Path.Geom.isRoughly(bb1.YMin, bb2.YMin, prec)
            and Path.Geom.isRoughly(bb1.YMax, bb2.YMax, prec)
        ):
            return faces
    return None
