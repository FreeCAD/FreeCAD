...
def tsp_solver_points(points, routeStartPoint=None, routeEndPoint=None):
    ...
    points = []
    for pos in subs:
        points.append({"x": pos.x, "y": pos.y, "d": holeDiameter})

    # get reordered list of points
    points = PathUtils.tsp_solver_points(
        holes,                      # list of dictionaries
        ...
        tunnels.append(
            {
                ...
                "startX": wires[i].OrderedEdges[0].firstVertex().X,
                "startY": wires[i].OrderedEdges[0].firstVertex().Y,
                "endX": wires[i].OrderedEdges[-1].lastVertex().X,
                "endY": wires[i].OrderedEdges[-1].lastVertex().Y,
                "flipped": False,
            }
        )

    # get reordered tunnels
    tunnels = PathUtils.tsp_solver_tunnels(
