...
def loopdetect(obj, edge1, edge2):
    ...
    Path.Log.track()
    e1hash = edge1.hashCode()
    e2hash = edge2.hashCode()
    candidates = [w for w in obj.Shape.Wires for e in w.Edges if e.hashCode() in (e1hash, e2hash)]
    loop = set([w for w in candidates if candidates.count(w) > 1])  # return the duplicate item
    if len(loop) == 1:
        return loop.pop().Edges
    else:
        # unique wire is not found
        return None
...
def horizontalFaceLoop(obj, faces, subNames):
    ...
    for edges in clustersSorted:
        ...
        if not subNames or not names or not all(f in names for f in subNames):
