...
class ContourMap:
    ...
    def highlightBorders(self, threshold=0, new_state=1):
        ...
        if new_state > 1:
            numpy.maximum(
                self.m[1:-1, 1:-1], (h_sum + v_sum > threshold) * new_state, out=self.m[1:-1, 1:-1]
            )
        else:
            numpy.maximum(self.m[1:-1, 1:-1], (h_sum + v_sum > threshold), out=self.m[1:-1, 1:-1])
    ...
    def bigShift(self):
        ...
        numpy.maximum(self.m[1:-1, 1:-1], (self.tmp[0:-9, 2:-8] > 0) * 2, out=self.m[1:-1, 1:-1])
    ...
    def shiftBorder(self, value):
        ...
        if value < 0:
            ...
            while i > 0:
                ...
                i -= 1
                if (i % 2) != 0:
                    self.highlightBorders(threshold=2, new_state=2)
                    # Expand more in diagonal direction

    def or_not(self, a):
        numpy.maximum(self.m[1:-1, 1:-1], (a.m[1:-1, 1:-1] == 0) * 3, out=self.m[1:-1, 1:-1])

    def subtract(self, a):
        if a is not None:
            numpy.minimum(self.m[1:-1, 1:-1], (a.m[1:-1, 1:-1] == 0) * 3, out=self.m[1:-1, 1:-1])
    ...
    def getBorder(self, climb=False):  # return list of (i,j,z) or empty list
        while True:
            ...
            if self.m[r, c] == 1:
                nodes = self._traceWaterLine(self.m, r, c, self.z, not ccw)[::-1] + nodes
    ...
    def _traceWaterLine(self, m, r0, c0, z, ccw):
        ...
        while True:
            ...
            while True:
                # check diagonal
                if m[r + drd, c + dcd] == 1:
                    answer.append((c + (dcd + 1) // 2 - 1, r + (drd + 1) // 2 - 1, z))
                    r += drd
                    c += dcd
                    tr = r
                    tc = c
                ...
                # check horizontal or vertical direction
                    ...
                    if t > 1:  # turn to +-90 degrees found
                        answer.append((c + (2 * dco > dcd) - 1, r + (2 * dro > drd) - 1, z))
                        tr = r
                        tc = c
                    r += dro
                    c += dco
    ...
    def projectToLine(self, p0, p1, p):
        ...
        if length > 0:
            ux = dx / length
            uy = dy / length  # unity vector
            sp = ux * (p[0] - p0[0]) + uy * (p[1] - p0[1])  # scalar product
            cross = (p0[0] + ux * sp, p0[1] + uy * sp)
        else:
            sp = 0
            cross = p0
        return ((p[0] - cross[0]) ** 2 + (p[1] - cross[1]) ** 2, sp > 0 and sp < length, cross)
    ...
    def appendNodes(self, head, tail, max_distance):
        ...
        if tail[0] == tail[-1]:  # cyclic list
            ...
            distance = math.sqrt((head[-1][0] - tail[0][0]) ** 2 + (head[-1][1] - tail[0][1]) ** 2)
    ...
    def prependNodes(self, target, source, max_distance):
        max_distance /= self.sampleInterval
        distance1 = math.sqrt(
            (target[0][0] - source[0][0]) ** 2 + (target[0][1] - source[0][1]) ** 2
        )
        distance2 = math.sqrt(
            (target[0][0] - source[-1][0]) ** 2 + (target[0][1] - source[-1][1]) ** 2
        )
    ...
    def nodesToPath(self, nodes, clearanceHeight, speed):
        ...
        output.append(Path.Command("G0", {"Z": clearanceHeight, "F": speed.vertRapid}))
        output.append(
            Path.Command(
                "G0",
                {"X": xmin + nodes[0][0] * si, "Y": ymin + nodes[0][1] * si, "F": speed.horizRapid},
            )
        )
        ...
        for i in range(1, last + 1):
            ...
            if i < last:
                nxt = nodes[i + 1]
                if (
                    (nxt[0] - this[0]) * (this[1] - prev[1])
                    == (nxt[1] - this[1]) * (this[0] - prev[0])
                    and (nxt[0] - this[0]) * (this[0] - prev[0]) > 0
                    and (nxt[0] - this[0]) * (this[2] - prev[2])
    ...
    def copy(self, out=None):
        if out is not None:
            ...
            out.tmp = None
            return out
        return ContourMap(self.xmin, self.ymin, self.z, self.sampleInterval, self.m[1:-1, 1:-1])
    ...
    def interlayerAppend(self, trace, nodes, slope, max_distance):
        ...
        if nodes[0] == nodes[-1]:  # cyclic list
            ...
            distance = math.sqrt(
                (trace[-1][0] - nodes[0][0]) ** 2 + (trace[-1][1] - nodes[0][1]) ** 2
            )
        if distance * self.sampleInterval < max_distance:
            # Prepend ramp to the tail
            if self.appendNodes(trace, self.prependRamp(nodes, self.z, slope, point), max_distance):
                return True
        return False
    ...
    def dump(self, filename="map.txt"):
        with open(filename, "w") as fd:
            ...
            for j in self.m[::-1]:
                fd.write(("%3i " % nr) + "".join([".+*#"[v] for v in j]) + "\n")
                nr -= 1
