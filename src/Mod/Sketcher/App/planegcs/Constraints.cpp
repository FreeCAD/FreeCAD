...
void ConstraintPointOnSegment::errorgrad(double* err, double* grad, double* param)
{
    ...
    DeriVector2 p(point, param);
    DeriVector2 l1(line.p1, param);
    DeriVector2 l2(line.p2, param);

    DeriVector2 v_line = l2.subtr(l1);
    DeriVector2 v_l1p = p.subtr(l1);
    DeriVector2 v_l2p = p.subtr(l2);

    // point to line distance (=h) and its derivative (=dh)
    // see ConstraintC2LDistance::errorgrad for explanation
    double darea = 0.0;
    double area = v_line.crossProdNorm(v_l1p, darea);
    darea = std::signbit(area) ? -darea : darea;
    ...
    double dDist;
    double dist = v_toNearest.length(dDist);
