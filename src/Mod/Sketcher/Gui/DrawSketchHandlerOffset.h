...
private:
    ...
    TopoDS_Shape makeOffsetShape(bool allowOpenResult = false)
    {
        ...
        if (onlySingleLines && sourceWires.size() > 1 && !branchPoints.empty()) {
            ...
            // Find intersections between a line (point + normalized direction) and a circle.
            ...
            auto lineCircleIntersection = [](const Base::Vector3d& linePoint,
                ...
                s1 = (-B + sqrtDisc) / 2.0;
                s2 = (-B - sqrtDisc) / 2.0;
                result1 = linePoint + lineDir * s1;
                result2 = linePoint + lineDir * s2;
                return (fabs(disc) < 1e-20) ? 1 : 2;
            };
            ...
            auto circleCircleIntersection = [](const Base::Vector3d& c1,
                ...
                result1 = Base::Vector3d(mx + h * dy / d, my - h * dx / d, 0);
                result2 = Base::Vector3d(mx - h * dy / d, my + h * dx / d, 0);
                return (fabs(hSq) < 1e-20) ? 1 : 2;
            };
                ...
                for (size_t i = 0; i < numEdges; i++) {
                    ...
                    if (intFound) {
                        ...
                        if (t1 > 0 && t1 < 1 && t2 > 0 && t2 < 1) {
                            ...
                            // Large offset: offset lines would cross through the interior.
                            // Connect via circle-circle intersection between the semicircles
                            // at the two outer endpoints.
                            Base::Vector3d cc1, cc2;
                            ...
                            if (nCC > 0) {
                                ...
                                if (joinType == 0) {
                                    ...
                                    double aCc1 = atan2(ccPt.y - e1Outer.y, ccPt.x - e1Outer.x);
                                    double cAngle1 = atan2(dir1.y, dir1.x);
                                    ...
                                    double aCc2 = atan2(ccPt.y - e2Outer.y, ccPt.x - e2Outer.x);
                                    double cAngle2 = atan2(dir2.y, dir2.x);
                            ...
                            if (nLC > 0) {
                                ...
                                if (nLC == 2 && s1 > 0 && s2 > 0) {
                                    trimPt = (s1 > s2) ? lc1 : lc2;
                                }
                                addLineEdge(off1End, trimPt);

                                // Record the arc range covered by this connection
