 ...
 *
 * This class provides an NVI interface for extension. Alternatively, specialisation of those
 * functions may be effected if it is opted to instantiate and specialise.
 *
...
protected:
    ...
    // Default implementation is that on every mouse click it redraws and the mode is changed to the
    // next seek On the last seek, it changes to SelectMode::End If this behaviour is not
    // acceptable, then the function must be specialised (or overloaded).
            ...
            for (int index = redundants.size() - 1; index >= 0; index--) {
                ...
                if (redundantconstraintindex > lastsketchconstraintindex) {
                    ...
                    THROWM(Base::RuntimeError,
                           "Redundant is not an Autoconstraint - Please report!\n");
                }
            }
    ...
    /** This function is to be called when redundancies or conflicting constraints should not
     * appear, and updated solver information is required (for example, after addition of new
     * constraints). It does not modify the SketchObject properties. It will throw if redundant or
     * conflicting constraints arise*/
...
protected:
    // The initial size may need to change in some tools due to the configuration of the tool, so
    // resetting may lead to a different number than the compiled time value
