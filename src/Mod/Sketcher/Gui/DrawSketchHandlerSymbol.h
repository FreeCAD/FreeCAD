...
#ifndef SKETCHERGUI_DrawSketchHandlerSymbol_H
#define SKETCHERGUI_DrawSketchHandlerSymbol_H
...
class DrawSketchHandlerSymbol;

namespace ConstructionMethods
{

enum class SymbolConstructionMethod
{
    Width,
    Height,
...
using DSHSymbolController = DrawSketchDefaultWidgetController<
    DrawSketchHandlerSymbol,
    ...
    ConstructionMethods::SymbolConstructionMethod,
    /*bool PFirstComboboxIsConstructionMethod =*/true>;

using DSHSymbolControllerBase = DSHSymbolController::ControllerBase;

using DrawSketchHandlerSymbolBase = DrawSketchControllableHandler<DSHSymbolController>;


class DrawSketchHandlerSymbol: public DrawSketchHandlerSymbolBase
{
    friend DSHSymbolController;
    friend DSHSymbolControllerBase;

public:
    explicit DrawSketchHandlerSymbol(ConstructionMethod constrMethod = ConstructionMethod::Width)
        : DrawSketchHandlerSymbolBase(constrMethod)
        , length(0.0)
        , handleId(0)
        , fileName("")
        , cachedFileName("")
        , cachedBaseShapes({}) {};
    ~DrawSketchHandlerSymbol() override = default;
...
private:
    ...
    {
        return QStringLiteral("Sketcher_Pointer_Symbol.svg");
    }
    ...
    {
        return true;  // Symbol tool must show the line edit to make sense
    };

    QPixmap getToolIcon() const override
    {
        return Gui::BitmapFactory().pixmap("Sketcher_CreateSymbol");
    }

    QString getToolWidgetText() const override
    {
        return QString(QObject::tr("Symbol parameters"));
    }

    bool canGoToNextMode() override
    {
        if (state() == SelectMode::SeekSecond && length < Precision::Confusion()) {
            // Prevent validation of null Symbol.
            return false;
        }
        return true;
    }
...
private:
    ...
    void createShape(bool onlyeditoutline) override
    {
        ...
        if (cachedFileName != fileName || cachedBaseShapes.empty()) {
            if (!fileName.empty()) {
                cachedFileName = fileName;
                // This is the one-time slow operation to get the template shapes.
                cachedBaseShapes = Part::getShapesFromSVG(cachedFileName);
            }
            else {
    ...
    {
        return lookupSymbolHints(static_cast<int>(constructionMethod()), static_cast<int>(state()));
    }
    ...
    static HintTable getSymbolHintTable();
    static std::list<Gui::InputHint> lookupSymbolHints(int method, int state);
};

template<>
auto DSHSymbolControllerBase::getState(int labelindex) const
{
    switch (labelindex) {
...
template<>
void DSHSymbolController::configureToolWidget()
{
    if (!init) {  // Code to be executed only upon initialisation
        ...
        toolWidget->setComboboxLabel(
            ...
            QApplication::translate("TaskSketcherTool_Symbol", "Symbol")
        );

        // 1. Scan for symbol files and store the map
...
template<>
void DSHSymbolController::adaptDrawingToComboboxChange(int comboboxindex, int value)
{
    if (comboboxindex == WCombobox::FirstCombo) {
        ...
        if (handler->pathMap.contains(fontName)) {
            handler->fileName = handler->pathMap.value(fontName).toStdString();
        }
...
template<>
void DSHSymbolControllerBase::doEnforceControlParameters(Base::Vector2d& onSketchPos)
{
...
template<>
void DSHSymbolController::adaptParameters(Base::Vector2d onSketchPos)
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            auto& firstParam = onViewParameters[OnViewParameter::First];
...
template<>
void DSHSymbolController::computeNextDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            auto& firstParam = onViewParameters[OnViewParameter::First];
...
template<>
void DSHSymbolController::addConstraints()
{
...
Gui::InputHint DrawSketchHandlerSymbol::switchModeHint()
{
    return {QObject::tr("%1 switch mode"), {Gui::InputHint::UserInput::KeyM}};
}

DrawSketchHandlerSymbol::HintTable DrawSketchHandlerSymbol::getSymbolHintTable()
{
...
std::list<Gui::InputHint> DrawSketchHandlerSymbol::lookupSymbolHints(int method, int state)
{
    const auto SymbolHintTable = getSymbolHintTable();

    auto it = std::find_if(
        SymbolHintTable.begin(),
        SymbolHintTable.end(),
    ...
    return (it != SymbolHintTable.end()) ? it->hints : std::list<Gui::InputHint> {};
}

}  // namespace SketcherGui


#endif  // SKETCHERGUI_DrawSketchHandlerSymbol_H
