...
using DrawSketchHandlerPointBase =
    DrawSketchDefaultWidgetHandler<DrawSketchHandlerPoint,
...
template<>
auto DrawSketchHandlerPointBase::ToolWidgetManager::getState(int labelindex) const
{
    switch (labelindex) {
        case WLabel::First:
        case WLabel::Second:
...
template<>
void DrawSketchHandlerPointBase::ToolWidgetManager::beforeFirstMouseMove(Base::Vector2d onSketchPos)
{
    ...
    onViewParameters[WLabel::Second]->setPoints(Base::Vector3d(0., 0., 0.),
                                                Base::Vector3d(onSketchPos.x, onSketchPos.y, 0.));
...
template<>
void DrawSketchHandlerPointBase::ToolWidgetManager::adaptDrawingToLabelChange(int labelindex,
                                                                              double value)
{
    switch (labelindex) {
        case WLabel::First:
    ...
    onViewParameters[WLabel::First]->setPoints(
        Base::Vector3d(0., 0., 0.),
        Base::Vector3d(dHandler->editPoint.x, dHandler->editPoint.y, 0.));
    onViewParameters[WLabel::Second]->setPoints(
        Base::Vector3d(0., 0., 0.),
        Base::Vector3d(dHandler->editPoint.x, dHandler->editPoint.y, 0.));
}

template<>
void DrawSketchHandlerPointBase::ToolWidgetManager::doEnforceWidgetParameters(
...
template<>
void DrawSketchHandlerPointBase::ToolWidgetManager::doChangeDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (onViewParameters[WLabel::First]->isSet && onViewParameters[WLabel::Second]->isSet) {
...
template<>
void DrawSketchHandlerPointBase::ToolWidgetManager::addConstraints()
{
