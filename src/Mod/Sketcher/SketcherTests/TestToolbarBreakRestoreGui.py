...
class TestToolbarBreakRestoreGui(unittest.TestCase):
    ...
    def test_save_records_implicit_toolbar_row(self):
        self.activate_workbench("SketcherWorkbench")
        view_toolbar = self.wait_for_toolbar("shared:View")
        sketcher_toolbar = self.wait_for_toolbar("wb:SketcherWorkbench:Sketcher")
        self.record_toolbar_state(view_toolbar)
        self.record_toolbar_state(sketcher_toolbar)

        self.show_toolbar(view_toolbar)
        self.show_toolbar(sketcher_toolbar)
        self.main_window().addToolBar(QtCore.Qt.TopToolBarArea, view_toolbar)
        self.main_window().addToolBar(QtCore.Qt.TopToolBarArea, sketcher_toolbar)
        self.main_window().removeToolBarBreak(view_toolbar)
        self.main_window().removeToolBarBreak(sketcher_toolbar)
        self.pump(250)

        view_geometry = view_toolbar.geometry()
        sketcher_geometry = sketcher_toolbar.geometry()
        sketcher_toolbar.setGeometry(
            sketcher_geometry.x(),
            view_geometry.bottom() + 1,
            sketcher_geometry.width(),
            sketcher_geometry.height(),
        )
        self.assertNotEqual(view_toolbar.geometry().y(), sketcher_toolbar.geometry().y())
        self.assertFalse(self.main_window().toolBarBreak(sketcher_toolbar))

        self.activate_workbench("PartWorkbench")

        top = (
            FreeCAD.ParamGet("User parameter:BaseApp/MainWindow/WorkbenchLayouts")
            .GetGroup("SketcherWorkbench")
            .GetGroup("Layout")
            .GetGroup("Top")
        )
        entries = [top.GetGroup(str(index)) for index in range(top.GetInt("Count", 0))]
        sketcher_index = next(
            index for index, entry in enumerate(entries) if entry.GetString("Toolbar") == "Sketcher"
        )
        self.assertGreater(sketcher_index, 0)
        self.assertTrue(entries[sketcher_index - 1].GetBool("Break", False))
