...
def areas(ship, draft, roll=0.0, trim=0.0, yaw=0.0, n=30):
	...
	shape.translate(Vector(0.0,0.0,-draft*Units.translateUnit('m')))
	...
	areas = [[xmax/Units.translateUnit('m'), 0.0]]
...
def displacement(ship, draft, roll=0.0, trim=0.0, yaw=0.0):
	...
	for solid in shape.Solids:
		...
		try:
			...
			continue
		# Get the data
		vol = vol + common.Volume/Units.translateUnit('m^3')
		for s in common.Solids:
			sCoG  = s.CenterOfMass
			cog.x = cog.x + sCoG.x*s.Volume/Units.translateUnit('m^4')
			cog.y = cog.y + sCoG.y*s.Volume/Units.translateUnit('m^4')
			cog.z = cog.z + sCoG.z*s.Volume/Units.translateUnit('m^4')
	cog.x = cog.x / vol
	cog.y = cog.y / vol
	cog.z = cog.z / vol
	Vol = L*B*abs(bbox.ZMin)/Units.translateUnit('m^3')
