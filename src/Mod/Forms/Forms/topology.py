...
def resize_structured_cage(
...
) -> tuple[list, list] | tuple[list, list, list[float], dict[tuple[int, int], float]]:
    ...
    Return vertices and faces, followed by vertex and edge sharpness when
    return_sharpness is enabled.
    """
...
def catmull_clark_limit_points(vertices, faces, edge_sharpness=None, vertex_sharpness=None):
    ...
    opposite_vertices = [[] for _ in vertices]
    for face_index, face in enumerate(faces):
