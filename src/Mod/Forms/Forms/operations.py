...
def _surface_straightened_controls(obj, indices, line=None):
    ...
    response = [[0.0] * len(indices) for _ in indices]
    base_count = len(obj.ControlPoints)
...
def bridge_boundaries(obj, boundary_edges):
    """Bridge two boundary loops, incorporating existing local refinement."""
    if not getattr(obj, "FormType", "").startswith("Forms::"):
        raise TypeError("The object is not a Forms object")
    cage = ControlCage.from_object(obj)
    local = bool(getattr(obj, "TMeshData", "") or getattr(obj, "LocalEdgeInserts", ()))
    if local:
        mesh = _object_tmesh(obj, cage)
        selected = {tuple(sorted(map(int, edge))) for edge in boundary_edges}
        counts = mesh.edge_counts()
        if not selected or any(counts.get(edge) != 1 for edge in selected):
            raise ValueError("Bridge requires edges on open boundary loops")
        points, faces, controls, edges, corners = materialize_tmesh(
            cage.vertices, cage.faces, mesh, cage.edge_sharpness, cage.vertex_sharpness
        )
        cage = ControlCage(points, faces, corners, edges)
        incident = {}
        for edge in cage.boundary_edges:
            for vertex in edge:
                incident.setdefault(vertex, edge)
        # A selected logical edge can span several baked edges. Either end
        # identifies its unique manifold boundary loop in the new cage.
        boundary_edges = [incident[controls[first]] for first, _second in selected]
    else:
        require_base_topology(obj, "Bridge boundaries")
    cage = cage.bridge_boundaries(boundary_edges, allow_unequal=local).compacted()
    check_sampling(len(cage.faces), 3)
    obj.CageMode = "Editable"
    cage.write(obj)
...
def unweld_segment(obj, segment_edges, separate_forms=True) -> tuple[App.DocumentObject, ...]:
    ...
    Return the resulting objects as a tuple of one or two document objects.
    """
    if not getattr(obj, "FormType", "").startswith("Forms::"):
        raise TypeError("The object is not a Forms object")
    if str(obj.FormType) == "Forms::Surface":
