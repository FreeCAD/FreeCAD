...
class FormFeatureProxy:
    ...
    def _ensure_match_properties(obj):
        ...
        if "MatchContinuity" not in obj.PropertiesList:
            ...
            obj.MatchContinuity = ["Connected", "Tangent", "Curvature"]
            obj.MatchContinuity = "Tangent"
        elif "Curvature" not in obj.getEnumerationsOfProperty("MatchContinuity"):
            current = str(obj.MatchContinuity)
            obj.MatchContinuity = ["Connected", "Tangent", "Curvature"]
            obj.MatchContinuity = current
        for name, kind, default, description, readonly in (
            ("MatchPositionTolerance", "App::PropertyLength", 0.001, "Maximum G2 seam gap", False),
            ("MatchAngularTolerance", "App::PropertyAngle", 0.1, "Maximum G2 tangent-plane angle", False),
            ("MatchCurvatureTolerance", "App::PropertyFloat", 0.01,
             "Relative G2 curvature error; flat supports use inverse opening size as the scale", False),
            ("MatchPositionError", "App::PropertyLength", 0.0, "Measured maximum seam gap", True),
            ("MatchAngularError", "App::PropertyAngle", 0.0, "Measured maximum tangent-plane angle", True),
            ("MatchCurvatureError", "App::PropertyFloat", 0.0, "Measured relative curvature error", True),
            ("MatchStatus", "App::PropertyString", "", "Result of continuity verification", True),
        ):
            if name not in obj.PropertiesList:
                obj.addProperty(kind, name, "Match", description)
