...
from .sessions import active_form_session, finish_active_form_session, set_active_form_session
...
class FormEditSession(FormEditToolsMixin):
    ...
    def _populate_pipe_segment_table(self):
        ...
        if table is None:
            return
        from .pipe_parameters import decode_segment_overrides, decode_segment_sample_overrides
    ...
    def _pipe_diameter_changed(self, key, spin):
        if self.cleaned:
            return
        from .pipe_parameters import set_segment_diameter
    ...
    def _pipe_samples_changed(self, key, spin):
        if self.cleaned:
            return
        from .pipe_parameters import set_segment_samples
    ...
    def _resume_selection_after_tool(self):
        ...
        if self.viewer is not None and self.tool_previous_selection_enabled is not None:
            try:
                ...
                # A destroyed viewer no longer has selection state to restore.
                pass
        self.tool_previous_selection_enabled = None
    ...
    def _dimension_axes(self, property_name) -> tuple[int, ...]:
        if property_name == "Length":
            return (0,)
        if property_name == "Width":
            return (1,)
    ...
    def _structured_segments(self) -> tuple[int, ...]:
        form_type = str(self.obj.FormType)
        if form_type == "Forms::Box":
    ...
    def start(self):
        if active_form_session() is not None:
            finish_active_form_session()
        set_active_form_session(self)
        try:
            self._start()
        except Exception:
            self.cleanup()
    ...
    def cleanup(self):
        ...
        if active_form_session() is self:
            set_active_form_session(None)
        self.cleaned = True
