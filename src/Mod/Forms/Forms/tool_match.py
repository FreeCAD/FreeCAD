...
class MatchTool(ToolController):
    ...
    def _update_match_preview(self, _index=None):
        ...
        continuity, tangent_mode = self._continuity_modes(obj)
        try:
            shape, status = preview_match_shape(obj, edges, support, continuity, tangent_mode,
                                                with_diagnostics=True)
            self._set_match_preview_shape(shape)
            self.match_preview_status.setText(
                status or App.Qt.translate("Forms_Edit", "Match preview")
            )
    ...
    def _continuity_modes(self, obj):
        mode = str(self.match_mode.currentData())
        if mode.startswith("Curvature"):
            return "Curvature", mode[len("Curvature"):]
        if mode == "Connected":
            return "Connected", str(getattr(obj, "MatchTangentMode", "AdjacentFaces"))
        return "Tangent", mode
