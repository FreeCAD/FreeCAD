...
{
    // We probably don't want to updateContents everytime
    // updateContents();
...
void AssemblyLink::onChanged(const App::Property* prop)
{
    ...
    if (prop == &Rigid) {
        Base::Console().Warning("hello on changed \n");
        updateContents();
        if (!Rigid.getValue()) {
            // when the assemblyLink becomes flexible, we need to make sure its placement is null
            // or it's going to mess up moving parts placement within.
...
{
    Base::Console().Warning("updateContents \n");
...
void AssemblyLink::synchronizeComponents()
{
    ...
    for (auto* obj : assemblyGroup) {
        ...
        for (auto* obj2 : assemblyLinkGroup) {
            // AssemblyLink should contain only Links or sub AssemblyLinks.
            auto* link2 = dynamic_cast<App::Link*>(obj2);
            auto* asmLink = dynamic_cast<AssemblyLink*>(obj);
            ...
            if (link2) {
                ...
                linkedObj = asmLink->LinkedObject.getValue();  // not recursive
            }
            else {
                continue;
            }
    ...
    for (auto* obj : assemblyLinkGroup) {
        ...
        if (!obj->isDerivedFrom<App::Link>()) {
            // AssemblyLink should contain only Links.
            continue;
        }

        auto* linkedObj = obj->getLinkedObject(false);  // not recursive
...
void AssemblyLink::synchronizeJoints()
{
    ...
    for (size_t i = 0; i < assemblyJoints.size(); ++i) {
        ...
        copyPropertyIfDifferent<App::PropertyVector>(joint, lJoint, "Offset");
        copyPropertyIfDifferent<App::PropertyFloat>(joint, lJoint, "Rotation");
    ...
    AssemblyObject::recomputeJointPlacements(getJoints());
}
...
AssemblyObject* AssemblyLink::getLinkedAssembly()
{
    ...
    if (!assembly) {
        ...
        assembly = assemblyLink->getLinkedAssembly();
    }

    return assembly;
}

AssemblyObject* AssemblyLink::getParentAssembly()
{
    std::vector<App::DocumentObject*> inList = getInList();
    for (auto* obj : inList) {
        auto* assembly = dynamic_cast<AssemblyObject*>(obj);
