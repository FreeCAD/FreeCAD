...
{
    auto assembly =
        ASMTAssembly::assemblyFromFile(std::string(TEST_DATA_PATH) + "/runPreDragBackhoe1.asmt");
...
{
    auto assembly =
        ASMTAssembly::assemblyFromFile(std::string(TEST_DATA_PATH) + "/runPreDragBackhoe2.asmt");
...
{
    ASMTAssembly::runSinglePendulumSuperSimplified2();  // DOF has infinite acceleration due to zero
                                                        // mass and inertias
    EXPECT_TRUE(true);
}
