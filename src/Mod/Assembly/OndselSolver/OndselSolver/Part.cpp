...
Part::Part(const std::string& str)
    : Item(str)
{}
...
void Part::fillEssenConstraints(
...
//	pEdot =
//mEdot->timesFullColumn(partFrame->qEdot)->plusFullColumn(mE->timesFullColumn(partFrame->qEddot));
// }
...
void Part::calcpTpE()
{
    ...
    pTpE =
        (pCpEtimesqEdot->transposeTimesFullColumn(aJ->timesFullColumn(aC->timesFullColumn(qEdot))))
            ->times(4.0);
}
...
void Part::calcppTpEpEdot()
{
    ...
    auto a4J = aJ->times(4.0);
    auto term1 =
        EulerParameters<double>::pCTpEtimesColumn(a4J->timesFullColumn(aC->timesFullColumn(qEdot)));
    auto pCpEtimesqEdot = EulerParameters<double>::pCpEtimesColumn(qEdot);
