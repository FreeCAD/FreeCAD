...
inline std::shared_ptr<EulerParameters<T>> FullMatrix<T>::asEulerParameters()
{
    ...
    T dum = 0.0;
    T dumSq = 0.0;
    // auto qE = CREATE<EulerParameters<double>>::With(4); //Cannot use CREATE.h in subclasses of
    // std::vector. Why?
...
inline std::shared_ptr<EulerAngleszxz<T>> FullMatrix<T>::eulerAngleszxz()
{
    ...
    if (Numeric::equaltol(std::abs(cthe1x), 1.0, 1.0e-6)) {
        ...
        [[maybe_unused]] auto aaaa = std::atan2(
            this->at(0)->at(2),
            -this->at(1)->at(
                2));  // Check missing minus is needed above. Smalltalk has missing minus too.
