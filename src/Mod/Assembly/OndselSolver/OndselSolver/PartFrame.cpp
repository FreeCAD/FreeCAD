...
PartFrame::PartFrame()
{}
PartFrame::PartFrame(const std::string& str)
    : CartesianFrame(str)
{}
...
{
    if (std::find(redundantEqnNos->begin(), redundantEqnNos->end(), aGeu->iG)
        != redundantEqnNos->end()) {
...
void PartFrame::constraintsReport()
{
    ...
    if (redunCons->size() > 0) {
        std::string str = "MbD: " + part->classname() + std::string(" ") + part->name
...
void PartFrame::fillEssenConstraints(
...
void PartFrame::fillConstraints(
...
{
    markerFramesDo([&](std::shared_ptr<MarkerFrame> markerFrame) {
        markerFrame->fillqsuWeights(diagMat);
    });
}
...
{
    markerFramesDo([&](std::shared_ptr<MarkerFrame> markerFrame) {
        markerFrame->fillqsudotWeights(diagMat);
    });
}
