...
MbD::DispCompIeqctJeqcIe::DispCompIeqctJeqcIe(EndFrmsptr frmi, EndFrmsptr frmj, size_t axis)
    : DispCompIeqcJeqcIe(frmi, frmj, axis)
{
...
void MbD::DispCompIeqctJeqcIe::calc_ppvaluepEIpt()
{
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        auto ppriIeJeIepEIpti = ppAjOIepEITpti->dot(rIeJeO) - pAjOIepEITi->dot(mprIeJeOpt)
            - pAjOIept->dot(mprIeJeOpEITi) - aAjOIe->dot(mpprIeJeOpEITpti);
...
void MbD::DispCompIeqctJeqcIe::calc_ppvalueptpt()
{
    ...
    ppriIeJeIeptpt = ppAjOIeptpt->dot(rIeJeO) - pAjOIept->dot(mprIeJeOpt)
        - pAjOIept->dot(mprIeJeOpt) - aAjOIe->dot(mpprIeJeOptpt);
}
