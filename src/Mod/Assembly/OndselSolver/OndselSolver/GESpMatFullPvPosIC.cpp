...
void GESpMatFullPvPosIC::preSolvewithsaveOriginal(SpMatDsptr spMat,
                                                  FColDsptr fullCol,
                                                  bool saveOriginal)
{
...
void GESpMatFullPvPosIC::doPivoting(size_t p)
{
    ...
    if (pivotRowLimit == SIZE_MAX || p >= pivotRowLimit) {
        pivotRowLimit =
            *std::find_if(pivotRowLimits->begin(), pivotRowLimits->end(), [&](size_t limit) {
                return limit > p;
            });
    }
    ...
    if (max < singularPivotTolerance) {
        auto itr = std::find_if(pivotRowLimits->begin(), pivotRowLimits->end(), [&](size_t limit) {
            return limit > pivotRowLimit;
        });
