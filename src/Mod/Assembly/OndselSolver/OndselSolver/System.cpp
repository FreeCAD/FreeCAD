...
System::System(const std::string& str)
    : Item(str)
{}
...
void MbD::System::runDragStep(std::shared_ptr<System> self,
...
void MbD::System::partsJointsMotionsLimitsForcesTorquesDo(
...
std::shared_ptr<std::vector<std::shared_ptr<Constraint>>> System::essentialConstraints()
{
    ...
    this->partsJointsMotionsDo([&](std::shared_ptr<Item> item) {
...
std::shared_ptr<std::vector<std::shared_ptr<Constraint>>> System::displacementConstraints()
{
    ...
    this->jointsMotionsDo([&](std::shared_ptr<Joint> joint) {
...
std::shared_ptr<std::vector<std::shared_ptr<Constraint>>> System::allConstraints()
{
    ...
    this->partsJointsMotionsLimitsDo([&](std::shared_ptr<Item> item) {
...
{
    auto maxPart = std::max_element(parts->begin(), parts->end(), [](auto& a, auto& b) {
        return a->m < b->m;
    });
...
{
    return std::all_of(limits->cbegin(), limits->cend(), [](auto& limit) {
...
{
    std::for_each(limits->cbegin(), limits->cend(), [](auto& limit) {
        limit->deactivate();
    });
}
