...
MbD::GeneralSpline::GeneralSpline(Symsptr arg)
    : AnyGeneralSpline(arg)
{}
...
void MbD::GeneralSpline::initxdegreexsys(Symsptr arg,
                                         size_t order,
                                         ...
                                         std::shared_ptr<std::vector<double>> yarr)
{
    ...
    xs = xarr;
    ys = yarr;
    if (!Numeric::isIncreasingVector(xs.get())) {
        throw std::runtime_error("x must be increasing.");
    }
    computeDerivatives();
}
...
void MbD::GeneralSpline::computeDerivatives()
{
    ...
    if (degree == 0) {
        throw std::runtime_error("ToDo: Use ZeroDegreeSpline");
    }
    auto n = xs->size();
    auto p = degree;
    auto np = n * p;
