...
{
    auto itr = std::find_if(set->begin(), set->end(), [sptr](Symsptr sym) {
        return sptr.get() == sym.get();
    });
    if (itr != set->end()) {
        auto answer = std::make_shared<Sum>();
        answer->terms = terms;
    ...
    for (const auto& term : *terms) {
        ...
        if (newTerm->isSum()) {
            ...
            productTerms->insert(productTerms->end(),
                                 newTerm->getTerms()->begin(),
                                 newTerm->getTerms()->end());
        }
        else {
    ...
    for (const auto& term : *sumTerms) {
        sumOfProductsOfSums =
            std::static_pointer_cast<Sum>(Symbolic::times(sumOfProductsOfSums, term));
    }
...
{
    auto itr = std::find_if(set->begin(), set->end(), [this](Symsptr sym) {
        return this == (sym.get());
    });
    if (itr != set->end()) {
        auto answer = std::make_shared<Sum>();
