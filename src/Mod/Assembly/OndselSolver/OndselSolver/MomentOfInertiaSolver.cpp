...
{
    auto aJpp = std::make_shared<FullMatrix<double>>(ListListD {{1, 0, 0}, {0, 2, 0}, {0, 0, 3}});
...
void MbD::MomentOfInertiaSolver::doFullPivoting(size_t p)
{
    ...
    if (p != pivotRow) {
        aJcmPcopy->swapElems(p, pivotRow);
    }
    if (p != pivotCol) {
...
void MbD::MomentOfInertiaSolver::calcAPp()
{
    ...
    if (lam0 == lam1) {
        if (lam1 == lam2) {
            ...
            if (eigenvector1->at(1) < 0.0) {
                eigenvector1->negateSelf();
            }
            if (eigenvector2->at(2) < 0.0) {
                eigenvector2->negateSelf();
            }
        ...
        eigenvector1->normalizeSelf();
        if (eigenvector0->at(0) < 0.0) {
            eigenvector0->negateSelf();
        }
        if (eigenvector1->at(1) < 0.0) {
            eigenvector1->negateSelf();
        }
...
FColDsptr MbD::MomentOfInertiaSolver::eigenvectorFor(double lam)
{
    ...
    if ((row2->length() / norm0) > 1.0e-5) {
        throw std::runtime_error("3rd row should be very small.");
    }
...
void MbD::MomentOfInertiaSolver::calcJppFromDiagJcmP()
{
    ...
    std::sort(sortedJ->begin(), sortedJ->end(), [](double a, double b) {
        return a < b;
    });
...
void MbD::MomentOfInertiaSolver::calcJppFromFullJcmP()
{
    ...
    auto c = a00 * a12 * a12 + (a11 * a02 * a02) + (a22 * a01 * a01) - (a00 * a11 * a22)
        - (2.0 * a01 * a02 * a12);
    auto aDiv3 = a / 3.0;
