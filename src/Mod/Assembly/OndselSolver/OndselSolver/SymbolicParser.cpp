...
void MbD::SymbolicParser::parseString(const std::string& expr)
{
    ...
    if (stack->size() != 1) {
        notify("Stack size error, compiler bug!");
    }
}
...
{
    //"Test to see if tokenType matches aType. If so, advance to the next token, leaving the stack
    //unchanged"
...
void MbD::SymbolicParser::xLetter()
{
    ...
    *buffer << hereChar;
    while (true) {
        ...
        if (!std::isalnum(hereChar) && hereChar != '_') {
            break;
        }
        *buffer << hereChar;
    }
        ...
        if (peekForTypeNoPush("(")) {
            ...
            if (expression()) {
                ...
                if (stack->size() > startsize) {
                    ...
                    if (peekForTypeNoPush(")")) {
                        Symsptr args = stack->top();  // args is a Sum with "terms" containing the
                                                      // actual arguments
                        stack->pop();
