...
void LDUSpMatParPvPrecise::doPivoting(size_t p)
{
    ...
    while (lookForFirstNonZeroInPivotCol) {
        ...
        if (spRowi->find(p) == spRowi->end()) {
            if (i <= (ssize_t)p) {
                throwSingularMatrixError("");  // Use ssize_t because i can be negative
            }
        }
        else {
    ...
    if (p != rowPivoti) {
        ...
        if (aip != std::numeric_limits<double>::min()) {
            rowPositionsOfNonZerosInPivotColumn->at(markowitzPivotColCount - 1) = rowPivoti;
        }
    }
