...
void SystemSolver::initializeLocally()
{
    setsOfRedundantConstraints =
        std::make_shared<std::vector<std::shared_ptr<std::set<std::string>>>>();
...
void SystemSolver::runAllIC()
{
    while (true) {
        ...
        if (std::find(discontinuities->begin(), discontinuities->end(), "REBOUND")
            != discontinuities->end()) {
            preCollision();
...
bool SystemSolver::needToRedoPosIC()
{
    ...
    auto itr = std::find_if(setsOfRedundantConstraints->begin(),
                            setsOfRedundantConstraints->end(),
                            [&](auto& set) {
                                for (auto& name : *set) {
                                    if (newSet->find(name) == newSet->end()) {
                                        return false;
                                    }
                                }
                                return true;
    ...
    if (setsOfRedundantConstraints->size() >= 2) {
        auto it = std::find_if(setsOfRedundantConstraints->begin(),
                               setsOfRedundantConstraints->end(),
                               [&](auto set) {
    ...
    setsOfRedundantConstraints->push_back(newSet);
    this->partsJointsMotionsDo([](auto item) {
...
void SystemSolver::partsJointsMotionsForcesTorquesDo(
    const std::function<void(std::shared_ptr<Item>)>& f)
{
    system->partsJointsMotionsForcesTorquesDo(f);
}

void MbD::SystemSolver::partsJointsMotionsLimitsDo(
    const std::function<void(std::shared_ptr<Item>)>& f)
{
    system->partsJointsMotionsLimitsDo(f);
}

void MbD::SystemSolver::partsJointsMotionsLimitsForcesTorquesDo(
    const std::function<void(std::shared_ptr<Item>)>& f)
{
