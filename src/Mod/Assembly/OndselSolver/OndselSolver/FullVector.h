...
public:
    FullVector()
        : Array<T>()
    {}
    FullVector(std::vector<T> vec)
        : Array<T>(vec)
    {}
    FullVector(size_t count)
        : Array<T>(count)
    {}
    FullVector(size_t count, const T& value)
        : Array<T>(count, value)
    {}
    FullVector(typename std::vector<T>::iterator begin, typename std::vector<T>::iterator end)
        : Array<T>(begin, end)
    {}
    FullVector(std::initializer_list<T> list)
        : Array<T> {list}
    {}
...
template<typename T>
inline void
FullVector<T>::atiplusFullVectortimes(size_t i1, std::shared_ptr<FullVector<T>> fullVec, T factor)
{
    for (size_t ii = 0; ii < fullVec->size(); ii++) {
        auto i = i1 + ii;
...
template<typename T>
inline void FullVector<T>::equalSelfPlusFullVectortimes(std::shared_ptr<FullVector<T>> fullVec,
                                                        T factor)
{
    for (size_t i = 0; i < this->size(); i++) {
...
inline void FullVector<double>::normalizeSelf()
{
    ...
    if (length == 0.0) {
        throw std::runtime_error("Cannot normalize a null vector.");
    }
...
inline bool FullVector<T>::isIncreasingIfExceptionsAreLessThan(double tol)
{
    ...
    for (size_t i = 1; i < this->size(); i++) {
        auto previous = next;
        next = this->at(i);
        if (previous > next && (previous - tol > next)) {
            return false;
        }
    }
    return true;
