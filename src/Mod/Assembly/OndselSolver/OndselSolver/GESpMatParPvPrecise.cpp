...
void GESpMatParPvPrecise::doPivoting(size_t p)
{
    ...
    while (lookForFirstNonZeroInPivotCol) {
        ...
        if (spRowi->find(p) == spRowi->end()) {
            if (i <= (ssize_t)p) {
                throwSingularMatrixError("doPivoting");  // Use ssize_t because i can be negative
            }
        }
        else {
    ...
    if (max < singularPivotTolerance) {
        throwSingularMatrixError("doPivoting");
    }
}

void GESpMatParPvPrecise::preSolvewithsaveOriginal(SpMatDsptr spMat,
                                                   FColDsptr fullCol,
                                                   bool saveOriginal)
{
    // assert(false);
    ...
    for (size_t i = 0; i < m; i++) {
        ...
        if (maxRowMagnitude == 0) {
            throwSingularMatrixError("preSolvewithsaveOriginal");
        }
