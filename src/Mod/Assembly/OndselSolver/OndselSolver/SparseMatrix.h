...
template<>
inline void
SparseMatrix<double>::atijminusDiagonalMatrix(size_t i1, size_t j1, DiagMatDsptr diagMat)
{
    auto n = diagMat->nrow();
    for (size_t ii = 0; ii < n; ii++) {
...
template<typename T>
inline void
SparseMatrix<T>::atijplusFullMatrixtimes(size_t i, size_t j, FMatDsptr fullMat, T factor)
{
    for (size_t ii = 0; ii < fullMat->nrow(); ii++) {
