...
MbD::PiecewiseFunction::PiecewiseFunction(Symsptr var,
...
Symsptr MbD::PiecewiseFunction::expandUntil(Symsptr,
                                            std::shared_ptr<std::unordered_set<Symsptr>> set)
{
    ...
    std::transform(functions->begin(),
                   ...
                   [&](auto& func) {
                       return func->expandUntil(func, set);
                   });
    return std::make_shared<PiecewiseFunction>(xx, expansions, transitions);
}

Symsptr MbD::PiecewiseFunction::simplifyUntil(Symsptr,
                                              std::shared_ptr<std::unordered_set<Symsptr>> set)
{
    ...
    std::transform(functions->begin(),
                   ...
                   [&](auto& func) {
                       return func->simplifyUntil(func, set);
                   });
...
Symsptr MbD::PiecewiseFunction::differentiateWRTx()
{
    ...
    std::transform(functions->begin(),
                   ...
                   [&](auto& func) {
                       return func->differentiateWRT(xx);
                   });
