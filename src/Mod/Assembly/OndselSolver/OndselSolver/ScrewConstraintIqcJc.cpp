...
MbD::ScrewConstraintIqcJc::ScrewConstraintIqcJc(EndFrmsptr frmi, EndFrmsptr frmj)
    : ScrewConstraintIJ(frmi, frmj)
{
...
{
    pGpEI =
        zIeJeIe->pvaluepEI()->times(2.0 * M_PI)->minusFullRow(thezIeJe->pvaluepEI()->times(pitch));
}
...
{
    ppGpEIpEI = zIeJeIe->ppvaluepEIpEI()
                    ->times(2.0 * M_PI)
