...
public:
    EulerParametersDot(size_t count)
        : EulerArray<T>(count)
    {}
    EulerParametersDot(size_t count, const T& value)
        : EulerArray<T>(count, value)
    {}
    EulerParametersDot(std::initializer_list<T> list)
        : EulerArray<T> {list}
    {}
    static std::shared_ptr<EulerParametersDot<T>>
    FromqEOpAndOmegaOpO(std::shared_ptr<EulerParameters<T>> qe, FColDsptr omeOpO);
...
template<typename T>
inline std::shared_ptr<EulerParametersDot<T>>
EulerParametersDot<T>::FromqEOpAndOmegaOpO(std::shared_ptr<EulerParameters<T>> qEOp,
                                           FColDsptr omeOpO)
{
    // auto answer = CREATE<EulerParametersDot<T>>::With(4);	//Cannot use CREATE.h in subclasses
    // of std::vector. Why?
