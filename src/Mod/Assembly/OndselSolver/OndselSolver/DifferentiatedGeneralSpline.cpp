...
MbD::DifferentiatedGeneralSpline::DifferentiatedGeneralSpline(Symsptr arg,
                                                              Symsptr spline,
                                                              size_t derivOrder)
...
{
    return std::static_pointer_cast<GeneralSpline>(generalSpline)
        ->derivativeAt(derivativeOrder, xx->getValue());
}
...
Symsptr MbD::DifferentiatedGeneralSpline::differentiateWRTx()
{
    ...
    auto deriv =
        std::make_shared<DifferentiatedGeneralSpline>(arg, generalSpline, derivativeOrder + 1);
    return deriv;
}
