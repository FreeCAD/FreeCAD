...
PrescribedMotion::PrescribedMotion(const std::string& str)
    : Joint(str)
{}
...
void MbD::PrescribedMotion::initMotions()
{
    ...
    std::static_pointer_cast<EndFrameqct>(frmI)->rmemBlks =
        (std::make_shared<FullColumn<Symsptr>>(xyzBlkList));
    auto xyzRotBlkList = std::initializer_list<Symsptr> {phiBlk, theBlk, psiBlk};
    std::static_pointer_cast<EndFrameqct>(frmI)->phiThePsiBlks =
        (std::make_shared<FullColumn<Symsptr>>(xyzRotBlkList));
}
