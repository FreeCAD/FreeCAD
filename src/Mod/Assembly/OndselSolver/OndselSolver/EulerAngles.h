...
public:
    EulerAngles()
        : EulerArray<T>(3)
    {}
    EulerAngles(std::initializer_list<T> list)
        : EulerArray<T> {list}
    {}
...
inline void EulerAngles<Symsptr>::calc()
{
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        if (axis == 1) {
            ...
            throw std::runtime_error("Euler angle rotation order must be any permutation of 1,2,3 "
...
inline std::shared_ptr<EulerAnglesDot<T>> EulerAngles<T>::differentiateWRT(T var)
{
    ...
    std::transform(this->begin(), this->end(), derivatives->begin(), [var](T term) {
