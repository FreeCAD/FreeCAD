# Contributing to Parashell

Thanks for your interest in improving Parashell. Parashell is an AI CAD platform forked from [FreeCAD](https://github.com/FreeCAD/FreeCAD), and we welcome contributions of every size — bug reports, fixes, features, and documentation.

## Quick Start

1. **Find or open an issue.** Use the [issue tracker](https://github.com/TryParashell/Parashell/issues) to report a bug or propose a change. A good issue describes the problem clearly and the minimum acceptable resolution.
2. **Fork and branch.** Fork the repository and create a topic branch on your fork for your work.
3. **Make focused changes.** Keep each pull request scoped to a single, well-defined problem. Match the existing code style (see `.clang-format`, `.clang-tidy`, and `.pre-commit-config.yaml`).
4. **Build and test.** Ensure your changes compile cleanly and pass the project self-tests on the target platforms before opening a PR.
5. **Open a pull request.** Write a clear title and description. If your change affects the UI, include before/after screenshots.

## Reporting Security Issues

**Do not** report security vulnerabilities through public issues or pull requests. Email **ops@parashell.cloud** instead, and see [SECURITY.md](SECURITY.md) for the full policy. If the issue also affects upstream FreeCAD, please report it to the FreeCAD developers as well.

## Code of Conduct

By participating in this project you agree to abide by our [Code of Conduct](CODE_OF_CONDUCT.md). Be respectful, considerate, and forgiving.

## Contribution Process

Because Parashell is a fork of FreeCAD, we follow the FreeCAD Contribution Process (FCP) reproduced below. Where the FCP refers to FreeCAD-specific governance, organization roles, or infrastructure, read it as guidance that Parashell adopts in spirit and applies to this repository and its maintainers.
