...
#define QUANTITY_UNIT(name, symbol) \
    do { \
        const auto actual = Base::UnitRegistry::lookup(symbol); \
        ASSERT_TRUE(actual.has_value()) << #name << " (" << symbol << ")"; \
        EXPECT_EQ(actual->getUnit(), Quantity::name.getUnit()) << #name << " (" << symbol << ")"; \
        EXPECT_DOUBLE_EQ(actual->getValue(), Quantity::name.getValue()) \
            << #name << " (" << symbol << ")"; \
    } while (false);
