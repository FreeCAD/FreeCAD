...
TEST(BaseQuantity, psi_parse_spaced)
{
    const auto qParsed = Quantity::parse("1 psi");
    EXPECT_EQ(qParsed.getValue(), 6.8947448254939996);
}

TEST(BaseQuantity, psi_parse_no_space)
{
    const auto qParsed = Quantity::parse("1psi");
    EXPECT_EQ(qParsed.getValue(), 6.8947448254939996);
}

TEST(BaseQuantity, psi_parse_user_str)
{
    const auto qParsed = Quantity::parse("1psi");
    EXPECT_EQ(qParsed.getUserString(), "6894.74Pa");
}

TEST(BaseQuantity, psi_parse_safe_user_str)
{
    const auto qParsed = Quantity::parse("1psi");
    EXPECT_EQ(qParsed.getSafeUserString(), "6894.74Pa");
}

TEST(BaseQuantity, psi_parse_unit_type)
{
    const auto qParsed = Quantity::parse("1psi");
    EXPECT_EQ(qParsed.getUnit().getTypeString(), "Pressure");
}

TEST(BaseQuantity, psi_to_Pa)
{
    const auto result = Quantity::parse("1psi").getValueAs(Quantity::Pascal);
    const auto expect = 6894.7448254939991;

    EXPECT_EQ(result, expect);
}

TEST(BaseQuantity, psi_to_KPa)
{
    const auto result = Quantity::parse("1psi").getValueAs(Quantity::KiloPascal);
    const auto expect = 6.8947448254939996;

    EXPECT_EQ(result, expect);
}

TEST(BaseQuantity, psi_to_MPa)
{
    const auto result = Quantity::parse("1psi").getValueAs(Quantity::MegaPascal);
    const auto expect = 0.0068947448254939999;

    EXPECT_EQ(result, expect);
}

TEST(BaseQuantity, voltage_unit)
{
...
TEST(BaseQuantity, voltage_val_smaller)
{
