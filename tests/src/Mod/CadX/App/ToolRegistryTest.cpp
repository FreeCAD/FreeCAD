...
TEST(CadXToolRegistry, RejectsMalformedRootBeforeExecutor)
{
    ...
    const auto nested
        = registry.execute("cadx.test", R"({"mode":"ok","items":[{"id":"x","extra":1}]})");
...
TEST(CadXToolRegistry, RejectsNonObjectAndMissingRequiredArguments)
{
    ...
    EXPECT_EQ(registry.execute("cadx.test", "[]").errorCode, "CADX_TOOL_ARGUMENTS_INVALID");
    EXPECT_EQ(registry.execute("cadx.test", R"({})").errorCode, "CADX_TOOL_ARGUMENTS_INVALID");
    EXPECT_EQ(registry.execute("cadx.test", R"({"mode":3})").errorCode, "CADX_TOOL_ARGUMENTS_INVALID");
}
