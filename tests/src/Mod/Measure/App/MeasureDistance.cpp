...
// Auto falls back to the finite extrema and reports the gap: 10 - 2 = 8.
...
// off both edges. Auto falls back to the nearest endpoints, 3 apart in Z.
...
// Datum planes are infinite, so neither side offers a snap and extrema would not
// converge; the pair must take the datum path and read the 5.0 normal offset.
...
// A plane offers no snap, so an explicit mode cannot apply and must not push the
// pair into extrema on unbounded geometry; the datum path still reports 5.0.
TEST_F(MeasureDistance, testExplicitSnapOnDatumPlanesKeepsDatumPath)
{
    App::Document* doc = getDocument();
    auto datum1 = doc->addObject<Part::DatumPlane>("Datum1");
    auto datum2 = doc->addObject<Part::DatumPlane>("Datum2");
