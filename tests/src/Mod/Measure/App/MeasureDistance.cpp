...
// The closest pair is x-ambiguous, so assert the distance components, not the positions.
...
// A single circular-edge wire isn't reduced to a circle, so extrema measures rim-to-rim.
...
TEST_F(MeasureDistance, testCollinearLinesMeasureTheGap)
{
    App::Document* doc = getDocument();
    auto p1 = doc->addObject<Part::Feature>("Line1");
    p1->Shape.setValue(makeLine(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(2.0, 0.0, 0.0)));
    auto p2 = doc->addObject<Part::Feature>("Line2");
    p2->Shape.setValue(makeLine(gp_Pnt(10.0, 0.0, 0.0), gp_Pnt(12.0, 0.0, 0.0)));
...
TEST_F(MeasureDistance, testCrossingLinesMeasureNearestPoints)
{
    App::Document* doc = getDocument();
    auto p1 = doc->addObject<Part::Feature>("Line1");
    p1->Shape.setValue(makeLine(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(2.0, 0.0, 0.0)));
    auto p2 = doc->addObject<Part::Feature>("Line2");
    p2->Shape.setValue(makeLine(gp_Pnt(0.0, 0.0, 3.0), gp_Pnt(0.0, 2.0, 3.0)));
...
// The committed fixture was authored before any Snap property existed.
...
// Restore() accepts an out-of-range index that setValue() would reject (#27404).
TEST_F(MeasureDistance, testRestoredBadIndexIsAuto)
{
    App::Document* doc = getDocument();
    auto p1 = doc->addObject<Part::Feature>("Shape1");
    p1->Shape.setValue(makeCircle(gp_Pnt(0.0, 0.0, 0.0)));
...
// An explicit snap choice survives save and reload (#13708).
...
TEST_F(MeasureDistance, testMixedAutoNoneBypassesShortcut)
{
    App::Document* doc = getDocument();
    auto p1 = doc->addObject<Part::Feature>("Shape1");
    p1->Shape.setValue(makeCircle(gp_Pnt(0.0, 0.0, 0.0)));
...
TEST_F(MeasureDistance, testCenterOnStraightEdgeDegrades)
{
    App::Document* doc = getDocument();
...
// No cursor at recompute, so both sides take the deterministic first endpoint.
...
TEST_F(MeasureDistance, testAxisOnPlanarFaceDegrades)
{
    App::Document* doc = getDocument();
    auto box = doc->addObject<Part::Feature>("Box");
    box->Shape.setValue(BRepPrimAPI_MakeBox(gp_Pnt(10.0, 0.0, 0.0), 5.0, 5.0, 5.0).Solid());
...
// Datum planes are infinite, so extrema would not converge.
TEST_F(MeasureDistance, testParallelDatumPlanesTakeDatumPath)
{
    App::Document* doc = getDocument();
    auto datum1 = doc->addObject<Part::DatumPlane>("Datum1");
    auto datum2 = doc->addObject<Part::DatumPlane>("Datum2");
    datum2->Placement.setValue(Base::Placement(Base::Vector3d(0.0, 0.0, 5.0), Base::Rotation()));
    doc->recompute();

    auto md = doc->addObject<Measure::MeasureDistance>("Distance");
    md->Element1.setValue(datum1, {""});
    md->Element2.setValue(datum2, {""});
    doc->recompute();

    EXPECT_DOUBLE_EQ(md->Distance.getValue(), 5.0);
}

TEST_F(MeasureDistance, testExplicitSnapKeepsDatumPath)
{
    App::Document* doc = getDocument();
    auto datum1 = doc->addObject<Part::DatumPlane>("Datum1");
    auto datum2 = doc->addObject<Part::DatumPlane>("Datum2");
    ...
    md->Element1.setValue(datum1, {""});
    md->Element2.setValue(datum2, {""});
    md->Snap1.setValue(static_cast<long>(Measure::MeasureSnapMode::Vertex));
    doc->recompute();

    EXPECT_DOUBLE_EQ(md->Distance.getValue(), 5.0);
}

TEST_F(MeasureDistance, testTypeUsesSnapping)
{
    ...
    EXPECT_FALSE(Measure::MeasureSnap::typeUsesSnapping("LENGTH"));
    EXPECT_FALSE(Measure::MeasureSnap::typeUsesSnapping("AREA"));
