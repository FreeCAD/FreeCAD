...
TEST_F(TopoShapeExpansionTest, mapSubElementsToTest)
{
    // Arrange
    // Creating tree Edges, the last one will be mapped on the others
    auto edge1 {
    ...
    auto edgeToMap {
        TopoShape(BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, -1.0, 0.0), gp_Pnt(0.0, 1.0, 0.0)).Edge(),
                  3)};
    // Creating the map for the edge that will be mapped, otherwise we won't see anyting about it on
    // the other maps
    edgeToMap.makeShapeWithElementMap(edgeToMap.getShape(), TopoShape::Mapper(), {edgeToMap});

    auto edges {std::vector<TopoShape> {edge1, edge2}};
    // Creating the map for the edges in the vector otherwise mapSubElementTypeForShape will throw
    // an exception If we create the element map on the original edges it won't be copied
    // automatically to those in the vector
    edges[0].makeShapeWithElementMap(edge1.getShape(), TopoShape::Mapper(), {edge1});
    edges[1].makeShapeWithElementMap(edge2.getShape(), TopoShape::Mapper(), {edge2});

    auto edgesWithOp {std::vector<TopoShape> {edge1, edge2}};
    // Creating the map for the edges in the vector otherwise mapSubElementTypeForShape will throw
    // an exception If we create the element map on the original edges it won't be copied
    // automatically to those in the vector
    edgesWithOp[0].makeShapeWithElementMap(edge1.getShape(), TopoShape::Mapper(), {edge1});
    edgesWithOp[1].makeShapeWithElementMap(edge2.getShape(), TopoShape::Mapper(), {edge2});

    // Act
    edgeToMap.mapSubElementsTo(edges);
    edgeToMap.mapSubElementsTo(edgesWithOp, "DUMMY");

    // Assert
    // On both edges we should find the tag of the edge that was mapped
    // Also, we shouldn't find the op "DUMMY" as it's applied to other edges
    EXPECT_NE(edges[0].getElementMap()[0].name.find(POSTFIX_TAG + std::to_string(edgeToMap.Tag)),
              0);
    EXPECT_NE(edges[1].getElementMap()[0].name.find(POSTFIX_TAG + std::to_string(edgeToMap.Tag)),
              0);
    EXPECT_EQ(edgesWithOp[0].getElementMap()[0].name.find("DUMMY"), -1);
    EXPECT_EQ(edgesWithOp[1].getElementMap()[0].name.find("DUMMY"), -1);

    // On both edges we should find the tag of the edge that was mapped
    // Her we should find the op "DUMMY" as it's applied to these edges
    EXPECT_NE(edges[0].getElementMap()[0].name.find(POSTFIX_TAG + std::to_string(edgeToMap.Tag)),
              0);
    EXPECT_NE(edges[1].getElementMap()[0].name.find(POSTFIX_TAG + std::to_string(edgeToMap.Tag)),
              0);
    EXPECT_NE(edgesWithOp[0].getElementMap()[0].name.find("DUMMY"), 0);
    EXPECT_NE(edgesWithOp[1].getElementMap()[0].name.find("DUMMY"), 0);
}
