...
TEST_F(TopoShapeExpansionTest, MapperHistoryModified)
{
    ...
    auto edge {BRepBuilderAPI_MakeEdge(vertex1, vertex2).Edge()};
    auto edgeR {BRepBuilderAPI_MakeEdge(gp_Pnt(-1.0, 0.0, 0.0), gp_Pnt(0.0, 1.0, 0.0)).Edge()};
    auto edgeF {BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 1.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge()};
    auto wire {BRepBuilderAPI_MakeWire(edge, edgeR, edgeF).Wire()};
    ...
    reshape->Replace(edge, BRepBuilderAPI_MakeEdge(vertex1, vertex2).Edge());
    reshape->Apply(edge);
    ...
    // same values
    EXPECT_EQ(historyMprHst.modified(edge).size(), 1);
    EXPECT_EQ(reshapeMprHst.modified(edge).size(), 1);
    EXPECT_EQ(fixMprHst.modified(edge).size(), 1);
}
