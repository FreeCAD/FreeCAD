...
TEST_F(RevolutionTest, ProjectAxisRespectsNestedBodyPlacements)
{
    auto outerPart = getDocument()->addObject<App::Part>("OuterPart");
    outerPart->Placement.setValue(Base::Placement(Base::Vector3d(100.0, 0.0, 0.0), Base::Rotation()));
    auto innerPart = getDocument()->addObject<App::Part>("InnerPart");
    outerPart->addObject(innerPart);
    innerPart->Placement.setValue(Base::Placement(Base::Vector3d(10.0, 0.0, 0.0), Base::Rotation()));
    auto body = getBody();
    innerPart->addObject(body);
    body->Placement.setValue(Base::Placement(Base::Vector3d(5.0, 0.0, 0.0), Base::Rotation()));

    auto axis = addAxis(gp_Pnt(115.0, 0.0, 5.0), gp_Pnt(115.0, 1.0, 5.0));
    auto revolution = addRevolution();
    revolution->ReferenceAxis.setValue(axis, {"Edge1"});
    revolution->ProjectAxis.setValue(true);
    ...
    ASSERT_FALSE(revolution->isError()) << revolution->getStatusString();
    EXPECT_NEAR(revolution->Base.getValue().x, 0.0, Precision::Confusion());
    EXPECT_NEAR(revolution->Base.getValue().z, 0.0, Precision::Confusion());
    EXPECT_NEAR(volumeOf(revolution->Shape.getValue()), torusVolume(360.0), volumeTolerance);
}
