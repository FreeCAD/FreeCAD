...
TEST(EdgesTest, BorderColorsTryFromCornersReturnsNullopt)
{
    // Same trace as PaddingTryFromCornersReturnsNulloptWithMessage, T = Base::Color.
    DiagnosticsCapture capture;

    const Corners corners {Value {Numeric {.value = 4.0, .unit = "px"}}};

    EXPECT_FALSE(BorderColors::tryFrom(Value {corners.tuple()}).has_value());
}
...
{
    // Guards the Margins -> Padding coercion this gate must keep working: the four edge
    // kinds share element type Numeric and are structurally interchangeable, so a plain
    // kind-equality check here would be a regression (it broke
    // ParserTest.ResolveTypedPaddingFromMarginsToken during Task 3's first pass).
    DiagnosticsCapture capture;
