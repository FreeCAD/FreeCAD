...
TEST_F(ParserRobustnessTest, ShortHexColorTokenIsContainedByResolve)
{
    InMemoryParameterSource broken(
        std::list<Parameter> {{.name = "ShortHex", .value = "#fff"}},
        ParameterSource::Metadata {.name = "Broken Hex Source"}
    );
    manager.addSource(&broken);

    EXPECT_NO_THROW({
        const auto resolved = manager.resolve("ShortHex");
        EXPECT_TRUE(resolved.has_value());
    });
}
