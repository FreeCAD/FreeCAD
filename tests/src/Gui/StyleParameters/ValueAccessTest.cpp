...
TEST(ValueAccessTest, ShadesWithNonNumericSpecDoesNotEscape)
{
...
// Regression (reviewer tritao, round 2, point 1): with exactly one gradient operand, blend()
// called Value::get<Base::Color>() on the other operand without checking it.
TEST(ValueAccessTest, BlendWithNonColorOperandDoesNotEscape)
{
    Gui::StyleParameters::ParameterManager manager;
    InMemoryParameterSource source(
        std::list<Parameter> {
