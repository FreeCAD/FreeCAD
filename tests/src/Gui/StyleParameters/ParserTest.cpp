...
{
    // A Padding tuple has no corner-named elements; constructing Corners from it (through the
    // real parser pipeline, via the *total* constructor -- not tryFrom) produces zero radii for
    // all corners rather than throwing, and reports why, exactly like CornersTest's
    // RejectsAPaddingTuple. Without the kind gate at Corners.h's validated(), expand() would
    // silently backfill zeros for a Padding-kind tuple with no diagnostic at all.
...
TEST_F(ParserTest, GradientKindMismatchDegrades)
{
    ...
    EXPECT_FALSE(LinearGradient::tryFrom(result).has_value());
    EXPECT_NO_THROW({ LinearGradient degraded(result.get<Tuple>()); });
}
...
// Every Numeric/Tuple argument now goes through requireArgument<T> just like the color
// arguments above, instead of silently degrading via the total get<T>(). Two of these
// (shades' spec tuple, and lighten's amount) are the exact degrade-to-nonsense cases the
// whole-branch review called out: lighten(#ff0000, #00ff00) used to resolve to unmodified red
// (amount degraded to 0), and shades(#ff0000, 10px) used to resolve to an empty tuple.
...
// The shading functions' range/min/max/pivot/q parameters all have ArgumentParser defaults;
// requireArgument<Numeric> must only reject an *explicit* wrongly-typed value, not the default
// itself. Every shade()/shades() test elsewhere in this file that omits these already exercises
// the default path implicitly; this test pins it down explicitly for the record.
TEST_F(ParserTest, ShadeOmittedShadingParametersUseDefaults)
{
    Parser parser("shade(#ff0000, 0.3)");
    auto expr = parser.parse();

    EXPECT_NO_THROW(expr->evaluate({.manager = &manager, .context = {}}));
}
