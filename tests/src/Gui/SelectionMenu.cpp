...
public:
    ...
    Gui::PickData pick(QAction* action, const std::vector<Gui::PickData>& selections)
    {
        return onPicked(action, selections);
    }
...
private Q_SLOTS:
    void initTestCase()
    {
        ...
        if (!Gui::Application::Instance) {
            ...
            auto macroPreferences = App::GetApplication().GetParameterGroupByPath(
                "User parameter:BaseApp/Preferences/Macro"
            );
            _scriptToPyConsole = macroPreferences->GetBool("ScriptToPyConsole", true);
            _macroPreferenceGroup = macroPreferences;
            macroPreferences->SetBool("ScriptToPyConsole", false);
            _guiApplication = std::make_unique<Gui::Application>(true);
        }
    }

    void cleanupTestCase()
    {
        _guiApplication.reset();
        if (_macroPreferenceGroup) {
            _macroPreferenceGroup->SetBool("ScriptToPyConsole", _scriptToPyConsole);
        }
    }
    ...
    void normalPickAddsSelectionAndClearsPreselection()
    {
        TestableSelectionMenu menu;
        const auto selections = makeSelections();
        menu.populate(selections);
        ...
        Gui::Selection().setPreselect(_docName.c_str(), "First", "Face1");
        QVERIFY(Gui::Selection().hasPreselection());
        const auto picked = menu.pick(firstAction, selections);

        QVERIFY(picked.obj == _firstObject);
        QCOMPARE(QString::fromStdString(picked.subName), QStringLiteral("Face1"));
        QVERIFY(!Gui::Selection().hasPreselection());
        QVERIFY(Gui::Selection().isSelected(_docName.c_str(), "First", "Face1"));
    }
...
private:
    ...
    Base::Reference<ParameterGrp> _macroPreferenceGroup;
    bool _scriptToPyConsole {true};
};
