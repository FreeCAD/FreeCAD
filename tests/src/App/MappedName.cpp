...
{
    return s ? Base::asBytes(s) : Base::BytesView {};
}

static Base::BytesView bv(const char* s, std::size_t size)
{
    return Base::asBytes(s, size);
}
...
TEST(MappedName, defaultConstruction)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), Base::BytesView {}));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), Base::BytesView {}));
}
...
TEST(MappedName, namedConstruction)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), Base::BytesView {}));
}
...
TEST(MappedName, namedConstructionWithMaxSize)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TE")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), Base::BytesView {}));
}
...
TEST(MappedName, constructFromIndexedNameNoIndex)
{
    ...
    // Assert
    EXPECT_EQ(mappedName.dataBytes().data(), Base::asBytes(indexedName.getType()).data());
...
TEST(MappedName, constructFromIndexedNameWithIndex)
{
    ...
    // Assert
    EXPECT_NE(mappedName.dataBytes().data(), Base::asBytes(indexedName.getType()).data());
    EXPECT_EQ(mappedName.isRaw(), false);
...
TEST(MappedName, copyConstructorWithPostfix)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXTEST")));
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName2.dataBytes(), bv("TESTPOSTFIXTEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName2.postfixBytes(), bv("ANOTHERPOSTFIX")));
}
...
TEST(MappedName, copyConstructorStartpos)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("ST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXTEST")));
}
...
TEST(MappedName, copyConstructorStartposAndSize)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("ST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POST")));
}
...
TEST(MappedName, fromRawData)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TESTTEST", 8)));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), Base::BytesView {}));
}
...
TEST(MappedName, fromRawDataCopy)
{
    ...
    Data::MappedName temp = Data::MappedName::fromRawData(
        Base::asBytes(testBytes.data(), testBytes.size())
    );
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TESTTEST", 8)));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("TESTPOSTFIX")));
}
...
TEST(MappedName, fromRawDataCopyStartposAndSize)
{
    ...
    EXPECT_EQ(mappedName.empty(), false);
    EXPECT_EQ(mappedName.size(), 13);
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("STTEST", 6)));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("ABCDEFG")));
}
...
TEST(MappedName, additionOperators)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXTESTPOST1POST2POST3POST4")));
    ...
    EXPECT_EQ(mappedName.size(), 35);
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXTESTPOST5POST6POST7POST8")));
}
...
TEST(MappedName, append)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIX")));
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXANOTH")));
}
...
TEST(MappedName, appendMappedNameObj)
{
    ...
    EXPECT_TRUE(Base::equalBytes(mappedName.dataBytes(), bv("TEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.postfixBytes(), bv("POSTFIXTESTSTPOSTF")));
}
...
TEST(MappedName, toRawBytes)
{
    ...
    // Act & Assert
    EXPECT_TRUE(Base::equalBytes(mappedName.toRawBytes().view(), bv("TESTPOSTFIXTEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.toRawBytes(3).view(), bv("TPOSTFIXTEST")));
    EXPECT_TRUE(Base::equalBytes(mappedName.toRawBytes(7, 3).view(), bv("TFI")));
    EXPECT_TRUE(Base::equalBytes(mappedName.toRawBytes(502, 5).view(), Base::BytesView {}));
}
...
TEST(MappedName, toBytes)
{
    ...
    // Act & Assert
    EXPECT_TRUE(Base::equalBytes(mappedName.toBytes().view(), bv("TESTPOSTFIXTEST")));
}
...
TEST(MappedName, findPreservesEmbeddedNullBytes)
{
    ...
    // Act & Assert
    EXPECT_EQ(mappedName.find(Base::asBytes(needle.data(), needle.size())), 1);
...
TEST(MappedName, rfindPreservesEmbeddedNullBytes)
{
    ...
    // Act & Assert
    EXPECT_EQ(mappedName.rfind(Base::asBytes(needle.data(), needle.size())), 1);
...
TEST(MappedName, endsWithPreservesEmbeddedNullBytes)
{
    ...
    // Act & Assert
    EXPECT_EQ(mappedName.endsWith(Base::asBytes(suffix.data(), suffix.size())), true);
