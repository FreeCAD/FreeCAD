...
if [[ ${HOST} =~ .*darwin.* ]]; then
    ...
    # Note that CI sets this per target: 11.0 (Intel & ARM legacy), 15.0 (ARM modern)
    # - macOS 11+ Intel: Qt 6.7, legacy QuickLook generator (.qlgenerator)
    # - macOS 11-14 ARM: Qt 6.7, legacy QuickLook generator (.qlgenerator)
    # - macOS 15+ ARM: Qt 6.8 (LTS), modern QuickLook App Extensions (.appex)
