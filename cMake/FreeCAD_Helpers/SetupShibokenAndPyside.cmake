macro (SetupShibokenAndPyside)
    ...
    option (
        FREECAD_USE_SHIBOKEN
        "Links to the shiboken library at build time. If OFF its Python module is imported at runtime"
        ON
    )
    ...
    # Shiboken2Config.cmake may explicitly set CMAKE_BUILD_TYPE to Release which causes CMake to
    # fail to create Makefiles for a debug build. So as a workaround we save and restore the value
    # after checking for Shiboken2.
    ...
    if (Shiboken${SHIBOKEN_MAJOR_VERSION}_FOUND)
        # Shiboken config file was found but it may use the wrong Python version Try to get the
        # matching config suffix and repeat finding the package
        set (SHIBOKEN_PATTERN .cpython-${Python3_VERSION_MAJOR}${Python3_VERSION_MINOR})

        file (
            GLOB
            SHIBOKEN_CONFIG
            "${Shiboken${SHIBOKEN_MAJOR_VERSION}_DIR}/Shiboken${SHIBOKEN_MAJOR_VERSION}Config${SHIBOKEN_PATTERN}*.cmake"
        )
        if (SHIBOKEN_CONFIG)
        ...
        if (TARGET Shiboken6::libshiboken AND SHIBOKEN_MAJOR_VERSION EQUAL 6)
            set_target_properties (
                Shiboken6::libshiboken PROPERTIES INTERFACE_COMPILE_DEFINITIONS "NDEBUG"
            )
        endif ()
    endif ()

    # pyside2 changed its cmake files, this is the dance we have to dance to be compatible with the
    # old (<5.12) and the new versions (>=5.12)
    if (NOT SHIBOKEN_INCLUDE_DIR AND TARGET Shiboken${SHIBOKEN_MAJOR_VERSION}::libshiboken)
        get_property (
            SHIBOKEN_INCLUDE_DIR
            TARGET Shiboken${SHIBOKEN_MAJOR_VERSION}::libshiboken
    ...
    if (NOT PYSIDE_INCLUDE_DIR AND TARGET
                                   PySide${PYSIDE_MAJOR_VERSION}::pyside${PYSIDE_MAJOR_VERSION}
    )
        get_property (
            PYSIDE_INCLUDE_DIR
    ...
    endif (NOT PYSIDE_INCLUDE_DIR AND TARGET
                                      PySide${PYSIDE_MAJOR_VERSION}::pyside${PYSIDE_MAJOR_VERSION}
    )
    ...
    find_package (PySide${PYSIDE_MAJOR_VERSION}Tools QUIET) # PySide utilities (uic & rcc
                                                            # executables)
    ...
    if (BUILD_GUI)
        file (
            WRITE ${CMAKE_BINARY_DIR}/Ext/PySide/QtGui.py
        ...
        if (PYSIDE_MAJOR_VERSION LESS 6)
            file (WRITE ${CMAKE_BINARY_DIR}/Ext/PySide/QtSvgWidgets.py
                  "from PySide${PYSIDE_MAJOR_VERSION}.QtSvg import QGraphicsSvgItem\n"
            ...
            file (WRITE ${CMAKE_BINARY_DIR}/Ext/PySide/QtSvgWidgets.py
                  "from PySide${PYSIDE_MAJOR_VERSION}.QtSvgWidgets import *\n"
            )
            file (WRITE ${CMAKE_BINARY_DIR}/Ext/PySide/QtWebEngineWidgets.py
                  "from PySide${PYSIDE_MAJOR_VERSION}.QtWebEngineWidgets import *\n"
    ...
    if (APPLE AND NOT BUILD_WITH_CONDA)
        install (DIRECTORY ${CMAKE_BINARY_DIR}/Ext/PySide DESTINATION MacOS)
    else ()
        install (DIRECTORY ${CMAKE_BINARY_DIR}/Ext/PySide DESTINATION Ext)
    endif ()
    ...
    if (NOT SHIBOKEN_INCLUDE_DIR)
        message (
            WARNING
                "Shiboken${PYSIDE_MAJOR_VERSION} include files not found, FREECAD_USE_SHIBOKEN automatically set to OFF"
        )
    ...
    if (FAILURE)
        ...
        execute_process (
            COMMAND
                ${PYTHON_EXECUTABLE} -c
                "import shiboken${SHIBOKEN_MAJOR_VERSION};print(shiboken${SHIBOKEN_MAJOR_VERSION}.__version__, end='')"
            RESULT_VARIABLE FAILURE
    ...
    if (NOT PYSIDE_INCLUDE_DIR)
        message (
            WARNING
                "PySide${PYSIDE_MAJOR_VERSION} include files not found, FREECAD_USE_PYSIDE automatically set to OFF"
        )
        set (FREECAD_USE_PYSIDE OFF)
    endif ()

    # Independent of the build option PySide modules must be loaded at runtime. Print a warning if
    # it fails.
    execute_process (
        COMMAND
            ${PYTHON_EXECUTABLE} -c
            "import PySide${PYSIDE_MAJOR_VERSION};import os;print(os.path.dirname(PySide${PYSIDE_MAJOR_VERSION}.__file__), end='')"
        RESULT_VARIABLE FAILURE
        OUTPUT_VARIABLE PRINT_OUTPUT
    )
    if (FAILURE)
        ...
        execute_process (
            COMMAND
                ${PYTHON_EXECUTABLE} -c
                "import PySide${PYSIDE_MAJOR_VERSION};print(PySide${PYSIDE_MAJOR_VERSION}.__version__, end='')"
            RESULT_VARIABLE FAILURE
...
# Macros similar to FindQt4.cmake's WRAP_UI and WRAP_RC, for the automatic generation of Python code
# from Qt4's user interface ('.ui') and resource ('.qrc') files. These macros are called: -
# PYSIDE_WRAP_UI - PYSIDE_WRAP_RC
...
macro (PYSIDE_WRAP_UI outfiles)
    ...
    foreach (it ${ARGN})
        ...
        if (WIN32 OR APPLE)
            ...
            # Especially on Open Build Service we don't want changing date like pyside2-uic
            # generates in comments at beginning., which is why we follow the tool command with a
            # POSIX-friendly sed.
            add_custom_command (
                OUTPUT ${outfile}
                COMMAND "${PYSIDE_UIC_EXECUTABLE}" ${UICOPTIONS} "${infile}" -o "${outfile}"
                COMMAND sed "/^# /d" "${outfile}" >"${outfile}.tmp" && mv "${outfile}.tmp"
                        "${outfile}"
                MAIN_DEPENDENCY "${infile}"
            )
        endif ()
...
macro (PYSIDE_WRAP_RC outfiles)
    ...
    foreach (it ${ARGN})
        ...
        # ADD_CUSTOM_TARGET(${it} ALL DEPENDS ${outfile} )
        if (WIN32 OR APPLE)
            add_custom_command (
                OUTPUT ${outfile}
            ...
            # Especially on Open Build Service we don't want changing date like pyside-rcc generates
            # in comments at beginning, which is why we follow the tool command with in-place sed.
            add_custom_command (
                OUTPUT "${outfile}"
                COMMAND "${PYSIDE_RCC_EXECUTABLE}" ${RCCOPTIONS} "${infile}" ${PY_ATTRIBUTE} -o
                        "${outfile}"
                # The line below sometimes catches unwanted lines too - but there is no date in the
                # file anymore with Qt5 RCC, so commenting it out for now... COMMAND sed "/^# /d"
