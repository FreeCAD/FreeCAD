macro(SetGlobalCompilerAndLinkerSettings)
    ...
    if(MSVC)
        ...
        set(CMAKE_C_STANDARD_LIBRARIES
            "kernel32.lib user32.lib gdi32.lib winspool.lib SHFolder.lib shell32.lib ole32.lib oleaut32.lib uuid.lib comdlg32.lib advapi32.lib winmm.lib comsupp.lib Ws2_32.lib dbghelp.lib "
        )
        ...
        if(CCACHE_PROGRAM)
            # By default Visual Studio generators will use /Zi which is not compatible with ccache,
            # so tell Visual Studio to use /Z7 instead.
            ...
            string(REGEX REPLACE "/Z[iI]" "/Z7" CMAKE_CXX_FLAGS_RELEASE
                                 "${CMAKE_CXX_FLAGS_RELEASE}")
        endif(CCACHE_PROGRAM)
        option(
            FREECAD_USE_MP_COMPILE_FLAG
            "Add /MP flag to the compiler definitions. Speeds up the compile on multi processor machines"
            ON)
        ...
        # Mark 32 bit executables large address aware so they can use > 2GB address space NOTE: This
        # setting only has an effect on machines with at least 3GB of RAM, although it sets the
        # linker option it doesn't set the linker switch 'Enable Large Addresses'
    ...
    if(MINGW)
        if(CMAKE_COMPILER_IS_CLANGXX)
            ...
            # https://gcc.gnu.org/bugzilla/show_bug.cgi?id=12477 Actually '-Wno-inline-dllimport'
            # should work to suppress warnings of the form: inline function 'foo' is declared as
            # dllimport: attribute ignored But it doesn't work with MinGW gcc 4.5.0 while using
            # '-Wno-attributes' seems to do the trick.
            set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Wno-attributes")
            set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Wno-attributes")
            set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -Wl,--export-all-symbols")
            # set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} -Wl,--export-all-symbols")
