...
def _fallback_chamfer_forward(xyz1, xyz2):
    dist = torch.cdist(xyz1, xyz2, p=2) ** 2
    dist1, idx1 = torch.min(dist, dim=2)
    dist2, idx2 = torch.min(dist, dim=1)
    return dist1, dist2, idx1.to(torch.int64), idx2.to(torch.int64)
...
class ChamferFunction(torch.autograd.Function):
    ...
    def forward(ctx, xyz1, xyz2):
        if chamfer is None:
            dist1, dist2, idx1, idx2 = _fallback_chamfer_forward(xyz1, xyz2)
        else:
