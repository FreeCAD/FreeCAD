...
class DressupPathBoundary(object):
    ...
    def execute(self, obj):
        ...
        if len(obj.Base.Path.Commands) > 1:  # if finite path, assume there is more than one line.
            ...
            while cmd.Name not in PathGeom.CmdMoveAll:
                commands.append(obj.Base.Path.Commands[nextcmd])
                nextcmd+=1
                cmd = obj.Base.Path.Commands[nextcmd]
            commands.append(cmd)
              
            pos = cmd.Placement.Base  # fake pos, unrelated to machine position which could be anything, see bug #4260
